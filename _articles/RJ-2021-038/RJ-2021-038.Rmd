...
abstract: Index numbers are descriptive statistical measures useful in economic settings
  for comparing simple and complex magnitudes registered, usually in two time periods.
  Although this theory has a large history, it still plays an important role in modern
  today's societies where big amounts of economic data are available and need to be
  analyzed. After a detailed revision on classical index numbers in literature, this
  paper is focused on the description of the R package [*IndexNumber*](https://CRAN.R-project.org/package=IndexNumber)
...
Index numbers are formally defined in Section [2](#sec1). Section
[2.1](#si) introduces simple index numbers. Concretely, simple index
numbers in series and in chain are distinguished. In Section
[2.2](#cin), non-weighted and weighted complex index numbers are
presented. Details on the usage of *IndexNumber* package are considered
from Section [3](#indexnumberinpractice). The four real data sets
contained in the library are also described briefly. Note that two of
them are used in this paper in order to illustrate the calculation of
index numbers. They are available on the website of the *Spanish
Statistical Office (INE)*, <http://www.ine.es>.
...
real example. Table [1](#tab0) shows the number (thousands) of
economically active women and men in Spain from the first trimester of
2002 (taken as a reference value). Remark that four trimesters of each
year are denoted by T1, T2, T3 and T4, respectively.
...
::: {#tab0}
  ...
  : Table 1: Number (thousands) of economically active women and men in
  Spain from first trimester of 2002.
:::

This dataset, included in *IndexNumber* package, can be obtained from
the Economically Active Population Survey (EPA) elaborated by the
...
defined as follows:
$$\label{niinserie}
I_0^t(X)=\frac{x_t}{x_0}\cdot 100.   (\#eq:niinserie)$$
For each $t\in \{0,1,\dots,T\}$, this measure has a natural
interpretation. Fixed a certain variable of interest $X$, the index
number in series in $t$ shows the percentage of variation of the
magnitude in this instant of time with respect to the reference value
(in this case, the initial one).
...
-   When prices are considered, the *relative price* of a product $i$,
    ...
    where $p_{it}$ denotes the price at instant $t$ and $p_{i0}$, the
    price in the base period.
-   The *relative amount* of a product $i$ can be written as
    $$q_0^t=\frac{q_{it}}{q_{i0}},$$
    where $q_{it}$ denotes the produced or sold amount at instant $t$
    and $q_{i0}$, the amount for the base period.
...
included in Table [1](#tab0). Thus, the simple index numbers in series
for the economically active women and men in Spain from the first
trimester of 2002 is given in Table [2](#tab1).
...
::: {#tab1}
  ...
  : Table 2: Simple index numbers in series for number (thousands) of
  economically active women and men in Spain from the first trimester of
  2002.
:::

Comparing the evolution of the index numbers in series for the
population of women and men shown in Table [2](#tab1) has an interest,
for instance, to analyze the effect of variable sex in the Spanish labor
market. Note that the number of economically active women and men in the
second trimester of 2003 is 7.2% and 4.2% larger than in the reference
time, respectively. Therefore, women increasing is slightly larger than
men.
...
$$\label{niinchain}
IC^t(X)=\frac{x_t}{x_{t-1}}\cdot 100.   (\#eq:niinchain)$$
Again, this index number can be naturally interpreted. It is worth
mentioning that these measures the variation of the characteristic under
study with respect to the previous value in a fixed instant $t$. For
instance, these index numbers reflect the percentage variation that the
variable experiments between two consecutive values in time series
settings.
...
::: {#tab4}
  ...
  : Table 3: Simple index numbers in chain for number (thousands) of
  economically active women and men in Spain from the first trimester of
  2002.
:::

Table [3](#tab4) shows the evolution in time of the number (thousands)
of economically active women and men in Spain from the trimester of 2002
(see Table [1](#tab0)). According to the obtained results, we emphasize
as relevant that the number of economically active women and men in the
second trimester of 2003 increases 1.4% and 0.9%, respectively, with
respect to the previous trimester.
...
Table [2](#tab1), it is possible to obtain the corresponding simple
index number in chain for women also for the first trimester of 2003
(contained in Table [3](#tab4)). That is,
...
$$I_0^t(X)=\frac{x_t}{x_{0}}\cdot 100=\frac{x_t}{x_{t-1}}\frac{x_{t-1}}{x_{t-2}}\cdots   \frac{x_2}{x_{1}}\frac{x_1}{x_{0}} \cdot 100=\frac{IC^t(X)}{100}\cdot\frac{IC^{t-1}(X)}{100}\cdot \cdots \cdot \frac{IC^{2}(X)}{100}\cdot \frac{IC^{1}(X)}{100}\cdot 100.$$
For instance, if we consider the simple index numbers in chain shown in
Table [3](#tab4), it is possible to obtain the simple index number in
series for the first trimester of 2003 contained in Table [2](#tab1).
...
with $t\in \{1,\dots,T\}$, is denoted by $Rate^t(X)$ for each
$t\in\{1,\dots,T\}$. It can be calculated from the simple index in chain
as follows:
$$Rate^{t}(X)=\frac{x_{t}-x_{t-1}}{x_{t-1}}\cdot100=IC^t(X)\cdot 100.$$
This definition can be extended to any pair of instants in
...
In particular, if the consecutive observations correspond to two
different years, months, or trimesters the variation rate is called
interanual variation rate, monthly variation rate, and quarterly
variation rate, respectively. From the data shown in Table [2](#tab1),
the quarterly variation rates have been calculated in Table
[4](#tabrateh).
...
::: {#tabrateh}
  ...
  : Table 4: Quarterly variation rate for number (thousands) of
  economically active women and men in Spain.
:::
...
\end{split}$$
Therefore, $x_{t+k}=\left(\frac{100+T_k}{100}\right)^k \cdot x_t$ and,
as consequence,
$\frac{x_{t+k}}{x_t}=\left(\frac{100+T_k}{100}\right)^k.$ Then,
$$T_k=100\cdot\left(\frac{x_{t+k}}{x_t} \right)^{1/k}.$$
From data shown in Table [2](#tab1), we have calculated the quarterly
average variation rate in $2002$ for women and men as
$$\left(\sqrt[3]{\frac{7751.50}{7442.70}}-1\right)\cdot 100 = 1.36  and \left(\sqrt[3]{\frac{11472.80}{11192.30}}-1\right)\cdot 100=0.83, respectively.$$
According to the results obtained, the average variation rate for women
is considerably bigger than the corresponding for men in this period.
...
described in Table [6](#tab1110). It shows the unitary value (euros) of
prices of combustibles and other energy resources for the main home in
Spain from 2005 to 2015. In this case, the data source is again the
Spanish Statistics National Institute (INE).
...
::: {#tab1110}
  ...
  : Table 6: Unitary value (euros) of combustibles and other energy
  resources for the main home in Spain from 2005 to 2015.
:::

Table [7](#tab671110) shows the (thousands of units) consumption of
...
Table [6](#tab1110). Of course, this data set can also be obtained from
the Spanish Statistics National Institute (INE).
...
::: {#tab671110}
  ...
  : Table 7: Consumption (thousands of units) of combustibles and other
  energy resources for the main home in Spain from 2005 to 2015.
:::
...
instance, in Table [5](#tab111). In particular, knowing the variation of
a magnitude in time $t$ with respect to the base period has an interest.
In this sense, it is worth mentioning that non-weighted complex index
numbers can be easily obtained as the arithmetic, geometric and harmonic
means of the simple index numbers in series for the considered
magnitudes. Anyway, their mathematical expressions are described below:
...
-   The *Sauerbeck index* (cf. [@sauerbeck1895index]) at time $t$ for X,
    ...
    $$\label{niSauerbeck}S^t(X)=\frac{1}{n}\sum_{i=1}^n \frac{x_{it}}{x_{i0}}\cdot 100,\text{ for each t }\in\{0,\cdots,T\} .
    (\#eq:niSauerbeck)$$
-   The *Geometric mean index* at time $t$, $G^t(X)$, is calculated as
    follows:
    $$\label{niGeometric}G^t(X)=\sqrt[n]{\prod_{i=1}^n \frac{x_{it}}{x_{i0}}}\cdot 100,\text{ for each t }\in\{0,\cdots,T\}. 
    (\#eq:niGeometric)$$
...
-   The *Harmonic mean index* at time $t$, $H^t(X)$, is determined by
    $$\label{niHarmonic}H^t(X)=\frac{n}{\sum_{i=1}^n\frac{x_{i0}}{x_{it}}}\cdot 100,\text{ for each t }\in\{0,\cdots,T\}.  
    (\#eq:niHarmonic)$$
...
-   The *Bradstreet-Dûtot index* at time $t$, $BD^t(X)$, is introduced
    ...
    $$\label{niBDutot}BD^t(X)=\frac{\sum_{i=1}^n x_{it}}{\sum_{i=1}^n x_{i0}}\cdot 100,\text{ for each t }\in\{0,\cdots,T\}.  
    (\#eq:niBDutot)$$
...
prices of combustibles and other energy resources for the main home in
Spain from 2005 to 2015. From the information in Table [6](#tab1110),
the four index numbers previously described are numerically shown in
Table [8](#tab1110000), and their evolution is depicted in Figure
[1](#nwjointevolution).
...
::: {#tab1110000}
  ...
  : Table 8: Non-weighted complex price indexes in seriea for the
  unitary value of combustibles and energy resources for the main home
  in Spain from 2006 to 2015.
:::

![Figure 1: Joint evolution of the non-weighted complex price indexes in
series for the unitary value of combustibles and energy resources for
the main home in Spain from 2006 to 2015.](1.png){#nwjointevolution
...
-   The *Carli index* (cf. [@carli1804]) at time $t$ for X, $C^t(X)$, is
    ...
    $$\label{niCarli}C^t(X)=\frac{1}{n}\sum_{i=1}^n \frac{x_{it}}{x_{i t-1}}\cdot 100, \text{ for each t }\in\{1,\cdots,T\}. 
    (\#eq:niCarli)$$
-   The *Jevons index* at time $t$, $J^t(X)$, is calculated following
    [@jevons1863serious] as follows:
    $$\label{niJevons}J^t(X)=\sqrt[n]{\prod_{i=1}^n \frac{x_{it}}{x_{i t-1}}}\cdot 100,\text{ for each t }\in\{1,\cdots,T\}.  
    (\#eq:niJevons)$$
    Given a collection of $n$ magnitudes, the geometric mean index at
    $t$ is obtained as $n^{th}$-root of the product of simple index
...
-   The *Dûtot index* at time $t$, $D^t(X)$, is introduced in
    ...
    as follows:
    $$\label{niDutot}D^t(X)=\frac{\sum_{i=1}^n x_{it}}{\sum_{i=1}^n x_{i t-1}}\cdot 100,\text{ for each t }\in\{1,\cdots,T\}.
    (\#eq:niDutot)$$
...
prices of combustibles and other energy resources for the main home in
Spain from 2005 to 2015, detailed in Table [6](#tab1110). These index
numbers are numerically detailed in Table [9](#tab11100002_2).
...
::: {#tab11100002_2}
  ...
  : Table 9: Non-weighted complex price indexes in chain for the unitary
...
table such as Table [10](#0tab111). For instance, the use of the amount
of production of different products or the use of prices may result of
interest, depending on the setting under study.
...
-   The *Laspeyres index* [@laspeyres1871ix] analyzes the variations of
    ...
    $$\label{niLaspeyres}
    L^t(X,Y)=\frac{\sum_{i=1}^n \frac{x_{it}}{x_{i0}}x_{i0}y_{i0}}{\sum_{i=1}^n x_{i0}y_{i0}}\cdot 100 = \frac{\sum_{i=1}^n x_{it}y_{i0}}{\sum_{i=1}^n x_{i0}y_{i0}}\cdot 100,\text{ for each t }\in\{0,\cdots,T\}.  
    (\#eq:niLaspeyres)$$
...
-   The *Paasche index* is an alternative index to the Laspeyres index
    ...
    $$\label{niPaasche}
    P^t(X,Y)=\frac{ \sum_{i=1}^n\frac{x_{it}}{x_{i0}}x_{i0}\cdot y_{it} }{\sum_{i=1}^n x_{i0}\cdot y_{it}}\cdot100=  \frac{\sum_{i=1}^n  x_{it}y_{it}}{\sum_{i=1}^n x_{i0}y_{it}}\cdot 100, \text{ for each t }\in\{0,\cdots,T\}.   
    (\#eq:niPaasche)$$
-   The *Marshall-Edgeworth index* (cf. [@marshall1887remedies],
    ...
    $$\label{niEdgeworth}
    E^t(X,Y)=\frac{\sum_{i=1}^n x_{it}(y_{i0}+y_{it})}{\sum_{i=1}^n x_{i0}(y_{i0}+y_{it}) }\cdot 100, \text{ for each t } \in\{0,\cdots,T\}. 
    (\#eq:niEdgeworth)$$
-   The *Fisher index* is equal to the geometric mean of the index
    numbers under the approaches of Laspeyres and Paasche:
    $$\label{niFisher}F^t(X,Y)=\sqrt{L^t(X,Y)\cdot P^t(X,Y)}, \text{ for each t }\in\{0,\cdots,T\}.
    (\#eq:niFisher)$$
...
$$\label{nivalue}IV_{0}^t(p,q)=\frac{V_t(p,q)}{V_0(p,q)}=\frac{\sum_{i=1}^n p_{it}q_{it}   }{\sum_{i=1}^n p_{i0}q_{i0} }, \text{ for each t } \in\{0,\cdots,T\}. 
(\#eq:nivalue)$$
It satisfies that
$IV_{0}^t(p,q)=L^t(p_0,q)\cdot P^t(p_0,q)=L^t(p,q_0)\cdot P^t(p,q_0)=F^t(p_0,q)\cdot F^t(p,q_0)$.

From data contained in Tables [6](#tab1110) and [7](#tab671110), these
five index numbers were determined. They are in Table [11](#tab1110000b)
(from tenth to twelfth column), and they are graphically depicted in
Figure [2](#wjointevolution).
...
::: {#tab1110000b}
  ...
  : Table 11: Weighted complex price indexes for the unitary value of
  combustibles and energy resources for the main home in Spain from 2006
  to 2015.
:::

![Figure 2: Joint evolution of the weighted complex price indexes for
the unitary value of combustibles and energy resources for the main home
in Spain from 2006 to 2015.](2.png){#wjointevolution width="100%"
...
*IndexNumber* is a tool that R users can use in order to determine
several classical index numbers that describe the evolution of a single
...
*micEconIndex* [@micEconIndex], *IndexNumR* [@indexnumR], and
*PriceIndices* [@PriceIndices] packages also allow to compute complex
index numbers but only when the considered magnitudes are prices and
quantities. It is worth mentioning that *IndexNumber* library was
designed under a more general perspective by extending to any type of
magnitude. Moreover, none of the above-referenced packages implement
simple index numbers, and they do not offer graphical tools to
facilitate the analysis of time evolution series either. Additionally,
*IndexNumber* can be seen as an additional basic library that can also
be exploited by non-experts R users. For instance, inputs of functions
are numeric vectors or matrices containing the magnitude values, much
more flexible than data structures that other packages consider. As for
computational complexity, it is also relatively simple because, unlike
*IndexNumR* that implements multilateral methods, the number of required
elementary operations is smaller. Finally, it is convenient to note that
*IndexNumber* package also provides four new recent real data sets.
...
::: {#table_summary1}
  ...
  --------------------------- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  `aggregated.index.number`   Function to obtain several non-weighted index numbers: the Sauerbeck index number (\@ref(eq:niSauerbeck)), the Geometric index number (\@ref(eq:niGeometric)), the Harmonic index number (\@ref(eq:niHarmonic)) the Bradstreet-Dûtot index number (\@ref(eq:niBDutot)), the Carli index number (\@ref(eq:niCarli)), the Jevons index number (\@ref(eq:niJevons)) and the Dûtot index number in (\@ref(eq:niDutot)).

  `edgeworth.index.number`    Function to calculate the Marshall-Edgeworth index number (\@ref(eq:niEdgeworth)).

  `fisher.index.number`       Function to calculate the Fisher index number (\@ref(eq:niFisher)).

  `index.number.chain`        Function to calculate the simple index number in chain (\@ref(eq:niinchain)).

  `index.number.serie`        Function to calculate the simple index number in series (\@ref(eq:niinserie)).

  `laspeyres.index.number`    Function to determine the Laspeyres index number (\@ref(eq:niLaspeyres)).

  `paasche.index.number`      Function to obtain the Paasche index number (\@ref(eq:niPaasche)).
...
associated parameters in the corresponding function. Table [13](#table2)
describes the different options to determine those index numbers whose
implementation was included in *IndexNumber* package. However, not all
of the mentioned options are required since only some of them are
specific for each particular class of index number. Thus, Table
[14](#table3) summarizes the arguments associated with each function.
Examples of usage for the implemented functions are described in the
next section.
...
::: {#table2}
  ...
  `type`          A chain of characters to indicate the type of non-weighted index number to evaluate the evolution of a set of magnitudes (even for different products). By considering `base="serie"`, if we introduce `type="arithmetic"`, we obtain the Sauerbeck index number in (\@ref(eq:niSauerbeck)). If we introduce `type="geometric"`, we obtain the Geometric index in (\@ref(eq:niGeometric)). If we choose `type="harmonic"`, we obtain the Harmonic mean index in (\@ref(eq:niHarmonic)). If we write `type="BDutot"`, we will obtain the Bradstreet-Dûtot index in (\@ref(eq:niBDutot)). This argument is only required in the function `aggregated.index.number`. Otherwise, if we take `base="chain"` and `type="Carli"`, we obtain the Carli index number in (\@ref(eq:niCarli)). If we introduce `type="Jevons"`, we obtain the Jevons index in (\@ref(eq:niJevons)) and if we choose `type="Dutot"`, we obtain the Dûtot index in (\@ref(eq:niDutot)). This argument is only required in the function `aggregated.index.number`.
  ...
  : Table 13: Summary of arguments for functions in the *IndexNumber*
  package.
:::
...
numbers in series (\@ref(eq:niinserie)) in R software on the real data
partially given in Table [1](#tab0). Remember that it depicts the number
(thousands) of economically active women and men in Spain. As we
mentioned before, this information is also included in the data set
`ActivePeople` in *IndexNumber* package. The first trimester of 2002 is
...
We include a graphical summary of the evolution of this magnitude under
the fixed criteria in Figure [3](#women_in_serie), by using
`opt.plot=TRUE`. We also summarize the most relevant information in
terms of the instant in which the maximum and minimum values are reached
choosing `opt.summary=TRUE`.

![Figure 3: Evolution of the simple index number in series for the
number (thousands) of economically active women in
Spain.](inseriewoman_new.png){#women_in_serie width="100%"
alt="graphic without alt text"}

Analogously, the user can obtain the corresponding results for the data
associated to `TotalMen`. The required code in this case is the shown
below:
...
Results contained in Table [2](#tab1) have been obtained using both
functions of *IndexNumber* package in R.

### Simple index numbers in chain in R

Again, we take the data set `ActivePeople` to determine the
corresponding simple index number in chain (\@ref(eq:niinchain)) for the
number (thousands) of economically active women in Spain. Note that the
reference value in each instant of time is the immediately previous one
in the series.
...
Also in this case, the option `opt.summary=TRUE` summarizes the most
relevant information about the corresponding simple index number. The
option `opt.plot=TRUE` provides a graphical representation of the
evolution of the magnitude as Figure [4](#women_in_chain) depicts.

![Figure 4: Evolution of the simple index number in chain for the number
(thousands) of economically active women in
Spain.](inchainwomen_new.png){#women_in_chain width="100%"
alt="graphic without alt text"}

Table [3](#tab4) also includes the numerical analysis of the evolution
of economically active men in Spain. To determine these values, we use
the following code:
...
Table [6](#tab1110) to show its performance in practice. That table
describes the unitary value of prices of several energy resources for
the period 2005-2015. As we have mentioned, this data set is available
in library *IndexNumber* under the name `ECResources` (in particular,
from the second to the sixth column).
...
[8](#tab1110000) were obtained by using the code in R that we show in
the current section.

**Sauerbeck index in R**

Next, we determine the Sauerbeck index (\@ref(eq:niSauerbeck)) in R
software. To this aim, we use the corresponding function
`aggregated.index.number()` by adding, as option, `type="arithmetic"`.
Recall that it corresponds to an average by stages. In this case, we
also include a graphical description of the joint evolution of prices in
Figure [5](#sauerbeck) with `opt.plot=TRUE`, and a numerical summary of
such magnitude (with `opt.summary=TRUE`).

![Figure 5: Sauerbeck index evolution for the prices of several energy
resources for the period 2005-2015.](sauerbeck_new.png){#sauerbeck
width="100%" alt="graphic without alt text"}
...
mean index (\@ref(eq:niGeometric)). To obtain for the case of prices of
the energetic resources, we slightly change the parameters of the
function `aggregated.index.number()`. We have to introduce the parameter
...
index (\@ref(eq:niHarmonic)). With regard to the previous cases, the
main difference is the parameter `type` that, in this case, has to take
the value `"harmonic"`.
...
The Bradstreet-Dûtot index (\@ref(eq:niBDutot)) is determined in R
software by using `aggregated.index.number()` with the parameter
`type="BDutot"`. We illustrate the case of obtaining the indicated index
for the prices of energetic resources. Again, the output has the same
structure as the previous cases.
...
[9](#tab11100002_2). They involves the usage of `base="chain"`.

**Carli index in R**

The Carli index (\@ref(eq:niCarli)) is obtained in R software through
...
The Jevons index (\@ref(eq:niJevons)) can be determined in R software by
using `aggregated.index.number()` with the parameter `type="Jevons"`.
Again, we illustrate the case of obtaining the indicated index for the
prices of energetic resources. The required code is the one displayed
below:
...
The Dûtot index (\@ref(eq:niDutot)) is determined in R with
...
Next, we enumerate the capabilities of *IndexNumber* package in R
software to determine weighted complex index numbers. The results in
Table [11](#tab1110000b) were also obtained by using the functions of R
that we show below.
...
resources given in Table [7](#tab671110). This information is also
included in the data set `ECResources` of the package *IndexNumber*.
...
obtain the Laspeyres index (\@ref(eq:niLaspeyres)) in R. *IndexNumber*
package allows this through `laspeyres.index.number()` by adding these
as parameters: the matrix of the magnitudes to be evaluated, the matrix
containing the weights, and several options of graphical and numerical
...
evolution can be analyzed as Figure [6](#laspeyres) depicts.

![Figure 6: Laspeyres index evolution for the prices of several energy
resources for the period 2005-2015.](laspeyres_new.png){#laspeyres
width="100%" alt="graphic without alt text"}

**Paasche index in R**

Analogously, the joint evolution of magnitudes under the Paasche index
(\@ref(eq:niPaasche)) can be also determined by using *IndexNumber*
...
(\@ref(eq:niEdgeworth)) is also possible in R software through the use
...
(\@ref(eq:niFisher)). In this case, `fisher.index.number()` provides a
measure of the considered magnitude. Again, the options `opt.plot` and
`opt.summary` allow the obtaining of additional information that may be
...
The complex index for $v$ given in (\@ref(eq:nivalue)) can be easily
obtained as the Bradstreet-Dûtot index for the value in each instant of
time. Recall that the value is obtained as the product of the amount of
good by its price in each instant of time. Thus, the function
...
the *IndexNumber* package provides R users a set of functions to
calculate index numbers. Concretely, this library allows the calculation
of simple index numbers in series and in chain. Furthermore, complex
index numbers are also implemented. In particular, the non-weigthed
index numbers included are the Sauerbeck, Geometric mean, Harmonic mean,
