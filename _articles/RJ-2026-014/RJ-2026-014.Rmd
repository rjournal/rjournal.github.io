...
abstract: |
  ...
  introduce the latest version of onewaytests,
  which offers a wide range of tools conducting analysis for one-way
  independent groups layout. In this study, we make significant
  improvements to the onewaytests package by adding 15 new one-way tests
  to the existing seven tests available in the work done by (Dag et al. 2018).
  With this update, users now have a total of 22
  tests for comprehensive assessments. We completely rewrite the
  function of the graphical approaches from scratch to improve the
...
- name: H. Erkin Sulekli
  ...
  - Sihhiye, Ankara
  - Turkey
  email: erkin.sulekli@gmail.com
- name: Malwane M. A. Ananda
  affiliation: Department of Mathematical Statistics, University of Nevada
  address:
  - Las Vegas, NV
  - USA
  email: malwane.ananda@unlv.edu
- name: Samaradasa Weerahandi
  affiliation: X-Techniques
  address:
  - Edison, NJ
  - USA
  email: weerahandi@aol.com
date: '2026-04-27'
...
[@WELCH:1951], Alexander-Govern test [@alexander1994], James second
order test [@JAMES:1951], Brown-Forsythe test [@brown1974a; @brown1974b]
and Kruskal-Wallis test [@kruskal1952] are provided in
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package version 1.5 [@Dag2017onewaytests]. The
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package is compact and comprehensive since it provides descriptive
statistics, assessment of the assumptions, and summarizes results using
various graphics in addition to performing statistical analysis. This
helps researchers evaluate their data from multiple angles.

In this paper, we present the upgraded version of
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package,
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests) 3.
This version includes Alvandi's *F* test [@sadooghi2012], Alvandi's
generalized p-value test [@sadooghi2012], approximate *F* test
[@asiribo1990coping], adjusted Welch's heteroscedastic *F* test
[@WELCH:1951], B square test [@ozdemir2006one], Box *F* test
[@box1954some], Cochran *F* test [@cochran1937], generalized test
equivalent to parametric bootstrap test [@weerahandi2019], generalized
test equivalent to fiducial test [@weerahandi2019], Johansen *F* test
[@johansen1980welch], modified Brown-Forsythe test
[@mehrotra1997improving], permutation *F* test [@berry2002fisher],
Scott-Smith *F* test [@scott1971interval], Welch-Aspin test
[@aspin1948], and Weerahandi's generalized *F* test [@weerahandi1995] in
addition to the tests in version 1.5. Along with the newly added 15
tests, there are a total of 22 tests for one-way independent groups
designs in this extended version. Additionally, two independent samples
tests including Student's t-test, Welch's t-test, and Mann-Whitney U
test are added to the package.
...
There is a user-friendly web application based on
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package. This web-tool is updated with the addition of 15 one-way
independent groups tests, along with graphical improvements. Also,
web-tool is made compatible with mobile devices. The web-tool is freely
available at [http://www.softmed.hacettepe.edu.tr/onewaytests](url).
...
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests) 3
package is demonstrated by using a real dataset. Third, the upgraded web
application is introduced. At last, general conclusions and further
...
[@sadooghi2012], Alexander-Govern test [@alexander1994], Alvandi's
generalized p-value test [@sadooghi2012], ANOVA, approximate *F* test
[@asiribo1990coping], adjusted Welch's heteroscedastic *F* test
[@WELCH:1951], B square test [@ozdemir2006one], Brown-Forsythe test
[@brown1974a; @brown1974b], Box *F* test [@lix1998trim], Cochran *F*
test [@cochran1937], Weerahandi's generalized *F* test
[@weerahandi1995], James second order test [@JAMES:1951; @JAMES:1954],
Johansen *F* test [@johansen1980welch], Modified Brown-Forsythe test
[@mehrotra1997improving], Permutation *F* test [@berry2002fisher], Scott
Smith *F* test [@scott1971interval], Welch-Aspin test [@aspin1948],
Welch's heteroscedastic *F* test [@WELCH:1951], generalized test
equivalent to parametric bootstrap test [@weerahandi2019], and
generalized test equivalent to fiducial test [@weerahandi2019] test the
null hypothesis $H_0: \mu_1=\mu_2= \ldots =\mu_k$ versus alternative
$H_1:$ at least one $\mu_j$ ($j=1,2,\ldots, k$) is different. All of
these tests are parametric, with the exception of Permutation *F* test.
...
require the assumption of normality [@winkler2014]. Welch's
heteroscedastic *F* test with trimmed means and Winsorized variances
...
shown in Table [1](#tab:T1){reference-type="ref"
reference="oneway22tests"}.

::: {#oneway22tests}
  --------------------------------------------------------------------------------------------------------------------------
  Existing Methods in the [@dag2018onewaytests]                                                       
  ------------------------------------------------------------------------ --------------- ---------- ----------------------
  ...
  **Methods Formulated in the Extension**                                                                 
...
@sadooghi2012 presented a practical test that can be used when the group
variances are heterogeneous. The test statistic for Alvandi's *F* test
is
$$F_{AF}=\displaystyle\sum\limits_{j=1}^k (n_j/S_j^2)(\bar{X}_{j}-\bar{X}_{w})^2,
\label{eq:texor_dummy_1}   (\#eq:texor-dummy-1)$$
where
$$\label{eq:Xw}
 \bar{X}_{w}=\displaystyle\sum\limits_{j=1}^k (n_j/S_j^2)\bar{X_j}/\displaystyle\sum\limits_{j=1}^k (n_j/S_j^2).   (\#eq:Xw)$$
The weights for each group are
$w_j=({n_j/S_j^2})/{\displaystyle\sum\limits_{j=1}^k (n_j/S_j^2)}$. The
null hypothesis is rejected when $F_{AF}\geqslant(k-1) F_{k-1, r}$ where
r is obtained with $r={2m_T}/({m_T-(k-1)})$ and
$m_T={\sum\limits_{j=1}^k (1-w_j)(n_{j}-1)/(n_j-3)}$.

### Alexander-Govern test

@alexander1994 proposed the following test statistic as an alternative
to ANOVA when group variances are not homogeneous:
...
$$\label{eq:wj}
{w}_{j}={(1/S_j^{'2})/}{\sum_{j=1}^{k}(1/S_j^{'2})}.   (\#eq:wj)$$
The null hypothesis is rejected when
$\chi_{AG}^2 \geqslant \chi_{k-1}^2$.

### Alvandi's generalized p-value test

@sadooghi2012 presented a generalized p-value test. Alvandi's
generalized p-value can be computed by simulation according to the
following two steps [@cavus2021testing]. The first step is computing
$T_w$ value (observed value of Cochran's statistic)
$$T_w=\displaystyle\sum\limits_{j=1}^k (n_j/S_j^2)(\bar{X}_{j}-\bar{X}_{w})^2,
\label{eq:texor_dummy_2}   (\#eq:texor-dummy-2)$$
...
\label{eq:texor_dummy_3}   (\#eq:texor-dummy-3)$$
...
\label{eq:texor_dummy_4}   (\#eq:texor-dummy-4)$$
The second step repeats a large number of times (${M}$). Let ${M(t)}$ be
the number of times that $T \geqslant {T_w}$, then Alvandi's generalized
...
$$\label{anova}
F=\frac{\sum\limits_{j=1}^k(n_j)(\bar{X}_{j}-\bar{X}_{..})^2/(k-1)}{\sum\limits_{i=1}^n\sum\limits_{j=1}^k(X_{ij}-\bar{X}_{j})^2/(n-k)}.   (\#eq:anova)$$
...
[@asiribo1990coping] proposed the following test statistic that has
approximately F distribution with $f_{1}$ and $f_{2}$ degrees of
freedom:
$$U_{0}=\frac{{n}{-k}}{k-1}\frac{{\displaystyle\sum\limits_{j=1}^k {n_{j}{(\bar{X}_{j}-\bar{X}_{..})^2}}}{}}{{\displaystyle\sum\limits_{j=1}^k f_jS_j^2}},
\label{eq:texor_dummy_5}   (\#eq:texor-dummy-5)$$
...
follows [@cavus2021testing]:
$$\label{eq:welch_1}	
F_w=\frac{\displaystyle\sum_{j=1}^kw_{j}(\bar{X}_{j}-\sum_{j=1}^kh_{j}\bar{X}_{j})^2}{(k-1)+2(k-2)(1/(k+1))\displaystyle\sum_{j=1}^k(1/n_{j}-1)(1-h_{j})^2}.   (\#eq:welch-1)$$
In Equation \@ref(eq:welch-1),
$w_{j}=n_{j}/((n_{j}-1)/(n_{j}-3)s_j^2)$, and
$h_{j}=w_{j}/\displaystyle\sum_{j=1}^kw_{j}$. The null hypothesis is
rejected when $F_w$ \> $F_{{k-1},  1/ \nu}$ where $\nu$ is obtained with
$v=\frac{(k^2-1)/3}{\displaystyle\sum_{j=1}^k(1-h_{j})^2/(n_{j}-1)}$.

### B square test

[@ozdemir2006one] developed a new and simple approximation procedure
which intends to create an easy and applicable alternative to ANOVA. The
test statistic is
$$B^2=\displaystyle\sum\limits_{j=1}^k[c \lbrace log (1+\frac{(\frac{\bar{X}_j{-X^+}}{S_{\bar{X}_j}})^2}{v_j})\rbrace^\frac 1 2]^2 ,
\label{eq:texor_dummy_6}   (\#eq:texor-dummy-6)$$
where
...
@cochran1937 proposed the following test statistic:
$$\label{eq:Cochran}
T_{Cochran}={\displaystyle\sum\limits_{j=1}^k ({n_j}/{S_j^2})}(\bar{X}_j-\bar{X}_w)^2,   (\#eq:Cochran)$$
...
[@weerahandi2019] showed that the parametric bootstrap test presented by
[@krishnamoorthy2007] is a regular generalized test. The test is carried
out as follows. The null hypothesis is rejected when the generalized
p-value given by the following formula is less than the desired nominal
level, say 0.05:
...
\geq \sum_{j=1}^k \frac{n_j \left(\bar{x}_j - \bar{x}_s \right)^2 }{s_j^2}  \right) ,   (\#eq:Pval)$$
...
[@weerahandi2019] also showed that the fiducial test developed by
[@li2011] is also a regular generalized test. The null hypothesis is
rejected when the generalized p-value given by the formula below is less
than the desired nominal level:

$$\label{Pval2}
p=Pr \left(\sum_{j=1}^k  T_j^2 
-  \frac{ (\sum_{j=1}^k T_j \sqrt[]{n_j} /s_j )^2}
{\sum_{j=1}^k n_j/s_j^2}  \geq \sum_{j=1}^k \frac{n_j \left(\bar{x}_j - \bar{x}_s \right)^2 }{s_j^2}  \right) ,   (\#eq:Pval2)$$
...
is
$$J=\sum_{j}t_j^2,
\label{eq:texor_dummy_9}   (\#eq:texor-dummy-9)$$
...
[@johansen1980welch] proposed the following test statistics:
$$T_{JF}=\frac{\sum_{j=1}^k(w_j(\bar{x}_j-\sum_{j=1}^k(h_j\bar{x}_j))^{2})}{c},
    \label{eq:texor_dummy_10}   (\#eq:texor-dummy-10)$$
where $w_j=n_j/\sigma_j$, $h_j=w_j/\sum\limits_{j=1}^k(w_j)$,
$c=(k-1)+2A-6A/(k+1)$ and
$A=\displaystyle\sum\limits_{j=1}^k({(1-h_j)^{2}}/{(n_j-1})$. The null
hypothesis is rejected when $T_{JF} \geqslant F_{v_{1},v_{2}}$ where
$v_1=k-1$ and $v_2=k-1(k+1)/3A$.

### Kruskal-Wallis test

@kruskal1952 presented the nonparametric alternative to ANOVA. The test
statistics is
...
[@mehrotra1997improving] proposed a modification of Brown-Forsythe test
to overcome the problem of higher than acceptable rate of false
positives. The test statistics is
$$F_{MBF}=\frac{\displaystyle\sum\limits_{j=1}^kn_{j}(\bar{X}_{j}-\bar{X}_{..})^2}{\displaystyle\sum\limits_{j=1}^k (1-\frac{n_j}{n})s_j^2}.
\label{eq:texor_dummy_11}   (\#eq:texor-dummy-11)$$
...
f_2=\frac{[\displaystyle\sum\limits_{j=1}^k (1-{n_j}/{n})\sigma_j^2]^2}{{\displaystyle\sum\limits_{j=1}^k {(1-{n_j}/{n})^2\sigma_i^4}/{(n_j-1)}}}.   (\#eq:mbf-df)$$
The degrees of freedom of the test statistic can be calculated by using
the Equation \@ref(eq:mbf-df).

### Permutation *F* test

[@berry2002fisher] presented a permutational test as an alternative to
ANOVA. The test statistics is

$$F_{PF}=\frac{{T-n\bar{\bar{x}}^2}/{(k-1)}}{{V-T}/{(n-k)}},
\label{eq:texor_dummy_12}   (\#eq:texor-dummy-12)$$
...
$$Y_{ij} =
\begin{cases}
X_{(g_{j}+1)j} & if  X_{ij}  \leq X_{(g_j+1)j} ,\\
X_{ij} & if X_{(g_j+1)j}< X_{ij}< X_{(n_j-g_j)j},\\
X_{(n_j-g_j)j} & if X_{ij}\geq X_{(n_j-g_j)j}.
\end{cases}$$
...
[@weerahandi1995] provided an extension of the classical F-test for the
...
\frac{n_2 s_2^2}{Y_2}, ..., \frac{n_k s_k^2}{Y_k} , 
    \right)
    \label{eq:texor_dummy_15}   (\#eq:texor-dummy-15)$$
...
The [**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package is a comprehensive collection of functions specifically designed
for analyzing data from one-way independent groups designs. This
versatile package provides a wide range of tools to facilitate
...
the work done by [@gorman2014ecological], available in the
[**palmerpenguins**](https://CRAN.R-project.org/package=palmerpenguins)
package [@Horst2020palmerpenguins]. For implementation of the package,
we utilize body mass as a response variable and species as a grouping
variable.

After successfully installing and loading the
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package, researchers have access to a wide range of functions
specifically designed for analyzing one-way independent groups designs.
These functions are incredibly useful when researchers compare k
populations with respect to continuous outcomes. The one-way tests
provided by this package not only calculate the test statistics and
p-values, but they also provide the pairwise comparisons for valuable
insights by evaluating the hypothesis of the statistical process. With
the [**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package, researchers can easily conduct thorough and in-depth analyses
of their one-way independent groups designs, gaining valuable insights
and making well-informed conclusions.

In this part, we use the functions, originally developed in our
previously published work [@dag2018onewaytests] to describe data and
assess the main assumptions, variance homogeneity and normality.

The `describe()` functions return the output involving sample size,
mean, standard deviation, median, minimum value, maximum value, 25th
percentile, 75th percentile, skewness, kurtosis and number of missing
values (NA) for one-way layout.
...
Researchers perform variance homogeneity tests with the `homog.test()`
function. It offers three variance homogeneity tests; Levene, Bartlett,
...
Researchers can assess normality using the `nor.test()` function. This
function offers six normality tests: Shapiro-Wilk, Shapiro-Francia,
Lilliefors (also known as Kolmogorov-Smirnov test), Anderson-Darling,
Cramer-von Mises, and Pearson Chi-square tests.
...
In the [**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package, the first argument is specified as a formula where the
...
In our previously published paper [@dag2018onewaytests], we introduced
this package that included seven one-way tests. In this latest version
of the package, we have expanded its capabilities by adding 15 new
functions specifically designed for one-way independent groups designs.
These additional functions provide users with a wider range of options
for conducting their analyses. The complete list of all the one-way
tests, their corresponding functions, and the method name of the
`onewaytests()` function can be found in Table
[1](#tab:T1){reference-type="ref" reference="oneway22tests"}.

In this section, we introduce a generic function called `onewaytests()`
that encompasses a total of 22 one-way tests. By utilizing this
function, researchers can conveniently apply any of the available
one-way tests. To specify the desired method, researchers simply need to
provide the method argument within the `onewaytests()` function. The
list of all methods is provided under Method in Table
[1](#tab:T1){reference-type="ref" reference="oneway22tests"}.
...
researchers' convenience. The `onewaytests()` function provides four
different types of output. We present each of four kinds to introduce
the outputs. As an example, we have incorporated Alvandi's *F* test,
Scott-Smith test, Weerahandi's generalized *F* test, and James second
order test, which can also be applied using `af.test()`, `ss.test()`,
`wgf.test()`, `james.test()` functions, respectively, to demonstrate the
practical usage of the `onewaytests()` function. The test results can be
displayed by setting the verbose argument to TRUE. The results are
returned as an object of class `htest`.
...
For more comprehensive results, the `summary()` function can be used, as
...
freedom for numerator (`num df`) and denominator (`denom df`). Also,
`p-value` is the significance value of the test statistic. Since the
p-value is lower than 0.05, there is enough evidence to conclude that
the difference between the penguin species in terms of body mass is
statistically significant (F = 318.6896, p-value \< 2.2\^-16).
...
with the degrees of freedom (`df`). Also, `p-value` is the significance
value of the test statistic. Since the p-value is lower than 0.05, one
can conclude that the difference between the species is statistically
significant ($\chi_{AG}^2$ = 639.8695, p \< 2.2\^-16).
...
Here, `p-value` is the significance value of the test statistic. Since
the p-value is lower than 0.05, there is sufficient evidence to conclude
that the difference between the penguin species in terms of body mass is
...
In the output, `Jtest` is the James second order test statistic (*J*),
`criticalValue` is the critical value ($h(\alpha)$) associated to the
significance level ($\alpha$). The null hypothesis is rejected when *J*
exceeds $h(\alpha)$. We conclude that there is a statistically
significant difference between the penguin species since
$\textit{J} = 637.3792$ is larger than $h(\alpha) = 6.108713$.

The [**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package provides the capability to perform pairwise comparisons when a
statistically significant difference is observed. It offers several
options for controlling the type I error rate, including `bonferroni`,
`holm` [@holm1979], `hochberg` [@hochberg1988], `hommel` [@hommel1988],
`BH` [@benjamini1995], `BY` [@benjamini2001], and `none` in `paircomp()`
function. The default is set to \"`bonferroni`\". The development of
`paircomp()` function is extensively placed in the article work done by
[@dag2018onewaytests]. The `paircomp()` function is capable of
recognizing the output of the `onewaytests()` function. In this part, we
include the pairwise comparison following to Alvandi's *F* test to
protect the compactness of the paper.
...
In this part, we rewrite the `gplot` function from scratch. The `gplot`
function offers different graphic types, box-and-whisker plot, violin
plot, box-and-whisker plot with violin lines and error bars. These
options can be selected in the `type` argument with \"boxplot\",
\"violin\", \"boxplot-violin\", and \"errorbar\", respectively. There
exists width argument involving three numeric values. These numeric
values specify the widths for the boxes of box-and-whisker plots
(defaults to 0.3), violin plots (defaults to 1.0), and the little lines
at the tops and bottoms of the error bars (defaults to 0.2),
respectively. Also, there exists the `dots` argument to draw
observations on the plots. The `binwidth` argument offers to change the
size of dots. Users can change the colors of the plots with
`color_manual` argument. Moreover, researchers can set color palette
using `wes_palette` function available in
[**wesanderson**](https://CRAN.R-project.org/package=wesanderson)
package (FantasticFox1 palette is used as a default). Additionally, the
theme of plot can be changed with `theme` argument. The list of all
theme is available with `ggtheme` function available in
[**ggplot2**](https://CRAN.R-project.org/package=ggplot2) package
[@Wickham2016ggplot2]. We use the classic dark-on-light ggplot2 theme as
a default theme by setting `theme_bw()` to `theme` argument. Researchers
can use standard deviation or standard error with `option` argument
while drawing error bars. Also, researchers can add mean bar to error
bars with `add` argument. Additionally, the labels of x and y axes and
title can be changed with `xlab`, `ylab`, and `title` arguments,
respectively.

In this part, we draw some graphic types to illustrate the usage of
`gplot` function. These graphics involve box-and-whisker plot with
violin line and dots (Figure [1](#graph:graphics){reference-type="ref"
reference="graph:graphics"}a), box-and-whisker plot with violin line
(Figure [1](#graph:graphics){reference-type="ref"
reference="graph:graphics"}b), box-and-whisker plot (Figure
[1](#graph:graphics){reference-type="ref" reference="graph:graphics"}c),
violin plot (Figure [1](#graph:graphics){reference-type="ref"
reference="graph:graphics"}d), mean $\pm$ standard deviation graph
(Figure [1](#graph:graphics){reference-type="ref"
reference="graph:graphics"}e), and mean $\pm$ standard deviation graph
with mean bar (Figure [1](#graph:graphics){reference-type="ref"
reference="graph:graphics"}f). These graphics can be obtained via
following codes:
...
<tr>
<td style="text-align: center;"><span>a. Box-and-whisker plot with
violin lines and dots </span></td>
<td style="text-align: center;"><span>b. Box-and-whisker plot with
violin lines</span></td>
</tr>
<tr>
...
<tr>
<td style="text-align: center;"><span>c. Box-and-whisker
plot</span></td>
<td style="text-align: center;"><span>d. Violin plot</span></td>
</tr>
<tr>
<td style="text-align: center;"><img src="graph5.png"
style="width:100.0%" alt="graphic without alt text" /></td>
<td style="text-align: center;"><img src="graph6.png"
style="width:100.0%" alt="graphic without alt text" /></td>
</tr>
<tr>
<td style="text-align: center;"><span>e. Mean <span
class="math inline">±</span> standard deviation graph</span></td>
<td style="text-align: center;"><span>f. Mean <span
class="math inline">±</span> standard deviation graph with
bar</span></td>
</tr>
</tbody>
</table>
<figcaption>Figure 1: Graphical approaches for one-way
layout</figcaption>
</figure>
...
built upon the
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package, providing a convenient and accessible way to perform
statistical analysis. This web-tool has recently undergone significant
updates and enhancements. One of the major improvements is the addition
of 15 one-way independent groups tests, allowing users to conduct a wide
range of statistical comparisons. Furthermore, the web-tool now features
enhanced graphics, providing users with clear and visually appealing
representations of their data. Moreover, we have made the web-tool
compatible with a mobile phone application due to the growing popularity
of mobile devices. This means that users can conveniently access and
utilize the web-tool on their smartphones or tablets, making statistical
analysis more accessible and convenient than ever before. Researchers
can freely access the web application by visiting the following link,
[http://www.softmed.hacettepe.edu.tr/onewaytests](url).

We utilize the [**shiny**](https://CRAN.R-project.org/package=shiny)
package [@Chang2017shiny] to create the web-interface of the
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package. This tool is specifically designed to cater to the needs of new
R users who may not have extensive programming experience, as well as
applied researchers who require a straightforward interface for
...
interpret their data by harnessing the power of the
[**shiny**](https://CRAN.R-project.org/package=shiny) package. There is
an example well-known data set called iris data collected by
[@anderson1935irises] placed in the web-tool, also available in R, to
help users learn the usage of the tool.
...
<tbody>
<tr>
<td style="text-align: center;"><img src="figures/webtool1.png"
style="width:100.0%" alt="graphic without alt text" /></td>
<td style="text-align: center;"><img src="figures/webtool2.png"
style="width:100.0%" alt="graphic without alt text" /></td>
</tr>
<tr>
<td style="text-align: center;"><span>a. Data upload</span></td>
<td style="text-align: center;"><span>b. Describe data</span></td>
</tr>
<tr>
<td style="text-align: center;"><img src="figures/webtool3.png"
style="width:100.0%" alt="graphic without alt text" /></td>
<td style="text-align: center;"><img src="figures/webtool4.png"
style="width:100.0%" alt="graphic without alt text" /></td>
</tr>
<tr>
<td style="text-align: center;"><span>c. One-way tests</span></td>
<td style="text-align: center;"><span>d. Graphics</span></td>
</tr>
</tbody>
</table>
<figcaption>Figure 2: Data analysis interfaces of onewaytests
web-tool</figcaption>
</figure>

Researchers can obtain descriptive statistics using the \"Describe
data\" tab (Figure [2](#fig:webtool){reference-type="ref"
reference="fig:webtool"}b). In this tab, the assumptions of variance
...
[2](#fig:webtool){reference-type="ref" reference="fig:webtool"}c). This
tab also allows users to make pairwise comparisons (Bonferroni, Holm,
Hochberg, Hommel, Benjamini-Hochberg, Benjamini-Yekutieli, no
...
well-designed [**ggplot2**](https://CRAN.R-project.org/package=ggplot2)
graphics (box-and-whisker plot, violin plot, and error bars) in the
\"Graphics\" tab (Figure [2](#fig:webtool){reference-type="ref"
reference="fig:webtool"}d). The web tool is freely available at
http://www.softmed.hacettepe.edu.tr/onewaytests.
...
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests), which
provides a variety of tools for one-way independent groups designs. We
make significant enhancements to the package by introducing 15 new
one-way tests (Scott-Smith test, Box *F* test, Johansen *F* test,
Generalized tests equivalent to Parametric Bootstrap and Fiducial tests,
Alvandi's *F* test, Alvandi's generalized p-value, approximate *F* test,
B square test, Cochran test, Weerahandi's generalized *F* test, modified
...
order test, Kruskal-Wallis test) in the work done by
[@dag2018onewaytests]. Users now have a total of 22 tests available for
...
In this study, we improve the web tool of
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package, ensuring its compatibility with a wide range of mobile devices.
This significant update allows users to effortlessly access and utilize
the web tool on their smartphones or tablets. This mobile compatibility
opens up a world of possibilities, enabling researchers and
statisticians to seamlessly integrate statistical analysis into their
daily routines, regardless of their location or device.

The [**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package also provides additional capabilities to enhance your data
...
Currently, the
[**onewaytests**](https://CRAN.R-project.org/package=onewaytests)
package provides a comprehensive set of tools for conducting analysis on
one-way independent groups designs. These tools include conducting
one-way tests, performing pairwise comparisons, obtaining descriptive
statistics, conducting two-sample tests, utilizing graphical approaches,
and assessing variance homogeneity and normality through the use of
tests and plots. Additionally, it is worth noting that the package and
its associated web-tool will be regularly updated to ensure that users
