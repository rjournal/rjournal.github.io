...
![Figure 1: Frequency of survival model evaluation metrics appearing in
journals such as \"*Annals of Statistics*\", \"*Biometrika*\",
...
from 2010 to 2021.](bothbarline.png){#figbarline width="100%"
...
2021) in Figure [1](#figbarline) and Figure [2](#figpie), we have found
that among these 136 research papers, the popular predictive metrics for
survival models mainly include: concordance index (C-index), Brier score
(BS), integrated Brier score (IBS), integrated absolute error (IAE),
integrated square error (ISE) and mean absolute error (MAE)
[@Harrell.1982; @Brier.1950; @Graf.1999; @Hoora.2017; @Schemper.1992].

![Figure 2: This graphic shows the percentage of metrics appearing from
2010 to 2021 in journals such as \"*Annals of Statistics*\",
\"*Biometrika*\", \"*Journal of the American Statistical Association*\",
\"*Journal of the Royal Statistical Society, Series B*\", \"*Statistics
in Medicine\", \"Artificial Intelligence in Medicine*\", \"*Lifetime
Data Analysis*\". It indicates that the use of C-index is steadily
increasing and it has become a dominant predictive measure recently.
Meanwhile, other metrics do not have an adequate seat in survival
prediction practices.](pie.png){#figpie width="100%"
alt="graphic without alt text"}

From Figure [1](#figbarline), we can see that the two most frequently
used metrics are C-index and IBS, while the other metrics are less used.
This may be partly due to the fact that the available predictive metrics
are scattered across several R packages, and differences in input data
of these tools make it difficult for non-specialists to use or compare
the survival models [@survcomp.2011; @ipred.2009]. Hence, software which
provides easy input data formats and includes most popular metrics is
desirable [@survcomp.2011; @Wang.2019].

Figure [2](#figpie) indicates that the use of C-index is steadily
increasing and it has become a dominant predictive measure recently
[@Jing.2019; @Amico.2021]. On the one hand, this implies that the lack
of handy evaluation metrics tools since other measures are also very
important in survival analysis but do not have an adequate seat in
survival prediction practices [@Nemati.2021; @Li.2021; @ensor.2021]. On
the other hand, it reminds us that providing an accurate C-index measure
which can take various situations into account is extremely important
...
described by [@Ishwaran.2008], \"for sample pairs, where observed time
$X_i = X_j$ and both are deaths, count 1 if predicted outcomes are
tied.\" This error is mainly due to the fact that the above packages
ignore this possible scenario in practice. Omitting such scenarios can
result in the loss of information from survival data, especially when
the sample size is small with high data collection costs. In
*SurvMetrics*, we have implemented the method described by
[@Ishwaran.2008] to cover all tied scenarios. There are also functions
available in the *ipred* package to calculate BS and IBS values, but a
list of survfit objects is required in the calculation process
[@ipred.2009]. However, for most survival models, only a survival
probability vector or matrix is provided, making calculations of such
values a challenging problem [@Ishwaran.2008]. It is not easy for
non-specialists to get IBS using the *ipred* package directly on the
...
and some implementing details. The following Table [1](#tab1) presents
some popular R packages and the metrics they provide.
...
::: {#tab1}
  ...
  *survcomp*       $\checkmark$   $\checkmark$   $\checkmark$       $✗$            $✗$            $✗$

  *ipred*              $✗$        $\checkmark$   $\checkmark$       $✗$            $✗$            $✗$

  *mlr*                $✗$            $✗$            $✗$            $✗$            $✗$        $\checkmark$

  *SurvMetrics*    $\checkmark$   $\checkmark$   $\checkmark$   $\checkmark$   $\checkmark$   $\checkmark$
  ---------------------------------------------------------------------------------------------------------

  : Table 1: R packages that are commonly used to evaluate survival
  predictions.
:::

As can be seen in Table [1](#tab1), the proposed *SurvMetrics* package
implements all popular evaluation metrics, while other packages mostly
implement just one or two. With our *SurvMetrics* package, one can
compare and evaluate different survival models in a convenient and
comprehensive way.
...
As shown in Table [3](#tab3-1res), they return 0 or NA for all cases of
survival data tied. However, in practice, cancer patients are usually
surveyed annually or monthly after surgery for survival status and with
the advent of big data, tied samples are becoming commonplace. Simply
...
1.  The comparable sample pair is defined as:
    $$\label{compairs}
      np_{ij}\left(X_{i}, \delta_{i}, X_{j}, \delta_{j}\right) = \max(I\left(X_{i} \geqslant X_{j}\right) \delta_{j},I\left(X_{i} \leqslant X_{j}\right) \delta_{i}).   (\#eq:compairs)$$

2.  The complete concordance (CC) is defined as:
    $$\label{CC}
      \text{CC} = \sum_{i,j}I({\rm sign}(Y_i,Y_j)={\rm csign}(X_i,X_j)|np_{ij}=1),   (\#eq:CC)$$
    where
    $$\label{sign}
      {\rm sign}(Y_i,Y_j) = I\left(Y_{i} \geqslant Y_{j}\right) -I\left(Y_{i} \leqslant Y_{j}\right)   (\#eq:sign)$$
    and
    $$\label{csign}
      {\rm csign}(X_{i}, \delta_{i}, X_{j}, \delta_{j})=I(X_{i} \geqslant X_{j}) \delta_{j}-I\left(X_{i} \leqslant X_{j}\right) \delta_{i}.   (\#eq:csign)$$

3.  We derive the partial concordance (PC):
    $$\label{PC}
    \begin{aligned}
...
4.  The C-index value can be given by:
    $$\label{CI}
      C\text{-}index =\frac{\text{concordance}}{\text{permissible}} = \frac{CC+0.5\times PC}{\sum_{i,j}np_{ij}(X_i,\delta_i,X_j,\delta_j)},   (\#eq:CI)$$
...
To further illustrate the proposed `Cindex()` function in *SurvMetrics*
...
::: {#tab3-1data}
  ...
  **sample**                  $S_1$   $S_2$   $S_3$   $S_4$   $S_5$   $S_6$   $S_7$   $S_8$
  ...
  : Table 2: A simulation data set which can include all the possible
  sample pairs, where [time]{.sans-serif} and [status]{.sans-serif} are
  the observed survival time and survival status of samples $S_1...S_8$,
...
differences behind these R packages in Table [3](#tab3-1res).
...
::: {#tab3-1res}
  ...
  *SurvMetrics*          0.5            0.5            0.5             1             0.5            0.5            0.5            0.5
  ...
  : Table 3: Differences of C-index calculated by the *Hmisc* package,
  the *survival* package, the *survcomp* package and the *SurvMetrics*
  package.
:::
...
$$\label{BS}
BS(t^*)=\frac{1}{N}\sum_{i=1}^N\left[\frac{(\hat{S}(t^*|z_i))^2}{\hat{G}(X_i)}\cdot I(X_i<t^*,\delta_i=1)+\frac{(1-\hat{S}(t^*|z_i))^2}{\hat{G}(t^*)}\cdot I(X_i\ge t^*)\right],   (\#eq:BS)$$
...
In the `Brier()` function of our *SurvMetrics* R package, $t^*$ is set
to median survival time as default.
...
$$\label{IBS}
  IBS=\frac{1}{\max\limits_i{(X_i)}} \int_0^{\max\limits_i(X_i)}BS(t)dt.   (\#eq:IBS)$$

But the calculation of IBS using `sbrier()` function from the *ipred*
package does not always go smoothly [@ipred.2009] . If a list of
`survfit` objects is incorrectly specified, then the package currently
...
integration to the `IBS()` function, and our program will take care of
...
$$\label{IAE}
  IAE=\int_{t}|S(t)-\hat{S}(t|X)| dt   (\#eq:IAE)$$
and
$$\label{ISE}
  ISE =\int_{t}(S(t)-\hat{S}(t|X))^{2} dt,   (\#eq:ISE)$$
...
non-parametric KM estimator in the `IAEISE()` function and this makes
...
$$\label{MAE}
  MAE=\frac{1}{n} \sum_{i=1}^{N}\left(\delta_{i}\left|Y_{i}-{X}_{i}\right|\right),   (\#eq:MAE)$$
...
in the `MAE()` function. Similar to MSE, the lower the MAE, the higher
...
![Figure 3: This graphic shows the Cox model prediction accuracy based
on three different scenarios via 5 evaluation metrics by the
*SurvMetrics* package. The sample size is 300, the dimension is 25, and
control the censoring rate about 30%. The prediction performance of the
Cox model keeps decreasing in three scenarios, which satisfies the data
generated methods. However, in some complex situations where C-index and
BS cannot distinguish models, IBS, IAE, and ISE, may be useful
alternatives.](simuladataexp.png){#fig3.1 width="100%"
alt="graphic without alt text"}

From Figure [3](#fig3.1), one may see that the prediction performance of
the Cox model keeps decreasing in three scenarios as expected. However,
the difference in terms of C-index and BS are not significant for
Scenario2 and Scenario3 while in terms of IBS, IAE and ISE, a sharp
difference and a clear trend can be observed.
...
In this section, a kidney dataset from the *survival* R package is used
to illustrate the usage of the *SurvMetrics* package using popular
survival models widely used in biostatics and biomedical study.
...
::: {#tab3.2}
  ...
  : Table 4: This table shows the prediction accuracy by using
  *SurvMetrics* package to compare the Cox and RSF models based on the
  kidney data set from *survival* R package. In this case, the values of
  C-index are close, and the values of IAE and ISE are far different.
  When using C-index alone is hard to differentiate the predictive power
  of the two models, alternative measures provided in *SurvMetrics* may
  help give a more accurate result.
:::

![Figure 4: This graphic shows the prediction accuracy by using
*SurvMetrics* package to compare the Cox and RSF models based on the
kidney data set from *survival* R package. In this case, using IBS, IAE
and ISE can give a more clear conclusion that Cox model performs
better.](kidneyres.png){#fig3.2 width="100%"
alt="graphic without alt text"}

The results presented in Table [4](#tab3.2) and Figure [4](#fig3.2) show
that using C-index alone is hard to differentiate the predictive power
of the two models. In this case, alternative measures provided in the
proposed package may help if further evaluation is needed. This result
is also consistent with the simulated scenario mentioned above.
...
the *SurvMetrics* package, six evaluation metrics are present.
Evaluation metrics used in more complicated settings such as
time-dependent AUC for joint modelling of longitudinal and survival
data, and concordance index for competitive risks are still being
developed and will be added to the *SurvMetrics* package in the future.
Meanwhile, we are also working to provide a more user-friendly interface
to facilitate both statistical and non-statistical clinical research
workers in evaluating survival models.
