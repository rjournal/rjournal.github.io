...
In many fields, there is interest in estimating the lagged relationship between an exposure (or treatment) and an outcome. In such cases, the length of the lag or how the exposure effect varies is often unknown. The lagged relationship can take either of two forms. The first form is the association between the exposure at one time point and an outcome distributed across several subsequent times. For example, the impact of advertisement persists beyond that single time point of the investment [@koyck_1954_econDLM;@palda_1965_advertise]
or exposure to an environmental pollutant affects mortality on the same day and each of the following days [@schwartz_distributed_2000]. The second form is an exposure assessed longitudinally and an outcome assessed post-exposure. Examples of this form are maternal exposure to environmental chemicals during pregnancy on birth outcomes and children's health and development [@wilson_2017_bias;@chiu2023prenatal;@hsu_prenatal_2023] and the effect of training and recovery activities over multiple days on athlete wellness [@schliep_2021_wellness]. A popular statistical method to estimate the time-varying association between exposure and outcome is a distributed lag model (DLM).

The DLM regresses a scalar outcome on the exposure measured at preceding time points [@schwartz_distributed_2000;@gasparrini_distributed_2010]. The DLM framework is particularly useful as it allows for estimating time-resolved exposure effects and quantifies the temporal relationship between the exposure and the outcome. Because the repeated measurements of exposure are often correlated, DLMs typically include a smoothing constraint to add temporal structure to the estimated time-specific effects and to regularize the exposure effects in the presence of multicollinearity across the measurements. Constraints include polynomials, splines, and Gaussian processes [@zanobetti_generalized_2000; @warren_spatial-temporal_2012]. Notably, @mork_estimating_2023 introduced an approach for constrained DLM using regression tree structures based on the Bayesian additive regression tree (BART) framework [@chipman2010]. More recent methods have extended the tree structured DLM framework in several directions, including nonlinear exposure-response relationships [@mork_treed_2022], models with lagged interactions among multiple exposures [@mork_estimating_2023], and models with heterogeneous lag effects [@mork_heterogeneous_2023].
...
relationships using splines for the smoothing constraint [@gasparrini_dlnm_2013]. @mork_treed_2022 and @mork_estimating_2023 compare model performance between
...
modification by a single continuous factor [@demateis_penalized_2024] and
...
consider mixtures or heterogeneity [@dempsey2025bayesianvariableselectiondistributed]. Hence, the
...
exposures are in the $T$ time points prior to outcome assessment, as in \@ref(eq:eqnDLMts),
...
autocorrelation within the exposure measurements. @mork_estimating_2023
...
Estimating the function in \@ref(eq:eqnDLMM)
...
high-dimensional parameter space. @mork_estimating_2023 introduced the
treed distributed lag mixture model (TDLMM) that structures the
parameters in \@ref(eq:eqnDLMM) using a DLM tree pair. Figure
...
formulation in \@ref(eq:eqng2) and \@ref(eq:eqng-I). Each DLM tree $\mathcal{T}_{ap}$ is also
associated with exposure $m$, indicated by $S_{ap} = m$. With TDLMM, the
main effect of exposure $m$ at time $t$ in \@ref(eq:eqnDLMM) is
$$\label{eqn:TDLMMmain}
...
exposure $m_2$ at time $t_2$ in \@ref(eq:eqnDLMM)
is
$$\label{eqn:TDLMMint}
...
@mork_treed_2022 introduced the treed distributed lag nonlinear
...
such as fetal sex [@rosa_association_2019]. The set of factors, referred to as
...
@mork_heterogeneous_2023 extended TDLM to the heterogeneous distributed lag
...
in our previous large data analyses with this software [@mork_estimating_2023;@mork_heterogeneous_2023;@mork_treed_2022] we have used upwards of 10,000 burn-in
iterations.
