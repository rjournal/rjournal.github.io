...
All tables and figures can be reproduced with the file `article.R`, distributed with the \pkg{MSTest} package (`inst/examples/article.R`) and also available from the package's public GitHub repository. Because the Monte Carlo procedures are simulation-based, exact reproduction requires the random seed set in `article.R`. On an Apple MacBook with an M1 processor running macOS Sonoma 14.2.1 and 16 GB of RAM, the full script runs in approximately three minutes of elapsed time, which varies with hardware and parallelization settings.
