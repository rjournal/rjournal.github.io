...
one of the poles while the other half are keyed toward the opposite pole [@Savalei:2014;@Vigil-Colet:2020].
...
FA-based control operates at two levels [see @Ferrando:2003]: first, at the level of the factor structure obtained in the
...
Within the general FA modeling, two main approaches exist at present [@Savalei:2014;@delaFuente:2020]. The first is fully confirmatory, and the solution is identified by restricting all the loadings on the additional Acquiescence (ACQ) factor to have the same unit value [@Billiet:2000]. The second is exploratory or semi-confirmatory [@Ferrando:2003]: First, (a) an unrestricted ACQ factor with (possibly) different loadings and (b) an also unrestricted (EFA) direct “content” solution are obtained. Second, the direct content solution is either analytically rotated (fully exploratory solution) or rotated against a specified or semi-specified target (semi-confirmatory solution). The pros and cons of both approaches have been discussed and compared by @Savalei:2014 and @delaFuente:2020. Both studies concluded that the confirmatory approach is more robust and user-friendly than the EFA with target rotation. However, it is also more sensitive to violation of the unit-weight loading assumption for the ACQ factor.

Within the general FA-based controlling procedure, two main approaches exist at present [@Savalei:2014;@delaFuente:2020]. The first is fully confirmatory, and, in it, the structural FA solution is identified by restricting all the loadings on the additional Acquiescence (ACQ) factor to have the same unit value [@Billiet:2000]. The second is exploratory or semi-confirmatory [@Ferrando:2003]: First, an unrestricted ACQ factor with (possibly) different loadings together with a direct (i.e. non-rotated) unrestricted or exploratory (EFA) “content” solution are obtained. Second, the direct content solution is either analytically rotated (in a fully exploratory solution) or rotated against a specified or semi-specified target (in a semi-confirmatory solution). The pros and cons of both approaches have been discussed by @Savalei:2014 and @delaFuente:2020. Both studies concluded that the confirmatory approach is more robust and user-friendly than the semi-confirmatory EFA with target rotation. However, it is also more sensitive to violation of the unit-weight loading assumption for the ACQ factor.
...
where **Z** is an $n \times 1$ random vector of observed item scores;
...
where $\boldsymbol{\Phi}$ is $p\times p$ correlation matrix containing the
...
[see @Joreskog:1969]. Although a restricted solution can be obtained by using different sets of constraints, the most usual consist of imposing an independent-cluster structure [e.g. @McDonald:2000]: each item has only a non-zero loading in one factor, having zero loadings in all the others.
...
parameter values in (2) (see e.g. @DeMars:2014; @Ferrando:2010).
...
cleaned covariance/correlation matrix $\mathbf{R}_{corr}$ in which the
...
$\mathbf{R}_{corr}$ instead of $\mathbf{R}$ . This solution will now fit well,
...
items is not the same at least in one factor [e.g. @Lorenzo-Seva:2009].
...
effects before it is used as input for further structural analyses
...
approach initially proposed by @Billiet:2000. In effect,
...
the first case, and product-moment correlations in the second case
[see e.g. @Ferrando:2013 for further details].
In the
...
partializing, and the resulting content loadings will be biased [see @tenBerge:1999].
Having said that, however, we also note that the strict
...
as $\mathbf{R}_{corr}$ = $\bf{R}$
$- \boldsymbol{\alpha}$$\boldsymbol{\alpha}^{'}$. The $\mathbf{R}_{corr}$
...
alternative ways. The first is to input $\mathbf{R}_{corr}$ specified as a
...
directly because $\mathbf{R}_{corr}$ is a covariance matrix (i.e.
...
the RMSR and GFI as overall measures of misfit [@McDonald:1995],
...
comparative fit with respect to the null independence model (see @Tanaka:1993, for points b and c). These indices are provided in the SIREN
...
the respondents under study [see @Ferrando:2016]. For
...
| CFI = 1 <br> GFI = .998 <br> RMSEA = 0 <br> SRMR = .024 | CFI = .922 <br> GFI = .974 <br> RMSEA = .088 <br> SRMR = .049 |
| *Note: GOF = Goodness of Fit Indices*       |                                               |
Table: (\#tab:tab3) Toy Example Results
...
Table \@ref(tab:tab3) are predictable.
@Ferrando:2010 and @DeMars:2014 can be summarized as
...
above can be viewed as an extension-analysis estimate [e.g. @McDonald:1978] obtained by the method of moments.
...
factor loadings estimates [@Fabrigar:1999]. To help the reader to
...
changing the results. All analyses were conducted with R [@RCore:2024]. The quality of the estimates was assessed using the average bias
$$\label{EQ6}
...
[Table 4](#tab:tab4){reference-type="ref" reference="tab4"} and
[Table 5](#tab:tab5){reference-type="ref" reference="tab5"}). The loadings
...
```{r tab4, echo = FALSE, layout="l-page"}
library(gt)
library(dplyr)
...
# Build the gt table 
content_data %>%
  # Row groupings by Section
  gt(groupname_col = "Section") %>%
  # Rename columns to match your kable call
  cols_label(
    Condition1 = "",
    Condition2 = "",
    Item1 = "1", Item2 = "2", Item3 = "3", Item4 = "4", Item5 = "5",
    Item6 = "6", Item7 = "7", Item8 = "8", Item9 = "9", Item10 = "10",
    Item11 = "11", Item12 = "12", Item13 = "13", Item14 = "14", Item15 = "15",
    Item16 = "16", Item17 = "17", Item18 = "18", Item19 = "19", Item20 = "20"
  ) %>%
  # Add spanning headers for factor 1 and 2  
  tab_spanner(
    label = "factor 1",
    columns = Item1:Item10
  ) %>%
  tab_spanner(
    label = "factor 2",
    columns = Item11:Item20
  ) %>%
  ...
  tab_footnote(
    footnote = "Bal = Balanced; LU = Low Unbalanced; HU = High Unbalanced; EQ = equal; LH = Low Heterogeneity; HH = High Heterogeneity"
  ) %>%
  # Format numbers to 3 decimals (optional, for consistent appearance)
  fmt_number(
    columns = starts_with("Item"),
    decimals = 3
  ) %>%
  # Appearance tweaks to resemble kableExtra table
  tab_options(
    table.font.size = 10,
...
acq_data %>%
  ...
  fmt_number(
    columns = Item1:Item20, decimals = 3
  ) %>%
  tab_options(
    table.font.size = 10,
    row_group.as_column = FALSE,
...
[Table 5](#tab:tab5){reference-type="ref" reference="tab5"}. The bias in
...
(unlike the confirmatory method of @Billiet:2000), these
...
(M = 16.4, S.D. = 1.1) from three previous studies [@Morales-Vives:2018; @Morales-Vives:2020; @Morales-Vives:inpress]. Therefore, further details about this data can be obtained
...
the Psychological Maturity Assessment Scale questionnaire [PSYMAS, @Morales-Vives:2013], which assesses
...
carried out by @Morales-Vives:2013, shows that
...
carried out by @Morales-Vives:2013. All the
...
results by @Morales-Vives:2013. Overall, the
...
.436, as was expected, because the study carried out by @Morales-Vives:2013 already showed that these factors are
...
cons [@Savalei:2014; @delaFuente:2020].
