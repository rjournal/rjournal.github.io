## This file generates Figure 5c of our manuscript. ##
...
dist_sample <- function(n, p) {
    gc <- mvdc(normalCopula(p), rep("norm", 2),
               rep(list(list(mean = 0, sd = 1)), 2))
    return(rMvdc(n, gc))
}
param0 <- 0
param <- c(seq(0.05, 0.95, 0.125), 0.975, 0.9999)
xlab <- expression(rho)
...
saveRDS(pplt, file = "data/fig5c.rds")
