...
The R extension of the markdown language [@Xie2018;@rmarkdown]
enables reproducible statistical reports with
nice typesetting in HTML, Microsoft Word, and LaTeX. Moreover, since
recently [@R, version 4.2], Rs manual pages include
support for mathematical expressions [@Sarkar2022;@Viechtbauer2022],
which is already a big improvement. However, except
for special cases such as regression models [@equatiomatic]
...
documents [@Xie2020] and Shiny Apps [@Chang2022]. 
...
R Markdown to be able to follow this article [@knitr;@rmarkdown],
...
or in a specific environment [@Wickham2019]. The present package
...
the result of the calculation, but instead an R call [see, e.g., @Wickham2019, 
...
function is invoked [see the vignette on custom print methods in @knitr]. 
An R Markdown code chunk with `mathout(term)` thus produces:
...
the order proposed by Murrell and Ihaka [-@murrell2000].
...
and Ihaka [-@murrell2000] for graphical annotations and arrows in R figures.
...
instance, if we consider a function such as `g <- function(...) ...`, 
the name *g* is not transparent to R, because only the function body 
is represented. We can still display functions in the form `head(x) = body` 
if we embed the object to be shown into a call `"<-"(head, body)`.
...
Above, the quoted term is an abbreviation for `call("integrate", quote(sin), ...)`, 
with `sin` being an R symbol, not a function. While the R function 
`integrate()` can handle both symbols and functions, mathml needs the 
...
techniques [e.g., @Wickham2019] to return only the left part an
...
psychology using the features of package mathml. Schwarz [-@schwarz1994] presents
...
on two or more channels. In Schwarz's [-@schwarz1994] model, decision-making
builds on a process of noisy accumulation of information over time
[e.g., @ratcliff2016]. In redundant stimuli, the model assumes a
...
[@miller] with 13 onset asynchronies
...
reader is referred to the original paper [@schwarz1994].
...
for displaying mathematical formulas in R [@murrell2000], but
...
[@green1977], R code is notoriously hard to read, and the poor
legibility of the language is one of the main sources of mistakes. For
illustration, we consider again Equation 10 in Schwarz [-@schwarz1994].
...
[@breqn] is mostly a proof of concept. Moreover, mathml
only works in one direction, that is, it is not possible to translate
from LaTeX or HTML back to R [see @latex2r, for an example].
...
[via pandoc, @pandoc]. The source code of the package is found
...
[rolog](https://CRAN.R-project.org/package=rolog) [@rolog]. If a
version of SWI-Prolog [@swipl] is found on the system,
...
[rswipl](https://CRAN.R-project.org/package=rswipl) [@rswipl].
...
providing an interface to a SWI-Prolog distribution [@swipl] 
...
`lm(EOT ~ T0 + Therapy, data=d)` in mathematical form [see also @equatiomatic], 
...
[pracma](https://CRAN.R-project.org/package=pracma), @pracma], so
