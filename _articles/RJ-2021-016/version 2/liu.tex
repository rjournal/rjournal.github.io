...
Overall AIC:  101.8515 
\end{verbatim}
\code{uTAR} has the following arguments: \code{y} is a vector of observed time seres, \code{p1} and \code{p2} are the AR order of regime 1 and regime 2, respectively, \code{d} is the delay, and \code{thrV} contains the external threshold variable $z_{t}$ which should have the same length as that of \code{y}. For SETAR models, \code{thrV} is not needed, i.e., setting \code{thrV} to NULL. \code{thrQ} determines the lower and upper quantiles to search for threshold value. \code{Trim} defines the lower and upper trimmings to control the minimum sample size in each regime and determine $[\underline{r}, \overline{r}]$ for estimation.  \code{include.mean} is a logical value for including
...
Table \ref{table:SMC} lists functions in \pkg{NTS} that implement the aforementioned SMC procedures. Comparing to the existing R package \pkg{SMC} coming with one generic function, \pkg{NTS} provides various functions for statistical inference and are much more user-friendly.
\begin{table}[h]
\begin{center}
\footnotesize
