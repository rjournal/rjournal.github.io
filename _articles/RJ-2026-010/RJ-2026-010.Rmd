...
- name: Luiz M. Carvalho
  ...
  - USA
  - |
    [luiz.fagundes@fgv.br](luiz.fagundes@fgv.br){.uri}
...
including the power prior [@ibrahim2000power], the normalized power
prior [@duan2006evaluating; @carvalho2021normalized], the Bayesian
hierarchical model, commensurate priors [@hobbs2012commensurate], and
robust meta-analytic predictive priors [@schmidli2014robust].
Unfortunately, software implementations of these methods may be
difficult to come across. Where they do exist, the implementations can
differ so substantially that it becomes troublesome to utilize multiple
packages to compare methods. For example, the
[**RBesT**](https://CRAN.R-project.org/package=RBesT) package
[@weberApplyingMetaAnalyticPredictivePriors2021], which implements the
robust meta-analytic predictive prior, accepts data in an aggregate
format where each row represents a study or group and uses a single
function that accommodates Gaussian, binomial, and Poisson models. In
contrast, the [**NPP**](https://CRAN.R-project.org/package=NPP) package
[@han2021npp], which implements the normalized power prior, provides
separate functions for different outcome types, each requiring a
distinct data format. For instance, `NPP::LMNPP_MCMC()` requires users
to provide vectors of individual-level responses for the current and
historical data, whereas `NPP::BerNPP_MCMC()` requires a vector of two
elements (number of trials and number of successes) for each data set.
...
programming language [@carpenter2017stan], implemented in the
[**cmdstanr**](https://CRAN.R-project.org/package=cmdstanr) package
[@cmdstanr_pkg]. Although
...
package [@landau2023instantiate], which depends on
...
(GLMs) [@mccullagh1989generalized], whose likelihood function is given
by
$$\begin{aligned}
...
\end{aligned}   (\#eq:glm-likelihood-matrix)$$
...
designed to enable users familiar with the `R` programming language
[@rlanguage], particularly its `glm()` function in the
...
::: {#tab:comparison}
  ...
  : (#tab:T1) Features present in various R packages providing
  support for prior elicitation on the basis of historical data.
:::

### Power prior {#sec:pp}

The power prior (PP) of @ibrahim2000power, developed for settings with a
single historical data set, involves discounting the likelihood of the
historical data by a value $a_{01} \in [0, 1]$ (often referred to as the
*discounting parameter*) along with eliciting an initial prior $\pi_0$.
...
\end{aligned}   (\#eq:pp-fixeda0-singledataset)$$
where
...
the prior. @ibrahim2015power provides an overview of how to select
...
\end{aligned}   (\#eq:pp-fixeda0-initialprior)$$
...
[@shen2022bayesppd] via the function `glm.fixed.a0()`.
...
sampling [@neal2003slice] otherwise. Slice samplers are easier to tune
than Metropolis-type samplers, but it is difficult to implement
multivariate versions of slice samplers. As a result, most practical
implementations conduct slice sampling on the full conditional
distributions. Unfortunately, these samplers can be slow to converge and
exhibit poor mixing in high-dimensional or strongly correlated settings
[@neal2003slice; @murrayEllipticalSliceSampling2010; @bloem-reddySliceSamplingHamiltonian2016].
...
### Normalized power prior {#sec:npp}
...
posterior violates the likelihood principle
[@duan2006evaluating; @neuenschwander2009note]. This leads to the
normalized power prior (NPP), which, for multiple historical data sets,
is given by
$$\begin{aligned}
...
simplified version of the two-step approach described by
[@carvalho2021normalized]. The procedure is summarized in Algorithm
...
package [@Gronau2020]. Because bridge sampling cannot be directly
...
often resulting in highly correlated samples
[@robertsOptimalScalingVarious2001]. Moreover, the
[**NPP**](https://CRAN.R-project.org/package=NPP) package uses a Laplace
approximation [@tierney1986accurate] to estimate the normalizing
constant, which is faster than the bridge sampling approach in
[**hdbayes**](https://CRAN.R-project.org/package=hdbayes) but can be
inaccurate in small-sample or high-dimensional settings
[@shunLaplaceApproximationHigh1995]. Finally, each distribution in the
exponential family corresponds to a separate function in
...
### Normalized asymptotic power prior {#sec:napp}

@ibrahim2000power showed that, under large samples of the historical
data set, the PP in \@ref(eq:pp-fixeda0-singledataset) converges to a
multivariate normal density, i.e.,
$$\begin{aligned}
...
the "asymptotic power prior" [@ibrahim2015power]
...
$$J(\boldsymbol{\mathbf{\beta}}, \tau) = \begin{pmatrix}
     \frac{\partial \boldsymbol{\mathbf{\beta}}}{\partial \boldsymbol{\mathbf{\beta}}'} & \frac{\partial \boldsymbol{\mathbf{\beta}}}{\partial \tau} \\
     \frac{\partial \phi}{\partial \boldsymbol{\mathbf{\beta}}'}       & \frac{\partial \phi}{\partial \tau}
  \end{pmatrix}
  %
  = \begin{pmatrix}
...
### Bayesian hierarchical model {#sec:bhm}
...
### Robust meta-analytic predictive prior {#sec:RMAP}

The *robust* meta-analytic predictive (RMAP) prior [@schmidli2014robust]
extends the BHM framework introduced in Section [2.5](#sec:bhm) by
incorporating a vague (non-informative) component to mitigate
prior--data conflict. As shown by @schmidli2014robust, the BHM in
...
Although [@schmidli2014robust] recommends approximating the MAP prior
...
$$\begin{aligned}
    ...
    \label{eq:texor_dummy_1}
\end{aligned}   (\#eq:texor-dummy-1)$$
where
...
### Commensurate prior {#sec:cp}

The commensurate prior (CP) of [@hobbs2012commensurate] is a
hierarchical prior. In the traditional CP, the current data regression
coefficients,
...
$$\begin{aligned}
   ...
   &\beta_j | \beta_{0j}, \tau_j \sim N\left( \beta_{0j}, \tau_j^{-1} \right),
    ...
    %
    \label{eq:comm_hm}
\end{aligned}   (\#eq:comm-hm)$$
Following @hobbs2012commensurate, we elicit a spike-and-slab prior on
the commensurability parameters $\tau_j$. When there is only one
historical data set ($H = 1$), the CP implemented in
[**hdbayes**](https://CRAN.R-project.org/package=hdbayes) corresponds to
the traditional CP of @hobbs2012commensurate.
...
The R packages
[**psborrow**](https://CRAN.R-project.org/package=psborrow)
[@psborrow_pkg] and
[**psborrow2**](https://CRAN.R-project.org/package=psborrow2)
[@psborrow2_pkg] provide propensity score-based implementations of the
CP, with the former relying on
...
### Latent exchangeability prior {#sec:LEAP}

The latent exchangeability prior (LEAP), developed by @alt2023leap,
...
models. [@kass1995bayes] shows that the preference of $M_2$ over $M_1$
can be expressed in terms of the Bayes factor, given by
$$\begin{aligned}
    \text{BF}(D) = \frac{Z_2(D)}{Z_1(D)} = \frac{ \int L_2(\boldsymbol{\mathbf{\theta}}_2 | D) \pi_2(\boldsymbol{\mathbf{\theta}}_2) d\boldsymbol{\mathbf{\theta}}_2 }{ \int L_1(\boldsymbol{\mathbf{\theta}}_1 | D) \pi_1(\boldsymbol{\mathbf{\theta}}_1) d\boldsymbol{\mathbf{\theta}}_1 }, \label{eq:texor_dummy_2}
\end{aligned}   (\#eq:texor-dummy-2)$$
...
### Logistic regression example {#sec:dataanalysis_logistic}

We now illustrate the functionality in
[**hdbayes**](https://CRAN.R-project.org/package=hdbayes) by analyzing
the data on two clinical trials presented in Section 4.2 of
@chen1999prior using logistic regression to model progression to HIV in
a cohort of patients treated with zidovudine (AZT). More details and
full output can be found in the vignette "AIDS-Progression" that
accompanies [**hdbayes**](https://CRAN.R-project.org/package=hdbayes).
The historical data come from the ACTG019 study [@Volberding1990], which
was a double-blind placebo-controlled clinical trial comparing AZT with
a placebo in people with CD4 cell counts below 500. The sample size of
complete observations for this study was $n_0 = 822$. The response
variable (`outcome`) is binary, taking the value 1 for death,
development of AIDS or AIDS-related complex (ARC) and 0 otherwise. The
covariates considered are CD4 cell count (`cd4`), `age`, `treatment` and
`race`.

Our goal is to analyze data from a more recent study, ACTG036
[@Merigan1991], which includes $n=183$ observations. We use the methods
implemented in [**hdbayes**](https://CRAN.R-project.org/package=hdbayes)
...
[@posterior_pkg]. One may utilize functions in the
[**posterior**](https://CRAN.R-project.org/package=posterior) package,
...
[@bayesplot_pkg] to visualize MCMC diagnostics, such as trace plots and
autocorrelation (ACF) plots. This feature allows users to leverage
...
![Figure 1: []{#fig:a0_prior_and_normconst
label="fig:a0_prior_and_normconst"} **Beta prior distribution for
$a_{01}$ and estimated log-normalizing constants as a function of
$a_{01}$ for the AIDS progression logistic regression model.** In panel
A, the vertical dotted line marks the mean value $1/2 (n/n_0)$. Panel B
shows the estimated log-normalizing constant obtained using
Algorithm [1](#alg:npp){reference-type="ref" reference="alg:npp"}. Note
that $\log Z_h(0) = 0$ for all $h$.
](figures/prior_and_normconst.png){#fig:a0_prior_and_normconst
...
![Figure 2: []{#fig:models label="fig:models"} **Coefficient estimates
for the AIDS progression logistic regression model**. For each
coefficient (vertical panels), we show the posterior mean (dot) and the
$95\%$ equal-tailed credible interval (solid vertical line) for each of
the models considered. Horizontal lines (dotted: ACTG036; dashed:
ACTG019) indicate the MLE for each
coefficient.](figures/posterior_estim.png){#fig:models width="100%"
alt="graphic without alt text"}
...
\label{eq:texor_dummy_3}
\end{aligned}   (\#eq:texor-dummy-3)$$

$$\begin{aligned}
\mathcal{M}_2&:  \mu_i = \Phi\left(\boldsymbol{\mathbf{x}}_i'\boldsymbol{\mathbf{\beta}}\right),
\label{eq:texor_dummy_4}
\end{aligned}   (\#eq:texor-dummy-4)$$
...
\@ref(eq:prior-mal), and another to estimate the normalizing constant of
...
$0.1$ is shown. Following [@kass1995bayes], we mark the
...
![Figure 3: []{#fig:link_selection label="fig:link_selection"} **Log
Bayes factor comparing logit (Model 1, $\mathcal{M}_1$) and probit
($\mathcal{M}_2$) links for various values of the discounting parameter
$a_0$**. We show $\log\left(m_1(a_0)/m_2(a_0)\right)$. Horizontal lines
mark the $\log\text{BF}_{12} \geq 0$ (dashed) and
$\log \text{BF}_{12} \geq \log(3)$ (dotted) thresholds.
](figures/a0_logZ.png){#fig:link_selection width="100%"
alt="graphic without alt text"}
::::
