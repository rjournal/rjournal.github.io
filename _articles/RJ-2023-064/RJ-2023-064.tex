...
\begin{figure}
\includegraphics[width=1\linewidth]{tikz/figrelationships} \caption{Relationships between models. Models coloured blue are implemented in \CRANpkg{rater}. DS: Dawid–Skene. LCM: latent class model.}\label{fig:relationships}
\end{figure}
...
\CRANpkg{rater} fit using the \texttt{get\_stanmodel()} function, or convert the
fit into a \texttt{mcmc.list} from the \CRANpkg{coda} package using the
\texttt{as\_mcmc.list()} function. Functions in the \CRANpkg{rstan} and
...
Second, \CRANpkg{rater} implements the functions \texttt{loo()} and \texttt{waic()}
...
\CRANpkg{ggplot2} 3.4.4 (Wickham 2016),
...
\CRANpkg{rstan} 2.32.3 (Stan Development Team 2023).
