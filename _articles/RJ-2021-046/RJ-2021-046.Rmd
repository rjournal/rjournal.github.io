...
kripp.boot of package kripp.boot [@kripp.boot], function krippalpha of
package [icr](https://CRAN.R-project.org/package=icr) [@icr], and
functions krippen.alpha.raw and krippen.alpha.dist of package
[irrCAC](https://CRAN.R-project.org/package=irrCAC) [@irrcac]. However,
these packages fail to provide a number of useful features. In this
article we present package
...
-   conforms to the R idiom by providing S3 methods confint, influence,
    plot, and summary;
...
Section [2](#situate){reference-type="ref" reference="situate"}, we
locate Krippendorff's $\alpha$ among statistical procedures. In
Section [2.1](#genesis){reference-type="ref" reference="genesis"}, we
first develop a special case of Krippendorff's $\alpha$ (call it
$\alpha_\text{\textsc{sed}}$) in a well-known parametric setting, and
then we present $\alpha$ in its most general (i.e., nonparametric) form.
In Section [2.2](#mrpp){reference-type="ref" reference="mrpp"}, we show
that $\alpha$ is a type of multiresponse permutation procedure. In
Section [2.3](#sklarsomega){reference-type="ref"
reference="sklarsomega"}, we generalize $\alpha_\text{\textsc{sed}}$ in
a fully parametric fashion, arriving at Sklar's $\omega$. In
Section [3](#inference){reference-type="ref" reference="inference"}, we
describe our package's bootstrap inference for $\alpha$ and compare the
performance of our procedure to that of two alternative approaches. In
Section [4](#robust){reference-type="ref" reference="robust"}, we
briefly discuss robustness and influence. In
Section [5](#illustrations){reference-type="ref"
reference="illustrations"}, we provide a thorough demonstration of
[krippendorffsalpha](https://CRAN.R-project.org/package=krippendorffsalpha)'s
usage before concluding in Section [6](#summary){reference-type="ref"
reference="summary"}.
...
question. In Sections [2.1](#genesis){reference-type="ref"
reference="genesis"}--[2.3](#sklarsomega){reference-type="ref"
reference="sklarsomega"}, we will show, by examining Krippendorff's
$\alpha$'s place among statistical procedures, that $\alpha$ is, in
fact, a sensible measure of agreement. Also, establishing a context for
$\alpha$ may help practitioners make educated decisions regarding
$\alpha$'s use.

The UML class diagram [@fowler2004uml] shown below in
Figure [1](#fig:kripp){reference-type="ref" reference="fig:kripp"}
provides a conceptual roadmap for our development. Briefly, a special
case of $\alpha$ (which we denote as Alpha(SED) or
$\alpha_\text{\textsc{sed}}$) arises naturally in the context of the
one-way mixed-effects ANOVA model. Alpha(SED) can then be generalized in
a nonparametric fashion to arrive at Krippendorff's $\alpha$ as it has
been presented by @hayes2007answering (see @gwet2015 for development of
nonparametric $\alpha$ in terms of agreement rather than discrepancies);
this nonparametric form of $\alpha$ is a (slightly modified)
multiresponse permutation procedure. Alternatively,
$\alpha_\text{\textsc{sed}}$ can be generalized in a parametric fashion
to arrive at Sklar's $\omega$, a Gaussian copula-based methodology for
measuring agreement.

![A UML class diagram that shows the relationships between
Krippendorff's $\alpha$ and other statistical
procedures.](kripp.png){#fig:kripp}
...
$$\begin{aligned}
\label{intraclass}
\hat{\alpha}&=1-\frac{\widehat{\sigma_\varepsilon^2}}{\widehat{\sigma_\tau^2+\sigma_\varepsilon^2}}=1-\frac{\frac{1}{n_u(n_c-1)}\sum_{i=1}^{n_u}\sum_{j=1}^{n_c}(Y_{ij}-\bar{Y}_{i\text{\raisebox{1pt}{\scalebox{.6}{$\bullet$}}}})^2}{\frac{1}{n_un_c-1}\sum_{i=1}^{n_u}\sum_{j=1}^{n_c}(Y_{ij}-\bar{Y}_{\text{\raisebox{1pt}{\scalebox{.6}{$\bullet$}}}\text{\raisebox{1pt}{\scalebox{.6}{$\bullet$}}}})^2},\end{aligned}$$
where $\bar{Y}_{i\text{\raisebox{1pt}{\scalebox{.6}{$\bullet$}}}}$ and
$\bar{Y}_{\text{\raisebox{1pt}{\scalebox{.6}{$\bullet$}}}\text{\raisebox{1pt}{\scalebox{.6}{$\bullet$}}}}$
denote the arithmetic means for the $i$th unit and for the entire
sample, respectively. The form of this estimator is not surprising, of
course, since it is well-known that assuming Gaussianity leads to
variance estimators involving sums of weighted squared deviations from
sample arithmetic means.

We can eliminate the arithmetic means in
([\[intraclass\]](#intraclass){reference-type="ref"
reference="intraclass"}) by employing the identity
$$\sum_{i=1}^n(x_i-\bar{x}_\text{\raisebox{1pt}{\scalebox{.6}{$\bullet$}}})^2=\frac{1}{2n}\sum_{i=1}^n\sum_{j=1}^n(x_i-x_j)^2.$$
This gives $$\begin{aligned}
\label{intranew}
\hat{\alpha}&=1-\frac{\frac{1}{2n_un_c(n_c-1)}\sum_{i=1}^{n_u}\sum_{j=1}^{n_c}\sum_{k=1}^{n_c}(Y_{ij}-Y_{ik})^2}{\frac{1}{2n_un_c(n_un_c-1)}\sum_{i=1}^{n_u}\sum_{j=1}^{n_c}\sum_{k=1}^{n_u}\sum_{l=1}^{n_c}(Y_{ij}-Y_{kl})^2}.\end{aligned}$$
Now, let $d^2(x,y)=(x-y)^2$, and rewrite
([\[intranew\]](#intranew){reference-type="ref" reference="intranew"})
as $$\begin{aligned}
\label{sed}
\hat{\alpha}&=1-\frac{D_o}{D_e}=1-\frac{\frac{1}{2n_un_c(n_c-1)}\sum_{i=1}^{n_u}\sum_{j=1}^{n_c}\sum_{k=1}^{n_c}d^2(Y_{ij},Y_{ik})}{\frac{1}{2n_un_c(n_un_c-1)}\sum_{i=1}^{n_u}\sum_{j=1}^{n_c}\sum_{k=1}^{n_u}\sum_{l=1}^{n_c}d^2(Y_{ij},Y_{kl})},\end{aligned}$$
...
$\alpha_\text{\textsc{sed}}$. As we mentioned above, this form of
...
Table [1](#distance){reference-type="ref" reference="distance"}). Note
that package
[krippendorffsalpha](https://CRAN.R-project.org/package=krippendorffsalpha)
...
::: {#distance}
  ...
  : Several distance functions that may be appropriate for use in
  Krippendorff's $\alpha$.
:::

## Alpha as a multiresponse permutation procedure {#mrpp}

In any case, ([\[sed\]](#sed){reference-type="ref" reference="sed"}) is
nonparametric for arbitrary $d^2$ since then the estimator
$\hat{\alpha}$ does not usually correspond to a well-defined population
parameter $\alpha$. This more general form of Krippendorff's $\alpha$
is, in fact, a special case of the so-called multiresponse permutation
procedure (MRPP). The MRPPs form a class of permutation methods for
discerning differences among two or more groups in one or more
dimensions [@permutation]. Note, however, that although $\alpha$ can be
viewed as an MRPP (as we are about to show), $\alpha$ has been modified
for the purpose of measuring agreement rather than discerning
differences.
...
be written as $$\delta=\sum_{i=1}^aC_i\theta_i,$$ where $C_i>0$ are
group weights such that $\sum_iC_i=1$;
...
In the preceding sections, we generalized $\alpha_\text{\textsc{sed}}$
in a nonparametric fashion by substituting other notions of distance for
the squared Euclidean distance. Now, we will present a fully parametric
generalization of $\alpha_\text{\textsc{sed}}$, namely, Sklar's $\omega$
[@sklarsomega].
...
Y_i = F_i^{-1}(U_i) & \;\sim\; F_i,\end{aligned}$$ where
$\mathbf{\Omega}$ is a correlation matrix, $\Phi$ is the standard
...
$\alpha_\text{\textsc{sed}}$) is a special case of Sklar's $\omega$, let
the copula correlation matrix $\mathbf{\Omega}$ be block diagonal, where
the $i$th block corresponds to the $i$th unit $(i=1,\dots,n_u)$ and has
a compound symmetry structure. That is,
$$\mathbf{\Omega}= \mathop{\mathrm{diag}}(\mathbf{\Omega}_i),$$ where
$$\mathbf{\Omega}_i = \bordermatrix{ & c_1 & c_2 & \dots & c_{n_c} \cr
...
[irrCAC](https://CRAN.R-project.org/package=irrCAC), also performs well.
By contrast, we found that the bootstrapping procedure recommended by
@alphaboot, which is implemented in packages kripp.boot and
[icr](https://CRAN.R-project.org/package=icr), generally performs rather
poorly, producing intervals that are far too narrow (e.g., 95% intervals
achieve 74% coverage).
...
Table [2](#tab:interpret){reference-type="ref"
reference="tab:interpret"}, but we suggest---as do Krippendorff and
...
krippendorffsalpha: Measuring Agreement Using Krippendorff's Alpha
Coefficient Version 1.1 created on 2021-01-13. copyright (c) 2020-2021,
John Hughes For citation information, type
citation(\"krippendorffsalpha\"). Type help(package =
krippendorffsalpha) to get started.
:::
...
1,2,3,3,2,2,4,1,2,5,NA,3, + NA,3,3,3,2,3,4,2,2,5,1,NA, +
1,2,3,3,2,4,4,1,2,5,1,NA), 12, 4) R\> nominal

\[,1\] \[,2\] \[,3\] \[,4\] \[1,\] 1 1 NA 1 \[2,\] 2 2 3 2 \[3,\] 3 3 3
3 \[4,\] 3 3 3 3 \[5,\] 2 2 2 2 \[6,\] 1 2 3 4 \[7,\] 4 4 4 4 \[8,\] 1 1
2 1 \[9,\] 2 2 2 2 \[10,\] NA 5 5 5 \[11,\] NA NA 1 1 \[12,\] NA 3 NA NA
:::
...
with sample size $n_b=$ 1,000 (argument confint defaults to TRUE, and
control parameter bootit defaults to 1,000). We set control parameter
parallel equal to FALSE because the dataset is too small to warrant
parallelization of the bootstrap computation. Finally, we set argument
verbose equal to TRUE so that a progress bar is shown during the
bootstrap computation. The computation took less than one second.

::: example
R\> set.seed(42) R\> fit.full = krippendorffs.alpha(nominal, level =
\"nominal\", control = list(parallel = FALSE), + verbose = TRUE)

\|++++++++++++++++++++++++++++++++++++++++++++++++++\| 100
:::

As is customary in R, one can view a summary by passing the fit object
to summary.krippendorffsalpha, an S3 method. If krippendorffs.alpha was
called with confint = TRUE, summary displays a 95% confidence interval
by default. The confidence level can be specified using argument
conf.level. In any case, the quantile method [@davison1997bootstrap] is
used to estimate the confidence limits. Any arguments passed to
summary.krippendorffsalpha via $\dots$ are passed on to R's quantile
function. This allows the user to control, for example, how the sample
...
parallel FALSE bootit 1000

Results:

Estimate Lower Upper alpha 0.7429 0.4644 1
:::
...
influence.glm), computes DFBETA statistics [@Young2017Handbook-of-Reg],
...
::: example
R\> fit.sub = krippendorffs.alpha(nominal\[-6, \], level =
\"nominal\", + control = list(parallel = FALSE)) confint(fit.sub)

0.025 0.975 0.6616541 1.0000000
:::

We see that excluding the sixth unit leads to $\alpha\in(0.66,1.00)$.
The new 95% interval was returned by S3 method
confint.krippendorffsalpha, whose level argument defaults to 0.95, in
keeping with R's other confint methods. Note that
confint.krippendorffsalpha, like summary.krippendorffsalpha, passes any
$\dots$ arguments on to the quantile function.

We conclude this example by producing a visual display of our results
(Figure [2](#fig:nominal){reference-type="ref"
reference="fig:nominal"}). The figure was produced via a call to S3
method plot.krippendorffsalpha, which in turn calls hist and abline, and
does not show a kernel density estimate. Function
plot.krippendorffsalpha is capable of producing highly customized plots;
see the package documentation for details. Since $\hat{\alpha}$ is close
to 1 and the dataset is small, the bootstrap distribution is
substantially skewed to the left. Thus, these data provide a textbook
example of the importance of bootstrapping.

::: example
R\> plot(fit.sub, xlim = c(0, 1), xlab = \"Bootstrap Estimates\", main =
\"Nominal Data\", + density = FALSE)
:::

![A plot of the results from our analysis of the nominal data. The
histogram shows the bootstrap sample, the solid orange line marks the
value of $\hat{\alpha}$, and the dashed blue lines mark the 95%
confidence limits.](nominal.pdf){#fig:nominal}
...
Figure [\[fig:interval\]](#fig:interval){reference-type="ref"
reference="fig:interval"}, are 323 pairs of T2\* relaxation times (a
magnetic resonance quantity) for femoral cartilage [@nissi2015t2] in
patients with femoroacetabular impingement
(Figure [3](#fig:fai){reference-type="ref" reference="fig:fai"}), a hip
...
quantitatively assessing cartilage health.
  ...
  ------- ------- ------- ------- ------- ------- --------- ----------- ----------- ----------- ----------- -----------
           $u_1$   $u_2$   $u_3$   $u_4$   $u_5$   $\dots$   $u_{319}$   $u_{320}$   $u_{321}$   $u_{322}$   $u_{323}$
   $c_1$   27.3    28.5    29.1    31.2    33.0    $\dots$     19.7        21.9        17.7        22.0        19.5
   $c_2$   27.8    25.9    19.5    27.8    26.6    $\dots$     18.3        23.1        18.0        25.7        21.7
  ------- ------- ------- ------- ------- ------- --------- ----------- ----------- ----------- ----------- -----------

![An illustration of femoroacetabular impingement (FAI). Top left:
normal hip joint. Top right: cam type FAI (deformed femoral head).
Bottom left: pincer type FAI (deformed acetabulum). Bottom right: mixed
type (both deformities present).](fai.pdf){#fig:fai}
...
::: example
R\> data(cartilage) R\> cartilage = as.matrix(cartilage)
:::
...
Setting argument verbose to TRUE causes the fitting function to display
a progress bar once again. The computation took five seconds to
complete.

::: example
R\> set.seed(12) R\> fit.sed = krippendorffs.alpha(cartilage, level =
\"interval\", verbose = TRUE, + control = list(bootit = 10000, parallel
= TRUE, + nodes = 3))

Control parameter 'type' must be \"SOCK\", \"PVM\", \"MPI\", or \"NWS\".
Setting it to \"SOCK\".

\|++++++++++++++++++++++++++++++++++++++++++++++++++\| 100
:::

A call of function summary.krippendorffsalpha produced the output shown
below.
...
krippendorffs.alpha(data = cartilage, level = \"interval\", verbose =
TRUE, control = list(bootit = 10000, parallel = TRUE, nodes = 3))

Control parameters:

bootit 10000 parallel TRUE nodes 3 type SOCK

Results:

Estimate Lower Upper alpha 0.8369 0.808 0.8648
:::
...
Figure [4](#fig:cartilage){reference-type="ref"
reference="fig:cartilage"} provides a visual display of the cartilage
...
![A plot of the results from our analysis of the cartilage data. The
histogram and kernel density estimate (dotted black curve) show the
bootstrap sample, the solid orange line marks the value of
$\hat{\alpha}$, and the dashed blue lines mark the 95% confidence
limits.](cartilage_sed.pdf){#fig:cartilage}
...
user-defined distance function must deal explicitly with NAs if the data
at hand exhibit missingness. There are no missing values in the
cartilage data, but we illustrate the handling of NA anyway.

::: example
R\> L1.dist = function(x, y) +  + d = abs(x - y) + if (is.na(d)) + d =
0 + d +
:::

Now we call krippendorffs.alpha, supplying our new distance function via
the level argument.

::: example
R\> fit.L1 = krippendorffs.alpha(cartilage, level = L1.dist, verbose =
TRUE, + control = list(bootit = 10000, parallel = TRUE, + nodes = 3))
...
not only our $\alpha_\text{\textsc{sed}}$ analysis but also a Sklar's
$\omega$ analysis that assumed a non-central $t$ marginal distribution
to accommodate slight asymmetry.
...
Estimate Lower Upper alpha 0.6125 0.5761 0.648
:::
