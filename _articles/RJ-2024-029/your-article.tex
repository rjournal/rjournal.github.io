...
\address{Shubo Sun\\
	...
	Coral Gables, FL, USA\\
	\email{sxs3935@miami.edu}}

\address{Zifeng Zhao\\
	University of Notre Dame\\
	Mendoza College of Business\\
	Notre Dame, IN, USA\\
	\email{zifeng.zhao@nd.edu}}

\address{Feiyu Jiang\\
	Fudan University\\
	School of Management\\
	Shanghai, China\\
	\email{jiangfy@fudan.edu.cn}}

\address{Xiaofeng Shao\\
	University of Illinois at Urbana-Champaign\\
	Department of Statistics\\
	Champaign, IL, USA\\
	\email{xshao@illinois.edu}}





\section*{Appendix: Further details of comparison settings}
{We provide a full description of the R packages, functions as well as settings utilized in Table \ref{table:comparison-no-change-point} and \ref{table:comparison-of-methods} in the comparison between SNCP and other change-point detection methods. We note that the same function with the same setting is used across all comparisons for each method.}

{For SNCP, we use the \texttt{SNSeg\_Uni()} function from the \textbf{SNSeg} package, selecting a confidence level of \texttt{confidence = 0.9} and setting the trimming parameter to $\epsilon = 0.05$ for detecting a change in a single mean. For BinSeg and PELT, we apply the \texttt{cpt.mean()} function from the \textbf{changepoint} package. The settings for \texttt{cpt.mean()} are consistent for both BinSeg and PELT, with \texttt{penalty = 'MBIC'} (the Modified Bayes Information Criterion penalty in \cite{zhang2007modified}), \texttt{Q = 5} (the maximum number of change-points to search for), \texttt{minseglen = 1} (the minimum number of observations between two change-points), and \texttt{test.stat = 'Normal'} (assuming a normal distribution for the data). The only difference is that we vary the input parameter \texttt{method} from \texttt{BinSeg} to \texttt{PELT}.}

{For the MOSUM method, we use the \texttt{mosum()} function from the \textbf{MOSUM} package, employing the both-sided MOSUM variance estimator. Specifically, we set \texttt{G = 100} for the moving sum bandwidth and \texttt{eta = 0.4} based on the criterion argument \texttt{criterion = "eta"}, indicating a minimal mutual distance of changes calculated as $G \times \eta = 40$. For the other key input arguments, we set the length of the right summation window \texttt{G.right = G}, \texttt{var.est.method = "mosum"} to specify the MOSUM-based variance estimator from equation (10) of \cite{mosum2021}, and \texttt{alpha = 0.1} for a significance level of 10\%. } 

{Finally, for ECP, we utilize the \texttt{e.divisive()} function from the \textbf{ecp} package. The ECP method is executed with a significance level of \texttt{sig.lvl = 0.05}, a maximum of \texttt{R = 199} random permutations for each iteration of the permutation test, and a minimum distance between two change-points set to \texttt{min.size = 30}. The number of change-point locations to estimate is set to be \texttt{k = NULL} such that all significant estimated change-points are returned.} 
