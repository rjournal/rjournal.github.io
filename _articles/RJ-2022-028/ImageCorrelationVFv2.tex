...
\title{Quantifying Population Movement Using a Novel Implementation of
...
\abstract{%
Movements in imagery captivate the human eye and imagination. They are
also of interest in variety of scientific disciplines that study
spatiotemporal dynamics. Popular methods for quantifying movement in
imagery include particle image velocimetry and digital image
correlation. Both methods are widely applied in engineering and
materials science, but less applied in other disciplines. This paper
describes an implementation of a basic digital image correlation
algorithm in R as well as an extension designed to quantify persistent
movement velocities in sequences of three or more images. Algorithms are
applied in the novel arena of landscape ecology to quantify population
movement and to produce vector fields for easy visualization of complex
movement patterns across space. Functions to facilitate analyses are
available in \CRANpkg{ICvectorfields} \citep{ICvf}. These methods and
functions are likely to produce novel insights in theoretical and
landscape ecology because they facilitate visualization and comparison
of theoretical and observed data in complex and heterogeneous
environments.
}
...
Travelling waves and spiral waves emerge from mathematical models of
...
\CRANpkg{amt} package \citep{amt}, which incorporate the impact of
...
\CRANpkg{IDE} \citep{IDE}, \CRANpkg{deSolve} \citep{deSolve}, and
\CRANpkg{ReacTran} \citep{ReacTran}. These packages are designed
...
In contrast to the R packages and approaches I have cited above, this
...
\CRANpkg{ncf} R package \citep{ncf}. The \CRANpkg{ncf} package relies on
...
Projecting population data from a domain with two spatial coordinates
...
\CRANpkg{ICvectorfields} package \citep{ICvf}, in which the IC is the
abbreviation for Image Correlation. I demonstrate these approaches using
the \CRANpkg{ICvectorfields} package to analyze a simulated data set as
well as the larch budmoth data set provided with the \CRANpkg{ncf} R
...
used in the \CRANpkg{ICvectorfields} R package starting with a standard
...
persistent movement. The \CRANpkg{ICvectorfields} package capitalizes on
...
wrapper package in R called \CRANpkg{fftwtools} \citep{fftwtools}. Input
raster images and raster stacks are read and manipulated using the
\CRANpkg{terra} package \citep{terra}.

\hypertarget{digital-image-correlation}{%
\subsection{Digital image correlation}\label{digital-image-correlation}}

One of the earliest applications of cross-correlation in image analysis
...
\CRANpkg{ICvectorfields}, the \(f\) and \(g\) matrices are padded with
...
\emph{Xcov2D} function in the \CRANpkg{ICvectorfields} package.
...
cross-correlation. In \CRANpkg{ICvectorfields}, shifts to the right and
...
function in \CRANpkg{ICvectorfields} or using a sub-grid of equal sized
regions of interest as in the \emph{DispField} function in
\CRANpkg{ICvectorfields}. Note that all of the functions in
\CRANpkg{ICvectorfields} that use DIC or variations of it, translate
...
correct spatial units is obtained using functions in the \CRANpkg{terra}
...
In STIC, the three dimensional array is first lagged by duplicating it
...
Cross-correlation between the pairs of reference and lagged matrices is
...
\ref{tab:function-table-tex}). Two functions in \CRANpkg{ICvectorfields}
...
in \CRANpkg{ICvectorfields} \citep{ICvf} using an example in which data
...
\CRANpkg{ncf} package \citep{ncf}. The data from the simulated example
are embedded in \CRANpkg{ICvectorfields}. For visualization of results,
the demonstrations call functions in several R packages: These include
\CRANpkg{ggplot2} \citep{ggplot2}, \CRANpkg{ggnewscale}
\citep{ggnewscale}, \CRANpkg{metR} \citep{metR}, and \CRANpkg{terra}
\citep{terra}.
...
The model used to simulate data to test and demonstrate the
functionality of \CRANpkg{ICvectorfields} is a convection-diffusion
...
convention used in \CRANpkg{ICvectorfields}. Note that the
...
the \CRANpkg{ReacTran} R package \citep{ReacTran}, using a finite
...
\CRANpkg{ICvectorfields}.

The data are imported and then converted from table format to a raster
...
To analyze displacement based on a pair of images, I chose to use the
standard implementation of DIC in the \emph{DispField} function of
\CRANpkg{ICvectorfields}. The first two arguments of the
...
The output of \emph{DispField} is in data table format. Because the data
...
plotting functionality in \CRANpkg{ggplot2} with extensions in the
\CRANpkg{ggnewscale} and \CRANpkg{metR} packages.
...
Before proceeding to the next demonstration I will illustrate one of the
...
\citep{Bjornstad2002}. These data are embedded in the \CRANpkg{ncf} R
package.

After loading the \CRANpkg{ncf} package as well as
\CRANpkg{ICvectorfields}, the data can be loaded, converted to a raster
...
The study region covers a large geographic area and so it is likely the
...
Calling \emph{DispFieldSTall} returns a data frame object that is
...
The \CRANpkg{ICvectorfields} R package implements standard Digital Image
...
extension implemented in \CRANpkg{ICvectorfields} in a new arena:
...
methods in \CRANpkg{ICvectorfields} will be useful because they
