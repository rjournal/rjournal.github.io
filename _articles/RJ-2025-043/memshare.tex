...
\subsection{\texorpdfstring{\texttt{SharedObject} baseline}{SharedObject baseline}}\label{sharedobject-baseline}
...
\subsection{R's threading model}\label{rs-threading-model}
...
\subsection{PDE-based Mutual Information}\label{pde-based-mutual-information}
...
\subsection{\texorpdfstring{The \texttt{memshare} API}{The memshare API}}\label{the-memshare-api}
...
\subsection{Shared memory semantics}\label{shared-memory-semantics}
...
\subsubsection{Master session}\label{master-session}
...
\subsubsection{Worker session}\label{worker-session}
...
\subsubsection{Diagnostic tools}\label{diagnostic-tools}
...
\subsubsection{\texorpdfstring{User-friendly wrapper functions for \texttt{apply} and \texttt{lapply}}{User-friendly wrapper functions for apply and lapply}}\label{user-friendly-wrapper-functions-for-apply-and-lapply}
...
\subsection{RNA-seq dataset via FireBrowse}\label{rna-seq-dataset-via-firebrowse}
...
\subsection{Performance and Memory}\label{performance-and-memory}
...
\caption{\label{tab:median-res-tab-static} The benchmark compares four types: \protect\texttt{memshare}, \protect\texttt{SharedObject}, a single-threaded baseline, and a parallel baseline. For \protect\texttt{memshare} and \protect\texttt{SharedObject}, the reported values are the medians over 100 iterations, while the baselines are the result from either a single-threaded R or a simple \protect\texttt{parApply} run using one iteration. Magnitude refers to the matrix size. Entries are given as \emph{Time Consumed (Memory Overhead)}, where time is measured in seconds and memory in megabytes (MB); the memory after call is mentioned in Appendix C.}\tabularnewline
\toprule\noalign{}
...
\caption{\label{tab:amad-res-tab-static} AMAD for the benchmark of \protect\texttt{SharedObject} vs \protect\texttt{memshare}.}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Magnitude
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsection{Application to Feature Selection by Mutual Information using Pareto Density Estimation}\label{application-to-feature-selection-by-mutual-information-using-pareto-density-estimation}
...
\section{Appendix A: code listing of benchmark}\label{appendix-a-code-listing-of-benchmark}
...
\section{Appendix B: ALTREP and shared memory in R}\label{appendix-b-altrep-and-shared-memory-in-r}
...
\section{Appendix C: benchmarking in detail}\label{appendix-c-benchmarking-in-detail}
...
ef{fig:figure1}.}\label{fig:figure1-detail}
\end{figure}
...
\caption{\label{tab:appendix-amad-res-tab-static} AMAD for the benchmark grid for \protect\texttt{SharedObject} and \protect\texttt{memshare} on Windows 10 via Boot Camp.}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Type
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\section{Appendix D: screenshot}\label{appendix-d-screenshot}
