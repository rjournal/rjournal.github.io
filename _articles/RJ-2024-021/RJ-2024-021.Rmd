---
title: 'Pomdp: A Computational Infrastructure for Partially Observable Markov Decision
  Processes'
date: '2025-03-08'
abstract: |
...
- name: Anthony R. Cassandra
  url: https://tonycassandra.com
  email: tony.cassandra@gmail.com
  affiliation: POMDP, Inc
...
[@hahsler:Kamalzadeh:2021] and, in the absence of solvers for R, 
we started the development of the `pomdp` 
...
affected by the agent's behavior [@Puterman1994]. 
...
A POMDP can be modeled as a _belief MDP_ where the underlying Markov model uses belief states instead of the original states of the environment. 
...
[@Astrom1965]. The model was also studied by the operations research community where the acronym POMDP was introduced [@Smallwood1973]. 
More recently, the POMDP framework was adapted for automated planning problems in artificial intelligence [@Kaelbling1998].
...
Reinforcement learning methods typically require observable states and perform a large amount of exploration, where the agent performs sub-optimal actions to learn about the environment. 
 Q-learning and some algorithms are already available in R packages like \CRANpkg{ReinforcementLearning} [@Proellochs2020].
...
* pomdp-solve [@Cassandra2015] is a C program to solve POMDPs using exact and approximate solvers.
* APPL [@APPL2022] provides the fast point-based POMDP solver SARSOP in C++.
* ZMDP software [@ZMDP2009] implements several approximate value iteration algorithms in C++.
* pyPOMDP [@pypomdp2013] is a Python 2.x toolbox for solving POMDPs.
* JuliaPOMDP [@juliapomdp2022] is a set of packages for defining and solving MDPs and POMDPs using the Julia programming language. 

R activity around POMDPs has also picked up with the packages \CRANpkg{sarsop} [@Bottiger2021] and \CRANpkg{pomdpSolve} [@Hahsler2022b] which interface the 
two popular POMDP solver programs APPL and pomdp-solver.

In this paper, we present \CRANpkg{pomdp} [@Hahsler2022] which was co-developed with
\CRANpkg{pomdpSolve} [@Hahsler2022b] to provide R users with 
...
to obtain a policy that is actionable in a primary care setting [@hahsler:Kamalzadeh:2021]. 
...
# Background for partially observable Markov decision processes
...
The used notation follows largely [@Kaelbling1998]. Several variations of
...
each state [@Puterman1994]. Even though
...
This issue is called the *curse of dimensionality* in dynamic programming.
Mundenk [@Mundhenk2000] has shown that finding the optimal policy for POMDPs is, in general, an 
...
More useful algorithms fall into the classes of approximate value iteration and approximate policy iteration [@Cassandra1998; @Hauskrecht2000], which often find good solutions for larger problems.
...
For communication with the solver, the package supports the widely used POMDP [@Cassandra2015] file specification and can use POMDPX files [@APPL2022] via package \CRANpkg{sarsop}. This means that
...
provides an easy-to-install distribution of the well-known fast C implementation of a set of solvers originally developed by one of the co-authors [@Cassandra2015]. 
...
* Approximate value iteration
    ...
    - SARSOP [@Kurniawati2008], Successive Approximations of the
        ...
        R package \CRANpkg{sarsop} [@Bottiger2021]).
...
* sparse matrix representation based on the \CRANpkg{Matrix} package [@Bates2022] for large transition and observation
matrices of low density,
* fast matrix operations,
* fast C++ implementations of loops using \CRANpkg{Rcpp} [@Eddelbuettel2013], and
* parallel execution using \CRANpkg{foreach} [@Microsoft2022].
...
POMDP and estimate beliefs, fast C++ implementations (using \CRANpkg{Rcpp} [@Eddelbuettel2013]) and
support for parallel execution using \CRANpkg{foreach} [@Microsoft2022] are provided.
To represent and visualize policy graphs the widely used and powerful \CRANpkg{igraph} package [@igraph2006]
with its advanced layout options is used. Interactive policy graphs can be produced based on the
\CRANpkg{visNetwork} [@Almende2022]. 
While the package does not directly provide functions to create \CRANpkg{ggplot2} visualizations [@Wickham2016] to 
...
3.  analyze and visualize the results with functions like `reward()`, 
    `plot_policy_graph()`, and `plot_value_function()`.
...
The `POMDP()` creator function has as its arguments the 
...
* A named list of dense or sparse matrices or the keywords `"identity"` and `"uniform"` representing the 
...
* An R function with the same arguments as `T_()`, `O_()` or `R_()` that
...
For example, transition and observation matrices can typically be represented efficiently as sparse matrices and the keywords `uniform` and `identity`, while rewards are typically more compactly specified as a data frame with rows describing the reward for
...
*  Transition probabilities, observation probabilities, and rewards can be accessed using functions ending in `_matrix()`.
   Given an action, a matrix is returned. 
   The user can request a dense or sparse matrix using the logical parameter `sparse`.
   To reduce the overhead associated with representing dense matrices in sparse format, sparse matrices are only returned
   if the density of the matrix is below 50%. The user can also specify `sparse = NULL`,
   which will return the data in the way it was specified by the user (e.g., a data frame). This saves the cost of conversion.
   Functions ending in `_val()` can be used to access individual values directly.
   
To allow a user-implemented algorithm direct access to the data in a uniform way, the function `normalize_POMDP()` can be used to create a 
...
POMDP problems are solved with the function `solve_POMDP()`. This function uses the low-level interface in the companion package 
...
For example, for using the SARSOP solver interfaced in package \CRANpkg{sarsop}, a function `solve_SARSOP()` is provided. 
...
The `solve_POMDP()` and `solve_SARSOP()` functions require a POMDP model and then 
...
The function `solve_POMDP()` returns a solved instance of the POMDP as a list that contains the original 
...
(transaction and observation probabilities). This simulation is implemented in function `simulate_POMDP()`
and includes fast C++ code using \CRANpkg{Rcpp} [@Eddelbuettel2013] and a native R implementation supporting
sparse matrix representation and sparse matrix operations. 
Both implementations support parallelization using \CRANpkg{foreach} [@Microsoft2022] 
...
using function `add_policy()` which provides a convenient way to combine POMDP problem descriptions with compatible policies.
...
We provide a `plot_value_function()` that visualizes the piecewise linear value function giving the reward over the belief space simplex as a line chart for two-state problems. Function `plot_belief_space()`
...
The function `policy()` returns the policy as a table (data frame) consisting of 
...
`policy_graph()` and visualized using the function `plot_policy_graph()`. 
...
`update_belief()`,
`optimal_action()`, and 
`reward()`. Together with the unified accessor functions and the POMDP specifications, 
...
# Toy Example: The Tiger problem

We will demonstrate how to use the package with a popular toy problem called the Tiger Problem [@Cassandra1994].
...
door. The available actions are `listen` for tiger noises or opening a
door (actions `open-left` and `open-right`).
...
list of distribution keywords and a matrix, while for the rewards, a data frame created with the `R_()` function is used. 
The `R_()` function accepts the arguments `action`,
`start.state`, `end.state`,
`observation`, and the reward `value`.
A missing value of `NA` indicates that the reward is valid for any 
state or observation.

The transition model can be visualized as a graph.

```{r, tiger-transition, echo = TRUE, fig.width = 8, fig.height = 5, out.width = "100%", fig.cap="Transition model of the Tiger problem.", fig.alt="A graph showing the transition model of the Tiger problem."}
...
The graph shows that the action `listen` stays with a probability of 1
in the same state (i.e., listening does not move the tiger).  The actions `open-left` and `open-right` lead to
...
## Solving the Tiger problem for an infinite time horizon
...
`reward()` function. For example, the expected reward for a correct
...
```{r tiger-infinite-horizon, echo = TRUE, fig.width = 8, fig.height = 8, out.width = "100%", fig.cap="The policy graph for the converged infinite-horizon solution of the Tiger problem.", fig.alt="A graph showing the infinite-horizon solution of the Tiger problem."}
plot_policy_graph(sol)
```

The function uses the \CRANpkg{igraph} package [@igraph2006] to produce the layout.
...
`estimate_belief_for_nodes()` which uses the solver output and searches along policy trajectories.
...
graph as an interactive HTML widget with movable vertices (see the manual page for `plot_policy_graph()`)
to let the user arrange the graph manually.
Larger policy graphs can also be exported in common formats
like graphML to be displayed and analyzed in large-scale network analysis tools like Gephi [@Jacomy2014]. 
...
```{r tiger-value-function, echo = TRUE, out.width = "100%", fig.cap="The value function for the solution of the converged Tiger problem.", fig.alt="A line chart with the piecewise linear segments of the value funciton."}
...
with two states. A more versatile function is `plot_belief_space()` which can produce
...
```{r tiger-simulation, echo = TRUE, out.width = "100%", fig.cap="Belief states reached in 50 simulated trajectories of horizon 5.", fig.alt="A dot chart showing the distribution of reached belief states."}
...
## Solving the Tiger problem for a finite time horizon
...
algorithm [@Zhang1996; @Cassandra1997].

```{r tiger-finite-horizon-pol, echo = TRUE}
...
```{r tiger-finite-horizon, echo = TRUE, fig.width = 8, fig.height = 8, out.width = "100%", fig.cap="Policy tree for the Tiger problem solved with a horizon of 4 and a uniform initial belief.", fig.alt="A graph showing the finite-horizon solution of the Tiger problem."}
plot_policy_graph(sol)
```
...
```{r tiger-finite-horizon-99, echo = TRUE, fig.width = 8, fig.height = 8, out.width = "100%", fig.cap="Policy tree for the Tiger problem solved with a horizon of 4 and an initial belief of 99 percent that the tiger is to the left.", fig.alt="A graph showing the finite-horizon solution of the Tiger problem."}
...
`r round(reward(sol, belief = c(.99, .01)), 3)` for the optimal policy. 
