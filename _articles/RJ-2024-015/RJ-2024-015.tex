...
\end{itemize}
If one's model is not supported by \CRANpkg{clarify}, one can omit the \texttt{fit} argument and just specify the \texttt{vcov} and \texttt{coefs} arguments, which will draw the coefficients from the distribution named in \texttt{dist} (\texttt{"normal"} by default).
...
Several packages offer methods for computing interpretable quantities from regression models, including \CRANpkg{emmeans} \citep{emmeans2024}, \pkg{margins} \citep{margins2021}, \CRANpkg{modelbased} \citep{modelbased2020}, and \CRANpkg{marginaleffects} \citep{marginaleffectsforthcoming}. Many of the quantities computed by these packages can also be computed by \CRANpkg{clarify}, the primary difference being that \CRANpkg{clarify} uses simulation-based inference rather than delta method-based inference.
\CRANpkg{marginaleffects} offers the most similar functionality to \CRANpkg{clarify}, and \CRANpkg{clarify} depends on functionality provided by \CRANpkg{marginaleffects} to accommodate a wide variety of regression models. \CRANpkg{marginaleffects} also offers simulation-based inference using \texttt{marginaleffects::inferences()} and support for arbitrary user-specified post-estimation functions using \texttt{marginaleffects::hypotheses()}. However, \CRANpkg{clarify} and \CRANpkg{marginaleffects} differ in several ways. The largest difference is that \CRANpkg{clarify} supports iterative building of more and more complex hypotheses through the \texttt{transform()} method, which quickly computes new quantities and transformations from the existing computed quantities, whereas \CRANpkg{marginaleffects} only supports a single transformation and, as of version 0.20.0, cannot use simulation-based inference for these quantities.
