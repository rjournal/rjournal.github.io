...
If one's model is not supported by \CRANpkg{clarify}, one can omit the `fit` argument and just specify the `vcov` and `coefs` arguments, which will draw the coefficients from the distribution named in `dist` (`"normal"` by default).
...
[^6]: Note that if a quantity is named `.b#`, e.g., `.b1`, it can only be referred to using the positional shortcut and not its name. That is, the positional shortcut takes precedence over the names of the quantities.
...
Several packages offer methods for computing interpretable quantities from regression models, including \CRANpkg{emmeans} [@emmeans2024], \pkg{margins} [@margins2021], \CRANpkg{modelbased} [@modelbased2020], and \CRANpkg{marginaleffects} [@marginaleffectsforthcoming]. Many of the quantities computed by these packages can also be computed by \CRANpkg{clarify}, the primary difference being that \CRANpkg{clarify} uses simulation-based inference rather than delta method-based inference.

\CRANpkg{marginaleffects} offers the most similar functionality to \CRANpkg{clarify}, and \CRANpkg{clarify} depends on functionality provided by \CRANpkg{marginaleffects} to accommodate a wide variety of regression models. \CRANpkg{marginaleffects} also offers simulation-based inference using `marginaleffects::inferences()` and support for arbitrary user-specified post-estimation functions using `marginaleffects::hypotheses()`. However, \CRANpkg{clarify} and \CRANpkg{marginaleffects} differ in several ways. The largest difference is that \CRANpkg{clarify} supports iterative building of more and more complex hypotheses through the `transform()` method, which quickly computes new quantities and transformations from the existing computed quantities, whereas \CRANpkg{marginaleffects} only supports a single transformation and, as of version 0.20.0, cannot use simulation-based inference for these quantities.
