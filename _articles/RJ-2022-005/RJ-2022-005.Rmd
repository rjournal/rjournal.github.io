...
are provided in Table [1](#tb:1){reference-type="ref" reference="tb:1"},
which expands on the METACRAN compilation [@metacran]. Although
considerably smaller in scope, a few R packages provide functions for
simulating data with change-points. CPsurv has functionality for
simulating both nonparametric survival data and parametric survival data
from the Weibull change-point distribution but requires existing data as
an argument and only allows for one change-point [@CPsurv].
SimSCRPiecewise simulates data using the piecewise exponential hazard
model within the Bayesian framework, however, this method requires at
least one covariate as an argument [@SimSCRPiecewise].
...
::: {#tb:1}
  ...
  : R packages for simulating time-to-event data
:::
:::
...
$(\tau_k, k=1,\ldots,K)$ can be expressed as $$\begin{aligned}
f(t)=\begin{cases}
...
\end{cases}\label{exponential pdf}\end{aligned}$$ with corresponding
hazard function $$\begin{aligned}
	h(t)=\begin{cases}
	\theta_1 & 0\leq t<\tau_1\\
	...
	\end{cases}
	\label{exponential hazard}\end{aligned}$$ We draw on the work of
[@Walke] in that we use the inverse hazard function to simulate survival
time $t$. [@Walke] uses a baseline hazard and relative hazards to
simulate each time interval between change-points, whereas our
simulation is based on the value of the scale parameter
($\theta_i, i = 1,\ldots, K+1$) corresponding to each interval as
specified by the user. Starting with the relationship between the
...
\end{cases}\end{aligned}$$ where $A=\theta_1\tau_1$,
$B=\theta_2(\tau_2-\tau_1)$, $C=\theta_3(\tau_3-\tau_2)$, and
$D=\theta_4(\tau_4-\tau_3)$. In cpsurvsim, this method of simulating
time-to-event data is considered the CDF method. An end-of-study time
horizon (or maximum measurement time) is specified by the user and all
simulated event times with values greater than the end time are censored
...
\end{cases}
\label{weibull pdf}\end{aligned}$$ with corresponding hazard function
$$\begin{aligned}
	h(t)=\begin{cases}
	...
	\end{cases}\end{aligned}$$ As with the exponential model, event times
can be simulated using the inverse hazard function (shown here with four
change-points) $$\begin{aligned}
H^{-1}(x)=\begin{cases}
...
$A=\frac{\theta_1}{\gamma}\tau_1^{\gamma}$,
...
simulating data are summarized in Table [2](#tb:2){reference-type="ref"
reference="tb:2"}
...
::: {#tb:2}
  ...
  : Summary of functions for simulating data using cpsurvsim
:::
:::
...
::: example
library(cpsurvsim) dta1 \<- exp_cdfsim(n = 50, endtime = 100, theta =
c(0.005, 0.01, 0.05), + tau = c(33, 66)) head(dta1)

time censor 1 100.00000 0 2 85.99736 1 3 78.21772 1 4 71.03138 1 5
100.00000 0 6 82.71520 1

dta2 \<- weib_cdfsim(n = 50, endtime = 100, gamma = 2, + theta =
c(0.0001, 0.0002, 0.0001), tau = c(33, 66)) head(dta2)

time censor 1 11.36844 1 2 100.00000 0 3 81.04904 1 4 100.00000 0 5
71.93590 1 6 56.40275 1
:::
...
::: example
dta3 \<- exp_memsim(n = 50, endtime = 100, theta = c(0.005, 0.01,
0.05), + tau = c(33, 66)) head(dta3)

time censor 1 93.64262 1 2 63.47413 1 3 84.54253 1 4 89.01574 1 5
73.92685 1 6 23.67631 1

dta4 \<- weib_memsim(n = 50, endtime = 100, gamma = 2, + theta =
c(0.0001, 0.0002, 0.0001), tau = c(33, 66)) head(dta4)

time censor 1 59.47848 1 2 100.00000 0 3 62.08739 1 4 100.00000 0 5
100.00000 0 6 100.00000 0
:::
...
study using cpsurvsim. We simulated data with one, two, three, and four
change-points using both the exponential and Weibull distributions. In
our simulation, time $t$ ranged from 0-100 and change-points occurred at
various times within that range. Sample sizes of 50, 100, and 500 were
tested and values of $\theta$ were chosen to demonstrate differences
between the simulation methods when the hazard rate changes (e.g.,
smaller to larger hazard versus larger to smaller hazard). For the
Weibull simulations, we set $\gamma=2$. We conducted 10,000 simulations
of each setting. We are primarily interested in comparing the ability of
these two simulation methods to simulate data with the correct
change-points $\tau_i$. Therefore, we compared how often the estimated
value ($\hat{\tau_i}$) was within a 10% range, in this case
...
change-points (see
Figure [\[fig:expplplots\]](#fig:expplplots){reference-type="ref"
reference="fig:expplplots"}). Sample size, however, had a large impact
on the accuracy regardless of the simulation method. When estimating one
change-point with a sample size of 50, there were a few simulation
templates where less than a third of estimates $\hat{\tau}$ were within
range of the known change-point
(Figure [1](#fig:exp1plplot){reference-type="ref"
reference="fig:exp1plplot"}). In general, accuracy improved as the
...
![One change-point.](exp1_plplot.pdf){#fig:exp1plplot
width="\\textwidth"}

![Two change-points.](exp2_plplot.pdf){#fig:exp2plplot
width="\\textwidth"}

![Three change-points.](exp3_plplot.pdf){#fig:exp3plplot
width="\\textwidth"}

![Four change-points.](exp4_plplot.pdf){#fig:exp4plplot
width="\\textwidth"}

Simulations for the Weibull distribution, however, revealed important
differences in accuracy between the two methods (see
Figure [\[fig:weibplplots\]](#fig:weibplplots){reference-type="ref"
reference="fig:weibplplots"}). Although accuracy of the two methods was
similar when simulating one change-point, there were many cases where
accuracy was very low, even with a sample size of 500
(Figure [5](#fig:weib1plplot){reference-type="ref"
reference="fig:weib1plplot"}). In almost one quarter ($4/18$) of the
...
(Figure [6](#fig:weib2plplot){reference-type="ref"
reference="fig:weib2plplot"}), accuracy of the first change-point was
...
(Figure [7](#fig:weib3plplot){reference-type="ref"
reference="fig:weib3plplot"}). In almost half of the simulation
...
simulating four change-points
(Figure [8](#fig:weib4plplot){reference-type="ref"
reference="fig:weib4plplot"}), for most scenarios there was a large drop
in accuracy at the first and third change-point for the memoryless
method compared to the inverse hazard method. At the second and fourth
change-points, however, there was a drop in accuracy for sample sizes of
50 and 100 whereas most simulation scenarios with a sample size of 500
had similar accuracy between the two methods. Every simulation scenario
for four change-points with a sample size of 500 using the memoryless
method had at least one change-point where the 95% confidence interval
did not include the known value of $\tau_i$. The known value of $\tau_4$
was, however, included in the 95% confidence interval for every scenario
with a sample size of 500.

![One change-point.](weib1_plplot.pdf){#fig:weib1plplot
width="\\textwidth"}

![Two change-points.](weib2_plplot.pdf){#fig:weib2plplot
width="\\textwidth"}

\

![Three change-points.](weib3_plplot.pdf){#fig:weib3plplot
width="\\textwidth"}

![Four change-points.](weib4_plplot.pdf){#fig:weib4plplot
width="\\textwidth"}
...
Figure [\[fig:weibgplots\]](#fig:weibgplots){reference-type="ref"
reference="fig:weibgplots"}. Values of $\gamma$, however, were similar
for both methods except when there were three change-points
(Figure [11](#fig:weib3gplot){reference-type="ref"
reference="fig:weib3gplot"}), in which case the estimates of $\gamma$
...
(Figure [12](#fig:weib4gplot){reference-type="ref"
reference="fig:weib4gplot"}).

![One change-point.](weib1_gplot.pdf){#fig:weib1gplot
width="\\textwidth"}

![Two change-points.](weib2_gplot.pdf){#fig:weib2gplot
width="\\textwidth"}

\

![Three change-points.](weib3_gplot.pdf){#fig:weib3gplot
width="\\textwidth"}

![Four change-points.](weib4_gplot.pdf){#fig:weib4gplot
width="\\textwidth"}
...
An important limitation of the exp_cdfsim and weib_cdfsim functions is
that they only accommodate up to four change-points. While it's possible
to have more than this many change-points in a dataset, it's also
important to make sure that there is a meaningful interpretation for
multiple change-points. Also, cpsurvsim only accommodates type I right
censoring. For the Weibull distribution, $\gamma$ is assumed fixed for
every interval between change-points. In our simulation study, we only
estimated an overall value of $\gamma$ due to convergence issues when
trying to estimate it within each interval between change-points. In an
effort to be concise, the accuracy of the scale parameters $\theta_i$
are not discussed here, however, in some cases this parameter may be of
more interest than the change-point $\tau$. Thus, we briefly discuss
these results in the appendix. Future versions of cpsurvsim could
incorporate additional features such as accommodating informative
censoring.
...
(Figure [\[fig:exptplots\]](#fig:exptplots){reference-type="ref"
reference="fig:exptplots"}). These exceptions were $\theta_2$ in the two
change-point (Figure [14](#fig:exp2tplot){reference-type="ref"
reference="fig:exp2tplot"}) and three change-point models
(Figure [15](#fig:exp3tplot){reference-type="ref"
reference="fig:exp3tplot"}), where the memoryless method with a sample
size of 100 had a much larger proportion of bias. We were only able to
estimate the shape parameters for the four change-point model when the
sample size was 500.

![One change-point.](exp1_tplot.pdf){#fig:exp1tplot width="\\textwidth"}

![Two change-points.](exp2_tplot.pdf){#fig:exp2tplot
width="\\textwidth"}

![Three change-points.](exp3_tplot.pdf){#fig:exp3tplot
width="\\textwidth"}

![Four change-points.](exp4_tplot.pdf){#fig:exp4tplot
width="\\textwidth"}
...
size was large (Figure [17](#fig:weib1tplot){reference-type="ref"
reference="fig:weib1tplot"}). Bias was generally smaller when using the
memoryless method to estimate $\theta_i$ in the two change-point Weibull
model (Figure [18](#fig:weib2tplot){reference-type="ref"
reference="fig:weib2tplot"}). On the other hand, bias was larger when
using the memoryless method to estimate the shape parameter for the
three change-point Weibull model
(Figure [19](#fig:weib3tplot){reference-type="ref"
reference="fig:weib3tplot"}). We were unable to estimate $\theta$ using
the results from the memoryless method for any of the four change-point
simulations.

![One change-point.](weib1_tplot.pdf){#fig:weib1tplot
width="\\textwidth"}

![Two change-points.](weib2_tplot.pdf){#fig:weib2tplot
width="\\textwidth"}

![Three change-points.](weib3_tplot.pdf){#fig:weib3tplot
width="\\textwidth"}
:::
