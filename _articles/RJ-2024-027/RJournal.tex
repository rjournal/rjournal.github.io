...
The appendix shows practical data pre-processing steps that are helpful for easily reading the relevant data files into R and extracting relevant survey data before using the functions from \CRANpkg{GeoAdjust}. 
...
\end{example}
%\caption{Reading DHS data into R.}
%\label{lst:readingDHS}
%\end{listing}
...
r = terra::rast("Nga_ppp_v2c_2015.tif")
\end{example}
%\caption{Data preprocessing: Reading geographical data into R.}
%\label{lst:readingGeo}
%\end{listing}

Once the external data files are read into R, we need to extract the variables of interest. Accordingly, the individual survey answers contained in the data frame \code{individualData} are first subsetted with respect to the age interval that we are interested in (20--49), and then merged with the cluster level information in the data frame \code{clusterData}. The merged data are then aggregated into the cluster centers.
...
\end{example}
%\caption{Extracting target variables and linking to geographic information.}
%\label{lst:mergeData}
%\end{listing}
...
pointsDegrees = st_as_sf(pointsDegrees, coords=c("long","lat"),
                         crs = crs_degrees)
\end{example}
%\caption{Collecting data into a data frame.}
%\label{lst:mainDataFrame}
%\end{listing}
...
\end{example}
%\caption{Final adjustments to geographic information.}
%\label{lst:NigeriaExample4}
%\end{listing}
...
\address{Andrea Riebler\\
  Department of Mathematical Sciences\\
  Norwegian University of Science and Technology\\
  Trondheim, Norway\\
  \email{andrea.riebler@ntnu.no}}

  \address{Geir-Arne Fuglstad\\
  Department of Mathematical Sciences\\
  Norwegian University of Science and Technology\\
  Trondheim, Norway\\
