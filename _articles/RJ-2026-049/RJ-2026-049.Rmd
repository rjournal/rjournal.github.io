---
title: 'SelectionBias: An R Package for Bounding Selection Bias in Causal Estimands'
date: '2026-09-30'
abstract: |
...
draft: no
author:
- name: Stina Zetterstrom
  affiliation: Uppsala University
  address:
  - Department of Statistics
  - Uppsala, Sweden
  url: https://www.statistik.uu.se/?languageId=1
  orcid: 0000-0003-0730-9425
  email: stina.zetterstrom@gmail.com
- name: Ingeborg Waernbaum
  affiliation: Uppsala University
  address:
  - Department of Statistics
  - Uppsala, Sweden
  url: https://www.statistik.uu.se/?languageId=1
  email: ingeborg.waernbaum@statistics.uu.se
  orcid: 0000-0002-4457-5311
type: package
output:
...
[@hernan2004structural]. This systematic error is commonly referred to as 
...
in the study [@lu2022toward]. In an empirical study, it is often of interest to 
...
@huang2015bounding, @greenland2003quantifying, @flanders2019limits, 
@smith2019bounding, @zetterstrom2022selection, @zetterstrom2024,
@zetterstrom2025investigations, and @duarte2024automated for selection bias. 
...
\CRANpkg{SelectionBias} is the bound presented in @smith2019bounding, hereafter
...
[@smith2019bounding;@smith2021multiple]. For these software tools, the user inputs
...
sensitivity parameters as the SV bound [@zetterstrom2025investigations] are also
...
calculating the AF, GAF, and CAF bounds [@zetterstrom2022selection;@zetterstrom2024].
...
pre-treatment covariates. The contents of the \CRANpkg{SelectionBias} package is:
  ...
  * `zika_learner`: a simulated dataset inspired by the study in 
  @de2018association and the zika example in @smith2019bounding. The dataset 
  ...
  * `sharpbound()`: a function that calculates the sharp lower and upper bound
  for the risk ratio or risk difference in either the total or subpopulation for
  sensitivity parameters given by the user, or calculated from
  `sensitivityparametersM()`.
  * `AFbound()`: a function that calculates the AF lower and upper bounds for 
  ...
  * `GAFbound()`: a function that calculates the GAF lower and upper bounds for
  ...
  either given by the user, or calculated from `sensitivityparametersM()`.
...
the bounds. In Section 3, the simulated example dataset `zika_learner` is 
described. In Section 4, the R package \CRANpkg{SelectionBias} is demonstrated 
using the `zika_learner` data. The performance and features of 
...
We assume *K* binary selection variables, $S_1,\dots,S_k,\dots,S_K$, where $S_k$ 
indicates if the subject passes the *k*th selection criterion, from which we 
...
that $Y=TY(1)+(1-T)Y(0)$, i.e. consistency is assumed [@DR74]. Additionally, we 
...
Bounds from the following works are considered; @smith2019bounding, 
@zetterstrom2022selection, @zetterstrom2024, and @zetterstrom2025investigations. 
...
[@zetterstrom2025investigations], referred to as the sharp bounds. However, it 
...
Table: (\#tab:bounds) Summary of the different bounds.
...
`zika_learner`. This is inspired by a case-control study that investigates the 
effect of zika virus on microcephaly [@de2018association] and the numerical zika 
example introduced in @smith2019bounding. The data can be loaded by

```{r, eval=TRUE, echo=TRUE}
data("zika_learner", package = "SelectionBias")
```

The zika example in @smith2019bounding covers the case of a single selection, 
...
selections are the binary variables `birth` and `public hospital`, and the 
...
in order to control for confounding [@de2018association], however, we only 
...
values for the sensitivity parameters in @smith2019bounding. The causal 
...
observational study of zika virus [@lebov2019international]. The variables 
included are:

  * `urban` ($V$). A binary, unobserved variable indicating whether the subject 
  lives in an urban area ($V=1$) or not ($V=0$). The probability of
  living in an urban area is set to 0.85 following numbers from the World Bank [@urban].
  * `ses` ($U$). A binary variable indicating
  socioeconomic status (high vs low), with probability set to 0.5.
  * `zika` ($T$). In 2016 there were approximately 34000 possible cases of zika virus
  during pregnancy [@de2017infection], and there are approximately 2.9 million
  births per year in Brazil [@diniz2019understanding]. The risk of getting zika was
  higher in urban areas [@ali2017environmental]. Thus, the prevalence of zika
  in the simulated dataset is around 1\% with a positive impact of $V$, see
  Table \@ref(tab:dgp) for details.
  * `mic_ceph` ($Y$). The estimated prevalence of microcephaly was 74
  cases per 10000 births and the controlled odds ratio of zika virus on
  microcephaly was estimated at 73.1 [@de2018association]. We mimic these
  ...
  * `birth` ($S_1$). Pregnancies that ended in a live or still birth were
  ...
  [@malta2019abortion]. The prevalence of birth in the simulated dataset is
  ...
  * `hospital` ($S_2$). Births that occurred in public hospitals were
...
Table: (\#tab:dgp) Data generating process for the dataset `zika_learner`. Models
...
The unobserved variables, `urban` ($V$) and `ses` ($U$) are constructed such
that the true sensitivity parameters in our DGP approximately match the assumed
values in @smith2019bounding. In the simulated dataset, the proportions of the
...
Table: (\#tab:total) Proportions and number of subjects for the simulated dataset, by treatment (zika) status and overall.
...
Table: (\#tab:sel1) Proportions and number of subjects for the simulated dataset, by treatment (zika) status and overall, after the first selection.
...
Table: (\#tab:sel2) Proportions and number of subjects for the simulated dataset, by treatment (zika) status and overall, after both selections.
...
<!-- %https://data.worldbank.org/indicator/SP.URB.TOTL.IN.ZS?locations=BR världsbanken om andel i urban Världsbanken siffror om urban vs rural i Brasilien.
% (https://agenciabrasil.ebc.com.br/en/geral/noticia/2020-12/number-births-registered-brazil-down-2019) Antalet födslar i Brasilien.
%(https://www.angloinfo.com/how-to/brazil/healthcare/health-system) public hospitals. -->

# R package SelectionBias

In this section, we describe the package \CRANpkg{SelectionBias}. The different
functions are illustrated using either the simulated dataset `zika_learner`, or
...
<https://cran.r-project.org/package=SelectionBias>, and can be installed and
loaded using:
...
`sensitivityparametersM()`. The input in the function is the model 
...
is `Mmodel`, which indicates whether the models in the M-structure are probit 
(`Mmodel = "P"`) or logit (`Mmodel = "L"`). The ninth and tenth arguments are 
`pY1_T1_S1` and `pY1_T0_S1`. They are the observed probabilities 
...
total population, `whichEst = "RR_tot"`, the DGP is found in Table \@ref(tab:dgp),
...
in the subpopulation [@zetterstrom2025investigations]. If they are not included, 
...
for all sensitivity parameters is `NULL`, and the user must then specify numeric 
values on the sensitivity parameters that are necessary for the bound for the 
chosen estimand. These arguments are also optional in case the argument `sens` 
...
sensitivity parameters is `NULL`, and the user must then specify numeric values
...
`"RD_sub"`). The second argument is `outcome`, where the user inputs 
...
The third argument is `treatment`, where the user inputs either the numeric 
...
is `selection` where the user inputs either the numeric vector with the observed 
selection variable, or the selection probability, $P(I_S=1)$. Its default value 
is `NULL` since it is only required when the causal estimands in the total 
...
$P(Y=1|T=1,I_S=1)$ and $P(Y=1|T=0,I_S=1)$. The sixth argument is `treatment`,
...
$P(T=1|I_S=1)$ and $P(T=0|I_S=1)$. The seventh argument is `selection` where the
...
population, `whichEst = "RR_tot"`. The sensitivity parameters are the 
...
the selection probability, $P(I_S=1)$. Its default value is `NULL` since it is 
only required when the causal estimands in the total population are of interest. 
If the subpopulation is of interest and `selection = NULL`, the outcome and 
...
# Comparison to other software for sensitivity analysis
...
The R packages \CRANpkg{tipr} [@mcgowan2022tipr] and \CRANpkg{sensemakr} 
[@sensemakr2024] solely focus on bias that arises from unmeasured confounding 
...
biases presented in @duarte2024automated are accompanied by a Python package. 
...
them [@ding2017sensitivity;@mathur2020sensitivity]. However, \CRANpkg{EValue} 
...
i.e. a type of Cornfield inequality [@cornfield1959smoking]. 
...
Table: (\#tab:packages) Summary of the features of the different packages.
...
[@smith2019bounding;@zetterstrom2025investigations;@zetterstrom2024;@zetterstrom2022selection].
For users to
become familiar with the functions, the R package also includes a simulated 
dataset, `zika_learner`, and the DGP the dataset is simulated from. The 
...
[@de2018association]. Hopefully, \CRANpkg{SelectionBias} can be a complement to 
...
1. *(Total population estimands $\beta_R$ and $\beta_D$)
	For some unmeasured variable(s) $U$: $Y \perp\hskip -7pt \perp I_S|(T=t,U=u)$, 
	for $t=0,1$.*

2. *(Subpopulation estimands $\beta_{R_S}$ and $\beta_{D_S}$)
	For some unmeasured variable(s) $U$: $Y(t) \perp\hskip -7pt \perp T|(I_S=1,U=u)$, 
	for $t=0,1$.*
