...
The work is organized as follows: Section [2](#definitions) begins with
a brief look of FCA and Simplification Logic. Section [3](#related)
presents other software libraries that implement FCA or related
paradigms' core notions. In Section [4](#design), an explanation of the
data structures, classes and constructor methods is covered.
Section [5](#fcaR) shows how to use the package, describing the
implemented FCA methods and the use of the simplification logic. In
Section [6](#examples), a real application of the package in developing
a recommender system is illustrated. Finally, some conclusions and
future works are presented in Section [7](#conclusions).

# Background on FCA {#definitions}

In this section, we present the basic notions in the FCA framework using
a running example (see Table [1](#tab:fc)). Note that the package
...
the reference [@ganter2016conceptual] for further details of this
framework.
...
::: {#tab:fc}
  ...
  : Table 1: A sample formal context. The attributes are P1 to P4 and
  the objects are named O1 to O4.
:::

A formal context is a triple $(G, M, I)$, where $G$ is a set of objects,
$M$ is a set of attributes and $I$ is a fuzzy relation between objects
and attributes, where $I(x, y) \in [0, 1]$ means the truth value to
which object $x$ possesses attribute $y$, indicating $I(x,y) = 0$ the
...
shown in Table [1](#tab:fc), the object named O4 fully possesses
...
Given a fuzzy set of objects $\mathcal{S}$, we can compute its intent as
the set of attributes that are shared by all objects in $\mathcal{S}$.
Analogously, we define the extent of a set $\mathcal{T}$ of attributes
as the set of objects which have *all* the attributes in $\mathcal{T}$.

In the above example, for $S=${,  }, we have
$\text{intent}(S)=${\^0.5/,  \^0.5/} because
$I(\mathrm{O1}, \mathrm{P2}) = 0.5$,
...
$\phi(\left\{{^{0.5}}\!/\mathrm{P1}\right\}) = \left\{{^{0.5}}\!/\mathrm{P1},\, \mathrm{P2},\, \mathrm{P4}\right\}$,
meaning that *every object* that has P1 with degree at least 0.5, also
has all the attributes {,  }. When $\mathcal{T}$ is closed, the pair
...
context represented in Table [1](#tab:fc), the pair ({,  }, {\^0.5/,  , 
}) is a concept, because
$\text{extent}(\left\{{^{0.5}}\!/\mathrm{P1},\, \mathrm{P2},\, \mathrm{P4}\right\})=\left\{\mathrm{O2},\, \mathrm{O3}\right\}$
and
$\text{intent}(\left\{\mathrm{O2},\, \mathrm{O3}\right\})=\left\{{^{0.5}}\!/\mathrm{P1},\, \mathrm{P2},\, \mathrm{P4}\right\}$,
i.e. a fixpoint is achieved.

In FCA, using these derivation operators, two operations can be used to
reduce a formal context:

-   Clarification, which is the removal of duplicated rows (objects) and
    columns (attributes) of the formal context since duplicates do not
...
The concept lattice of a formal context is the set of all concepts, with
the partial order $\le$ defined as follows: for two concepts
$(A_1, B_1)$ and $(A_2, B_2)$, we say that $(A_1, B_1) \le (A_2, B_2)$
if and only if $A_1 \subseteq A_2 \iff B_2 \subseteq B_1$. For instance,
$\left(\left\{{^{0.5}}\!/\mathrm{O2},\, {^{0.5}}\!/\mathrm{O3}\right\}, \left\{\mathrm{P1},\, \mathrm{P2},\, \mathrm{P4}\right\}\right)  \le \left(\left\{\mathrm{O2},\, \mathrm{O3}\right\}, \left\{{^{0.5}}\!/\mathrm{P1},\, \mathrm{P2},\, \mathrm{P4}\right\}\right)$,
since their intents verify
$\left\{{^{0.5}}\!/\mathrm{P1},\, \mathrm{P2},\, \mathrm{P4}\right\} \subseteq \left\{\mathrm{P1},\, \mathrm{P2},\, \mathrm{P4}\right\}$.
...
Figure [1](#fig:lattice), we find the Hasse diagram corresponding to our
running example.

![Figure 1: []{#fig:lattice label="fig:lattice"}Concept lattice for the
context in Table [1](#tab:fc). Arrows indicate the direction of the
order relationship between concepts.](fig-lattice.png){#fig:lattice
width="100%" alt="graphic without alt text"}

From the concept lattice and the order relationship, we can define
notions as subconcept, superconcept, infimum and supremum of a set of
concepts.
...
The standard context for a given formal context $\mathbb{K}$ is another
formal context $(\mathcal{J}, \mathcal{M}, \le)$, where $\mathcal{J}$
and $\mathcal{M}$ are the sets of join- and meet-irreducible elements of
$\mathbb{K}$ and $\le$ is the partial order defined in the concept
lattice. This standard context has a concept lattice isomorphic to that
of $\mathbb{K}$.
...
deduced. The Duquenne-Guigues basis in our example is given by:
  ...
  ---- --------------------------------------------------------------------------------------------- --------------- -------------------------------------------------------------------
    1:                                                                                 $\varnothing$  $\Rightarrow$  $\left\{{^{0.5}}\!/\mathrm{P2},\, {^{0.5}}\!/\mathrm{P4}\right\}$

    2:                                        $\left\{{^{0.5}}\!/\mathrm{P2},\, \mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P2}\right\}$

    3:                                        $\left\{\mathrm{P2},\, {^{0.5}}\!/\mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P4}\right\}$

    4:                         $\left\{\mathrm{P2},\, {^{0.5}}\!/\mathrm{P3},\, \mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P3}\right\}$

    5:   $\left\{{^{0.5}}\!/\mathrm{P1},\, {^{0.5}}\!/\mathrm{P2},\, {^{0.5}}\!/\mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P2},\, \mathrm{P4}\right\}$

    6:          $\left\{{^{0.5}}\!/\mathrm{P1},\, \mathrm{P2},\, \mathrm{P3},\, \mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P1}\right\}$
  ---- --------------------------------------------------------------------------------------------- --------------- -------------------------------------------------------------------
...
`[Ref]` $\displaystyle\frac{A\supseteq B}{A \Rightarrow B}$
...
implications with numbers 2, 3, 4, 5 and 6 are simplified to:
  ...
  ---- -------------------------------------------------------- --------------- ------------------------------
    2:                             $\left\{\mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P2}\right\}$

    3:                             $\left\{\mathrm{P2}\right\}$  $\Rightarrow$  $\left\{\mathrm{P4}\right\}$

    4:   $\left\{{^{0.5}}\!/\mathrm{P3},\, \mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P3}\right\}$

    5:                  $\left\{{^{0.5}}\!/\mathrm{P1}\right\}$  $\Rightarrow$  $\left\{\mathrm{P4}\right\}$

    6:   $\left\{{^{0.5}}\!/\mathrm{P1},\, \mathrm{P3}\right\}$  $\Rightarrow$  $\left\{\mathrm{P1}\right\}$
  ---- -------------------------------------------------------- --------------- ------------------------------
...
in Algorithm [1](#algo:closure).

<figure id="algo:closure">
<img src="alg/algoclosure.png" style="width:100.0%"
alt="graphic without alt text" />

<figcaption>Algorithm 1: <span
class="math inline"><em>S</em><em>L</em><sub><em>F</em><em>D</em></sub></span>
closure</figcaption>
</figure>

For instance, the closure of $S = \left\{{^{0.5}}\!/\mathrm{P2}\right\}$
with this algorithm is
$S^+ = \left\{{^{0.5}}\!/\mathrm{P2},\, {^{0.5}}\!/\mathrm{P4}\right\}$
...
of implications $\Gamma'$:
  ...
  ---- -------------------------------------------------------------------------------------- --------------- ---------------------------------------------
    1:                                                           $\left\{\mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P2}\right\}$

    2:                                                           $\left\{\mathrm{P2}\right\}$  $\Rightarrow$  $\left\{\mathrm{P4}\right\}$

    3:                  $\left\{\mathrm{P2},\, {^{0.5}}\!/\mathrm{P3},\, \mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P3}\right\}$

    4:                                                $\left\{{^{0.5}}\!/\mathrm{P1}\right\}$  $\Rightarrow$  $\left\{\mathrm{P2},\, \mathrm{P4}\right\}$

    5:   $\left\{{^{0.5}}\!/\mathrm{P1},\, \mathrm{P2},\, \mathrm{P3},\, \mathrm{P4}\right\}$  $\Rightarrow$  $\left\{\mathrm{P1}\right\}$
  ---- -------------------------------------------------------------------------------------- --------------- ---------------------------------------------
...
usability, see the
[*tidyverse*](https://CRAN.R-project.org/package=tidyverse) of
@tidyverse.
...
[*frbs*](https://CRAN.R-project.org/package=frbs) package [@frbs]
presents an implementation of various learning algorithms based on fuzzy
rule-based systems (FRBSs) for dealing with classification and
regression tasks. The
[*RKEEL*](https://CRAN.R-project.org/package=RKEEL) package [@rkeel] is
an R interface to KEEL, a popular Java software for a large number of
...
In Table [2](#tab:comparison), we present a comparison of the main
...
::: {#tab:comparison}
  ...
  : Table 2: Functionality comparison among different software libraries
  for FCA. (1) The rules can be used only for classification or
  regression; (2) *fcaR* is integrated with *arules* to import and
  export exact association rules; (3) some packages do not use natively
  fuzzy sets as representation, but use scaled or discretized contexts.
:::
...
programming, the need to be integrated with the *arules* package, and
its use in reproducible research.
...
The *R6* OOP paradigm is being increasingly used in the R ecosystem
...
implications. The main classes are presented in Table [3](#tab:classes).
Let us briefly describe the internal representation of the main object
classes.
...
are presented in Table [4](#tab:methodsFC).
...
::: {#tab:methodsFC}
  ...
  `standardize()`         Builds the standard context $(\mathcal{J}, \mathcal{M}, \le)$ for the given `"FormalContext"`
  ...
  `to_transactions()`     Converts the formal context to an object of class `"transactions"` from the *arules* package
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------
...
`fc$implications`, respectively. Initially, these instances are empty
(no concepts nor implications are included), and they are filled with
the corresponding data as needed.
...
summarized in Table [5](#tab:methodsCL).
...
described in Table [6](#tab:methodsIS).
...
::: {#tab:methodsIS}
  ...
  `to_arules()`          Converts a binary `"ImplicationSet"` to the `"rules"` class of *arules*
  ----------------------------------------------------------------------------------------------------------------------------------------------------------
...
the binary incidence matrix. Also, *fcaR* can export the
`"FormalContext"` to the `"transactions"` format, by using the
`to_transactions()` method of a binary `"FormalContext"`.

Analogously, a `"rules"` object can be used to create a
`"ImplicationSet"`. The LHS and RHS of the `"rules"` object are mapped
to the `lhs_matrix` and `rhs_matrix` of the `"ImplicationSet"`. In
addition, the method `to_arules()` can be used to convert any binary
`"ImplicationSet"` to the `"rules"` format.

These interfaces allow the users who usually work in other areas with
*arules* to use FCA methods and algorithms for transactions databases,
as an extension of what can do in *arules*, and then, if necessary, to
...
Another of the design goals of the *fcaR* package is that it could be
used in research and used in publications. All the implemented classes
in *fcaR* provide a `to_latex()` function so that concepts, implications
and the table of the formal context can be easily exported to LaTeX.
Also, the `"FormalContext"` and `"ConceptLattice"` classes have a
`plot()` method that allows to graphically represent that object types.
This `plot()` function allows to export the graphs in LaTeX format to be
...
mentioned in Section [2](#definitions). We will use the same formal
context shown in Table [1](#tab:fc).
...
`fc$attributes` and `fc$objects`, respectively. We can even plot a
heatmap (see Fig. [2](#fig:heatmap)) to show the sparsity of the
context, very useful for large contexts.

![Figure 2: The heatmap representing
the formal context shown in Table [1](#tab:fc), obtained by doing
`fc$plot()`. A grey scale is used, where white indicates the value 0 for
an object-attribute pair, while black indicates the value 1, with the
greys representing the intermediate values. This type of graph aids
visual inspection of the context, being able to check its sparsity,
density or possible patterns.](fig-context-heatmap.png){#fig:heatmap
...
them: `intent()`, `extent()` and `closure()`. They can be applied on
objects of type `"Set"`, representing fuzzy sets of objects or
attributes:
...
In addition, we can perform *clarification* on the formal context, by
using `fc$clarify()`, giving:
...
The command to compute the concept lattice for a `"FormalContext"` `fc`
is `fc$find_concepts()`. The lattice is stored in `fc$concepts`, which
is of the `"ConceptLattice"` class.
...
In addition, the user can compute concepts' support (the proportion of
objects whose set of attributes contains the intent of a given concept)
by means of `fc$concepts$support()`.
...
use the `sublattice()` function.

For instance, to build the sublattice generated by the concepts
$\left(\left\{{^{0.5}}\!/\mathrm{O1},\, {^{0.5}}\!/\mathrm{O4}\right\}, \left\{\mathrm{P2},\, \mathrm{P3},\, \mathrm{P4}\right\}\right)$
and
$\left(\left\{\mathrm{O2},\, \mathrm{O3}\right\}, \left\{{^{0.5}}\!/\mathrm{P1},\, \mathrm{P2},\, \mathrm{P4}\right\}\right)$,
...
such as `fc$concepts$sublattice(fc$concepts$support() > 0.5)`.
...
`subconcepts()` functions. Analogously, the `superconcepts()` function
can be used to compute the set of $(C_i, D_i)$ such that
$(A, B) \le (C_i, D_i)$.

For instance, if we take a sample concept $L=$ ({\^0.5/,  ,  ,  \^0.5/},
...
its infimum and supremum using the `supremum()` and `infimum()` methods
of the `"ConceptLattice"` object. Additionally, irreducible elements in
the lattice, with respect to join (supremum) and meet (infimum), can be
computed for a given concept lattice with the methods
`join_irreducibles()` and `meet_irreducibles()`.
...
`fc_std <- fc$standardize()` to save the new context to another
variable.

This is a method that applies to a `"FormalContext"` object, but is
closely related to its associated `"ConceptLattice"`, since the objects
and attributes of the newly created standard context are the meet- and
join- irreducible elements of the concept lattice.

The standard context for the example in Table [1](#tab:fc) and its
corresponding lattice are shown in Table [7](#tab:std).
...
::: {#tab:std}
  ...
  : Table 7: Left: Standard context associated to the context in
  Table [1](#tab:fc); M1 to M5 refer to the meet-irreducible elements of
  the formal context, and J1 to J5, to the join-irreducible ones. Right:
  concept lattice of the standard context, isomorphic to the one in
  Figure [1](#fig:lattice).
:::

::: {#tab:std}
  ----------------------------------------------------------------------------
   ![image](fig-std-lattice.png){width="100%" alt="graphic without alt text"}
...
of [NextClosure]{.smallcaps} has been implemented in C and made
accessible from *fcaR* thanks to an interface using *Rcpp*. As we have
...
by using the [NextClosure]{.smallcaps} algorithm, the command
`fc$find_implications()` is executed, and the result is stored in
`fc$implications`.

The set of implications is stored as an object of class
`"ImplicationSet"`, which can be inspected with the `print()` method or
simply with `fc$implications`. In order to get a subset of the
implications by providing its indexes, the standard R subsetting with
`[` will create another `"ImplicationSet"` with exactly the rules
required. For instance, `fc$implications[1:3]` gives:
...
These implications can be converted to LaTeX using the `to_latex()`
method, giving:
...
On the other hand, one can `filter()` the implications to retrieve just
those with specific attributes in the LHS and RHS. As before, the method
returns a new `"ImplicationSet"` object. For example, to get the
implications with attributes P1 and P2 in the LHS and attribute P4 in
the RHS, the user can execute
`fc$implications$filter(lhs = c(’P1’, ’P2’), rhs = ’P4’)`.

Some quantities can be computed from a set of implications:

-   Cardinality, that is, the number of implications, with
    `fc$implications$cardinality()`.

-   Size, which is the cardinality of the LHS and RHS of *each*
    implication, interpreted as fuzzy sets (thus non-integer sizes can
    appear): `fc$implications$size()`.

-   Support, the proportion of objects in the formal context whose set
    of attributes is a *superset* of the LHS of each implication:
...
In *fcaR*, the simplification logic has been implemented and made
accessible from an `"ImplicationSet"` object through method
`apply_rules`.

The list of equivalence rules applicable to an `"ImplicationSet"` is
stored in a `registry` object from the
[*registry*](https://CRAN.R-project.org/package=registry) package by
@registry.
...
we can use `comp` instead of `Composition` in the above command to
obtain the information about the composition rule.

The registry's use enables the user to extend the functionality provided
by the *fcaR* package. One can define and implement new equivalences
...
right-hand sides of the implications, `attributes` is the character
vector of attribute names, and returns the modified `LHS` and `RHS`.
This mechanism has been used in the package to implement additional
equivalence rules.
...
`"ImplicationSet"` if `reduce = TRUE` and only the closure if
`reduce = FALSE` (default). For instance, we can create a fuzzy set
where the attribute `P2` is present with:
...
*fcaR* package to perform medical diagnosis. We have focused on
...
are coded as fuzzy and graded attributes with values 0, $\frac{1}{6}$,
$\frac{1}{3}$, $\frac{1}{2}$, $\frac{2}{3}$, $\frac{5}{6}$ and 1,
...
In *fcaR*, this dataset is exposed to the user with the name `cobre32`,
...
![Figure 3: []{#fig:schizo label="fig:schizo"}Formal context of the
`cobre32` dataset. The default plot type allows us to identify patterns
occurring in the attributes, such as the sparsity of the `SCIDII`
variables. Additionally, we can see that the variables indicating
diagnosis, `dx_ss` and `dx_other` in the last two columns, are binary
and mutually exclusive.](formal_context.png){#fig:schizo width="100%"
alt="graphic without alt text"}
...
Figure [4](#fig:sublatticecobre32).

![Figure 4: []{#fig:sublatticecobre32
label="fig:sublatticecobre32"}Hasse diagram for a sublattice of the
`cobre32` formal context. Since the complete concept lattice is huge,
with the Hasse diagram of a sublattice we can understand the
relationship in a subset of the attributes. In this case, we can see how
variables `COSAS_2` and `FICAL_1` are related to the variable `dx_ss`
which indicates the schizophrenia
diagnosis.](fig-sublatticecobre32.png){#fig:sublatticecobre32
width="100%" alt="graphic without alt text"}
...
as those of the formal context. The `attribute_filter` argument
specifies which attributes are of interest, in our case, the diagnosis
attributes.
...
strict. For `S2`, maybe adding more attributes to it can help in
...
We can check that, for `S2`, if the presence of any symptom (of the LHS
of the implications) is verified, then the implications above could
directly tell us the diagnosis.

An extended version of the diagnosis system in this example has been
presented in @CORDERO2020113449, where the *fcaR* package has been used
to build a conversational recommender system based on fuzzy rules. In
that work, the recommender system designed with the help of *fcaR* has
obtained better results than those of the classical methods used in the
...
implications. Additionally, the *fcaR* package implements a logic to
...
*arules*. This way, the user of *arules* can benefit from the FCA tools
in *fcaR*, including the simplification logic.
