...
in the package
[*spherepc*](https://CRAN.R-project.org/package=spherepc). Research on
the LPG is underway in progress.
...
in the [*spherepc*](https://CRAN.R-project.org/package=spherepc) are
...
[@Hijmans2017]. The following codes yield an implementation of the
`PGA()` function.
...
Figure [1](#fig:PGA), where the principal geodesic properly extracts the
...
described in Tables [1](#table:PGA) and [2](#table:PGAout).

<figure id="fig:PGA">
<p><img src="figures/PGA(halfcircle).png" style="width:45.0%"
alt="graphic without alt text" /><img src="figures/PGA(S).png"
style="width:45.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 1: From left to right, half-great circle and S-shaped
data (blue) and the results (red) of principal geodesic analysis (PGA).
The principal geodesic detects the global trends of the noisy half-great
circle and the S-shaped data but cannot identify the circular variation
of the S-shaped data.</figcaption>
</figure>
...
In a spherical surface, as shown in Figure [1](#fig:PGA), the principal
...
thereby leading to numerical errors. @Lee2021 have proposed an exact
principal circle in an intrinsic way and its practical algorithm based
on gradient descent. The details are described in Section 3 of @Kim2020
and Appendix B of @Lee2021spherical:supp. The
[*spherepc*](https://CRAN.R-project.org/package=spherepc) package
...
Table [3](#table:PrincipalCircle), and its output is a three-dimensional
...
are described in Table [4](#table:GenerateCircle).
...
The results of the principal circle are shown in Figure [2](#fig:PC). As
one can see, the principal circle identifies the circular patterns of
the noisy half-great circle and circular dataset well.

<figure id="fig:PC">
<p><img src="figures/PC(halfcircle).png" style="width:47.0%"
alt="graphic without alt text" /><img src="figures/PC(circular).png"
style="width:47.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 2: Half-great circle data and circular data (blue)
and the results (red) of the principal circle from left to right. The
principal circle can identify the relatively small circular structure
(right) and the great circle structure (left).</figcaption>
</figure>
...
@Hauberg2016. The usage of the `SPC()` function is as follows.
...
The usage of the `SPC.Hauberg()` function is the same as that of the
...
[@Robotham2013], and
[*geosphere*](https://CRAN.R-project.org/package=geosphere)
[@Hijmans2017] packages. To implement the `SPC()` and `SPC.Hauberg()`
functions, we consider the waveform data used in @Liu2017level,
@Kim2020, and @Lee2021. The generating equation of waveform is
...
The above codes generate the results in Figure [3](#SPC:wave). As one
...
described in Tables  [5](#table:SPC) and [6](#table:SPCout),
respectively, which are the same for the `SPC.Hauberg()`.

<figure id="SPC:wave">
<p><img src="figures/Hauberg(wave).png" style="width:30.0%"
alt="graphic without alt text" /><img src="figures/SPC(wave).png"
style="width:30.0%" alt="graphic without alt text" /><img
src="figures/SPC(wave_noisy).png" style="width:30.0%"
alt="graphic without alt text" /></p>
<figcaption>Figure 3: Left and middle: The waveform data (blue) and the
results (red) of Hauberg’s principal curves (left) and spherical
principal curves. Right: The noisy waveform data (blue) and the result
(red) of spherical principal curves. All cases are implemented with
<span class="math inline"><em>q</em> = 0.05</span>. The two methods find
the true waveform of the data well. In particular, the spherical
principal curve tends to be smoother.</figcaption>
</figure>
...
`deletePoints`, described in Table [5](#table:SPC). If
...
The results are illustrated in Figure [4](#fig:SPCoption). The left
...
<figure id="fig:SPCoption">
<p><img src="figures/SPC(option1).png" style="width:45.0%"
alt="graphic without alt text" /><img src="figures/SPC(option2).png"
style="width:45.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 4: Left: Projection result (black) of SPC with <span
class="math inline"><em>q</em> = 0.1</span>. The spherical principal
curve (red) continuously represents the earthquake data (blue). Right:
The open curve of SPC with <span
class="math inline"><em>q</em> = 0.04</span> and
<code>deletePoints=TRUE</code>. The less <span
class="math inline"><em>q</em></span> is, the more the curve overfits
the data.</figcaption>
</figure>
...
[@Robotham2013], and
[*geosphere*](https://CRAN.R-project.org/package=geosphere)
[@Hijmans2017] packages. The detailed arguments and outputs of this
function are described in Tables [7](#table:LPG) and [8](#table:LPGout).
...
 [5](#fig:spiral), [6](#fig:zigzag), and [7](#fig:tree).
...
<p><img src="figures/spiral.png" style="width:32.0%"
alt="graphic without alt text" /><img src="figures/LPG(spiral).png"
style="width:32.0%" alt="graphic without alt text" /><img
src="figures/LPG(spiral_noisy).png" style="width:32.0%"
alt="graphic without alt text" /></p>
<figcaption>Figure 5: Left: Spiral data (blue) and the result (red) of
LPG with <span
class="math inline"><em>s</em><em>c</em><em>a</em><em>l</em><em>e</em> = 0.06</span>
and <span class="math inline"><em>n</em><em>u</em> = 0.1</span>. Right:
Noisy spiral data (blue) and the result (red) of LPG with <span
class="math inline"><em>s</em><em>c</em><em>a</em><em>l</em><em>e</em> = 0.12</span>
and <span class="math inline"><em>n</em><em>u</em> = 0.1</span>. Local
principal geodesics represent the spiral patterns of the (noisy) spiral
data. The larger the noise is, the larger scale is
required.</figcaption>
</figure>
...
<p><img src="figures/zigzag.png" style="width:32.0%"
alt="graphic without alt text" /><img src="figures/LPG(zigzag).png"
style="width:32.0%" alt="graphic without alt text" /><img
src="figures/LPG(zigzag_noisy).png" style="width:32.0%"
alt="graphic without alt text" /></p>
<figcaption>Figure 6: Left: zigzag data (blue). Middle: zigzag data
(blue) and the result (red) of with <span
class="math inline"><em>s</em><em>c</em><em>a</em><em>l</em><em>e</em> = 0.1</span>
and <span class="math inline"><em>n</em><em>u</em> = 0.1</span>. Right:
Noisy zigzag data (blue) and the result (red) of LPG with <span
class="math inline"><em>s</em><em>c</em><em>a</em><em>l</em><em>e</em> = 0.2, <em>a</em><em>n</em><em>d</em> <em>n</em><em>u</em> = 0.1</span>.
Local principal geodesics extract the zigzag structures of the (noisy)
zigzag data properly. The larger the noise is, the larger scale is
needed.</figcaption>
</figure>
...
several curves. As shown in the left panel of Figure [7](#fig:tree), the
...
<figure id="fig:tree">
<p><img src="figures/tree.png" style="width:49.0%"
alt="graphic without alt text" /><img src="figures/LPG(tree).png"
style="width:49.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 7: Tree data (blue) and the result (red) of LPG with
<span
class="math inline"><em>s</em><em>c</em><em>a</em><em>l</em><em>e</em> = 0.03</span>
and =0.2. The LPG function captures the complex structures of the data
well, provided that and are properly chosen.</figcaption>
</figure>

As displayed in Figures [5](#fig:spiral), [6](#fig:zigzag), and
[7](#fig:tree), the `LPG()` function identifies the non-geodesic or
...
Tables [7](#table:LPG) and [8](#table:LPGout).
...
<figure id="fig:earthquake">
<p><img src="figures/earthquake1.png" style="width:50.0%"
alt="graphic without alt text" /><img src="figures/earthquake2.png"
style="width:50.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 8: The distribution of significant earthquakes (8+ Mb
magnitude), and their three-dimensional visualization.</figcaption>
</figure>
...
Ocean since 1900. As shown in Figure [8](#fig:earthquake), the data
contain 77 observations distributed in the borders between the Eurasian,
Pacific, North American, and Nazca tectonic plates. The data have three
features: the observations are distributed globally, scattered, and form
non-geodesic structures. Because the tectonic plates are constantly
moving in different directions, identifying the hidden patterns of
borders is useful in geostatistics and seismology, as noted in
...
circle, respectively. As illustrated in Figure [9](#fig:PGAPC), the
principal geodesic (left) fails to identify the variations of the
earthquake data. The principal circle (right) captures the global trend
of the data, whereas the circle could not extract the local variations
of the data.

<figure id="fig:PGAPC">
<p><img src="figures/PGA(earthquake).png" style="width:47.0%"
alt="graphic without alt text" /><img src="figures/PC(earthquake).png"
style="width:47.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 9: Earthquake data (blue) and the results (red) of
the principal geodesic analysis and principal circle, from left to
right. The principal geodesic fails to find the non-geodesic feature of
the data, and the principal circle captures the circular pattern but
cannot identify the local variations of the data.</figcaption>
</figure>

<figure id="fig:SPC">
<p><img src="figures/SPCHauberg(earthquake).png" style="width:46.0%"
alt="graphic without alt text" /> <img src="figures/SPC(earthquake).png"
style="width:46.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 10: Earthquake data (blue) and implementation results
(red) with <span class="math inline"><em>q</em> = 0.1</span> of the
SPC.Hauberg and SPC functions, respectively, from left to right. Both
methods can represent the non-geodesic feature of the earthquake data.
The spherical principal curve particularly tends to be
smoother.</figcaption>
</figure>
...
curve with $q=0.1$. As shown in Figure [10](#fig:SPC), both methods
identify the curved feature of the earthquake data. The spherical
principal curve particularly tends to be more continuous than Hauberg's
principal curve.
...
Figure [11](#fig:SPC:change), the smaller $q$ is, the rougher the curve
is. On the contrary, the larger $q$ is, the smoother the curve is.

<figure id="fig:SPC:change">
<p><img src="figures/SPC(earthquake)q=015.png" style="width:50.0%"
alt="graphic without alt text" /><img
src="figures/SPC(earthquake)q=01.png" style="width:50.0%"
alt="graphic without alt text" /><img
src="figures/SPC(earthquake)q=003.png" style="width:50.0%"
alt="graphic without alt text" /><img
src="figures/SPC(earthquake)q=002.png" style="width:50.0%"
alt="graphic without alt text" /></p>
<figcaption>Figure 11: From left to right and top to bottom: Earthquake
data (blue) and the results (red) of the SPC with <span
class="math inline"><em>q</em> = 0.15, 0.1, 0.03 <em>a</em><em>n</em><em>d</em> 0.02</span>.
The larger the parameter <span class="math inline"><em>q</em></span> is,
the smoother the curve is, while it tends to underfit the data.
Conversely, the smaller the parameter <span
class="math inline"><em>q</em></span> is, the rougher the curve
is.</figcaption>
</figure>
...
Lastly, example 5 implements the `LPG()` function with different `scale`
and `nu`. As shown in Figure [12](#fig:LPG), the function represents the
curved pattern of the data, illustrating the slightly different
features.

<figure id="fig:LPG">
<p><img src="figures/LPG(earthquake).png" style="width:49.0%"
alt="graphic without alt text" /><img src="figures/LPG(earthquake)2.png"
style="width:49.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 12: From left to right, earthquake data (blue) and
the results of the LPG function with <span
class="math inline"><em>s</em><em>c</em><em>a</em><em>l</em><em>e</em> = 0.5</span>,
nu=0.2 and <span
class="math inline"><em>s</em><em>c</em><em>a</em><em>l</em><em>e</em> = 0.4</span>,
nu=0.3. Both the local principal geodesics implemented by different
parameters recognize the non-geodesic and scattered pattern of the data,
illustrating the different features.</figcaption>
</figure>

# Conclusions

In this paper, the R package
[*spherepc*](https://CRAN.R-project.org/package=spherepc) has
implemented various dimension reduction methods on a sphere. It includes
not only principal geodesic analysis (PGA), principal circle, and
principal curves of @Hauberg2016 as existing methods but also spherical
principal curves (SPC) and local principal geodesics (LPG) as new
approaches. The
