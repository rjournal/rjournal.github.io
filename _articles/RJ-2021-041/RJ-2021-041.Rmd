...
within the recipes package. We illustrate a more in-depth use-case in a
car pricing application, performing a transform-both-sides regression as
well as comparing the performance of several predictive models fit via
caret. Finally, we conclude by discussing the pros and cons of
...
be implemented well in existing R packages, bestNormalize puts them all
under the same umbrella syntax. This section describes each
...
in the [LambertW](https://CRAN.R-project.org/package=LambertW) package,
is essentially a mechanism that de-skews a random variable $X$ using
moments. The method is motivated by a system theory and is alleged to be
able to transform any random variable into any other kind of random
variable, thus being applicable to a large number of contexts. One of
the package's main functions is Gaussianize, which is similar in spirit
to the purpose of this package. However, this method may not perform as
well on certain shapes of distributions as other candidate
...
details on this transformation, consult the LambertW documentation.[^1]
While the transformations contained and implemented by bestNormalize are
reversible (i.e., 1-1), in rare circumstances, we have observed that the
lambert function can yield non-reversible transformations.
...
![ORQ normalization visualization on Fisher's iris
data.](figs/orq_vis-1.pdf){#fig:orq_vis width="1\\linewidth"}
...
In addition to the techniques above, the bestNormalize package performs
and evaluates:
...
The bestNormalize function selects the best transformation according to
an extra-sample estimate of the Pearson P statistic divided by its
...
![Simulated skewed data for simple
example.](figs/simple_hist-1.pdf){#fig:simple_hist width="2.5in"
height="1.3in"}
...
::: Soutput
#\> Best Normalizing transformation with 250 Observations #\> Estimated
Normality Statistics (Pearson P / df, lower =\> more normal): #\> -
arcsinh(x): 1.7917 #\> - Box-Cox: 1.0442 #\> - Center+scale: 3.0102
#\> - Exp(x): 9.5306 #\> - Log_b(x+a): 1.7072 #\> - orderNorm (ORQ):
1.1773 #\> - sqrt(x + a): 1.144 #\> - Yeo-Johnson: 1.1875 #\> Estimation
method: Out-of-sample via CV with 10 folds and 5 repeats #\> #\> Based
off these, bestNormalize chose: #\> Standardized Box Cox Transformation
with 250 nonmissing obs.: #\> Estimated statistics: #\> - lambda =
0.3254863 #\> - mean (before standardization) = -0.3659267 #\> - sd
(before standardization) = 0.9807881
:::
...
![The suite of transformations estimated by default in bestNormalize
(trained on simulated right-skewed
data).](figs/bn_plot-1.pdf){#fig:bn_plot width="1\\linewidth"}
...
![Summary of transformations performed on simulated right-skewed
data.](figs/hist_best-1.pdf){#fig:hist_best width="1\\linewidth"}
...
Relevant statistics: #\> - mean (before standardization) = 0.7383146
...
Estimated statistics: #\> - lambda = 0.3254863 #\> - mean (before
standardization) = -0.3659267 #\> - sd (before standardization) =
0.9807881
:::
...
#\> Estimated statistics: #\> - lambda = -0.7080476 #\> - mean (before
standardization) = 0.4405464 #\> - sd (before standardization) =
0.2592004
:::
...
::: Soutput
#\> Standardized Lambert WxF Transformation of type s with 250
nonmissing obs.: #\> Estimated statistics: #\> - gamma = 0.3729 #\> -
mean (before standardization) = 0.6781864 #\> - sd (before
standardization) = 0.7123011
:::
...
Original quantiles: #\> 0 #\> 0.001 0.268 0.721 1.299 4.161
:::

All normalization techniques in bestNormalize have their own class with
convenient S3 methods and documentation. For instance, we can use the
predict method to perform the transformation on new values using the
objects we have just created, visualizing them in a plot:

::: Sinput
xx \<- seq(min(x), max(x), length = 100) plot(xx, predict(arcsinh_obj,
newdata = xx), type = \"l\", col = 1) lines(xx, predict(boxcox_obj,
newdata = xx), col = 2) lines(xx, predict(yeojohnson_obj, newdata = xx),
col = 3) lines(xx, predict(orderNorm_obj, newdata = xx), col = 4)
:::

![Manually plotting transformations trained on simulated right-skewed
data.](figs/vis_data2-1.pdf){#fig:vis_data2 width="4.375in"
height="2.5in"}
...
plot the transformed data, x.t, stored in the transformation objects we
created previously.

![Normalized values for trained transformations on simulated
right-skewed data.](figs/hist_trans-1.pdf){#fig:hist_trans width="4in"
height="4in"}
...
statistic for each left-out fold/repeat into oospreds. Users can access
and visualize these results via a boxplot (see below), which may give
some insight into whether the transformation is truly preferred by the
normality statistic or if another (possibly simpler) transformation can
be applied that would achieve the approximately the same results. In
...
Leave-one-out CV can be optionally performed in \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize} via
the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 loo}}%
{loo} argument, which, if set to \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 TRUE}}%
{TRUE}, will compute the
leave-one-out CV transformations for each observation and method.
Specifically, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize} will be run \(n\) separate times
...
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize} uses repeated CV by default to estimate the
...
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 out\textunderscore of\textunderscore sample = FALSE}}%
{out\textunderscore of\textunderscore sample = FALSE}. This will
highly speed up the process. However, for reasons previously discussed,
ORQ normalization will always be chosen unless
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 allow\textunderscore orderNorm = FALSE}}%
{allow\textunderscore orderNorm = FALSE}. Therefore, a user might
as well use the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 orderNorm}}%
{orderNorm} function directly as opposed to only
setting \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 out\textunderscore of\textunderscore sample = FALSE}}%
{out\textunderscore of\textunderscore sample = FALSE} since
the end result will be the same (and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 orderNorm}}%
{orderNorm} will run much
...
parallelized via the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 cluster}}%
{cluster} argument and the \href{https://CRAN.R-project.org/package=parallel}{\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont parallel}}%
{parallel}}
and \href{https://CRAN.R-project.org/package=doRNG}{\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont doRNG}}%
{doRNG}} \citep{doRNG} packages. A cluster can be set up with
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 makeCluster}}%
{makeCluster} and passed to \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize} via the
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 cluster = }}%
{cluster = } argument.
...
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize} with 15 repeats of 10-fold CV on a
...
The \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 step\textunderscore best\textunderscore normalize}}%
{step\textunderscore best\textunderscore normalize} and the
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 step\textunderscore orderNorm}}%
{step\textunderscore orderNorm} functions can be utilized in
conjunction with the \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont recipes}}%
{recipes} package to preprocess data in machine
learning workflows with \href{https://CRAN.R-project.org/package=tidymodels}{\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont tidymodels}}%
{tidymodels}} \citep{tidymodels} or in
combination with \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont caret}}%
{caret}. The basic usage within \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont recipes}}%
{recipes} is
shown below; for implementation with \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont caret}}%
{caret}, refer to this paper's
application.
...
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 step\textunderscore best\textunderscore normalize}}%
{step\textunderscore best\textunderscore normalize} to speed up or
alter performance via the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 transform\textunderscore options}}%
{transform\textunderscore options}
argument, which passes a list of options to \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize}.
...
used ``under the hood'' by \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize} to estimate the
...
Via the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 new\textunderscore transforms}}%
{new\textunderscore transforms} argument, users can use
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize}'s machinery to compare custom, user-defined
...
function with those provided in the package. \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize}
requires two functions to implement this: the transformation function
and an associated \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 predict}}%
{predict} method. The custom cube-root
...
been implemented well in \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont nortest}}%
{nortest} already. In order to accommodate
...
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 norm\textunderscore stat\textunderscore fn}}%
{norm\textunderscore stat\textunderscore fn} argument, which takes
...
The user-defined function must take an argument \texorpdfstring%
...
::: Soutput
#\> Best Normalizing transformation with 100 Observations #\> Estimated
Normality Statistics (using custom normalization statistic) #\> -
arcsinh(x): 0.1958 #\> - Box-Cox: 0.1785 #\> - Center+scale: 0.2219
#\> - Exp(x): 0.3299 #\> - Log_b(x+a): 0.1959 #\> - orderNorm (ORQ):
0.186 #\> - sqrt(x + a): 0.1829 #\> - Yeo-Johnson: 0.1872 #\> Estimation
method: Out-of-sample via CV with 10 folds and 5 repeats #\> #\> Based
off these, bestNormalize chose: #\> Standardized Box Cox Transformation
with 100 nonmissing obs.: #\> Estimated statistics: #\> - lambda =
0.3281193 #\> - mean (before standardization) = -0.1263882 #\> - sd
(before standardization) = 0.9913552
:::
...
::: Sinput
(dont_do_this \<- bestNormalize(x, norm_stat_fn = function(x)
nortest::lillie.test(x)$p))
\end{Sinput}
\begin{Soutput}
#> Best Normalizing transformation with 100 Observations
...
Note: \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 bestNormalize}}%
{bestNormalize} will attempt to minimize this statistic by
...
\bgroup 
\begin{Sinput}
best_transform <- names(which.max(dont_do_this$norm_stats)) do_this \<-
dont_do_this$other_transforms[[best_transform]]
...
de-skew mileage, age, and price in a pricing model. See ?autotrader for
more information on this data set.

::: Sinput
data(\"autotrader\") autotrader$yearsold <- 2017 - autotrader$Year
:::
...
::: {#tab:unnamed-chunk-10}
  ...
  : Sample characteristics of 'autotrader' data.
:::
...
::: Soutput
#\> Best Normalizing transformation with 6283 Observations #\> Estimated
Normality Statistics (Pearson P / df, lower =\> more normal): #\> -
arcsinh(x): 3.4332 #\> - Box-Cox: 3.0903 #\> - Center+scale: 14.7488
#\> - Log_b(x+a): 3.4354 #\> - orderNorm (ORQ): 1.1514 #\> - sqrt(x +
a): 5.1041 #\> - Yeo-Johnson: 3.0891 #\> Estimation method:
Out-of-sample via CV with 10 folds and 5 repeats #\> #\> Based off
these, bestNormalize chose: #\> orderNorm Transformation with 6283
nonmissing obs and ties #\> - 6077 unique values #\> - Original
quantiles: #\> 0 #\> 2 29099 44800 88950 325556
:::
...
below). Even so, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 orderNorm}}%
{orderNorm} is chosen as it has the lowest
...
\bgroup 
\begin{Sinput}
p.t <- priceBN$x.t; m.t \<- mileageBN$x.t; yo.t <- yearsoldBN$x.t fit
\<- lm(p.t   m.t + yo.t)
:::
...
::: {#tab:unnamed-chunk-11}
  ...
  : TBS regression results for autotrader data.
:::
...
[visreg](https://CRAN.R-project.org/package=visreg) package [@visreg] to
perform our visualizations, using predict.bestNormalize in conjunction
with visreg's trans and xtrans options to view the relationship in terms
of the original unit for the response and covariate respectively
(formatting omitted).[^3] For the sake of illustration, we have also
plotted the estimated effect of a generalized additive (spline) model
fit with [mgcv](https://CRAN.R-project.org/package=mgcv) [@mgcv].

::: Sinput
visreg(fit, \"m.t\") visreg(fit, \"m.t\", partial = TRUE, trans =
function(price.t) predict(priceBN, newdata = price.t, inverse =
TRUE)/1000, xtrans = function(mileage.t) predict(mileageBN, newdata =
mileage.t, inverse = TRUE) )
:::

![TBS regression visualized on transformed units (left) and original
units (right).](figs/linear_visreg-1.pdf){#fig:linear_visreg
width="0.8\\linewidth"}
...
![Age effect on car price (re-transformed to original
unit).](figs/gam_tbs_model-1.pdf){#fig:gam_tbs_model
width="0.8\\linewidth"}
...
::: Sinput
library(tidymodels) library(caret) library(recipes)

set.seed(321) df_split \<- initial_split(autotrader, prop = .9) df_train
\<- training(df_split) df_test \<- testing(df_split)

rec \<- recipe(price   Make + model + mileage + status + Year, df_train)
step_mutate(years_old = 2017 - Year) step_rm(Year) step_log(price)
step_best_normalize(all_predictors(), -all_nominal())
step_other(all_nominal(), threshold = 10) step_dummy(all_nominal())
prep()

fit1 \<- train(price   ., bake(rec, NULL), method = 'glmnet') fit2 \<-
train(price   ., bake(rec, NULL), method = 'earth') fit3 \<- train(price
  ., bake(rec, NULL), method = 'rf')
...
::: {#tab:unnamed-chunk-14}
  ...
  : CV prediction accuracy of various ML methods.
:::
...
::: {#tab:unnamed-chunk-16}
  ...
  : Test data prediction accuracy of various ML methods. RMSE and MAE
  can be interpreted in terms of 2017 US dollars.
:::
...
learning workflows. We are enthusiastic in our support of bestNormalize,
and will continue to maintain the package while it is found to be useful
by R users. We hope to continue to build up the repertoire of candidate
transformations using the same infrastructure so that additional ones
can be considered by default in the future.

[^1]: As of version 1.2.0 of bestNormalize, lambert methods are not
    performed by default in bestNormalize, but they are still available
    via the allowlambert arguments.

[^2]: Since standardize=TRUE, the identity transformation is represented
    in Figure 7 by centerscale, which yields the exact same normality
    statistic.
...
[^3]: Alternatively, one can use
    ...
    visualize any transformation fit using bestNormalize; instructions
    are included in the package vignette.
