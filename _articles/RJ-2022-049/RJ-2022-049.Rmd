...
$$\label{eq:1}
\mathbb{E}[Y(\bar{a})]=\mu(\bar{a};\beta),   (\#eq:1)$$
...
$$\label{eq:2}
\mathbb{E}[Y(\bar{a})]=\int...\int\mathbb{E}[Y|\bar{A}=\bar{a}, \bar{L}=\bar{l}]\prod^{T}_{t=1}f(l_{t}|\bar{l}_{t-1},\bar{a}_{t-1})d\mu(l_{t}),   (\#eq:2)$$
...
$$\label{eq:3}
\mathbb{E}[Y|\bar{A}=\bar{a}]=\int...\int\mathbb{E}[Y|\bar{A}=\bar{a}, \bar{L}=\bar{l}]\prod^{T}_{t=1}f(l_{t}|\bar{l}_{t-1},\bar{a})d\mu(l_{t}).   (\#eq:3)$$

By comparing Equations \@ref(eq:2) and \@ref(eq:3), we see that
weighting the observed population by

$$\label{eq:4}
W_{l}=\prod^{T}_{t=1}\frac{f(L_{t}|\bar{L}_{t-1},\bar{A}_{t-1})}{f(L_{t}|\bar{L}_{t-1},\bar{A})}   (\#eq:4)$$
...
$$\label{eq:5}
\mathbb{E}^{*}[\delta(g(L_{t}))h(\bar{L}_{t-1},\bar{A})]=\mathbb{E}^{*}[\delta(g(L_{t}))]\mathbb{E}^{*}[h(\bar{L}_{t-1},\bar{A})]=0,   (\#eq:5)$$
where:
...
the observed population by $W_{l}$. To do so, the method 1) specifies a
set of $g(\cdot)$ functions,
...
$$\label{eq:6}
\sum^{n}_{i=1}rbw_{i}c_{ir}=0, \,\,\, 1 \le r \le n_{c},   (\#eq:6)$$
...
$$\label{eq:7}
\min_{rbw_{i}}\sum_{i}rbw_{i}\log (rbw_{i}/q_{i}).   (\#eq:7)$$
...
Figure [1](#fig:1) depicts the logic of RBW in a DAG. Following the
...
$rbw_{i}$ weights break the dependence between $A_{t}$ and $\bar{L}_{t}$
at each time point. That is, the weights create a pseudo-population
where the confounding arrows $L_{1}\rightarrow A_{1}$,
$L_{1}\rightarrow A_{2}$, and $L_{2}\rightarrow A_{2}$ are broken while
all others are preserved. It is important to note that $L_{1}$ is
*marginally* independent of both $A_{1}$ and $A_{2}$ in the
pseudo-population, but $L_{2}$ is *conditionally* independent of
$A_{2}$, given $L_{1}$ and $A_{1}$. Hence, RBW invokes a model for the
conditional mean of $L_{2}$ given $L_{1}$ and $A_{1}$ and balances the
residuals from this model across levels of $A_{2}$ and levels of
$(L_{1}, A_{1})$ (the observed past). This procedure avoids overcontrol
and collider-stratification bias when adjusting for post-treatment
...
![Figure 1: The underlying logic of residual balancing: $A_{t}$ denotes
the treatment at time $t$, $L_{t}$ is a vector of time-varying
confounders at time $t$, and $Y$ is the end-of-study outcome. Residual
balancing weights break the confounding arrows $L_{1}\rightarrow A_{1}$,
$L_{1}\rightarrow A_{2}$, and
$L_{2}\rightarrow A_{2}$.](tikz/fig1.png){#fig:1 width="100%"
...
The DAG of Figure [2](#fig:2) illustrates the point treatment situation.
...
![Figure 2: RBW in a point treatment: $A$ represents the treatment, $X$
is a vector of baseline confounders, and $Y$ denotes the outcome.
Residual balancing weights break the confounding arrow
$X\rightarrow A$.](tikz/fig2.png){#fig:2 width="100%"
alt="graphic without alt text"}
...
-   `rbwPoint()`, for constructing residual balancing weights to
...
-   `rbwPanel()`, for constructing residual balancing weights to
...
several real-world data sets (`advertisement`, `peace`, `campaign_long`,
and `campaign_wide`), which we describe and analyze in the examples
below.
...
[*rbw*](https://CRAN.R-project.org/package=rbw) to implement the method
...
$n_{c}$ balancing constraints described in Equation \@ref(eq:6).
...
$$\label{eq:8}
\min_{rbw_{i}}L^{p}=\sum^{n}_{i=1}rbw_{i}\log (rbw_{i}/q_{i})+\sum^{n_{c}}_{r=1}\lambda_{r}\sum^{n}_{i=1}rbw_{i}c_{ir}+\lambda_{0}\left(\sum^{n}_{i=1}rbw_{i}-n\right),   (\#eq:8)$$
...
$$\label{eq:9}
rbw_{i}^{*}=\frac{nq_{i}\text{exp}(-\sum^{n_{c}}_{r=1}\lambda_{r}c_{ir})}{\sum^{n}_{i=1}q_{i}\text{exp}(-\sum^{n_{c}}_{r=1}\lambda_{r}c_{ir})}.   (\#eq:9)$$

We can then insert Equation \@ref(eq:9) into $L^{p}$, leading to an
unrestricted dual problem:

$$\label{eq:10}
\max_{\lambda_{r}}L^{d}=-\log \left(\sum^{n}_{i=1}q_{i}\text{exp}\left(-\sum^{n_{c}}_{r=1}\lambda_{r}c_{ir}\right)\right),   (\#eq:10)$$
or equivalently,

$$\label{eq:11}
\min_{Z}L^{d}=\log \left(Q'\text{exp}(CZ)\right),   (\#eq:11)$$
where $Q=[q_{1},...,q_{n}]'$,
...
-   `M` is a vector of moment conditions to be met in the reweighted
    sample (per Equation \@ref(eq:6), this is a vector of zeros with
    length equal to the number of columns of `C` when the other
    functions in [*rbw*](https://CRAN.R-project.org/package=rbw) call
    `eb2()` internally).
...
-   `print_level` determines the level of printing:
    ...
    -   **2** detailed: print also the maximum absolute value of the
        deviations between the moments of the reweighted data and the
        target moments in each iteration.
    -   **3** very detailed: print also the step length of the line
...
the base weights is minimized. Additionally, `rbwPoint()` calls a
...
-   `baseline_x` is an expression for a set of baseline confounders
    stored in `data` or a character vector of the names of these
    variables.
-   `base_weights` is an *optional* vector of base weights (or its
    name). If no value is supplied, the function sets a vectors of ones
    with length equal to the sample size as the base weights.
...
set to `FALSE` by default, but users suspecting an interaction effect
may find it prudent to balance against it. Like `rbwPoint()`, `rbwMed()`
...
-   `treatment`, `data`, `base_weights`, `max_iter`, `print_level`, and
    `tol` are defined as in `rbwPoint()`.
...
-   `mediator` is a symbol or character string representing the mediator
    variable.
-   `zmodels` is a list of fitted `lm` or `glm` objects for
    post-treatment confounders of the mediator-outcome relationship.
    Users should set this argument to `NULL` if there are no
...
past treatments and past confounders. Then it calls `eb2()` to find a
set of weights, $rbw_{i}$, such that 1) residuals of the post-treatment
...
-   `data`, `base_weights`, `max_iter`, `print_level`, and `tol` are
    defined as in `rbwPoint()` and `rbwMed()`.
...
-   `future` is an integer indicating the number of future treatments in
    ...
    treatments $A_{t+1},...,A_{t+\texttt{future}}$. The default,
    `future = 1`, assumes away higher-ordered lagged effects of the
    covariates on the treatment. Users can leave out lagged effects
    entirely by setting `future` to zero.

The output returned by `rbwPanel()` is essentially the same as those
from `rbwPoint()` and `rbwMed()`. The only difference is that the
`weights` object, instead of a vector, is now a data frame with two
columns: the id variable and the residual balancing weights (the column
storing the weights is called `rbw`).

# Examples

We now illustrate the functions `rbwPoint()`, `rbwMed()`, and
`rbwPanel()` with data sets `advertisement`, `peace`, `campaign_long`,
and `campaign_wide`, which are included in
[*rbw*](https://CRAN.R-project.org/package=rbw). We expect users to
rarely need to call `eb2()` manually since its primary use is to be
...
$$\label{eq:12}
\mathbb{E}[Y(a^{*})|U]=\theta_{0}+\theta_{1}a^{*} +\theta_{2}^{T}U.   (\#eq:12)$$
...
`advertisement` does not include sampling weights, `rbwPoint()` will set
...
contributions. Hence, we create a `dose` variable to account for this.
If $a_{i}^{*}=\log(a_{i}+1)$, we can define our dose as $\log(1000+1)$:
...
[*ipw*](https://CRAN.R-project.org/package=ipw) packages. Recall from
the introduction that the parametric CBPS is similar to IPW in that it
...
Table [1](#tab:point-treatment-comparison) summarizes the findings. All
...
::: {#tab:point-treatment-comparison}
  ...
  : Table 1: Comparison of RBW, npCBPS, CBPS, and IPW for a Point
  Treatment Situation
:::
...
$$\label{eq:13}
\mathbb{E}[Y(a, m)]=\alpha_{0}+\alpha_{1}a +\alpha_{2}m+\alpha_{3}am.   (\#eq:13)$$
...
$$\label{eq:14}
\mathbb{E}[Y(a, m)|X]=\alpha_{0}+\alpha_{1}a +\alpha_{2}m+\alpha_{3}am + \alpha_{4}^{T}X.   (\#eq:14)$$
...
vector of ones as the base weights since `peace` does not include
...
before, but we now leave the `baseline_x` argument empty:
...
We summarize the results in Table [2](#mediation-comparison). The
...
::: {#mediation-comparison}
  ...
  : Table 2: MSM Results for the Controlled Direct Effects of Shared
  Democracy on Public Support for War: $\widehat{\text{CDE}}(m=0)$
  Equals the Coefficient for Shared Democracy;
  $\widehat{\text{CDE}}(m=1)$ Equals the Coefficient for Shared
  Democracy Plus the Coefficient for the Interaction Term
:::
...
$$\label{eq:15}
\mathbb{E}[Y(\bar{a})|X]=\beta_{0}+\beta_{1}\text{avg}(\bar{a})+\beta_{2}V+\beta_{3}\text{avg}(\bar{a})V+\beta_{4}^{T}X,   (\#eq:15)$$
...
permitted because the `weights` object returned by `rbwPanel()` is a
...
[*dplyr*](https://CRAN.R-project.org/package=dplyr) to rename the
weights so that our variable's name is consistent with the previous
examples:
...
We report the model results in Table [3](#time-varying-model). The
...
::: {#time-varying-model}
  ...
  : Table 3: MSM Results for the Cumulative Effect of Negative
...
The scalar objects `rbwPanel_time` and `CBPSPanel_time` store how long
...
[4](#tab:method-comparison) sums up these comparisons.
