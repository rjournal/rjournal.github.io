...
abstract: |
  ...
  LibTorch backend (via the torch package), provides a more scalable and
  computationally efficient approach. The package integrates three
...
development of Latent Binary Bayesian Neural Networks (LBBNN)
[@hubin2024sparse] allows for a scalable Bayesian approach to sparse
neural network learning, by including a Bernoulli inclusion indicator
variable for each weight. This allows using model uncertainty to reduce
a network to a fraction of its initial size, while maintaining all the
other advantages of doing Bayesian inference. In the frequentist
setting, sparsification is usually obtained by more ad hoc procedures
(see e.g. [@frankle2018lottery]).

Two limitations of LBBNNs as defined in [@hubin2024sparse] are addressed
in [@skaaret-lund2024sparsifying]. The first is that while LBBNNs are
sparser than BNNs during inference, they are still computationally
...
reparameterization trick (LRT) [@kingma2015variational], which makes it
...
including normalizing flows [@rezende2015variational], the variational
...
LBBNNs [@hoyheim2026explainable]. Input-skip refers to the design of a
...
function to learn. In [@hoyheim2026explainable], an experiment with
...
were popularized by [@neal1995bayesian] in his PhD-thesis, where
inference was done using Hamiltonian Monte Carlo (HMC)
[@duane1987hybrid] to sample from the posterior distribution of the
neural network weights. HMC is considered an improvement on traditional
MCMC techniques, as it uses Hamiltonian dynamics to explore the
parameter space more efficiently. However, HMC does not scale well with
large datasets, and very high-dimensional parameter spaces such as
neural network posteriors, as we typically see in modern applications.
In addition, deep and overparameterized neural network posteriors are
typically highly multi-modal, another area where HMC struggles.

An alternative method that is more scalable is Variational Inference
(VI) [@jordan1999introduction]. Rather than trying to sample from the
true posterior distribution, VI defines a parameterized approximate
posterior distribution, $q$, where the goal is to adjust the parameters
of $q$ to minimize the discrepancy from the true posterior distribution,
where the latter is typically measured by the KL-divergence. This
optimization based approach is often preferred in practice (given finite
...
(IVON) [@shen2024variational] uses an implicit mean-field Gaussian
approximation over the weights, where the mean and variance are updated
with online second order curvature estimates of the loss, achieving a
computational cost comparable to Adam, enabling scaling to large neural
network architectures. While such methods improve the scalability of
variational inference, recent work has also focused on improvements in
sampling based inference, such as Microcanonical Langevin Ensembles
(MILE) [@sommer2025microcanonicallangevinensemblesadvancing], which uses
deep ensembles to initialize short MCLMC [@robnik2023fluctuation]
chains. This achieves substantial computational gains over the No-U-Turn
Sampler (NUTS) [@hoffman2014no] algorithm, at the cost of no longer
guaranteeing asymptotic exactness of posterior sampling due to
...
learning frameworks such as PyTorch [@paszke2019pytorch] and TensorFlow
[@tensorflow2015-whitepaper], providing key features like high
performance GPU computation and automatic differentiation. While it is
possible to implement Bayesian neural networks directly using these
frameworks, both ecosystems provide probabilistic programming libraries
such as Pyro [@bingham2019pyro] (for PyTorch) and TensorFlow Probability
[@dillon2017tensorflow] which facilitate Bayesian modeling by providing
...
[@cabezas2024blackjax], providing efficient, accelerator-agnostic
implementations of MCMC algorithms including NUTS and HMC. In addition,
the `bde` [@arvanitis2026bde] library combines scikit-learn
...
Another approach is the `posteriors` package [@duffield2024scalable],
...
the `reticulate` [@reticulate] package, this requires a properly
...
considerations. In contrast, the R native `torch` [@torchR] package
builds directly on libtorch, the backend C++ library that also powers
PyTorch.
...
GPU-accelerated variational inference for BNNs is `BayesFluxR`
[@bayesfluxr] (itself a wrapper around a Julia backend), while the
others --- `bnns` [@bnns], `BNN` [@liang2018bayesian], `BLNN`
[@sharaf2020blnn], and `BoomSpikeSlab` [@boomspikeslab] rely on some
form of MCMC. Finally, `selectnn` [@mcinerney2025statistical] does a
combinatorial search. For a detailed comparison of these packages, see
Table [1](#tab:T1){reference-type="ref" reference="table:Rpackages"}.
...
\end{aligned}\label{eq:BNNprior}   (\#eq:BNNprior)$$
...
To implement the LBBNNs, we will follow @skaaret-lund2024sparsifying and
...
RNVP [@dinh2016density] with the numerically stable updates presented in
@kingma2016improved. For a more detailed overview of computational costs
related to both LRT and normalizing flows, please see Appendix A in
...
sparse representation, and having a linear connection between $x_0$ and
...
\end{aligned}$$
with $i = 1,...,1000$ and $j = 0,...,15$. The goal is to check if the
model is able to remove the non-relevant variables, in addition to
recovering the true linear effects. The data is generated as follows:
...
in the `trainlbbnn` function avoids printing to the console during
...
node. See [@hoyheim2026explainable] for a more formal definition of
active paths. `density` refers to the proportion of weights in the whole
...
the mean, and 95% credible intervals for the explanations. If only one
sample is used, the mean explanation and credible interval is given from
model averaging over the weights. If multiple samples are selected, the
overall mean explanation and confidence interval are derived from the
individual mean explanations of each individual data sample.
...
\end{aligned}$$
With $i = 1,...,1000$ and $j = 0,...,15$. In this case, we further
transform the output into a binary variable:
$$\begin{aligned}
   {y_i} = 
    \begin{cases}
...
![Figure 5: The structure of the network in the non-linear simulation
example after removing weights that are not included in active
paths.](figures/sim_nonlinear_global.png){#fig:simulation_ex
width="100%" alt="graphic without alt text"}

### Tutorial 3: Real data classification experiment

For this experiment, we use the Gallstone dataset [@gallstone_1150] from
the UCI machine learning repository. It consists of data from 319
...
validation, in line with the experiment described in [@gallstone_1150],
although we do not know the exact indices for the split, so the results
may vary. In [@gallstone_1150], the best result (85.42 $\%$) is obtained
using gradient boosting. Our sparse input-skip model obtained an
accuracy of $83.33\%$, using only roughly 5$\%$ of the total weights.
...
[2](#tab:T2){reference-type="ref" reference="bernoulli_inits"}. Figure
[7](#fig:inits){reference-type="ref" reference="fig:inits"} provides a
visualization of the initial distribution of the resulting Bernoulli
probabilities, $\alpha_{\texttt{init}}$.
...
![Figure 7: Empirical distributions of initial inclusion probabilities
for different initialization schemes.](figures/inits.png){#fig:inits
width="100%" alt="graphic without alt text"}

In the following experiment, we use the raisin dataset
[@ccinar2020classification], consisting of 900 samples, where the goal
is to classify two types of raisins based on 7 morphological features.
We use a network with 3 hidden layers, each consisting of 10 neurons. Of
the 900 samples, 720 are used for training and 180 for validation. We
compare results with different initializations, where the model is now
defined as:
...
![Figure 8: Density across epochs with different
initializations.](figures/density.png){#fig:density width="100%"
alt="graphic without alt text"}
...
seen in Table [3](#tab:T3){reference-type="ref"
...
good and comparable to the results reported in the original paper
[@ccinar2020classification], 0.8633 accuracy using a frequentist neural
network. In Figure [9](#fig:raisins_global){reference-type="ref"
...
<figure id="fig:raisins_global">
<table>
<caption> </caption>
<tbody>
<tr>
<td style="text-align: center;"><img src="figures/raisins_balanced.png"
style="width:100.0%" alt="graphic without alt text" /></td>
<td style="text-align: center;"><img src="figures/raisins_dense.png"
style="width:100.0%" alt="graphic without alt text" /></td>
<td style="text-align: center;"><img src="figures/raisins_polarized.png"
style="width:100.0%" alt="graphic without alt text" /></td>
<td style="text-align: center;"><img
src="figures/raisins_polarized_dense.png" style="width:100.0%"
alt="graphic without alt text" /></td>
</tr>
<tr>
<td style="text-align: center;">(a) balanced</td>
<td style="text-align: center;">(b) dense</td>
<td style="text-align: center;">(c) polarized</td>
<td style="text-align: center;">(d) polarized_dense</td>
</tr>
<tr>
<td style="text-align: center;"><img
src="figures/raisins_polarized_mild.png" style="width:100.0%"
alt="graphic without alt text" /></td>
<td style="text-align: center;"><img
src="figures/raisins_polarized_sparse.png" style="width:100.0%"
alt="graphic without alt text" /></td>
<td style="text-align: center;"><img src="figures/raisins_sparse.png"
style="width:100.0%" alt="graphic without alt text" /></td>
<td style="text-align: center;"></td>
</tr>
<tr>
<td style="text-align: center;">(e) polarized_mild</td>
<td style="text-align: center;">(f) polarized_sparse</td>
<td style="text-align: center;">(g) sparse</td>
<td style="text-align: center;"></td>
</tr>
</tbody>
</table>
<figcaption>Figure 9: Comparison of the global structure of the networks
after training with different initializations on the raisin
dataset.</figcaption>
</figure>
...
We use the KMNIST dataset [@clanuwat2018deep], consisting of 70,000
...
![Figure 10: Random 4×4 grid of KMNIST images with class
labels.](figures/kmnist_grid_labeled.png){#fig:kmnist_grid width="100%"
alt="graphic without alt text"}
...
in [@skaaret-lund2024sparsifying], but the purpose here is not to obtain
the best possible results, but rather demonstrate how a simple
convolutional LBBNN can be created. We note that convolutional LBBNNs
can also be very sparse without sacrificing too much predictive power in
this case. Active paths are not defined for convolutional architectures,
hence density within them is not available for this example.
...
And we can take a look at the first 5 draws of the 255th[^3] predicted
image posteriors:
...
global interpretability. Local explanations are also further available
