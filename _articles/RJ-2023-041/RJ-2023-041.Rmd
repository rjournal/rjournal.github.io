...
Table
`r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:table-variables-html)', '\\@ref(tab:table-variables-static)'))`)
...
```{r table-variables-html, eval = knitr::is_html_output(), echo=F, message=FALSE}
library(hydrotoolbox)
library(kableExtra)
...
```{r table-variables-static, eval = knitr::is_latex_output(), echo=F, message=FALSE}
library(hydrotoolbox)
library(kableExtra)
...
`r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:table-methods-html)', '\\@ref(tab:table-methods-static)'))`
...
```{r table-methods-html, eval = knitr::is_html_output(), echo=F, message=FALSE}
...
```{r table-methods-static, eval = knitr::is_latex_output(), echo=F, message=FALSE}
...
```{r read-like, echo = F, fig.show="hold", fig.align="center", out.width="52%", fig.cap= "Rectangular data formats/file types compatible with the $\\texttt{hm\\_build\\_generic}$ function. Top scheme depicts allowed delimited file arrangements while the bottom scheme is for excel files."}
...
```{r build-generic, echo=TRUE, eval=FALSE}
...
```{r mza-basin, echo = FALSE, fig.align="center", out.width="60%", fig.pos="h", fig.cap= "Map of the Mendoza basin including Guido hydrological station, cities, river and main mountain summits for reference."}
...
of Figure `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(fig:guido-plot-html)', '\\@ref(fig:guido-plot-static)'))`
please visit the HTML (online) version of the article.

```{r guido-plot-static, eval=knitr::is_latex_output(), echo=T, fig.align="center", out.width="100%", fig.cap = "Plot of the Guido's streamflow time series (hydrologial years 2010/11 to 2013/14)."}
# ggplot2 daily flow
guido %>% 
...
```{r guido-plot-html, eval=knitr::is_html_output(), echo=T, fig.align="center", out.width="100%", fig.cap = "Plot of the Guido's streamflow time series (hydrologial year 2010/11 to 2013/14)."}
# plotly daily flow
guido %>% 
  hm_plot(slot_name = "qd",
          col_name = list( "q_m3/s" ), 
...
As depicted in Figure `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(fig:guido-plot-html)', '\\@ref(fig:guido-plot-static)'))` the time 
...
```{r guido-agg-get, eval=TRUE, echo=TRUE}
...
```{r guido-ggplot, eval=T, echo=T, warning=F, fig.align="center", out.width="100%", fig.cap="Example of customized time-series plot using the $\\texttt{hm\\_plot()}$ method in combination with labeling, scaling and theme options available on $\\textbf{ggplot2}$."}
# library
library(ggplot2)
...
```{r cuevas-mb, eval=T, echo=T, warning=F, message=F, fig.align="center", out.width="100%", fig.cap="Model post-processing case example for the Cuevas basin annual glacier mass balance."}
