...
[@huang19; @ribeiro19; @forthmann20]. The *mpcmp* package is based on
the work of @huang. The hyper-Poisson regression model
...
package, called
[*DGLMExtPois*](https://CRAN.R-project.org/package=DGLMExtPois), in
which the maximum likelihood optimization procedure has been
accelerated. Of course, the package includes the common methods and
functions that allow for a complete analysis, including model
diagnostics. The package also provides the @huang implementation of the
CMP model that admits covariates in the dispersion parameter, mainly to
facilitate comparisons between hP and CMP fits. In conclusion, what we
want to demonstrate is the usefulness of CMP and hP models in a genuine
GLM framework to test the significance and to assess the size effect of
any explanatory variable on the mean value. At the same time, other
explanatory variables may take part in the explanation of the dispersion
structure, providing over- or under-dispersion depending on their
different values.
...
package has included this feature. Table [1](#table:comparison)
summarizes this comparison along with some additional features included
...
::: {#table:comparison}
  ...
  : Table 1: Feature comparison between packages *DGLMExtPois* (version
  0.2.1), *mpcmp* (version 0.3.6) and *COMPoissonReg* (version 0.7.0).
:::
...
$$\label{hp}
P\left[Y = y \right] = \frac{1}{_{1}F_{1}\left(1, \gamma; \lambda \right)} \frac{\lambda^{y}} {\left(\gamma \right)_{y}}, \qquad \lambda, \gamma > 0,    (\#eq:hp)$$
where
...
solution of
$$\label{eq:link}
\mu = \exp (\mathbf{X} \boldsymbol{\beta}) = \sum_{y = 0}^{\infty} y \times P[Y = y] .   (\#eq:link)$$
...
to it as double GLM, although we highlight that the hP distribution does
not properly belong to the exponential family for unknown $\gamma$.

The previous definition of the $hP_{\mu, \gamma}$ model [@hp] was based
on an alternative equation, different from (\@ref(eq:link)). Concretely,
the authors proposed a closed equation that involves the mean, the
variance and the normalizing constant. The substitution of the previous
equation by equation (\@ref(eq:link)) and the change of the optimization
algorithm have been the key points in the great reduction of the
computational time needed to fit real data provided by the *DGLMExtPois*
package.
...
solution of (\@ref(eq:link)) for a given $\mu_i$ and $\gamma_i$. This
converts the maximum likelihood procedure into a nonlinear constrained
...
$n$ non-linear equations from (\@ref(eq:link)). We have implemented it
using a sequential quadratic programming (SQP) gradient-based algorithm
[@kraft1994algorithm]; concretely, the NLOPT_LD_SLSQP algorithm included
in the [*nloptr*](https://CRAN.R-project.org/package=nloptr) package
[@nloptr].

In the implementation, it was necessary to compute the gradient of the
objective function and the gradient of restriction \@ref(eq:link), in
...
$$\label{scorebeta}
    S_{\boldsymbol{\beta}} = \frac{\partial l_{hP}}{\partial \boldsymbol{\beta}} = \sum_{i=1}^n \frac{Y_i - \mu_i}{V\left(\mu_i,\gamma_i\right)} \mu_i \mathbf{x}_i'   (\#eq:scorebeta)$$
where $V\left(\mu_i,\gamma_i\right)$ is the variance of an $hP$ with a
probability mass function given by \@ref(eq:hp), which depends on
$\mu_i$ and $\gamma_i$. This way, the MLE of $\boldsymbol{\beta}$ can be
characterized as the solution to the usual GLM score equations.*
:::
...
verified that
$$\label{scoredelta}
    S_{\boldsymbol{\delta}}= \frac{\partial l_{hP}}{\partial\boldsymbol{\delta}} = \sum_{i=1}^n \left\{\left(Y_i - \mu_i\right)\frac{Cov \left[Y_i \psi\left(\gamma_i+ Y_i\right) \right]}{V\left(\mu_i,\gamma_i\right)} - \psi\left(\gamma_i+Y_i\right) + E \left[ \psi\left(\gamma_i+Y_i\right) \right]  \right\} \gamma_i \mathbf{z}_i'   (\#eq:scoredelta)$$
...
This is the model with constant $\nu$ defined by @sellers10, later
extended to consider covariates in the $\nu$ equation
[@sellers12; @CHATLA18]. In contrast, the model defined by @huang is
given as
...
$$\log \nu = \mathbf{Z} \boldsymbol{\delta},$$
where additionally equation \@ref(eq:link) must be verified for each
observation, and the probabilities in \@ref(eq:cmp). We denote this
model as $CMP_{\mu, \nu}$.
...
In this context, what the *DGLMExtPois* package provides is, first, a
much faster implementation of the $hP_{\mu, \gamma}$ model than the
previous existing R code and, second, our own implementation of
$CMP_{\mu, \nu}$ to facilitate the comparison of the models.

*DGLMExtPois* has been created by trying to reproduce the syntax of GLM
fits. Thus, functions `glm.hP` and `glm.CMP` provide the corresponding
fits for $hP_{\mu, \gamma}$ and $CMP_{\mu, \nu}$, respectively. There
are also `print` and `summary` methods to visualize the fitted models
and `AIC`, `confint`, `predict`, `residuals` and `plot` methods to
...
some associated plots, such as QQ-plots and simulated envelopes. The
package additionally incorporates `expected`, a function which
...
is given in Table [2](#tab_functions).
...
in regression on count variables have been used to compare
*COMPoissonReg* and *DGLMExtPois* in terms of goodness of fit,
computational times and reliability. Finally, some guidelines are given
on the optimization process for estimating the models.
...
(`nhu`), the average income in dollars (`aid`), the average housing unit
age in years (`aha`), the distance to the nearest competitor in miles
(`dnc`) and the distance to store in miles (`ds`) as covariates. Here,
...
and one of them, `dnc`, in the dispersion parameter equation. The
dataset is also analyzed in [@Bonat] considering the Extended
Poisson-Tweedie distribution, but they also obtain an equidispersed
distribution.
...
The functions `glm.hP` and `glm.CMP` return S3 objects of classes
`"glm_hP"` and `"glm_CMP"`, respectively, with information on the fitted
models. The most suitable model according to AIC is Poisson. The fits
for these models, shown in Table [3](#tab_constant_dispersion), indicate
...
::: {#tab_constant_dispersion}
  ...
  : Table 3: Estimation of coefficients and standard errors for fitted
  models with a constant dispersion parameter. The last row is the
  estimation of $\delta_0$ and its standard error.
:::
...
also used the LRT to confirm the adequacy of `hp1` versus the `hp0`
...
note that low values of the `dnc` variable show over-dispersion while
high values show under-dispersion. Specifically, the point at which the
change occurs is 1.7069. There are 24 out of the 110 cases with
$\gamma_i > 1$, and thus over-dispersed, underlying
$hP(\mu_i, \gamma_i)$ distributions, see Figure [1](#fig:dispersion).
The plot suggests a certain correlation between under-dispersion and
high response variable values.

![Figure 1: Value of the dispersion parameter according to the response
variable. The points above the dashed line are greater than 1, so the
distribution is over-dispersed. These points occur for low values of the
response variable.](dispersion.png){#fig:dispersion width="100%"
...
The `plot` method provides two classical plots for the model
diagnostics: residuals against fitted values and a normal QQ-Plot. In
...
Figure [2](#fig:diagnostics)). The package also provides the possibility
...
functions (Figure [2](#fig:diagnostics)c). Finally, the diagnostic can
...
[2](#fig:diagnostics)d the cases for which the model assumptions are
...
<tbody>
<tr class="odd">
<td style="text-align: center;"><img src="diagnostics_a.png"
style="width:100.0%" alt="graphic without alt text" /></td>
<td style="text-align: center;"><img src="diagnostics_b.png"
style="width:100.0%" alt="graphic without alt text" /></td>
</tr>
<tr class="even">
<td style="text-align: center;">(a) <span>Residuals versus fitted
values.</span></td>
<td style="text-align: center;">(b) <span>Normal QQ plot.</span></td>
</tr>
<tr class="odd">
<td style="text-align: center;"><img src="diagnostics_c.png"
style="width:100.0%" alt="graphic without alt text" /></td>
<td style="text-align: center;"><img src="diagnostics_d.png"
style="width:100.0%" alt="graphic without alt text" /></td>
</tr>
<tr class="even">
<td style="text-align: center;">(c) <span>Bar_plot for the observed
marginal frequencies. The blue line represents the expected marginal
frequencies for the response variable.</span></td>
<td style="text-align: center;">(d) <span>Normal QQ plot with simulated
envelopes of residuals. The shaded gray region contains 95% of the
simulated points for each observation.</span></td>
</tr>
<tr class="odd">
<td style="text-align: center;"><span id="fig:diagnostics"
label="fig:diagnostics"></span></td>
<td style="text-align: center;"></td>
</tr>
</tbody>
</table>
<figcaption>Figure 2: Regression diagnostics. The plots do not show
evidence against the adequacy of the model.</figcaption>
</figure>
...
processor (3.20GHz) and 64 GB of RAM. Table [4](#tab_aic_time) shows the
AICs and the computational times obtained by the different models for
the different datasets. The first column of the table lists the names of
the datasets and the second column lists the number of observations of
each dataset. The best AICs results are highlighted in bold.
Unfortunately, the *COMPoissonReg* and *mpcmp* packages crash with some
datasets, which is indicated by a dash in the table. In relation to the
crashes, we clarify that all the fits have been carried out with the
default optional arguments. To obtain a better approximation of the
execution time needed to estimate a model, we have estimated each model
10 times and computed the average time required for the estimate. These
average times are presented in Table [4](#tab_aic_time) as follows. A
value of 1.0 means the model has the fastest average estimate time and
this fastest average time, in seconds, is shown in the last column. A
value of, e.g., 41.4 means the average estimate time of the model is
41.4 times larger than the average of the fastest. Table
[5](#tab_goodness_fit) shows the results of the $hP_{\mu, \gamma}$ and
$CMP_{\mu, \nu}$ models, fitted with the *DGLMExtPois* package,
according to other goodness of fit criteria.
...
::: {#tab_aic_time}
  ...
  : Table 4: Comparison of models based on AIC (the smallest value is
  highlighted in bold) and relative average estimate time. For the
  estimate times a value of 1.0 means the model is the fastest on
  average for the dataset. A value of, e.g., 41.4 means the average
  estimate time of the model is 41.4 times larger than the average of
  the fastest. The fastest time is expressed in seconds. The symbol --
  means that the model could not be estimated.
:::
...
::: {#tab_goodness_fit}
  ...
  : Table 5: Comparison of models based on additional goodness of fit
  criteria. MPB: mean prediction bias; MAD: mean absolute deviance;
  MSPE: mean squared predictive error and Pearson statistic. The best
...
-   **Takeover bids**. The response variable is the number of bids
    received by 126 US firms that were successful targets of tender
    offers between 1978--1985. We consider 9 explanatory variables on
    the defensive actions taken by the management of the target firm,
...
-   **Days absent**. The sample refers to 314 students sampled from two
    ...
    others, by @huang. The *COMPoissonReg* package crashes again with
...
-   **Cotton bolls**. Here, the response variable is the number of bolls
    ...
    *DLGMExtPois* and *mpcmp* packages). Here $hP_{\mu, \gamma}$
    performs worse and obtains a higher AIC. Another interesting point
    in relation to the $CMP_{\mu, \nu}$ fit is that, regardless of the
...
-   **Korea crashes**. The dataset contains crash count data, which is
    the response variable, collected at 162 railway-highway crossings in
...
-   **Toronto crashes**. The dataset provides Toronto intersection crash
    ...
    example, @lord. Whereas *COMPoissonReg* once again leads to a crash
    in the fit of a $CMP_{\theta, \nu}$ model, $CMP_{\mu, \nu}$ and
    ...
    *mpcmp* package. In this case, the *mpcmp* package is the fastest
    ...
    $hP_{\mu, \gamma}$ fit.
-   **Insurance claims**. Data consist of the numbers of policyholders
    of an insurance company who were exposed to risk, and the number of
    car insurance claims made by those policyholders in the third
    quarter of 1973. The response variable is the number of claims, with
    the number of policyholders being an offset for the mean. Apart from
    ...
    occasion the *COMPoissonReg* package does not fail and is the
    fastest, however its AIC is clearly the highest among the estimated
    models. Most of the cases show under-dispersion (52 and 44 for
    $hP_{\mu, \gamma}$ and $CMP_{\mu, \nu}$ respectively).
-   **Credit card reports**. The response variable is the number of
    major derogatory reports. Three explanatory variables are
    considered: age in years plus twelfths of a year, yearly income (in
    USD 10,000) and average monthly credit card expenditure. For this
    dataset the only model that can be estimated is $hP_{\mu, \gamma}$.
    This is a case of over-dispersed data, but as a result of
    introducing covariates in the dispersion parameter, 5
    under-dispersed cases appear in the $hP_{\mu, \gamma}$ model. In any
    case a negative binomial model fits better due to the strong
...
*mpcmp* packages, the $CMP_{\mu, \nu}$ model of the *mpcmp* package
fails in more datasets and provides better results in terms of AIC for
only one dataset.
...
3.  The $\lambda$ parameter is obtained as solution of \@ref(eq:link) by
    means of the base R `optimize` function.

4.  Each value of $Y$ in the sample is randomly generated using the
    `rhP` function.
...
::: {#tab_simulation}
  ...
  : Table 6: Simulation process for $hP_{\mu, \gamma}$ models. For each
...
[7](#tab_simulation_cmp). It can be seen that, although the estimates of
the $\boldsymbol{\delta}$ coefficients are still less precise than those
of the $\boldsymbol{\beta}$ coefficients, these estimates are less
biased and more precise than those of the $hP_{\mu, \gamma}$ model. In
this sense, the $CMP_{\mu, \nu}$ model leads to better estimates than
...
few thousand parameters. Table [8](#tab_optimization) shows the datasets
from the above performance comparison sorted by the dimension of the
objective function when fitting an $hP_{\mu, \gamma}$ model. Also, the
average estimation time for 10 estimations and the number of iterations
taken by the optimizer are listed. Clearly, the dimension of the
objective function has the greatest impact on optimization time, while
the number of iterations plays a less important role.
...
::: {#tab_optimization}
  ...
  : Table 8: Optimization process for $hP_{\mu, \gamma}$ models. For
  each dataset, the dimension of the objective function, the average
  optimization time (in seconds) and the number of iterations are shown.
:::

The `nloptr` optimization function from the *nloptr* package was used to
...
-   a fractional tolerance (`xtol_rel`) is used in the parameters $x$,
    so that the optimizer stops when $|\Delta x_i|/|x_i| < xtol\_rel$,
    with a default value for `xtol_rel` of 0.01.
...
that the *mpcmp* package fails to fit some datasets and the fits
obtained are, in general, somewhat worse than those obtained with the
*DGLMExtPois* package. The *COMPoissonReg* package allows covariates in
the location parameter, but it models a location parameter instead of
the mean. In addition, this latter package fails to fit many of the
...
and therefore
$$\label{der1f1lambda}
    \frac{\partial}{\partial \lambda} \log \left(_{1}F_{1}\left(1, \gamma; \lambda \right) \right) = \frac{1}{_{1}F_{1}\left(1, \gamma; \lambda \right)} \frac{\partial}{\partial \lambda} \, _{1}F_{1}\left(1, \gamma; \lambda \right) = \frac{\mu}{\lambda}.   (\#eq:der1f1lambda)$$
...
%\end{split}   (\#eq:der1f1gamma)$$

## Proof of result [1](#res_beta) {#proof-of-result-res_beta}
...
$$\label{scorelambda}
    \frac{\partial l_{hP}}{\partial \lambda_i} = \sum_{i=1}^n \left\{ \frac{Y_i}{\lambda_i} -  \frac{\partial}{\partial \lambda_i} \log \left(_{1}F_{1}\left(1, \gamma_i; \lambda_i \right) \right) \right\} = \sum_{i=1}^n \frac{Y_i - \mu_i}{\lambda_i},   (\#eq:scorelambda)$$
where the result \@ref(eq:der1f1lambda) has been used.
...
solution of equation \@ref(eq:link). Thus,
...
and clearing
$$\label{derlambdamu}
    \frac{\partial \lambda}{\partial \mu} = \frac{\lambda}{V\left(\mu,\gamma\right)}.   (\#eq:derlambdamu)$$

Therefore, plugging \@ref(eq:scorelambda) and \@ref(eq:derlambdamu) into
...
\@ref(eq:scorebeta) is that
$$\frac{\partial \mu_i}{\partial \boldsymbol{\beta}} = \mu_i \mathbf{x}_i'.$$

## Proof of result [2](#res_delta) {#proof-of-result-res_delta .unnumbered}
...
\end{aligned}$$
wherein we have used \@ref(eq:der1f1gamma).

The derivative of $\lambda_i$ with respect to $\gamma_i$ is also
obtained by explicit differentiation from equation \@ref(eq:link) with
...
\end{aligned}$$
and clearing:
$$\label{derlambdagamma}
    \frac{\partial \lambda}{\partial \gamma} = \lambda \frac{E \left[\left(Y-\mu\right) \psi\left(\gamma+Y\right) \right]}{V\left(\mu,\gamma\right)} = \lambda \frac{Cov \left[Y, \psi\left(\gamma+Y\right) \right]}{V\left(\mu,\gamma\right)}.   (\#eq:derlambdagamma)$$

Therefore, replacing \@ref(eq:derlambdagamma) in the score of $\gamma$
we obtain
$$\label{scoregamma}
    \frac{\partial l_{hP}}{\gamma_i} = \sum_{i=1}^n \left\{\left(Y_i - \mu_i\right)\frac{Cov \left[Y_i, \psi\left(\gamma_i+Y_i\right) \right]}{V\left(\mu_i,\gamma_i\right)} - \psi\left(\gamma_i+Y_i\right) + E \left[ \psi\left(\gamma_i+Y_i\right) \right]  \right\}.   (\#eq:scoregamma)$$

The only thing left to consider to get the expression
\@ref(eq:scoredelta) is that
$$\frac{\partial \gamma_i}{\partial \boldsymbol{\delta}} = \gamma_i \mathbf{z}'_i.$$

## Proof of result [3](#inf_mat) {#proof-of-result-inf_mat .unnumbered}
...
Considering the log-likelihood function \@ref(eq:logverhp) as a function
of $\boldsymbol{\beta}$, $\lambda'_i = \log \lambda_i$ and
...
The proof is straightforward from expressions \@ref(eq:der1f1lambda) and
\@ref(eq:derpochgamma).

## Gradient of the restriction \@ref(eq:link) {#gradient-of-the-restriction-eqlink}

Restriction \@ref(eq:link) can be expressed as
...
\end{aligned}$$
where expression \@ref(eq:der1f1lambda) has been used. Considering
expressions \@ref(eq:derpochgamma) and \@ref(eq:der1f1gamma), it is
verified that:
$$\begin{aligned}
...
\end{aligned}$$
from which it is straightforward to obtain the gradient for
$\boldsymbol{\delta}$.
