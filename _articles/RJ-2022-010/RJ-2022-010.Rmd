...
author:
- name: Sami Aldag
  affiliation: Department of Mathematics Engineering, Istanbul Technical University,
  address:
  - Istanbul, 34469,
  - |
    Turkey
- name: Dogukan Topcuoglu
  ...
  - |
    Turkey
- name: Gul Inan
  affiliation: Department of Mathematics, Istanbul Technical University,
  orcid: |
    0000-0002-3981-9211
  address:
  - Istanbul, 34469,
...
country between the years $1923$ and $1937$ via `R`. For that reason,
...
in Figure [1](#fig:mylabelfanatomy).

![Figure 1: An anatomy of a column bar
graph.](deneme2.png){#fig:mylabelfanatomy width="12cm" height="14cm"}
...
Figure [2](#fig:mylabelf1org) gives the amount of smallpox vaccine
...
In Figure [2](#fig:mylabelf1org), we can see that the background color
of the figure is white. There is no vertical axis and related
information on the vertical axis (e.g., axis line, axis title, axis tick
marks, and tick mark labels). We can get the frequencies of each column
bar through the data values placed inside the columns. Consequently, the
column bar heights are directly proportional to the data values they
represent. Since the height of the columns are taller and take space in
the figure plotting area, the data values are placed vertically inside
the columns. The horizontal axis refers to the time interval with linear
increments without having an axis title. Due to the white background
color of the figure, the column bars are filled in with black color
whereas the data values are colored in white for contrast. Due to a
large number of column bars and lack of space, bar widths and the
spacing between columns are kept short and the labels of the horizontal
axis tick marks are displayed vertically.
...
Figure [3](#fig:mylabelf1). The data values are placed onto the graphic
...
`geom_col()` layer. The main figure title consists of four lines.
...
texts). For that reason, several `annotate()` layers are further used to
run the full title rather than `ggtitle()` or `labs()` layer which
...
![Figure 2: Historical figure on \"Smallpox vaccine administered in
various regions of the country, between the period 1925-1937\" retrieved
from
<https://acikerisim.tbmm.gov.tr/handle/11543/553>.](Cicek_original.png){#fig:mylabelf1org
width="9cm" height="12cm"}

![Figure 3: Reproduced figure on \"Smallpox vaccine administered in
various regions of the country, between the period
1925-1937\".](Cicekrep2.png){#fig:mylabelf1 width="7cm" height="14cm"}

## *Overlapped bar graphs with two data series*

![Figure 4: Historical figure on \"The service of hospitals and
dispensaries within the Department of Control of Trachoma, 1925-1937\"
retrieved from <https://acikerisim.tbmm.gov.tr/handle/11543/553>
([$\blacksquare$]{style="color: black"} The number of inpatient
treatments $\square$ The number of surgeries
performed).](Trahom_original.png){#fig:mylabelf3org width="12cm"
height="11cm"}

![Figure 5: Reproduced figure on \"The service of hospitals and
dispensaries within the Department of Control of Trachoma, 1925-1937\"
([$\blacksquare$]{style="color: black"} The number of inpatient
treatments $\square$ The number of surgeries
performed).](Trahomrep.png){#fig:mylabelf3 width="12cm" height="12cm"}

Figure [4](#fig:mylabelf3org) presents the service of hospitals and
...
As in Figure [2](#fig:mylabelf1org), the background color of the
Figure [4](#fig:mylabelf3org) is white and there is no vertical axis and
...
Unlike Figure [2](#fig:mylabelf1org), there are no labels for the
horizontal axis tick marks now. However, the third line of main graph
title gives the clue that horizontal axis starts from $1925$ and goes to
$1937$. As the reviewer pointed out, we think that horizontal tick mark
labels here were unintentionally forgotten since this is the only bar
graph with missing horizontal tick mark labels in the government report.
However, this may result in a cognitive effort for the viewer if the
viewer would like to know the exact information for the number of
inpatient treatments and/or the number of surgeries performed throughout
the years, especially for the years in the middle of 1925 and 1937. In
that case, the viewer has to count the number of bars to get precise
...
In Figure [5](#fig:mylabelf3), the $100\%$ overlapping structure of
column bars is obtained via setting argument `position = "identity"` in
`geom_col()` layer. Note that the look of Figure [5](#fig:mylabelf3)
...
This grouping variable is also mapped into `fill` and `alpha` arguments
of the aesthetics of the main `ggplot()` call since the fill-in colors
...
components of `theme()` layer, assigning "black" color to the inpatient
treatments and "white" color to the surgeries performed via
`scale_fill_manual()` layer, and then assigning low level of
...
`scale_alpha_manual()` layer would yield the final look of the figure.
Hence, the order of elements of the vector of colors in
`scale_fill_manual()` layer and the order of elements of the vector of
transparency in `scale_alpha_manual()` layer are matched with the order
...
The white line segment in the first column of Figure [5](#fig:mylabelf3)
is integrated via an `annotate()` layer along with `rect` argument. On
the other hand, three-lined main graph title is run with `labs()` and
`annotate()` layers due the italicized font structure of the middle line
...
Figure [6](#fig:mylabelf4org) shows the service of the Zonguldak
...
outpatient treatment is not available in $1924$ and is coded as **NA**
in the data. While the columns for the number of inpatient treatments
are taller than that of outpatient treatment over the period $1925$ and
...
![Figure 6: Historical figure on \"The service of the Zonguldak
Government Hospital, 1924-1937\" retrieved from
<https://acikerisim.tbmm.gov.tr/handle/11543/553>
([$\blacksquare$]{style="color: black"} The number of inpatient
treatments $\square$ The number of outpatient
treatments).](Zonguldak_original.png){#fig:mylabelf4org width="12cm"
height="12cm"}

![Figure 7: Reproduced figure on \"The service of the Zonguldak
Government Hospital, 1924-1937\" ([$\blacksquare$]{style="color: black"}
The number of inpatient treatments $\square$ The number of outpatient
treatments).](Zonguldakrep.png){#fig:mylabelf4 width="12cm"
height="12cm"}
...
The `R` code structure of Figure [7](#fig:mylabelf4) is very challenging
...
not so, `geom_col(position = "identity")`, `scale_fill_manual()`, and
...
Figure [8](#fig:mylabelf5org) represents the workload of private
...
available in Figures [6](#fig:mylabelf4org) and  [8](#fig:mylabelf5org)
is reasonable. However, as the reviewer pointed out, this
inconsistencies i.e., aligning texts horizontally then vertically, is
visually distracting and may decrease the readability.

![Figure 8: Historical figure on \"The workload of private hospitals,
1926-1937\" retrieved from
<https://acikerisim.tbmm.gov.tr/handle/11543/553>
([$\blacksquare$]{style="color: black"} The number of inpatient
treatments $\square$ The number of outpatient
treatments).](Hususi_original.png){#fig:mylabelf5org width="12cm"
height="11cm"}

![Figure 9: Reproduced figure on \"The workload of private hospitals,
1926-1937\" ([$\blacksquare$]{style="color: black"} The number of
inpatient treatments $\square$ The number of outpatient
treatments).](Hususirep.png){#fig:mylabelf5 width="12cm" height="12cm"}

The `R` code structures of Figures [5](#fig:mylabelf3) and
[9](#fig:mylabelf5) are very similar to each other. The $70\%$
overlapping position of columns bars in Figure [9](#fig:mylabelf5) is
adjusted through setting `position = position_dodge(width = 0.3)` in
`geom_col()` layer. Furthermore, in year 1926, the column of outpatient
...
Figure [7](#fig:mylabelf4), `geom_col(position = "identity")` could not
handle this and the white rectangular column bar in year 1926 is drawn
manually via `annotate()` layer with `rect` argument.

In Figures [4](#fig:mylabelf3org), [6](#fig:mylabelf4org), and
[8](#fig:mylabelf5org), the legend is placed vertically at the top-left
of the plotting area, since the overall structure of the graphs are
left-skewed and the legend keys are ordered according to color, not
alphabetically. Furthermore, in
Figures [4](#fig:mylabelf3org)-[9](#fig:mylabelf5), the double
apostrophe, \" , in the title of second legend key is a typographic
symbol, called a ditto mark, which is used for repeated words above it.
In hand-written texts, ditto mark is used to save time and effort from
the writer. Since the government report includes around forty figures,
this may be the reason why ditto mark is also used in the legend key
titles. While this approach also reduces the amount of ink used in the
figures, it does not distort the readability of the graph. However, we
should note that in today's technology, the repetitive words can be
typed as needed with less effort.

We would like to note that the Figures [4](#fig:mylabelf3org),
[6](#fig:mylabelf4org), and [8](#fig:mylabelf5org) may look like as if
they were stacked bar graphs. However, since the government report
published in $1938$ includes the raw tabular data in it, we are $100\%$
confident to say that these figures are overlapped bar graphs.
Furthermore, we would also like to note that in stacked bar charts, the
height of the column bar gives the total frequency of all groups on a
given year. If one needs the frequency of a specific group on that year,
then an additional arithmetic operation such as subtraction should be
done to calculate it. Keeping in mind that these graphs are presented to
the public who has a low level of literacy, doing an extra mathematical
work in stacked bar charts would add a computational burden to the
viewer, which may not be possible for the viewer at that time.
...
can see from the Figures [4](#fig:mylabelf3org), [6](#fig:mylabelf4org),
and [8](#fig:mylabelf5org), the variables of interests, which are
...
Furthermore, the offset (dodging) in the Figure [8](#fig:mylabelf5org)
also confirms that columns are overlapped.

Lastly, the increasing trends in the number of inpatient treatments in
Figures [6](#fig:mylabelf4org) and [8](#fig:mylabelf5org) may indicate
that the hospital bed capacity increased over the years, whereas a
stabile trend shows that the hospital's bed capacity did not change over
the years. On the other hand, increasing trends in the number of
outpatient treatments may indicate an increase in general service
capacity of the hospital.

## *Grouped side-by-side bar graphs with two data series*

Figure [10](#fig:mylabelf6org) represents the workload of sample
...
respectively. Figure [10](#fig:mylabelf6org) is different from previous
ones since the columns for inpatient and outpatient treatments for the
same year are now placed side-by-side accordingly. This can be done by
setting the argument `position = position_dodge2()` in the `geom_col()`
layer. The grouping variable is mapped into `fill` argument of the
aesthetics of the main `ggplot()` call so that color of the levels of
...
coded as **NA** in the data. The length of the column bars for the
number of inpatient treatments are shorter than those of the outpatient
treatments over the years. However, to increase the dominance of the
number of inpatient treatments over the number of outpatient treatments,
the inpatient treatment group is colored in black and placed ahead of
...
The Figures [6](#fig:mylabelf4org) and  [10](#fig:mylabelf6org) are good
examples that `geom_col()` layer in *ggplot2* can handle missing values
in the data. In other words *ggplot2*can handle unequal length of data
...
Figure [6](#fig:mylabelf4org) and the sample hospitals in the
Figure [10](#fig:mylabelf6org) were launched in 1924 as the second-stage
...
for outpatient treatments in the Figures [6](#fig:mylabelf4org) and
[10](#fig:mylabelf6org) in 1924. After 1924, due to on-going efforts
...
![Figure 10: Historical figure on \"The workload of sample hospitals,
1924-1937\" retrieved from
<https://acikerisim.tbmm.gov.tr/handle/11543/553>
([$\blacksquare$]{style="color: black"} Inpatient treatment $\square$
Outpatient treatment).](Numune_original.png){#fig:mylabelf6org
width="12cm" height="12cm"}

![Figure 11: Reproduced figure on \"The workload of sample hospitals,
1924-1937\" ([$\blacksquare$]{style="color: black"} Inpatient treatment
$\square$ Outpatient treatment).](Numunerep.png){#fig:mylabelf6
width="12cm" height="12cm"}

Figure [12](#fig:mylabelf7org) represents the laboratory workload for
...
to the number of diagnoses. As in Figure [10](#fig:mylabelf6org), in
Figure [12](#fig:mylabelf7org), the paired columns for the same year are
...
![Figure 12: Historical figure on \"The laboratory workload for Malaria
struggle, 1925-1937\" retrieved from
<https://acikerisim.tbmm.gov.tr/handle/11543/553> ($\square$ The number
of blood tests [$\blacksquare$]{style="color: black"} The number of
diagnoses).](Sitma_original.png){#fig:mylabelf7org width="15cm"
height="11cm"}

![Figure 13: Reproduced figure on \"The laboratory workload for Malaria
struggle, 1925-1937\" ($\square$ The number of blood tests
[$\blacksquare$]{style="color: black"} The number of
diagnoses).](Sitmarep.png){#fig:mylabelf7 width="10cm" height="12cm"}
...
In the Figures [10](#fig:mylabelf6org) and [12](#fig:mylabelf7org), the
space between groups of bars is increased and now equals to the width of
a single bar in a given group. On the other hand, space between bars
within a group is not available.

The legends in the Figures [10](#fig:mylabelf6org) and
[12](#fig:mylabelf7org) are placed vertically at the top-left of the
...
as well. For example, in Figure [12](#fig:mylabelf7org), the first
legend is in white and the second one is in black. Here we note that in
the Figures [11](#fig:mylabelf6) and [13](#fig:mylabelf7), the legend
...
argument of the aesthetics of the main `ggplot()` call. The color of the
...
layer(). Lastly, from Figures [12](#fig:mylabelf7org), we can say that
the government kept administrating blood tests over the years with an
increasing trend and nearly $10\%$ of them were being turned out to be
positive.

## *Grouped side-by-side bar graph with more than two data series*

Figure [14](#fig:mylabelf12org) represents the drugs sent by the
...
In the Figure [14](#fig:mylabelf12org) we can see that there are four
different drug types: Arsenobenzol, Bizmopen, Mercury, and Iodine, where
they are filled in with black color, white color, textured with vertical
lines, and textured with dots, respectively. This textured design is
...
done by setting argument `position = position_dodge()` in the
`geom_col()` layer. The order in the placement of the groups is also
...
possible via `caption` argument in `labs()` layer of *ggplot2*.
...
the patients. In the Figure [14](#fig:mylabelf12org) the columns for the
Mercury filled with vertical lines are very eye-catching due to their
taller heights. While @Si says that Mercury had been commonly used to
cure Syphilis until discovery of penicillin around $1940$'s ,
@mumyakmaz7illet reported that the mercury was the most effective drug
to treat the patients ($95\%$ effective), and the Iodine was the second
effective drug.

The Figure [14](#fig:mylabelf12org) is a good example for use of
...
several `annotate()` layers along with `segment` argument to integrate
...
`annotate()` layers with `rect` argument for drawing the legend boxes,
several `annotate()` layers with `segment` argument for filling the
textured patterns in legend keys, and several `annotate()` layers with
`text` argument for typing the legend key titles were used.
...
![Figure 14: Historical figure on \"The drugs sent by the Department of
Control of Syphilis to cities for treatment, 1926-1937\" retrieved from
<https://acikerisim.tbmm.gov.tr/handle/11543/553>
([$\blacksquare$]{style="color: black"} Arsenobenzol $\square$ Bizmopen
[■■]{style="color: gray"} (textured pattern with vertical lines) Mercury
:: Iodine).](Frengi_original.png){#fig:mylabelf12org width="13cm"
height="18cm"}

![Figure 15: Reproduced figure on \"The drugs sent by the Department of
Control of Syphilis to cities for treatment, 1926-1937\"
([$\blacksquare$]{style="color: black"} Arsenobenzol $\square$ Bizmopen
[■■]{style="color: gray"} (textured pattern with vertical lines) Mercury
:: Iodine).](Frengirep.png){#fig:mylabelf12 width="12cm" height="14cm"}

## *Paired bar graphs with two data series*

Figure [16](#fig:mylabelf13org) shows the total number of serums and
...
Figure [16](#fig:mylabelf13org) breaks down the data into two panels as
...
![Figure 16: Historical figure on \"The service of produced and
consigned serum and vaccine in kilogram at the Central Hygiene
Institute, 1930-1937\" retrieved from
<https://acikerisim.tbmm.gov.tr/handle/11543/553> (The left-panel is for
produced items and the right-panel is for consigned times;
[$\blacksquare$]{style="color: black"} Serum $\square$
Vaccine).](Serum_original.png){#fig:mylabelf13org width="12cm"
height="19cm" width="9cm"}

![Figure 17: Reproduced figure on \"The service of produced and
consigned serum and vaccine in kilogram at the Central Hygiene
Institute, 1930-1937\" (The left-panel is for produced items and the
right-panel is for consigned times;
[$\blacksquare$]{style="color: black"} Serum $\square$
Vaccine).](Serumrep.png){#fig:mylabelf13 width="12cm" height="19cm"
width="9cm"}

![Figure 18: Historical figure on \"The service of birth and childcare
houses, 1926-1937\" retrieved from
<https://acikerisim.tbmm.gov.tr/handle/11543/553> (The left-panel is for
inpatient services and the right-panel is for outpatient services;
[$\blacksquare$]{style="color: black"} Child $\square$
Woman).](Dogum_original.png){#fig:mylabelf14org width="12cm"
height="14cm"}

![Figure 19: Reproduced figure on \"The service of birth and childcare
houses, 1926-1937\" (The left-panel is for inpatient services and the
right-panel is for outpatient services;
[$\blacksquare$]{style="color: black"} Child $\square$
Woman).](Dogumrep.png){#fig:mylabelf14 width="12cm" height="12cm"}
...
Figure [18](#fig:mylabelf14org) shows the service of birth and childcare
houses for women and children between the period $1926$ and $1937$. As
in Figure [16](#fig:mylabelf13org), the vertical axis breaks down the
data into two panels as inpatient services (left-panel) and outpatient
...
Figure [16](#fig:mylabelf13org), the vertical axis at the center denotes
...
`scale_y_continuous()` layer in *ggplot2*. The paired bar graphs in the
Figures [17](#fig:mylabelf13) and [19](#fig:mylabelf14) can be plotted
via using multiple `geom_col()` layers with some extra aesthetic work in
...
$$\label{key}
\nonumber
\text{Data-ink ratio} = 	\frac{\text{Ink used to describe the data}}{\text{Ink used to visualize graph}} .   (\#eq:key)$$
...
Figure [6](#fig:mylabelf4org), the pattern of the number of inpatient
...
Figure [20](#fig:mylabelf4reinter), we used a multi-line plot to display
the number of patients treatments and that of outpatient treatments
between 1924 and 1937 with `geom_line()` layer. A regular line is now
...
Unlike Figure [6](#fig:mylabelf4org), a vertical axis along with axis
...
Figure [6](#fig:mylabelf4org), in Figure [20](#fig:mylabelf4reinter), it
...
codes in the Supplementary to implement Figures [7](#fig:mylabelf4) and
[20](#fig:mylabelf4reinter)).

With the [*plotly*](https://CRAN.R-project.org/package=plotly) package,
Figure [20](#fig:mylabelf4reinter) can be further turned into an
...
![Figure 20: An alternative illustration of \"The service of the
Zonguldak Government Hospital, 1924-1937\" (------ The number of
inpatient treatments ■■■■■■ The number of outpatient
treatments).](zonguldak_reinterpreted.png){#fig:mylabelf4reinter
width="12cm" height="15cm"}

Another example can be the Figure [14](#fig:mylabelf12org), where the
...
treatment is displayed. In Figure [14](#fig:mylabelf12org), it is easy
...
faceting with `facet_wrap()` layer.

Figure [21](#fig:mylabelf12reinter) gives the amount of Arsenobenzol,
...
Figures [15](#fig:mylabelf12) and [21](#fig:mylabelf12reinter)).

![Figure 21: An alternative illustration of \"The drugs sent by the
Department of Control of Syphilis to cities for treatment, 1926-1937\"
(The upper-left panel is Arsenobenzol, the upper-right panel is
Bizmopen, the lower-left panel is Mercury, and the lower-rightpanel is
Iodine).](frengi_reinterpreted.png){#fig:mylabelf12reinter width="12cm"
height="12cm"}

Lastly, Figure [18](#fig:mylabelf14org) shows the service of birth and
...
Figure [21](#fig:mylabelf12reinter). Alternatively,
Figure [18](#fig:mylabelf14org) can be split into two sub-line plots
...
the Figure [22](#fig:mylabelf14reinter) which gives more realistic
...
characters) (please have a look at the `R` codes in the supplementary
material to implement Figures [19](#fig:mylabelf14) and
[22](#fig:mylabelf14reinter)).
...
![Figure 22: An alternative illustration of \"The service of birth and
childcare houses, 1926-1937\" (The upper-panel is for inpatient services
and the right-panel is for outpatient services; ■■■■■■ Child ------
Woman).](dogum_reinterpreted.png){#fig:mylabelf14reinter width="12cm"
height="25cm"}
...
in *ggplot2* package, we were mostly challanged with i) multi-line
titles with different font styles, ii) textured patterns, and iii) data
groups where the difference of the frequencies is not monotonic over the
years.
...
`element_textbox()` layer in the *ggtext* package. We also provided `R`
codes to produce the multi-lines in the Figure [2](#fig:mylabelf1org)
and  [14](#fig:mylabelf12org) in the supplementary material.
...
the Figure [15](#fig:mylabelf12) with *ggpattern* and presented it in
Figure [23](#fig:mylabelf12pat). The amount of lines to produce
Figure [23](#fig:mylabelf12pat) is now 51 (2171 characters), where `R`
...
![Figure 23: An alternative illustration of \"The drugs sent by the
Department of Control of Syphilis to cities for treatment, 1926-1937\"
([$\blacksquare$]{style="color: black"} Arsenobenzol $\square$ Bizmopen
[$\blacksquare$]{style="color: gray"} (textured pattern with vertical
lines) Mercury :: Iodine).](frengi_ggpattern.png){#fig:mylabelf12pat
...
in the fight against the infectious diseases in order to . Furthermore,
we also learned that, as in the Figure [13](#fig:mylabelf7), while
epidemics take a very long time to be diminished from the world and even
if the ratio of to is getting larger over the years, increasing the test
capacity was also an old school approach yielding the idea .
