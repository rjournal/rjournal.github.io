...
With only one function, \code{puzzle()}, complex pharmacometrics databases can easily be assembled. Users are able to select from different absorption processes such as zero- and first-order, or a combination of both. Furthermore, datasets containing data from one or more analytes, and/or one or more responses, and/or time dependent and/or independent covariates, and/or urine data can be simultaneously assembled. 

The \pkg{puzzle} package is a powerful and efficient tool that helps modelers, programmers and pharmacometricians through the challenging process of assembling pharmacometrics datasets.
}
...
In this tutorial, we present the \CRANpkg{puzzle} package to the pharmacometrics community, an open source tool for assembling pharmacometrics datasets in R. The \pkg{puzzle} package consists in a group of functions developed to simplify and facilitate the time consuming and error prone task of assembling pharmacometrics datasets. The datasets created are compatible with the formatting requirements of the NONMEM\textsuperscript{\textregistered}~software~\citep{Beal2009}. Moreover, as NONMEM\textsuperscript{\textregistered}~data structures are the gold standard for non-linear mixed effects modeling, the datasets created with the \pkg{puzzle} package are mostly compatible with other non-linear mixed effects softwares such as MONOLIX~\citep{Lixoft}, \CRANpkg{saemix}~\citep{Comets2011},  and \CRANpkg{nlmixr}~\citep{Fidler2019}.  The \pkg{puzzle} package is also available on CRAN and can be installed with the following R code: \code{install.packages("puzzle")}.
...
In this case, the variable type of observation (\textit{i.e.} TYPE) has been appended to the "NM\_PKPD.csv" file. Because a bolus administration has been specified with the argument \code{order} (\textit{i.e.} \code{order = 0}), TYPE has a value of 0 for dosing events, and a value of 1 and 2 for PK and PD records, respectively.  Moreover, the file "coercion.txt" has been generated. The content is printed below:
...
The \code{puzzle()} function builds pharmacometrics ready datasets from a group of tabulated files. For convenience, it always returns a ".csv" file or an R object of class "data.frame" as output. The path to the location of the files to be assembled can be set with the \code{directory} argument. Depending on the pharmacometric analysis to be performed, the tabulated files may contain PK, dose, covariates and/or PD information. 

The PK data may include drug concentration (\textit{i.e.} parent and/or metabolite/s) and/or urine information. The \code{pk} argument is used to input this type of data into the \code{puzzle()} function. The general form is as follows: 
\begin{example}
...
where \code{nm} is a list containing the PK information \code{pk}. Excel files are useful to simultaneously store PK information from two or more analytes. The  \code{puzzle()} function is smart enough to assemble all the PK information at once. To this end, each analyte data has to be contained in its own spreadsheet. Moreover, each sheet has to have the same items, and the items have to be in the same order. 


The PD information may comprise the time course of the clinical endpoint of interest, time to event data, and/or a response outcome. This type of information is handled with the \code{pd} argument. Its behavior is similar to \code{pk}, and thus, it accepts the same type of tabulated files and it requires a similar syntax. For example, in case of multiple endpoints stored in an ".xlsx" file, the user should define: 
\begin{example}
...
A characteristic of the NONMEM\textsuperscript{\textregistered}~datasets is that characters are not allowed, and all the items, except DAT, must be numeric. Hence, the command IGNORE in the \$INPUT block within a NONMEM\textsuperscript{\textregistered}~control stream is commonly used to ignore the label of the data items. The \code{puzzle()} function has the argument \code{ignore} to create a new item for this purpose. For example, if the user sets ignore = "C", the first column of the returned ".csv" file will contain an item called C. The argument \code{missingvalues} allows the user to specify a label for the missing values. The default value is \code{missingvalues = "."}.

The user can control how the records are arranged with the argument \code{arrange}. By default, records are arranged as follows: \code{arrange="ID,TIME,CMT,desc(EVID)"}. Moreover, complex date formatting is also supported. The argument \code{datetimeformat} defines the format for dates and times. By default, the format is \code{datetimeformat = "\%Y-\%m-\%d \%H:\%M:\%S"}. In addition to that, time units may be specified with the argument  \code{timeunits}. For example, \code{timeunits = "hours"}. It should be highlighted that the timezone will affect how times are computed from dates. Thus, depending on your location, times may be slightly different because the default value is \code{timezone = Sys.timezone()}. The user can also identify the person assembling the dataset with the argument \code{username}. The last argument of the \code{puzzle()} function is \code{nm}. It is used to name the output from the function, in other words, the returned ".csv" file. The syntax of the arguments of the  \code{puzzle()} function is presented in Table~\ref{table:syntax}.
