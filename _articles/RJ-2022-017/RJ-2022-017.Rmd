...
abstract: This paper is an introduction to the new package in R called [*smoots*](https://CRAN.R-project.org/package=smoots)
  ...
  series from any other research area. The *smoots* package also provides a useful
  tool for teaching time series analysis, because many practical time series follow
  an additive or a multiplicative component model.
author:
- name: Yuanhua Feng
...
*smoots* [version 1.0.1, @feng2019smoots] for data-driven local
...
proposals in @fenggriesfritz2020. Detailed discussion on the methodology
and the development of the algorithms may be found in that work. Here,
the main algorithm is a fully data-driven IPI [iterative plug-in,
...
@francisco2004plug. The algorithms included in the *smoots* package
differ to their proposal in several ways. (1) The IPI-algorithm is
usually superior to the DPI [see @beran2009modifying for detailed
discussion in models with i.i.d. errors]. (2) In @francisco2004plug the
...
[*locpol*](https://CRAN.R-project.org/package=locpol)
[@ojeda2018locpol],
...
model is defined by
$$\label{eq:Semi-MTS}
y_t= m\left(x_t\right) + \xi_t,   (\#eq:Semi-MTS)$$
...
$\xi_t$. Under the regularity conditions given in @buhlmann1996locally,
...
@fenggriesfritz2020, the required conditions are fulfilled by an ARMA
...
$$\label{eq:LWSformula}
Q = \sum_{t=1}^{n}\left\{y_{t} - \sum_{j=0}^{p}b_{j}\left(x\right)\left(x_{t} - x\right)^{j}\right\}^{2}W\left(\frac{x_{t} - x}{h}\right),   (\#eq:LWSformula)$$
...
considered:
$$\label{eq:kernelfunc}
W\left(u\right) = C_{\mu}\left(1-u^{2}\right)^{\mu}\mathbb{I}_{\left[-1,1\right]}\left(u\right),   (\#eq:kernelfunc)$$
where $C_{\mu}$ is a standardization constant that is indeed irrelevant
for calculating the weights in \@ref(eq:kernelfunc) and $\mu=0,$ 1,
...
$$\label{eq:hOpt}
h_{A}=n^{-\frac{1}{2k+1}}\left(\frac{2\nu + 1}{2\left(k - \nu\right)}\frac{2\pi c_{f}\left(k!\right)^{2}\left(d_{b}-c_{b}\right)R\left(K\right)}{I\left[m^{\left(k\right)}\right]\beta^{2}}\right)^{\frac{1}{2k+1}}.   (\#eq:hOpt)$$
...
model:
$$\label{eq:Semi-ARMA} 
\xi_t= \varphi_1\xi_{t-1} + ... + \varphi_r\xi_{t-r}+ \psi_1\varepsilon_{t-1} + ... + \psi_s\varepsilon_{t-s}+\varepsilon_t,   (\#eq:Semi-ARMA)$$
where $\varepsilon_t$ are i.i.d. innovations. A semiparametric ARMA
(Semi-ARMA) model is then defined by \@ref(eq:Semi-MTS) and
\@ref(eq:Semi-ARMA).

# The proposed IPI-algorithms {#sec:ipi}

Since both $c_{f}$ and $I\left[m^{\left(k\right)}\right]$ in
\@ref(eq:hOpt) are unknown, we need to obtain and insert appropriate
estimates of these two quantities into this formula to achieve a
selected bandwidth $\hat h_{A}$. However, the estimation of $c_{f}$ and
...
following lag-window estimator of $c_f$:
$$\label{eq:cfbw}
\hat c_{f, {\rm M}}=\frac{1}{2\pi}\sum_{l=-M}^{M} w_l \hat \gamma\left(l\right),   (\#eq:cfbw)$$
...
${\rm CF}\cdot h_A$, not $h_A$ itself, where
$$\label{eq:CF}
{\rm CF}= \left\{2k\left[\frac{2 K\left(0\right)}{R\left(K\right)}- 1\right]\right\}^{1/\left(2k+1\right)}   (\#eq:CF)$$
...
details on those topics we refer the reader to @fenggriesfritz2020.
...
2.  Then an IPI-procedure as proposed above to select the bandwidth for
    estimating $m^{\left(\nu\right)}$ according to \@ref(eq:hOpt) is
    carried out with fixed $\hat c_f$ obtained in i).
...
`msmooth()`, `tsmooth()`, `gsmooth()` and `knsmooth()`, designed for
estimating the trend in different ways. Data-driven estimation can be
carried out by each of the first two functions, where `msmooth()` is a
user-friendlier simplified version of `tsmooth()`. Local polynomial
...
and `knsmooth()`, respectively. In the first case, one should choose $p$
...
-   `p` reflects the order of polynomial and currently only `p = 1`
    (local linear) and `p = 3` (local cubic) are selectable.
-   `mu` corresponds to the smoothness parameter $\mu$ of the second
    order kernel function \@ref(eq:kernelfunc) used for weighting. Only
    `0`, `1`, `2` and `3` are valid options.
-   `Mcf` defines the method for estimating $c_f$. For `Mcf = "NP"`, the
    ...
    `"MA"` or `"ARMA"` are selected, $\xi_t$ in model \@ref(eq:Semi-MTS)
    is assumed to follow an AR, MA or ARMA process, respectively, during
    the bandwidth selection and thus, $c_f$ is estimated parametrically.
...
-   With the argument `bvc` the estimation of $c_f$ can be adjusted even
    ...
    will be enlarged by the factor CF as in \@ref(eq:CF). CF is omitted
    for `bvc = "N"`.
-   `bb` describes the boundary method. If `bb = 0` is selected, the
...
-   `alg` defines specific argument settings of `tsmooth()` as named
    ...
    the section [*The IPI-algorithm for estimating $m$*](#sec:trendEst)
    can be directly chosen by `alg = "A"` and `alg = "B"`, respectively.

In accordance with @fenggriesfritz2020, we propose the use of **AlgA**
...
them. They are: `tempNH`, `gdpUS`, `dax` and `vix`, which contain
...
`install.packages("smoots")` and `library(smoots)` can be used to
...
\@ref(eq:Semi-MTS) and \@ref(eq:Semi-ARMA), the time series of the mean
...
this purpose, the function `tsmooth()` is applied. The used settings of
...
Figure [1](#fig:ex1)(a) shows the observations together with the
estimated trend. Here, the selected bandwidth is 0.1221. We see, the
estimated trend fits the data very well. In particular, the trend
increases steadily after 1970 that might be a signal for possible global
warming in the last decades. The residuals are displayed in
Figure [1](#fig:ex1)(b), which look quite stationary. This indicates
that Model \@ref(eq:Semi-MTS) is a suitable approach for this time
series. Moreover, Figures [1](#fig:ex1)(c) and [1](#fig:ex1)(d)
illustrate the data-driven estimates of the first and second derivatives
of the trend respectively, which fit the features of the trend function
very well and provide us further details about the global temperature
changes.
...
which results in
$$\label{eq:resultEx1}
\tilde{\xi}_{t} = 0.7489\tilde{\xi}_{t-1} - 0.3332\varepsilon_{t-1} +\varepsilon_{t}.   (\#eq:resultEx1)$$
We see, the dependence of errors is dominated by a strong positive AR
parameter with a moderate negative MA coefficient.

![Figure 1: []{#fig:ex1 label="fig:ex1"} The NHTM series and the
estimated trend are displayed in (a). The residuals, as well as the
estimated first and second derivatives are shown in (b), (c) and (d),
respectively.](Plot1_TempChange_final.png){#fig:ex1 width="100%"
...
follows Models \@ref(eq:Semi-MTS) and \@ref(eq:Semi-ARMA), we achieve a
...
The results together with the log-data are displayed in
Figure [2](#fig:ex2)(a). We see that the two trend estimates in the
middle part coincide with each other. They differ from each other only
at the boundary points and the kernel estimate is affected by a clear
boundary problem. Thus, the local linear method should be used.
Residuals of this estimate are shown in Figure [2](#fig:ex2)(b). Again,
the estimated first and second derivatives are given in
Figures [2](#fig:ex2)(c) and [2](#fig:ex2)(d), respectively, which help
us to discover more detailed features of the economic development in the
US.
...
residuals:
$$\label{eq:resultEx2}
\tilde{\xi}_{t} = 0.9079\tilde{\xi}_{t-1} + 0.2771\varepsilon_{t-1} + \varepsilon_{t}.   (\#eq:resultEx2)$$

![Figure 2: []{#fig:ex2 label="fig:ex2"} The log-transformed GDP series
and the estimated trends are displayed in (a). The residuals based on
the estimated local linear trend, as well as the estimated first and
second derivatives are shown in (b), (c) and (d),
respectively.](Plot2_GDP_final.png){#fig:ex2 width="100%"
alt="graphic without alt text"}
...
Semi-Log-GARCH model is defined by
$$\label{eq:rt}
r_{t} = \sqrt{v\left(x_{t}\right)}\zeta_{t} \ \text{ with } \ \zeta_{t} = \sqrt{h_{t}}\eta_{t} \ \text{ and}   (\#eq:rt)$$

$$\label{eq:lnht}
\ln\left(h_{t}\right) = \omega + \sum_{i=1}^{l}\alpha_{i}\ln\left(\zeta_{t-i}^{2}\right) + \sum_{j=1}^{s}\beta_{j}\ln\left(h_{t-j}\right),   (\#eq:lnht)$$
...
\@ref(eq:Semi-MTS). Furthermore, define
...
$$\label{eq:lgARMA}
\xi_t = \sum_{i=1}^{l^{*}}\varphi_{i}\xi_{t-i} + \sum_{j=1}^{s}\psi_{j}\varepsilon_{t-j} + \varepsilon_{t}   (\#eq:lgARMA)$$
...
2.  Fit an ARMA($l^{*}, s$) model \@ref(eq:lgARMA) to the residuals
...
3.  Consider
    ...
    volatilities are estimated as
    $$\label{eq:volEstimator}
    	  \hat{\sigma}_t=\exp\left\{ \left[\tilde{\xi}_t - \hat{\varepsilon}_t + \hat{m}\left(x_t\right) - \hat{\mu}_{le}\right] / 2 \right\}.   (\#eq:volEstimator)$$
...
\@ref(eq:lnht) and \@ref(eq:lgARMA), which are
$\alpha_i = \varphi_{i} + \psi_{i}$, $\beta_{j} = -\psi_{j}$, where the
non-existing coefficients are assumed to be zero, and
...
@sucarrat2019log is implemented by estimating the trend function in the
logarithm of the squared returns `yt` with the function `msmooth()` of
...
For reference, `estim3.2 <-  smoots::msmooth(yt,  p = 1,  alg = "A")` is
...
red) are displayed in Figures [3](#fig:ex3)(a) and [3](#fig:ex3)(b).
Moreover, the selected bandwidths are 0.0869 and 0.1013, respectively.
Results in Figure [3](#fig:ex3)(b) indicate that the unconditional
...
of the log-data
$$\label{eq:resultEx3_1}
\tilde{\xi}_t = 0.9692\tilde{\xi}_{t-1} - 0.9221\varepsilon_{t-1} + \varepsilon_{t}\text{,}   (\#eq:resultEx3-1)$$
whereas the re-transformed Log-GARCH(1, 1) formula is given by
$$\label{eq:resultEx3_2}
\ln\left(h_{t}\right) = 0.0685 + 0.0471\ln\left(\zeta_{t-1}^{2}\right) + 0.9221 \ln\left(h_{t-1}\right)   (\#eq:resultEx3-2)$$
following the idea of @sucarrat2016log. The estimated conditional
volatility ($\sqrt{\hat h_t}$) and total volatility ($\hat{\sigma}_t$)
series are displayed in Figures [3](#fig:ex3)(c) and [3](#fig:ex3)(d).

![Figure 3: []{#fig:ex3 label="fig:ex3"} The demeaned DAX returns are
displayed in (a). The log-transformed squared returns and the estimated
trends are shown in (b). Based on the estimated local cubic trend, the
corresponding estimated conditional and total volatilities are
illustrated in (c) and (d), respectively.](Plot3_DAX_final.png){#fig:ex3
width="100%" alt="graphic without alt text"}

# The Semi-Log-ACD model {#sec:SemiLogACD}

A more general framework for modeling non-negative financial time series
is the ACD [autoregressive conditional duration, @engle1998acd] model,
...
approach were introduced by @bauwens2000lacd, where the Type I
...
data by @forstinger2018modelling. In this paper, the application of the
Semi-Log-ACD model will be illustrated by the CBOE Volatility Index
(VIX) from 1990 to July 2019, denoted by $V_t$, $t=1, ..., n$. The data
was again downloaded from Yahoo Finance. The Semi-Log-ACD model for
$V_t$ is defined by
$$\label{eq:SemiLogACD}
V_t=g\left(x_t\right) \lambda_t e_t,   (\#eq:SemiLogACD)$$
where $u_t=\lambda_t e_t$ follows a Log-ACD and $g\ge0$ is a smooth mean
function in $V_t$, $\lambda_t\ge0$ is the conditional mean and $e_t$ is
an i.i.d. series of non-negative random variables. It is assumed that
$E\left(\lambda_t\right)=E\left(e_t\right)=1$. Further investigation on
this model can be carried out similarly to that on the Semi-Log-GARCH
model by replacing $r_t^2$, $h_t$ and $\eta_t^2$ there with $V_t$,
$\lambda_t$ and $e_t$, respectively. The Semi-Log-ACD model can be
similarly estimated. Discussion of those details is omitted. For further
information we refer the reader to @forstinger2018modelling and
references therein.
...
![Figure 4: []{#fig:ex4 label="fig:ex4"} The VIX series is displayed in
(a). The log-transformed index and the estimated trends are shown in
(b). Based on the estimated local linear trend, the corresponding
estimated conditional and total means are illustrated in (c) and (d),
respectively.](Plot4_VIX_final.png){#fig:ex4 width="100%"
alt="graphic without alt text"}
...
The original series of $V_t$ is shown in Figure [4](#fig:ex4)(a). The
trend was estimated from the log-transformation of $V_t$ using both
*AlgA* and *AlgB* with the selected bandwidths 0.0771 and 0.1598,
respectively. The data (black), the local linear trend (red) and the
local cubic trend (blue) are displayed in Figure [4](#fig:ex4)(b). The
results using both algorithms are quite similar, except the local cubic
estimates are smoother. From the residuals of the local linear approach
the following ARMA(1, 1) model (see Step 2 in the code) is obtained:
$$\label{eq:resultEx4_1} 
\tilde{\xi}_{t} = 0.9626\tilde{\xi}_{t-1} - 0.0707\varepsilon_{t-1} + \varepsilon_{t}.   (\#eq:resultEx4-1)$$
Subsequently, the estimated log-form of the conditional mean function is
given by
$$\label{eq:resultEx4_2}
\ln\left(\lambda_{t}\right) = 0.0010 + 0.8919\ln\left(u_{t-1}\right) + 0.0707\ln\left(\lambda_{t-1}\right).   (\#eq:resultEx4-2)$$
The estimated conditional means and the total means in the original data
by the local linear approach are shown in Figures [4](#fig:ex4)(c) and
[4](#fig:ex4)(d). We see the results fit the data very well. This model
can be applied for forecasting the VIX in the future.

# Concluding remarks {#sec:conclusion}

In this paper the methodological background for developing the R package
*smoots* (version 1.0.1) is summarized. The usage of the main functions
in this package is explained in detail. To show the wide applicability
of this approach two new semiparametric models for analyzing financial
time series are also introduced. Data examples show that the proposed
...
the publication of the R package *smoots*. We would also like to thank
Dr. Marlon Fritz for helpful discussions.
:::
