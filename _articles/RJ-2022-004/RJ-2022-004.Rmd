...
expressed as absolute or computed data (Table
[1](#table:1){reference-type="ref" reference="table:1"}), but we also
weighted their strengths and weaknesses.
...
::: {#table:1}
  ...
  : The selection of indicators used for aggregation.
:::
...
[1](#figure:workflow){reference-type="ref"
reference="figure:workflow"}).

![Overall workflow for the research methodology, depicting the main
steps of the process, from the selection of indicators to the resulting
aggregate indices.](workflow){#figure:workflow width="100%"}
...
studies (Table [1](#table:1){reference-type="ref" reference="table:1"}),
but approach a different method and process data in R. The multiple
packages make this language a powerful tool for data processing and
visualization [@barry_collections_2018] with multiple possibilities of
application, especially in spatial analysis
[@lovelace_geocomputation_2019]. As the sub-indicators had different
measurement scales, variable standardization was handled using the
option scale=true in prcomp(). Complying with the minimum required rule
of 3:1 [@jollands_usefulness_2003], the use of this method allows
summarizing the entire set of individual indicators
...
variables (Table [1](#table:1){reference-type="ref"
reference="table:1"}) observed in 34 small towns located in Oltenia
region, in two reference years 1992 and 2018. The main characteristic of
this method is the reduction of a great number of variables to a smaller
number (principal components), as linear combinations of the variables
in the multivariate set, with minimum loss of information. Principal
components outputs where computed using prcomp() for an improved
numerical accuracy and displayed using screeplot().

![Scree plot of the principal components. The graphic representation of
variances allows for easier selection of relevant
components.](screeplot){#figure:screeplot width="100%"}
...
[2](#figure:screeplot){reference-type="ref"
reference="figure:screeplot"}). If we use the Kaiser criterion, we see
that only the first four (1992), respectively three (2018) components
have eigenvalues greater than 1. But these components explain only 79.5%
of the variation in the data for 1992 and 73.2% for 2018 and usually a
...
[2](#table:2){reference-type="ref" reference="table:2"}).
...
::: {#table:2}
  ...
  : Summary of Principal Component Analysis (PCA) used for the selection
  of principal components. The components with a cumulative variance
  above 0.75 are considered \"strong\".
:::
...
(Table [3](#table:3){reference-type="ref" reference="table:3"}). The
data showed that the highest value of the variable is 0.690,
respectively 0.634 (Table [4](#table:4){reference-type="ref"
reference="table:4"}), therefore we considered that the level of
correlation between each component is important above the value of 0.3.
...
::: {#table:3}
  ...
  : Changes in the variances of the first five principal components in
  the analyzed time interval: 1992-2018.
:::
...
::: {#table:4}
  ...
  $k_{1}$                           0.076   -0.338             0.064                                 0.407   -0.035                     -0.023
  $k_{2}$                           0.000            -0.175            -0.042                        0.128            -0.013            -0.084
  $k_{3}$                          -0.350            -0.129             0.278                       -0.621            -0.153   -0.015    0.133
  $k_{4}$                          -0.711             0.089   -0.113   -0.240                        0.338            -0.279   -0.047   -0.158
  $k_{5}$                          -0.483             0.215    0.128   -0.130                        0.043             0.164   -0.154   -0.010
  $k_{6}$                           0.233                      0.142   -0.096                        0.094   -0.384                     -0.060
  $k_{7}$                                   -0.175    0.223    0.170    0.125                                -0.215            -0.021    0.186
  $k_{8}$                                    0.212    0.051    0.064   -0.022                                -0.031    0.045    0.001   -0.210
  $k_{9}$                           0.683    0.266    0.145   -0.056                                          0.071   -0.183   -0.095 
  $k_{10}$                                   0.355   -0.322   -0.054    0.186                                 0.045   -0.286   -0.037 
  $k_{11}$                                   0.020    0.148    0.015    0.005                                -0.067   -0.083   -0.005   -0.211
  $k_{12}$                          0.697    0.175    0.313                                                  -0.030   -0.011   -0.018 
  $k_{13}$                                   0.239   -0.140   -0.104                                          0.004   -0.238   -0.116 

  : The first five values of the principal components for the variables
  $k_{1}$-$k_{13}$ in 1992 and 2018.
:::
...
number of libraries and schools) (Table
[4](#table:4){reference-type="ref" reference="table:4"}).
...
them graphically. The two graphs were generated in R using the ggplot2,
...
[3](#figure:pca){reference-type="ref" reference="figure:pca"}, 1992).
This aspect is changing entirely in 2018, if we observe the directions
along which we have the highest variance of the date. Thus, the
demographic variables (migration rate and elders) strongly influence
component 1, while the social-economic variables influence the second
component. For a better view of differences amongst towns, the points
representing the urban settlements, vary in size and intensity according
to the values of the third and fourth components (Figure
[3](#figure:pca){reference-type="ref" reference="figure:pca"}, 2018).

![Principal Components Analysis of shrinkage
indicators](pca){#figure:pca width="100%"}
...
[4](#figure:rdc){reference-type="ref" reference="figure:rdc"}).

![Future (2018-2030, left) and Overall (1992-2030, right) trends of
Regional Demographic Change (RDC-type) for small towns in South-West
Development Region of Oltenia](rdc){#figure:rdc width="100%"}
...
and faster ageing (IV) (Figure [4](#figure:rdc){reference-type="ref"
reference="figure:rdc"} & Table
[\[table:5\]](#table:5){reference-type="ref" reference="table:5"}).

::: tabular
rlrrrr & & &\
I & Faster ageing. Shrinking/Lower growth & 11 towns & 32% & 11 towns &
32%\
II & Slower ageing. Shrinking/Lower growth & 2 towns & 6% & 2 towns &
6%\
...
[1](#table:1){reference-type="ref" reference="table:1"}) considered
important in the shrinking process of cities. The values of CSI for 1992
and 2018 were calculated using the following formula:
...
variations and St.dev. values \> 0.04 (Table
[5](#table:6){reference-type="ref" reference="table:6"} & Figure
[5](#figure:scatterplot){reference-type="ref"
reference="figure:scatterplot"}).
...
::: {#table:6}
  ...
  : The computed values of CSI for each town, showing shrinkage in 1992,
  2018 and the 2030 forecast.
:::

![Variation of CSI values for the reference
period](scatterplot){#figure:scatterplot width="100%"}
...
period with the same future trend (Figure
[6](#figure:shrinking){reference-type="ref"
reference="figure:shrinking"}).

![City types according to CSI](shrinking){#figure:shrinking width="90%"}
...
[6](#figure:shrinking){reference-type="ref"
reference="figure:shrinking"}). Some of these towns have kept and
developed some forms of tourism activities (holiday homes, private
camps, accommodation and leisure facilities), benefiting from
considerable investments following various programmes financed by the EU
for fostering the tourism sector.
