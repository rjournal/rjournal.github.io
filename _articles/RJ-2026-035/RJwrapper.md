...
abstract: |
  ...
  work, we present a new R software package psvmSDR that implements a
  ...
  attractive alternative to the dr R package that implements classical
  SDR methods.
address:
- |
  Jungmin Shin\
...
author:
- by Jungmin Shin, Seung Jun Shin and Andreas Artemiou
bibliography:
- manuscript_publication.bib
title: "psvmSDR: An R Package for a Unified Algorithm for Sufficient
  Dimension Reduction via Principal Machines"
---
...
$\mathcal{S}_{Y\mid \mathbf{X}}= span\{\mathbf{B}\}$ in
...
$\boldsymbol{\Sigma}= \COV(\mathbf{X})$, and $[u]_+ = \max\{0, u\}$.
...
(\hat \alpha_k, \hat \boldsymbol{\beta}_k) = \mathop{\rm argmin}_{\alpha, \boldsymbol{\beta}} \boldsymbol{\beta}^{\top} \hat \boldsymbol{\Sigma}\boldsymbol{\beta}+ \frac{\lambda}{n} \sum_{i=1}^{n}L_k\left(\tilde y_{k,i}, \alpha + \boldsymbol{\beta}^\top \mathbf{z}_i \right), \qquad k = 1, 2, \cdots h,
...
Table [\[tbl:loss\]](#tbl:loss){reference-type="ref"
reference="tbl:loss"} provides a complete list of PMs implemented in
[**psvmSDR**](https://CRAN.R-project.org/package=psvmSDR) with the
corresponding loss functions.

::: tabular
clcllc Type & Method & Response & Margin (Residual) & Loss & `loss`\
\* RPM & SVM & \* $\begin{array}{cc}
         Y \in \mathbb{R},\\
         \tilde{Y}_k = \mathbb{1}\{Y \ge c_k\} \\
         \qquad~~ - \mathbb{1}\{Y < c_k\}
         \end{array}$ & \*$m_k = \tilde Y_k f$ & ${[1-m_k]}_+$ &
`‘svm’`\
& Logistic & & & $\log(1 + e^{-m_k})^{-1}$ & `‘logit’`\
& $L_2$-SVM & & & $\{[1-m_k]_+\}^2$ & `‘l2svm’`\
& LS-SVM & & & $[1-m_k]^{2}$ & `‘lssvm’`\
\*LPM& WSVM & \*${Y = \tilde Y_k \in \{-1,1\}}$ & \*$m = Yf$ &
$\pi_{k}(Y)[1-m]_+$ & `‘wsvm’`\
& Wlogistic & & & $\pi_{k}(Y)\log(1 + e^{-m})^{-1}$ & `‘wlogit’`\
& W$L_2$-SVM & & & $\pi_{k}(Y)\{[1-m]_+\}^2$ & `‘wl2svm’`\
& WLS-SVM & & & $\pi_{k}(Y)[1-m]^{2}$ & `‘wlssvm’`\
& Quantile & \*${Y = \tilde Y_k \in \mathbb{R}}$ &\*$r = Y - f$ &
$r \{c_k - I(r<0)\}$& `‘qr’`\
...
(\alpha_{0,k}, g_{0,k}) = \mathop{\rm argmin}_{\alpha \in \mathbb{R}, g \in \mathcal{H}} \VAR \{\psi(\mathbf{X})\}+\lambda \mathbb{E}\left[ {L}_k \left\{ \tilde{Y}_{k}, f(\mathbf{X}) \right\} \right], \qquad k = 1, 2, \cdots, h,
...
$$\begin{align}
\label{eqn:fx}
g(\mathbf{x}) = \sum_{j=1}^b \beta_j \left\{\psi_{j}(\mathbf{x})- \bar \psi_j \right\}
\end{align}   (\#eq:eqnfx)$$
with
$$\begin{align}
\label{eqn:psi_gen}
\psi_j(\mathbf{x})=\left\{\mathbf{k}(\mathbf{x})\right\}^{\top} \mathbf{q}_j / \lambda_j, \quad j=1, \cdots b, \qquad and \qquad \bar \psi_j = \sum_{i=1}^{n}\psi_{j}\left(\mathbf{x}_{i}\right)/n,
...
$$\begin{equation}
\label{eqn:sample_nonlinear_pm}
(\hat \alpha_k, \hat \boldsymbol{\beta}_k) = \mathop{\rm argmin}_{\alpha, \boldsymbol{\beta}} {\boldsymbol{\beta}}^{\top} \hat \boldsymbol{\Sigma}{\boldsymbol{\beta}}+\frac{\lambda}{n} 
...
$$\begin{align}
 \label{eqn:obj}
L(\boldsymbol{\beta}) = \boldsymbol{\beta}^\top Diag\left\{0, \hat \boldsymbol{\Sigma}\right\} \boldsymbol{\beta}+ \frac{\lambda}{n}\sum_{i=1}^{n}L_k\left(\tilde y_{k,i}, \boldsymbol{\beta}^\top \mathbf{z}_i \right).
\end{align}   (\#eq:eqnobj)$$
...
[\[tbl:loss\]](#tbl:loss){reference-type="ref" reference="tbl:loss"}.
...
<figure id="fig:comparison_methods" data-latex-placement="!t">
<figure id="fig:linear_time">
<img src="comp_time_linear_revised_pdf.png" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 1: Linear PSVM</figcaption>
</figure>
<figure id="fig:nonlinear_time">
<img src="comp_time_nonlinear_revised_pdf.png" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 2: Kernel PSVM</figcaption>
</figure>
<figcaption>Figure 3: Average computing times of the QP and CGD
...
Table [\[tbl:ft_list\]](#tbl:ft_list){reference-type="ref"
reference="tbl:ft_list"} summarizes the main functions along with their
...
::: tabularx
m1.8cm X m1.8cm m3.2cm Function & Description & Class & Compatible
methods\
`psdr()` & Basic function for applying linear PMs with various loss
functions & &\
r)1-2 `rtpsdr()` & Real-time sufficient dimension reduction through
principal (weighted) least squares SVM & &\
`npsdr()` & Basic function for applying kernel PMs with various loss
functions & `‘npsdr’` &\
:::
...
- `mtype`: a margin type, which is either margin (`"m"`) or residual
  (`"r"`) (See, Table [\[tbl:loss\]](#tbl:loss){reference-type="ref"
  reference="tbl:loss"}). Only needed when a user-defined loss is used.
  The default is \"m\".
...
<embed src="ex1_psdr.eps" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 4: Scatter plots of <span
class="math inline"><em>Y</em></span> versus the first sufficient
predictor <span
class="math inline"><em>B̂</em><sub>1</sub><sup>⊤</sup><strong>X</strong></span>
from the generic method <code>plot.psdr()</code>.</figcaption>
</figure>
...
Figure [5](#fig:ex1_wsvm){reference-type="ref" reference="fig:ex1_wsvm"}
...
<embed src="ex1_wsvm.eps" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 5: A scatter plot of the predicted projections on the
first and second sufficient predictors, i.e., <span
class="math inline"><em>b̂</em><sub>1</sub><sup><em>T</em></sup><strong>X</strong></span>,
<span
class="math inline"><em>b̂</em><sub>2</sub><sup><em>T</em></sup><strong>X</strong></span>,
respectively, estimated by PWSVM with <code>loss=‘‘wsvm’’</code>.
</figcaption>
</figure>
...
Table [\[tbl:loss\]](#tbl:loss){reference-type="ref"
reference="tbl:loss"}. A basic syntax of the arbitrary loss function
follows:
...
Figure [6](#fig:plot and Gn){reference-type="ref"
reference="fig:plot and Gn"}.
...
<figure id="fig:plot and Gn" data-latex-placement="!t">
<embed src="ex2_Gn.eps" style="width:55.0%" />
<figcaption>Figure 6: BIC-type criterion values computed from
...
Figure [7](#fig:nonlinear){reference-type="ref"
...
<figure>
<embed src="fig_psvm_new.eps" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 7: Linear PSVM</figcaption>
</figure>
<figure>
<embed src="fig_kpsvm_new.eps" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 8: Kernel PSVM</figcaption>
</figure>
<figcaption>Figure 9: Scatter plots of the response and the first
...
<figure id="fig:Boston_Housing" data-latex-placement="!t">
<figure id="fig:Boston_x1">
<embed src="fig_boston_psvm_d1.eps" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 10: A scatter plot with lowess curve of versus
estimated <span
class="math inline"><em>b̂</em><sub>1</sub><sup>⊤</sup><strong>X</strong></span>.</figcaption>
</figure>
<figure id="fig:Boston_x2">
<embed src="fig_boston_psvm_d2.eps" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 11: A scatter plot with lowess curve of versus
estimated <span
class="math inline"><em>b̂</em><sub>2</sub><sup>⊤</sup><strong>X</strong></span>.</figcaption>
</figure>
<figcaption>Figure 12: Plots of the reduction of the predictors against
...
[\[fig:Boston_Housing\]](#fig:Boston_Housing){reference-type="eqref"
reference="fig:Boston_Housing"}, we are able to claim that the
...
Figure [11](#fig:wisc_1){reference-type="ref" reference="fig:wisc_1"}
...
Figure [12](#fig:wisc_2){reference-type="ref" reference="fig:wisc_2"},
where the two classes are clearly separated by
$\hat{\phi}_1(\mathbf{x})$.

<figure id="fig:real_wdbc" data-latex-placement="!t">
<figure id="fig:wisc_1">
<embed src="fig_wdbc_linear.eps" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 13: A scatter plot of the predicted projections,
<span
class="math inline"><em>b̂</em><sub>1</sub><sup><em>T</em></sup><strong>X</strong></span>,
<span
class="math inline"><em>b̂</em><sub>2</sub><sup><em>T</em></sup><strong>X</strong></span>
estimated by PWLR with the WDBC dataset. With only 2 sufficient
predictors, the two classes are well separated.</figcaption>
</figure>
<figure id="fig:wisc_2">
<embed src="fig_wdbc_nonlinear.eps" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Figure 14: Displays boxplots of classification result using
kernel PWLR. Two classes are well classified with a sole nonlinear
mapping <span
class="math inline"><em>ϕ̂</em>(<strong>x</strong>)</span>.<br />
</figcaption>
</figure>
<figcaption>Figure 15: WDBC dimension reduction result.</figcaption>
</figure>
