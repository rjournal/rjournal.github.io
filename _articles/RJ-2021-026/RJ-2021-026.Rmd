...
The seedCCA package is recently developed mainly to implement the seeded
CCA. However, the package can fit a collection of the statistical
...
$U$ and $V$ is as follows: $$\label{cor1}
{\rm cor}(U,V)=\frac{\mathbf{a}^{{\rm {T}}}
\boldsymbol{\Sigma}_{xy}\mathbf{b}}
{\sqrt{\mathbf{a}^{{\rm {T}}}\boldsymbol{\Sigma}_x\mathbf{a}}
\sqrt{\mathbf{b}^{{\rm {T}}}\boldsymbol{\Sigma}_y\mathbf{b}}}.$$ We seek
to find $\mathbf{a}$ and $\mathbf{b}$ to maximize ${\rm cor}(U,V)$ by
...
1.  The first canonical variate pair
    ...
    ([\[cor1\]](#cor1){reference-type="ref" reference="cor1"}).

2.  The second canonical variate pair
    $(U_2 = \mathbf{a}_2^{{\rm {T}}}\mathbf{X},
    V_2 = \mathbf{b}_2^{{\rm {T}}}\mathbf{Y})$ is constructed from the
    maximization of ([\[cor1\]](#cor1){reference-type="ref"
    reference="cor1"}) with restriction that
...
3.  At the $k$ step, the $k$th canonical variate pair
    ...
    is obtained from the maximization of
    ([\[cor1\]](#cor1){reference-type="ref" reference="cor1"}) with
...
show the following relation: $$\label{eq2}
...
The relation in ([\[eq2\]](#eq2){reference-type="ref" reference="eq2"})
directly indicates that $\mathbf{M}_x$ and $\mathbf{M}_y$ form basis
matrices of
...
\label{eq3}\end{aligned}$$ In $\mathbf{R}_{x,u_1}$ and
$\mathbf{R}_{y,u_2}$, the numbers of $u_1$ and $u_2$ are called
termination indexes. They decide the number of projections of
${\boldsymbol{\Sigma}}_{xy}$ and ${\boldsymbol{\Sigma}}_{yx}$ onto
${\boldsymbol{\Sigma}}_x$ and ${\boldsymbol{\Sigma}}_y$, respectively.
Also define that $$\begin{aligned}
...
{\boldsymbol{\Sigma}}_{yx}.
\label{eq4}\end{aligned}$$ In [@seed], it is shown that
${\mathcal{S}}({\mathbf{M}}_{x,u_1}^{0})
= {\mathcal{S}}(\boldsymbol{\Sigma}^{-1}_{x}\boldsymbol{\Sigma}_{xy})$
and ${\mathcal{S}}({\mathbf{M}}_{y,u_2}^{0})
= {\mathcal{S}}(\boldsymbol{\Sigma}^{-1}_{y}
\boldsymbol{\Sigma}_{yx})$ in ([\[eq4\]](#eq4){reference-type="ref"
reference="eq4"}). Hence ${\mathbf{M}}_{x,u_1}^{0}$ and
${\mathbf{M}}_{y,u_2}^{0}$ can be used to infer $\mathbf{M}_x$ and
$\mathbf{M}_y$, respectively. One clear advantage to use
${\mathbf{M}}_{x,u_1}^{0}$ and ${\mathbf{M}}_{y,u_2}^{0}$ is no need of
the inversion of $\boldsymbol{\Sigma}_x$ and $\boldsymbol{\Sigma}_y$.
...
Recall $\mathbf{R}_{x,u_1}$ in ([\[eq3\]](#eq3){reference-type="ref"
reference="eq3"}) and ${\mathbf{M}}_{x,u_1}^{0}$ in
...
\label{helland}$$ It is noted that this PLS representation in
([\[helland\]](#helland){reference-type="ref" reference="helland"}) is
...
seedCCA(X, Y, type=\"seed2\", ux=NULL, uy=NULL, u=10, eps=0.01,
 cut=0.9, d=NULL, AS=TRUE,\
         scale=FALSE)

The main function seedCCA returns "seedCCA" class and three subclasses
depending on the values of type. The values of type and its resulting
...
The function seedCCA prints out estimated canonical coefficient matrices
for all subclasses, and additionally does canonical correlations for
"finalCCA" subclasses, although it produces more outputs. For details,
the readers are recommended to run ?seedCCA after loading the seedCCA
package. It should be noted that the seedCCA package must be loaded
before using all functions in the package.: of CCA and corpcor
([@corpcor]).
...
components of cor, xcoef, ycoef, Xscores and Yscores. The first
component is cor is the sample canonical correlations. The next two
ones, xcoef, and ycoef, are the estimated canonical matrices for
$\mathbf{X}$ and $\mathbf{Y}$. The last two components, which are
Xscores and Yscores, are the estimated canonical variates for
$\mathbf{X}$ and $\mathbf{Y}$. A command plot(object) constructs a plot
of the cumulative correlations against the number of canonical pairs.
The plot(object) will provide a 90% reference line as default, and users
can change the reference line with plot(object, ref=percent).
...
::: center
![Cumulative canonical correlation plot in pulp data in Section
3.2](pulp_cor.pdf){#cca width="12cm" height="6cm"}
:::

According to Figure [1](#cca){reference-type="ref" reference="cca"}(a)
and (b), with 80% cumulative canonical correlations, two canonical pairs
are enough, while three canonical pairs should be good with the default
90%.
...
order of seedCCA(X, Y\[,1\], type=\"cca\") and seedCCA(Y\[,1\], X,
type=\"cca\") does not matter, and any of them yields the same results.
Also, the commands of coef(object) and fitted(object) return the
...
results in "finalCCA" subclass. Regardless of the order of X and Y, the
lower dimensional set alone is reduced in the initial step. Therefore,
seedCCA(X, Y, type=\"seed1\") and seedCCA(Y, X, type=\"seed1\")
basically produce the same seeded CCA results. For type=\"seed1\", the
values of the options of ux, uy, u, eps, and AS affect the
implementation, whose defaults are NULL, NULL, 10, 0.01, and TRUE,
respectively.

Option u controls the maximum number of projections unless both ux and
uy are specified. The option ux=k works only when the dimension of the
first set X is bigger than that of the second set Y. Then, the maximum
number of projections becomes the value given in ux=k. The option uy
works in the opposite way to ux. The options of AS=TRUE and eps control
automatic termination of the projections before reaching the maximum
given in u, ux, or uy. The projection is terminated if the increment
gets less than the value given in eps. Then, the first candidate value,
which satisfies the stopping criteria, is suggested as a proper value of
projections. If any of ux, uy, and u are specified not enough to
guarantee the automatic stopping, a notice is provided to increase it.

After running seedCCA(X, Y, type=\"seed1\"), a plot for the proper
selection of u is automatically constructed, and a blue vertical bar in
the plot is the suggested value of u.
...
For fit.seed1.ab, the first set A is reduced in the initial step. The
output component initialMX0 is the estimate of
...
initialMY0 and newY are produced. So, it is observed that the canonical
correlations and suggested values of u from fit.seed1.ab and
fit.seed1.ba are equal, not to mention that fit.seed1.ab\$xcoef and
fit.seed1.ba\$ycoef are the same. The selection plot for u is reported
in Figure [2](#data1u){reference-type="ref" reference="data1u"}, and
three projections are suggested. Since ux is not given big enough in
seedCCA(A, B, type=\"seed1\", ux=2), the following warning is given:

    The maximum number of iterations is reached. So, users must choose u bigger than 2.

::: center
![Selection plot of u generated from seedCCA(A, B, type=\"seed1\") in
Section 3.4](data1u.pdf){#data1u width="6cm" height="6cm"}
:::

Next, we change the values of ux, uy, AS, and eps. Since the usage of
these options for type=\"seed1\" are the same as that for type=\"seed2\"
and type=\"pls\". To measure the computing time, the tictoc package
([@tictoc]) is used with Intel(R) Core(TM)i7 2.9GHz and 12GB Ram
computer.
...
Usage of AS should be noted. With bigger choices of u and AS=FALSE, the
running time of the function will be longer.
...
$\hat{\boldsymbol\Sigma}_{xy}$. The option cut=$\alpha$ determines a set
of the eigenvectors whose cumulative proportions of their corresponding
eigenvalues is bigger than equal to $\alpha$. For the set of
eigenvectors to be chosen conservatively, we set the default of cut at
0.9. Also, users can directly give the number of eigenvectors using d.
Unless d is NULL, the option cut is discarded. This means that cut works
only when d=NULL. If users want to use d, then a function covplot should
be run first. The function covplot has the option mind, which set the
number of the eigenvalues to show their cumulative percentages. Its
default is NULL, and then it becomes $\min(p,r)$. The function returns
the eigenvalues, the cumulative percentages, and the number of the
...
regularized and seeded CCAs via the tictoc package. For seedCCA, we use
the default value of cut.
...
::: center
![Scree plots for the selection of sets of eigenvectors to replace
$\rm{cov}(\mathbf{X}, \mathbf{Y})$ generated from covplot(X, Y, mind=10)
in Section 3.5](data2cov10s.pdf){#data2cov width="6cm" height="6cm"}
:::

Since type=\"seed2\" reduces the dimensions of X and Y at the
initialized CCA step, the output components of initialMX0, initialMY0,
newX and newY and d are reported.

The plot generated from covplot(X, Y, mind=10) is given in Figure
[3](#data2cov){reference-type="ref" reference="data2cov"}. According to
Figure [3](#data2cov){reference-type="ref" reference="data2cov"}, the
first two, three, and four eigenvalues account for 79.6%, 91.8%, and
95.9% of the total variation of $\hat{\boldsymbol{\Sigma}}_{xy}$,
respectively. Using 90% conservative guideline, it is determined that
the first three largest eigenvectors replace
$\hat{\boldsymbol{\Sigma}}_{xy}$ well enough.

The selection plot of ux and uy is given in Figure
[4](#data2uxuy){reference-type="ref" reference="data2uxuy"}. The figure
suggests that $u_x$ and $u_y$ are equal to 7 and 6, respectively.

::: center
![Selection plot of ux and uy generated from seedCCA(X, Y) in Section
3.5](data2uxuy3.pdf){#data2uxuy width="12cm" height="6cm"}
:::
...
The results shown in Figures [5](#fpair){reference-type="ref"
reference="fpair"}--[6](#spair){reference-type="ref" reference="spair"}
are equivalent to the analysis discussed in [@ecca].
...
::: center
![the first pair of canonical variates from regularized CCA and seeded
CCA marked with genotype in Section 3.5](1stpair.pdf){#fpair
width="12cm" height="6cm"}
:::

::: center
![the second pair of canonical variates from regularized CCA and seeded
CCA marked with diet in Section 3.5](2ndpair.pdf){#spair width="12cm"
height="6cm"}
:::

According to Figures [5](#fpair){reference-type="ref"
reference="fpair"}--[6](#spair){reference-type="ref" reference="spair"},
the first pair of canonical variates from both CCAs distinguish genotype
very well, but their second pairs marked with diet are quite complex. To
have more insight into the results for the second pair on a diet,
multivariate analysis of variance is fitted. Further pairwise comparison
is done via lsmeans ([@lsmeans]) with level 5% and $p$-values adjusted
...
estimator in seedCCA, one needs to implement seedCCA(X, Y,
type=\"pls\"). This results in "seedpls" subclass. An important matter
in partial least squares is that the first set of the variable must be
predictors. The response variable can be either univariate or
multivariate. Option u is recommended to set reasonably small because
the estimated coefficients are reported up to the value given in u. If
scale=TRUE, the predictors are standardized to have zero sample means
...
default of u in both coef and fitt is NULL. In both functions, usage of
u is equivalent. If u=k is specified, only the estimated coefficients
and fitted values computed from k projections are reported. All of the
coefficient estimates and fitted values are reported up to u, if u=NULL.
...
::: center
![Selection plot of u generated from seedCCA(X, Y1, u=10,
type=\"pls\")(left) and seedCCA(X, Y1, u=3, type=\"pls\",
scale=TRUE)(right) in Section 3.6](pls_selu.pdf){#pls_selu width="12cm"
height="6cm"}
:::

The selection of projections for two partial least squares by seedCCA(X,
Y1, u=10, type=\"pls\") and seedCCA(X, Y1, u=3, type=\"pls\",
scale=TRUE) is given in Figure [7](#pls_selu){reference-type="ref"
reference="pls_selu"}. According to Figure
[7](#pls_selu){reference-type="ref" reference="pls_selu"}, the proper
value of projection is suggested at 6 for fit.pls1.10 object, while the
