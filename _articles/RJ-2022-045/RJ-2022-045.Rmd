...
- name: Camille Frévent
  affiliation: University of Lille, CHU Lille
  address:
  - |-
    ULR 2694 - METRICS: Évaluation des technologies de santé et des
    pratiques médicales, INRIA‐MODAL
  - F-59000 Lille
  - |
    France
...
$\mathcal{W}$ by
$$\label{eq:cluster}
\mathcal{W} = \{ w_{i,j} \ / \ 1 \le |w_{i,j}| \le \frac{n}{2}, \ 1 \le i,j \le n \},   (\#eq:cluster)$$
where $w_{i,j}$ is the disc centered on $s_i$ that passes through $s_j$
and $|w_{i,j}|$ corresponds to the number of sites in $w_{i,j}$.
Figure [1](#fig:potentialclusters) illustrates the set of potential
clusters defined with a circular scanning window with
Equation \@ref(eq:cluster) on a set of eight administrative areas in
France.

![Figure 1: Set of potential clusters defined with a circular scanning
window of variable center and radius with Equation \@ref(eq:cluster) on
a set of eight administrative areas in France. Each potential cluster is
represented with a red circle.](clusters_all.png){#fig:potentialclusters
...
here, is to use a Monte-Carlo method (see Section [2.4](#sec:signif) for
more details).
...
nonparametric one.\
Figure [2](#fig:schema_multi) summarizes the different types of
...
![Figure 2: Summary of spatial scan statistics for multivariate data.
The table indicates the question that can be asked for the detection of
clusters. It then indicates the methods to be used according to the
distribution of the data as well as the ways to interpret the detected
clusters. Spatial scan statistics for multivariate data can be used to
detect spatial clusters on any type of spatial data (lattice,
geostatistical, point data) modeled by vectors. The detected clusters
can be characterized by computing the mean or median of each variable
inside and outside each cluster. ](code_RJ_multi.png){#fig:schema_multi
width="100%" alt="graphic without alt text"}
...
$\forall i \in \left[\!\left[ 1 ; n \right]\!\right], \ X_i \sim \left\{ \begin{array}{ll}
...
favored.\
Figure [3](#fig:schema_unifonc) summarizes the different types of
...
![Figure 3: Summary of spatial scan statistics for univariate functional
data. The table indicates the question that can be asked for the
detection of clusters on a set of curves. It then indicates the methods
to be used according to the distribution of the data as well as the ways
to interpret the detected clusters. Spatial scan statistics for
univariate functional data can be used to detect spatial clusters on any
type of spatial data (lattice, geostatistical, point data) observed over
a period of time. The detected clusters can be characterized by
computing the mean or median curve inside and outside each cluster.
](code_RJ_unifonc.png){#fig:schema_unifonc width="100%"
...
Figure [4](#fig:schema_multifonc) summarizes the different types of
...
![Figure 4: Summary of spatial scan statistics for multivariate
functional data. The table indicates the question that can be asked for
the detection of clusters of multivariate curves. It then indicates the
methods to be used according to the distribution of the data as well as
the ways to interpret the detected clusters. Spatial scan statistics for
...
](code_RJ_multifonc.png){#fig:schema_multifonc width="100%"
alt="graphic without alt text"}
...
statistic developed by @oja and detailed in
Section [2.1](#subsec:multi). They defined the pointwise multivariate
ranks as
...
and the sgn function is the same as in Section [2.1](#subsec:multi).\
...
Table [1](#tab:summary_performances) summarizes the methods and their
...
Table [2](#tab:computation_time) gives an idea of the computation time
...
::: {#tab:summary_performances}
  ...
  : Table 1: Performance in terms of power, true positive rate and false
  positive rate of spatial scan statistics for multivariate data (MG and
  MNP), univariate functional data (PFSS, DFFSS, NPFSS and URBFSS) and
...
::: {#tab:computation_time}
  ...
  for the different scan statistics methods among 169 sites with *a
  priori* clusters comprising between 1 and 50% of the sites (default
...
(Equation \@ref(eq:cluster)). If you want to select clusters according
...
(Figures [5](#fig:description_curves) and [6](#fig:description_mean)).
This step allows us to see if sites seem to aggregate and therefore if
launching a cluster detection is relevant, and if a temporal variation
of the concentrations is visible, in which case a functional method will
be more relevant than a multivariate approach summarizing each curve by
its mean.

![Figure 5: Daily concentration curves of $\text{NO}_2$, $\text{O}_3$,
...
period.](curves_real_data.png){#fig:description_curves width="100%"
alt="graphic without alt text"}

![Figure 6: Spatial distributions of the average concentrations of
$\text{NO}_2$, $\text{O}_3$, $\text{PM}_{10}$ and $\text{PM}_{2.5}$ over
the period from May 1, 2020 to June 25, 2020. A spatial heterogeneity of
the average concentration (over the study period) is observed for the
four pollutants. The spatial distributions of the average concentrations
of $\text{PM}_{10}$ and $\text{PM}_{2.5}$ are also observed to be
similar.](map_real_data.png){#fig:description_mean width="100%"
alt="graphic without alt text"}

The maps in Figure [6](#fig:description_mean) show a spatial
heterogeneity of the average concentration for each pollutant. Thus
spatial scan statistics seem to be suitable to highlight the presence of
*cantons*-level spatial clusters of pollutants concentrations. Moreover
since the curves in Figure [5](#fig:description_curves) show a marked
temporal variability during the period from May 1, 2020 to June 25, 2020
a functional approach is more appropriate. However for sake of
completeness we will also perform a multivariate spatial scan statistic
approach anyway. Since small clusters of pollution are more relevant for
interpretation because the sources of the pollutants are very localized,
we will consider an *a posteriori* filtering of maximum radius equal to
10 km.
...
explained in Section [3.1](#subsec:computing), [@spatialdisease]
...
noticed in Section [4.1](#subsec:airpollution), we will apply an *a
...
secondary clusters [@spatialscanstat]) by setting the `typeI` value at
1.
...
Once the scan procedure is completed, the plot function can be used. For
...
(Figure [7](#fig:mlc_mnp)). For the latter, since the system of the
...
<figure id="fig:mlc_mnp">
<p><img src="MLC_MNP_circle1.png" style="width:49.0%"
alt="graphic without alt text" /><img src="MLC_MNP1.png"
style="width:49.0%" alt="graphic without alt text" /></p>
<img src="MLC_MNP_schema1.png" style="width:49.0%"
alt="graphic without alt text" />
<figcaption>Figure 7: Visualization of the most likely cluster with the
function <code>plot</code> with the types <code>"map"</code> (panel a),
<code>"map2"</code> (panel b) and <code>"schema"</code> (panel c) for
the MNP scan procedure computed with the function
<code>SpatialScan</code> on the average concentrations of <span
class="math inline">NO<sub>2</sub></span>, <span
class="math inline">O<sub>3</sub></span>, <span
class="math inline">PM<sub>10</sub></span> and <span
class="math inline">PM<sub>2.5</sub></span>. The first two types of
visualization require a spatial object corresponding to the spatial
units (here the 169 <em>cantons</em> of the <em>Nord-Pas-de-Calais</em>
region of northern France). The visualization option
<code>"schema"</code> allows in the other case to draw a schema of the
...
The user can also use the function `plotSummary` to display the spider
chart corresponding to the detected cluster
(Figure [8](#fig:spiderchart)).

``` r
 plotSummary(res_mnp, type = "median", only.MLC = TRUE)
```

![Figure 8: Spider chart obtained with the function `plotSummary` for
...
cluster.](MNP_spider_chart.png){#fig:spiderchart width="100%"
alt="graphic without alt text"}

The MLC is located in the area of Lille. The summary and
Figure [8](#fig:spiderchart) show that it is a cluster of overpollution
(except for the pollutant $\text{O}_3$). This cluster is especially
characterized by high concentrations of $\text{NO}_2$ and
$\text{PM}_{2.5}$ which indicates pollution from road traffic and from
the residential sector (auxiliary heating in particular). As the adverse
health effects of air pollution (and their potential synergistic effect)
are well established, such a result could inform local stakeholders
about immediate interventions around the area of Lille to reduce the air
pollution levels.

We have obtained some first results however the curves on
Figure [5](#fig:description_curves) present a marked temporal
variability during the study period. Thus it could be interesting to
apply functional spatial scan statistics.
...
multivariate counterpart MRBFSS [@notre_fonctionnel1]. As mentioned in
Section [4.2](#subsec:resmulti) we decide to use the set of potential
clusters *a priori* in the Equation \@ref(eq:cluster) which corresponds
to the recommended approach of [@spatialdisease], and to the default
...
![Figure 9: Visualization of the detected clusters with the function
...
Lille.](MLC_urbfss.png){#fig:mlc_urbfss width="70.0%"
alt="graphic without alt text"}

Again the MLC is located in the area of Lille
(Figure [9](#fig:mlc_urbfss)).
...
curves inside and outside the cluster (Figure [10](#fig:curves)): this
...
<p><img src="curves_urbfss.png" style="width:49.0%"
alt="graphic without alt text" /><img src="summary_urbfss.png"
style="width:49.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 10: Characterization of the most likely cluster
detected by the URBFSS scan approach in the context of univariate
functional data consisting of the <span
class="math inline">NO<sub>2</sub></span> concentrations in northern
France over the period from May 1, 2020 to June 25, 2020 with the
functions <code>plotCurves</code> (left panel) and
<code>plotSummary</code> (right panel). The most likely cluster is
characterized by high concentration curves with a median concentration
curve higher than outside the cluster. </figcaption>
</figure>
...
![Figure 11: Visualization of the most likely cluster with the function
`plot` with `type = "map2"` for the MRBFSS scan procedure computed using
...
Lille.](MLC_MRBFSS.png){#fig:mlc_mrbfss width="70.0%"
alt="graphic without alt text"}

The detected cluster is exactly the same as before and is therefore
located in the urban area of Lille (Figure [11](#fig:mlc_mrbfss)).

Again we will display the curves in the cluster by adding the curve of
the global median (Figure [12](#fig:curves_multi)), as well as the
median curves inside and outside the cluster which show that this is a
cluster of high concentrations of $\text{NO}_2$, $\text{PM}_{10}$ and
$\text{PM}_{2.5}$ (Figure [13](#fig:summary_multi)). As mentioned in
Section [4.2](#subsec:resmulti), in environmental science it is
well-known that $\text{NO}_2$ and $\text{PM}_{2.5}$ are more frequent in
urban areas due to road traffic and population density so this is
consistent with the cluster observed here. As the adverse health effects
of air pollution and the combined effects of air pollutants are well
established, this result could enable interventions by local authorities
around the Lille area to reduce air pollution.
...
![Figure 12: Characterization of the most likely cluster detected by the
MRBFSS scan approach in the context of multivariate functional data
consisting of the $\text{NO}_2$, $\text{O}_3$, $\text{PM}_{10}$ and
...
](curves_mrbfss.png){#fig:curves_multi width="100%"
alt="graphic without alt text"}

![Figure 13: Characterization of the most likely cluster detected by the
MRBFSS scan approach in the context of multivariate functional data
consisting of the $\text{NO}_2$, $\text{O}_3$, $\text{PM}_{10}$ and
$\text{PM}_{2.5}$ concentrations in northern France over the period from
...
outside the cluster. ](summary_mrbfss.png){#fig:summary_multi
width="100%" alt="graphic without alt text"}
...
presented in the Section [2](#sec:method). The potential clusters
