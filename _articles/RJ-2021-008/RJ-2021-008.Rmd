...
[@dag:2018], [welchADF](https://CRAN.R-project.org/package=welchADF) by
[@villacorta:2017], [WRS2](https://CRAN.R-project.org/package=WRS2) by
[@wilcox:2018]. These packages contain only the Brunner-Dette-Munk,
...
([\[eq:1\]](#eq:1){reference-type="ref" reference="eq:1"}) .
...
standard deviations of normal groups computed as in
([\[eq:AG1\]](#eq:AG1){reference-type="ref" reference="eq:AG1"}).
...
([\[eq:AG2\]](#eq:AG2){reference-type="ref" reference="eq:AG2"}).
$$wi=\frac{1/S_{\bar{X}_i}^2}{\sum_{i=1}^k 1/S_{\bar{X}_i}^2} \label{eq:AG2}$$
The weight mean is computed using the $w_i$ in
([\[eq:AG3\]](#eq:AG3){reference-type="ref" reference="eq:AG3"}).
$$\bar{X}^*=\sum_{i=1}^k w_i\bar{X}_i \label{eq:AG3}$$ The values of
$t_i=(\bar{X}_i-\bar{X}^*)/S_{\bar{X}_i} \sim t_{n_i-1}$ are transformed
...
The test statistic of AG test is computed as in
([\[eq:AG4\]](#eq:AG4){reference-type="ref" reference="eq:AG4"}).
$$T_{AG}=\sum_{i=1}^k z_i^2 \label{eq:AG4}$$ The $H_0$ is rejected when
$T_{AG}>\chi^2_{(k-1); \alpha}$.

## Alvandi et. al. Generalized F (AGF) test

[@alvandi:2012] proposed the test statistic in
[\[eq:AGF1\]](#eq:AGF1){reference-type="ref" reference="eq:AGF1"} as an
...
([\[eq:AF1\]](#eq:AF1){reference-type="ref" reference="eq:AF1"}).
...
freedom of the AF test statistic is computed in
([\[eq:AF2\]](#eq:AF2){reference-type="ref" reference="eq:AF2"}).
...
[@box:1954] proposed the test statistic in
([\[eq:BO1\]](#eq:BO1){reference-type="ref" reference="eq:BO1"}).
...
fredom of the test statistic computed as in
([\[eq:BF1\]](#eq:BF1){reference-type="ref" reference="eq:BF1"}).
...
$X_{i1}, X_{i2}, ..., X_{in_i} \sim N(\mu_i, \sigma_i^2)$ and the
standard deviations of normal groups computed as in
([\[eq:B21\]](#eq:B21){reference-type="ref" reference="eq:B21"}).
$$S_{\bar{X}_i}=\frac{\sum_{i=1}^k \sum_{j=1}^{n_i} (X_{ij}-\bar{X}_i)^2}{n_i(n_i-1)} \label{eq:B21}$$
The weights computed using the $S_{\bar{X}_i}$ as in
([\[eq:B22\]](#eq:B22){reference-type="ref" reference="eq:B22"}).
$$w_i=\frac{1/S_{\bar{X}_i}^2}{\sum_{i=1}^k 1/S_{\bar{X}_i}^2} \label{eq:B22}$$
The weighed mean computed using the $w_i$'s as in
([\[eq:B23\]](#eq:B23){reference-type="ref" reference="eq:B23"}).
$$\bar{X}^*=\sum_{i=1}^k w_i\bar{X}_i \label{eq:B23}.$$ The values of
$t_i=(\bar{X}_i-\bar{X}^*)/S_{\bar{X}_i} \sim t_{n_i-1}$ are transformed
using Bailey's (1980) normality transformation.
...
test computed as in ([\[eq:B24\]](#eq:B24){reference-type="ref"
reference="eq:B24"}).
...
[@cochran:1937] proposes the test statistic in
([\[eq:CF1\]](#eq:CF1){reference-type="ref" reference="eq:CF1"}).
...
[@li:2011] proposed the test statistic in
([\[eq:FA1\]](#eq:FA1){reference-type="ref" reference="eq:FA1"}).
...
[@weerahandi:1995] proposed the test statistic in
([\[eq:GF1\]](#eq:GF1){reference-type="ref" reference="eq:GF1"}) using
...
([\[eq:tj\]](#eq:tj){reference-type="ref" reference="eq:tj"}).
...
[@mehrotra:1997] proposed the test statistic in
([\[eq:MBF1\]](#eq:MBF1){reference-type="ref" reference="eq:MBF1"}),
which is a modification of BF, to well-performing in case of small
sample size.
...
([\[eq:MBF2\]](#eq:MBF2){reference-type="ref" reference="eq:MBF2"}).
...
the test statistic of the PB test is computed as in
([\[eq:PB2\]](#eq:PB2){reference-type="ref" reference="eq:PB2"}).
$$\label{eq:PB2}
...
[@berry:2002] proposed the test statistic in
([\[eq:PF1\]](#eq:PF1){reference-type="ref" reference="eq:PF1"}) as the
permutational alternative of F-test.
$$T_{PF}=\frac{(T-N\bar{X}^*)/(k-1)}{(V-T)/(N-k)} \label{eq:PF1}$$ where
$T=\sum_{i=1}^k n_i\sum{x}_i^2$, $\bar{X}^*=1/N\sum{n_i\bar{x}_i}$ and
$V=\sum_{i=1}^k \sum_{j=1}^{n_i} X_{ij}^2$. The $H_0$ is rejected when
$T_{PF}>F_{k-1, N-k; \alpha}$.

## Scott-Smith (SS) test

[@scott:1971] proposed the test statistic in
([\[eq:SS1\]](#eq:SS1){reference-type="ref" reference="eq:SS1"}).
...
[@welch:1951] improved the test statistic in
[\[eq:W1\]](#eq:W1){reference-type="ref" reference="eq:W1"} based on the
...
Welch test computed as in [\[eq:W2\]](#eq:W2){reference-type="ref"
reference="eq:W2"}.
$$v=\frac{(k^2-1)/3}{\sum_{i=1}^k \frac{(1-h_i)^2}{n_i-1}} \label{eq:W2}$$

## Welch-Aspin (WA) test

[@aspin:1948] proposed the test statistic in
([\[eq:WA1\]](#eq:WA1){reference-type="ref" reference="eq:WA1"}) with a
...
data are observations and species are the labels of species of the corn
hybrids.

::: example
\# Call the doex package \> library(doex) \# print hybrid data of
Weerahandi (1995) \> hybrid

data species 1 7.4 A 2 6.6 A 3 6.7 A 4 6.1 A 5 6.5 A 6 7.2 A 7 7.1 B 8
7.3 B 9 6.8 B 10 6.9 B 11 7.0 B 12 6.8 C 13 6.3 C 14 6.4 C 15 6.7 C 16
6.5 C 17 6.8 C 18 6.4 D 19 6.9 D 20 7.6 D 21 6.8 D 22 7.3 D

\# observations of the hybrid data \> hybrid$data
 [1] 7.4 6.6 6.7 6.1 6.5 7.2 7.1 7.3 6.8 6.9 7.0 6.8 6.3 6.4 6.7 6.5 6.8 6.4 6.9 7.6 6.8 7.3
 
 # group labels of the hybrid data
> hybrid$species \[1\] A A A A A A B B B B B C C C C C C D D D D D
Levels: A B C D

\# The ggplot2 package can be used to plot the box plot of the data in
Figure 1. \> ggplot(hybrid, aes(x = species, y = data)) + \>
geom_boxplot() + \> ylab(\"Yield\") + \> xlab(\"Corn Species\")
:::

<figure id="fig:ex1">
<img src="boxplot_ex1.png" />
<figcaption>Boxplot of the data in Example 1.</figcaption>
</figure>

::: example
\# Look at the summary statistics of the data before using the tests. \#
Use psych package to obtain the descriptive statistics of the hybrid
data \> library(psych)
...
Descriptive statistics by group group: A vars n mean sd median trimmed
mad min max range skew kurtosis se X1 1 6 6.75 0.48 6.65 6.75 0.52 6.1
7.4 1.3 0.11 -1.7 0.19
---------------------------------------------------------- group: B vars
n mean sd median trimmed mad min max range skew kurtosis se X1 1 5 7.02
0.19 7 7.02 0.15 6.8 7.3 0.5 0.28 -1.72 0.09
---------------------------------------------------------- group: C vars
n mean sd median trimmed mad min max range skew kurtosis se X1 1 6 6.58
0.21 6.6 6.58 0.3 6.3 6.8 0.5 -0.13 -2.02 0.09
---------------------------------------------------------- group: D vars
n mean sd median trimmed mad min max range skew kurtosis se X1 1 5 7
0.46 6.9 7 0.59 6.4 7.6 1.2 0.04 -1.84 0.21

\# It is seen that the variances of the species are unequal \# Thus we
need to use the tests for equality of the group means under unequal
variances \# \# Examples of the use of the AF and GF tests on the hybrid
data are given in the follows. \# The following code performs the
Approximate F-test on the hybrid data.

\> library(doex) \> AF(hybrid$data,hybrid$species)

\# This function returns a result matrix consists of a test statistic,
degrees of freedom, and p-value of Approximate F-test as follows:

Test Statistic df1 df2 p-value Approximate F 1.8538 2 12 0.1943
:::
...
\# The p-value of the GF test is computed Monte-Carlo estimates and its
size is \# controlled with the rept parameter in the function. It is
implemented as \# default rept=10000 \# This function returns the
p-value of the Generalized F-test as follows:

p-value Generalized F 0.0492

\# The results of the AF and GF tests are different at the nominal level
0.05. \# It is needed to investigate the performance of these tests in
...
weight_data dose 1 22.69 0 2 26.59 0 3 28.85 0 4 28.03 0 5 29.05 0 6
23.61 0 7 22.21 0 8 26.81 0 9 26.01 0 10 25.98 0 . . . . . . . . . 70
26.31 500 71 30.61 500 72 26.48 500 73 24.31 500 74 27.98 500

\# The ggplot2 package can be used to plot the box plot of the data in
Figure 2. \> ggplot(born_weight_data, aes(x = dose, y = weight_data)) +
\> geom_boxplot() + \> ylab(\"Born Weight (gr)\") + \> xlab(\"Dose
Treatment\")
:::

<figure id="fig:ex2">
<img src="boxplot_ex2.png" />
<figcaption>Boxplot of the data in Example 2.</figcaption>
</figure>

::: example
\# Describe the born weight data by species using describe.by(.)
function \>
describe.by(born_weight_data$weight_data, born_weight_data$dose)

#The output of the describe.by function as follows:

Descriptive statistics by group group: 0 vars n mean sd median trimmed
mad min max range skew kurtosis se X1 1 20 25.73 2.02 26.1 25.74 2.43
22.21 29.05 6.84 -0.1 -1.16 0.45 ------------------------------- group:
5 vars n mean sd median trimmed mad min max range skew kurtosis se X1 1
19 23.52 3.9 22.75 23.51 4.28 16.34 30.95 14.61 0.01 -0.97 0.89
------------------------------- group: 50 vars n mean sd median trimmed
mad min max range skew kurtosis se X1 1 18 23.79 2.83 24.11 23.84 1.92
17.54 29.21 11.67 -0.28 0.03 0.67 ------------------------------- group:
500 vars n mean sd median trimmed mad min max range skew kurtosis se X1
1 17 23.72 4.08 24.31 23.76 3.84 16.13 30.61 14.48 -0.4 -0.91 0.99

\# It is seen that the variances of the dose groups may be unequal \# To
conclude whether the variance homogenity assumption is valid, \# Levene
test is used.

\> library(car) \> car::LeveneTest(weight_data   dose)

\# LeveneTest(.) function returns the test statistic and \# p-value of
Levene variance homogeneity test as follows:

Levene's Test for Homogeneity of Variance (center = median) Df F value
Pr(\>F) group 3 3.3819 0.0229 \* --- Signif. codes: 0 '\*\*\*' 0.001
'\*\*' 0.01 '\*' 0.05 '.'
...
\# The GF, AF, and PB are used to conclude there is a significance
difference between \# the mean born weight of mice according to used
dose group.

\> doex::GF(weight_data, dose)

p-value Generalized F 0.0331

\> doex::AF(weight_data, dose)

Test Statistic df1 df2 p-value Approximate F 1.9408 3 57 0.1484

\> doex::PB(weight_data, dose)

p-value Parametric Bootstrap 0.0366

\# The results of the GF and PB tests indicate that there is a
significant difference, \# while the result of the AF indicates that
there is no significant difference between \# the mean born weight of
mice according to used dose group. \# It is also needed to investigate
the performance of these tests in \# a Monte-Carlo simulation study.
:::
...
[@cavus:2019] proposed the penalized power of the test in
([\[eq:pp\]](#eq:pp){reference-type="ref" reference="eq:pp"}) to compare
the power of the test even if Type I error probabilities are different.
...
<figure id="fig:k3">
<img src="error_plot_k3.png" />
<figcaption>Type I error probability of the tests for <span
class="math inline"><em>k</em> = 3</span>.</figcaption>
</figure>

<figure id="fig:k5">
<img src="error_plot_k5.png" />
<figcaption>Type I error probability of the tests for <span
class="math inline"><em>k</em> = 5</span>.</figcaption>
</figure>

<figure id="fig:k7">
<img src="error_plot_k7.png" />
<figcaption>Type I error probability of the tests for <span
class="math inline"><em>k</em> = 7</span>.</figcaption>
</figure>

The boxplots in Figs. [3](#fig:k3){reference-type="ref"
reference="fig:k3"},[4](#fig:k5){reference-type="ref"
reference="fig:k5"},[5](#fig:k7){reference-type="ref"
reference="fig:k7"} are constructed for several heteroscedasticity
scenarios. In this way, the ability of the tests to control the Type I
error probability are obtained. According to the
Fig.[3](#fig:k3){reference-type="ref" reference="fig:k3"}, AF, AG, B2,
GF, JF, MBF, PB, WA and WE test controls the Type I error probability in
the [@bradley:1978] limits which are shown with dashed red lines.
However, the AGF, CF, PF, and SS test could not control the Type I error
probability for $k=3$. The GF test controls the Type I error probability
unlike in the case of $k=3$ in Fig.[4](#fig:k5){reference-type="ref"
reference="fig:k5"}. The AF, AG, B2, MBF, PB, and WA test control Type I
error probability for $k=7$. When the results are summarized, it is
concluded that the AGF, BF, BX, CF, MW, PF, and SS test could not
control the Type I error probability for each of the $k$'s. Thus, the
tests only which control Type I error probability in the limits are
considered in the next section for power comparisons to avoid making a
wrong decision.
...
Tables [1](#tab1){reference-type="ref" reference="tab1"},
[2](#tab2){reference-type="ref" reference="tab2"},
[3](#tab3){reference-type="ref" reference="tab3"}. The mean parameter of
the samples are shown as the effect size $\Delta_i$ in each line. This
means that the mean parameter of the samples are zero except the last
sample is $\Delta_i$.
...
Figs.[3](#fig:k3){reference-type="ref" reference="fig:k3"},
[4](#fig:k5){reference-type="ref" reference="fig:k5"} and
[5](#fig:k7){reference-type="ref" reference="fig:k7"} are used to
summarize the results. The tests which can control the Type I error
probability are obtained as robust tests with respect to the
[@bradley:1978]'s limits. Then, the penalized power of the robust tests
is calculated. The reason for using this method was to consider two
...
The [doex](https://CRAN.R-project.org/package=doex) package contains the
several tests for testing equality of normally distributed groups' means
under unequal variances. Most of these tests are not available in any R
package. Thus, we fill this gap by implementing the package in the
