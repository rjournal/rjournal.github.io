...
creation of these matrices. More recently, in reference the increasing use of \CRANpkg{sf} structures \citep{sf}, the \CRANpkg{sfdep} package \citep{sfdep} has presented generally similar functionality by wrapping \CRANpkg{spdep} functions with functions that follow the \CRANpkg{sf} naming convention (function names starting with \texttt{st\_}), as well as a ``use a data.frame for everything'' attitude.
