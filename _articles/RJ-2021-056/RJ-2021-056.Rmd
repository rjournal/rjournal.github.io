...
::: example
x\[x \> threshold\] \<- threshold x\[is.na(x)\] \<- median(x,
na.rm=TRUE)
:::
...
aggregate of interest, say the mean of x, evolves as it gets processed.
The instinctive way to do this is to edit the code by adding statements
to the script that collect the desired information.

::: example
meanx \<- mean(x, na.rm=TRUE) x\[x \> threshold\] \<- threshold meanx
\<- c(meanx, mean(x, na.rm=TRUE)) x\[is.na(x)\] \<- median(x,
na.rm=TRUE) meanx \<- c(meanx, mean(x, na.rm=TRUE))
:::
...
Figure [1](#fig:streams){reference-type="ref" reference="fig:streams"}.
The 'primary data flow' is developed by a user as a script. In the
previous example, this concerns processing x. When the script runs, some
kind of logging information, which we label the 'secondary data flow' is
derived implicitly by an abstraction layer.
...
![Primary and secondary data flows in an R script. The primary flow
follows the execution of an R script, while in the background a
secondary data flow (e.g. logging information) is
created.](fig/streams.pdf){#fig:streams width="\\textwidth"}
...
namespace of the package to manage option settings, while futile.logger
[@rowe2016futile.logger] implements a custom global option settings
manager that is somewhat comparable to R's own options() function.
...
script as in Figure [1](#fig:streams){reference-type="ref"
reference="fig:streams"} but to compare the performance of R
expressions. Both packages export a function that accepts a sequence of
expressions to profile. These functions take control of expression
...
[@gaslam2019unitizer], and tinytest [@loo2019tinytest]. The first three
packages (testthat, RUnit, and testit) all export assertion functions
that generate condition signals to convey information about test
results. Packages RUnit and testit use sys.source() to run a file
containing unit test assertions and exit on the first error while
testthat uses eval() to run expressions, capture conditions, and test
results and reports afterward. The unitizer framework is different
because it implements an interactive prompt to run tests and explore
their results. Rather than providing explicit assertions, unitizer
stores results of all expressions that return a visible result and
compares their output at subsequent runs. Interestingly, unitizer allows
for optional monitoring of the testing environment. This includes
environment variables, options, and more. This is done by manipulating
the code of (base) R functions that manage these settings and masking
the original functions temporarily. These masking functions then provide
parts of the secondary data flow (changes in the environment). Finally,
tinytest is based on the approach that is the topic of this paper, and
it will be discussed as an application below.
...
$$\begin{aligned}
 ...
 e_2(e_1(\{\})) &= \{(\texttt{"x"}, \texttt{10}),(\texttt{"y"}, \texttt{20})\}\end{aligned}$$
...
$$\begin{aligned}
e_2(e_1(\{\})) = (e_2\circ e_1)(\{\}).\end{aligned}$$ And in general, we
can express the final state $\mathcal{U}$ of any environment after
executing a sequence of expressions $e_1,e_2,\cdots,e_k$ as:
$$\begin{aligned}
\mathcal{U} = (e_k\circ e_{k-1}\circ \cdots\circ e_1)(\{\}),
\label{eq:compose}\end{aligned}$$ where we assumed without loss of
generality that we start with an empty environment. We will refer to the
sequence $e_1\ldots e_k$ as the 'primary expressions' since they define
a user's main data flow.
...
amounts to the cumbersome solution: $$\begin{aligned}
\mathcal{U}\cup\{(\texttt{"N"},k)\} = (n\circ e_k \circ n\circ e_{k-1}\circ n\circ \cdots n\circ e_{1})(\{\}),
\label{eq:insert}\end{aligned}$$ where the number of executed
expressions is stored in N. We shall refer to n as a 'secondary
expression', as it does not contribute to the user's primary data flow.

The above procedure can be simplified if we define a new function
composition operator $\circ_n$ as follows: $$\begin{aligned}
a\circ_n b = a\circ n \circ b.\end{aligned}$$ One may verify the
associativity property $a\circ_n(b\circ_n c)=(a\circ_n
b)\circ_n c$ for expressions $a$, $b$, and $c$, so $\circ_n$ can,
indeed, be interpreted as a new function composition operator. Using
this operator we get $$\begin{aligned}
\mathcal{U}\cup\{(\texttt{"N"},k-1)\} = (e_k\circ_n e_{k-1}\circ_n \cdots\circ_n e_1)(\{\}),
\label{eq:compose}\end{aligned}$$ which gives the same result as
Equation [\[eq:insert\]](#eq:insert){reference-type="ref"
reference="eq:insert"} up to a constant.
...
::: example
m: if (!exists(\"M\")) M \<- sum(memory.profile()) else M \<- c(M,
sum(memory.profile()))
:::
...
::: example
e1: x \<- rnorm(10) e2: x\[x\<0\] \<- 0 e3: print(x)
:::
...
::: example
v: if (!exists(\"V\")) V \<- logical(0) x0 \<- x if (identical(x0,x)) V
\<- c(V, FALSE) else V \<- c(V, TRUE) x0 \<- x
:::

After running $e_3\circ_v e_2\circ_v e_1$, the variable V equals c(TRUE,
FALSE), indicating that $e_2$ changed x, and $e_3$ did not.
...
The source() function reads an R script and executes all expressions in
the global environment. A simple variant of source() that counts
expressions as they get evaluated can be built using parse() and eval().

::: example
run \<- function(file)

expressions \<- parse(file) runtime \<- new.env(parent=.GlobalEnv)

n \<- 0 for (e in expressions) eval(e, envir=runtime) n \<- n + 1
message(sprintf(\"Counted runtime
:::
...
found by setting the parent of runtime equal to .GlobalEnv. Now, given a
file \"script.R\".

::: example
\# contents of script.R x \<- 10 y \<- 2\*x
:::

An interactive session would look like this.

::: example
\> e \<- run(\"script.R\") Counted 2 expressions \> e$x
  [1] 10
\end{example}
So, contrary to the default behavior of \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 source()}}%
{source()}, variables are assigned
in a new environment. This difference in behavior can be avoided by evaluating
expressions in \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 .GlobalEnv}}%
{.GlobalEnv}. However, for the next step, it is important
to have a separate runtime environment.


We now wish to give the user some control over the secondary data stream.  In
particular, we want the user to be able to choose when \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 run()}}%
{run()} starts
counting expressions. Recall that we demand that this is done by direct
communication to \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 run()}}%
{run()}. This means that side-effects such as setting a
special variable in the global environment or a global option is out of the
question. Furthermore, we want to avoid code inspection: the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 run()}}%
{run()}
function should be unaware of what expressions it is running exactly.  We start
by writing a function for the user that returns \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 TRUE}}%
{TRUE}.
\begin{example}
  start_counting <- function() TRUE
\end{example}
Our task is to capture this output from \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 run()}}%
{run()} when \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 start\_counting()}}%
{start\_counting()}
...
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 run()}}%
{run()} can find it. To achieve this, we use the following helper function.
\begin{example}
...
\end{example}
This function accepts a function (\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 fun}}%
{fun}) and an environment
(\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 envir}}%
{envir}). It returns a function that first executes \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 fun(...)}}%
{fun(...)},
copies its output value to \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 envir}}%
{envir}, and then returns the output to the
...
\begin{example}
  ...
  > f()
  [1] TRUE
  > store$counting \[1\] TRUE
:::

Observe that our call to f() returns TRUE as expected but also exported
a copy of TRUE into store. The reason this works is that an R function
'remembers' where it is created. The function f() was created inside
capture(), and the variable envir is present there. We say that this
'capturing' version of start_counting has a *local side-effect*: it
writes outside of its own scope but the place where it writes is
controlled.
...
::: example
\# contents of script1.R x \<- 10 start_counting() y \<- 2\*x
:::

In an interactive session, we would see this.

::: example
\> e \<- run(\"script1.R\") Counted 1 expressions \> e$x
  [1] 10
  > e$y \[1\] 20
:::
...
start_counting() function is only masked during the execution of run().
The captured version of start_counting()as a side effect stores its
output in store. We call this a 'local side-effect' because store is
never seen by the user: it is created inside run() and destroyed when
run() is finished.
...
akin to source() that can gather and communicate useful process metadata
while executing a script. Moreover, the user of the script can convey
information directly to the file runner, while it runs, without relying
on global side-effects. This is achieved by first creating a user-facing
function that returns the information to be sent to the file runner. The
file runner locally masks the user-facing version with a version that
copies the output to an environment local to the file runner before
...
::: example
start_counting \<- function(data) attr(data, \"n\") \<- 0 data
end_counting \<- function(data) message(sprintf(\"Counted attr(data,
\"n\") \<- NULL data
:::
...
::: example
\> out \<- 3 + start_counting + sin + cos + end_counting Counted 2
expressions \> out \[1\] 0.9900591
:::
...
We will first demonstrate working with the lumberjack pipe operator. The
function start_log() accepts an R object and a logger object. It
...
the output. It then calls the add() method of the logger with the input
and output so that the logger can compute and store the difference. The
dump_log() function accepts an R object, calls the \$dump() method on
the attached logger (if there is any), removes the logger from the
object and returns the object. An interactive session could look as
follows.

::: example
\> library(lumberjack) \> out \<- women \> start_log(simple$new()) %L>%
...
\end{example}
Here, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 simple\$new()}}%
{simple\$new()} creates a logger object that registers whether an R
object has changed or not. There are other loggers that compute more involved
differences between in- and output. The \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 \$dump()}}%
{\$dump()} method of the logger
writes the logging output to a csv file.  


For larger scripts, a file runner called \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 run\_file()}}%
{run\_file()} is available in
\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont lumberjack}}%
{lumberjack}. As an example, consider the following script. It converts
columns of the built-in \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 women}}%
{women} data set to SI units (meters and kilogram)
and then computes the body-mass index of each case.
\begin{example}
  # contents of script2.R
  start_log(women, simple$new()) women$height <- women$height \*
2.54/100 women$weight <- women$weight \* 0.453592
...
\begin{example}
  ...
  > read.csv("women_simple.csv")
    step                time                                 expression changed
  1    1 2019-08-09 13:11:25             start_log(women, simple$new())
FALSE 2 2 2019-08-09 13:11:25 women$height <- women$height \* 2.54/100
TRUE 3 3 2019-08-09 13:11:25 women$weight <- women$weight \* 0.453592
TRUE 4 4 2019-08-09 13:11:25
women$bmi <- women$weight/(women$height)^2    TRUE
  > head(e$women,3) height weight bmi 1 1.4732 52.16308 24.03476 2
1.4986 53.07026 23.63087 3 1.5240 54.43104 23.43563
:::
...
::: example
\# contents of bmi.R bmi \<- function(weight, height) weight/(height\^2)
:::

A simple tinytest test file could look like this.

::: example
\# contents of test_script.R data(women) women$height <- women$height \*
2.54/100 women$weight <- women$weight \* 0.453592 BMI \<- with(women,
bmi(weight,height) )

expect_true( all(BMI \>= 10) ) expect_true( all(BMI \<= 30) )
:::
...
::: example
\> source(\"bmi.R\") \> library(tinytest) \> out \<-
run_test_file('test_script.R') Running
test_script.R\...\...\...\...\..... 2 tests OK \> print(out,
passes=TRUE) ----- PASSED : test_script.R\<7--7\> call\|
expect_true(all(BMI \>= 10)) ----- PASSED : test_script.R\<8--8\> call\|
expect_true(all(BMI \<= 30))
:::

In this application, the file runner locally masks the expect\_\*()
...
Compared to the basic version described in the 'build your own source()'
section, this file runner keeps some extra administration, such as the
line numbers of each masked expression. These can be extracted from the
output of parse(). The package comes with a number of assertions in the
form of expect\_\*() functions. It is possible to extend tinytest by
registering new assertions. These are then automatically masked by the
file runner. The only requirement on the new assertions is that they
return an object of the same type as the built-in assertions (an object
of class ' tinytest').
...
executed. In practice, we use R's parse() and eval() to add secondary
data stream to user code, or build a special 'pipe' operator. Local
masking and local side effects allow a user to control the secondary
data flow without global side-effects. The result is a clean separation
of concerns between the primary and secondary data flow, that does not
rely on condition handling, is void of global side-effects, and that is
implemented in pure R.

[^1]: A native R Reference Class, an ' R6' object [@chang2019r6], or any
    other reference type object implementing the proper API.
