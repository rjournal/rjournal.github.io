...
abstract: >
  ...
  latent traits and their responses to specific items. One key aspect of IRT is 
  the assumption about the distribution of the latent variables, 
  which can influence parameter estimation accuracy. 
  While a normal distribution has been conventionally assumed, 
  this may not always be appropriate. When the assumption of normality is violated, 
  latent distribution estimation (LDE) can enhance parameter estimation accuracy 
  by accommodating non-normal characteristics. Despite there being several methods proposed 
  for LDE in IRT, there is a lack of software designed to handle their implementation. 
  This paper introduces \CRANpkg{IRTest}, a software program developed for 
  IRT analysis that incorporates LDE procedures. 
  It outlines the statistical foundation of LDE, 
  details the functionalities of \pkg{IRTest}, and provides examples of IRT analyses 
  to demonstrate the software's applications.
draft: true
author:  
...
[see @Clogg:1981;@Follmann:1988;@Haberman:2005;@McCutcheon:1987;@Xu+vonDavier:2008].
...
for notational convenience. The equation \@ref(eq:GPCM) represents the
...
Equation \@ref(eq:likeli) shows that the latent distribution is a component 
...
being the dominant choice, a different approach, such as minimizing the *approximate mean integrated squared error* [@Li:2022], can be applied
...
[Section 2.2](#role-of-latent-distribution), the results show that appropriately 
...
function `IRTest::reliability()` returns all the coefficients mentioned
