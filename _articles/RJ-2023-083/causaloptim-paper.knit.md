...
journal:
  ...
  firstpage: 1.0
  lastpage: ~
slug: causaloptim-paper
pdf_url: causaloptim-paper.pdf
...
\CRANpkg{causaloptim} includes a GUI using \CRANpkg{shiny} [@shiny]. The interface is launched in the user's default web browser by calling `specify_graph()`. 
...
<img src="InterfaceStart2.png" alt="A picture of the causaloptim Shiny app web interface." width="100%" />
<p class="caption">(\#fig:InterfaceStart)The Shiny web interface at lauch</p>
</div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="figure" style="text-align: center">
<img src="causaloptim-paper_files/figure-html5/iv-fig-1.png" alt="A picture of the instrumental variable DAG" width="100%" />
<p class="caption">(\#fig:iv-fig)Causal DAG for the IV example.</p>
</div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="figure">
<img src="InterfaceAddNode2.png" alt="Constructing the DAG" width="50%" /><img src="InterfaceAddedArcs2.png" alt="Constructing the DAG" width="50%" />
<p class="caption">(\#fig:DAG)Constructing the DAG</p>
</div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="figure">
<img src="InterfaceSetCard2.png" alt="Setting attributes" width="33%" /><img src="InterfaceCardSet2.png" alt="Setting attributes" width="33%" /><img src="InterfaceAttribSet2.png" alt="Setting attributes" width="33%" />
<p class="caption">(\#fig:Cardinality)Setting attributes</p>
</div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="figure">
<img src="InterfaceQuery2a.png" alt="The causal DAG and bounds" width="50%" /><img src="InterfaceBounds2a.png" alt="The causal DAG and bounds" width="50%" />
<p class="caption">(\#fig:causalDAG)The causal DAG and bounds</p>
</div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="sourceCode"><pre class="sourceCode r"><code class="sourceCode r"><span><span class='va'>graph</span> <span class='op'>&lt;-</span> <span class='fu'>igraph</span><span class='fu'>::</span><span class='fu'><a href='https://r.igraph.org/reference/graph_from_literal.html'>graph_from_literal</a></span><span class='op'>(</span><span class='va'>Z</span> <span class='op'>-</span><span class='op'>+</span> <span class='va'>X</span>, <span class='va'>X</span> <span class='op'>-</span><span class='op'>+</span> <span class='va'>Y</span>, </span>
...
<span><span class='va'>bounds</span></span></code></pre></div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="sourceCode"><pre class="sourceCode r"><code class="sourceCode r"><span><span class='va'>bounds_function</span> <span class='op'>&lt;-</span> <span class='fu'><a href='https://rdrr.io/pkg/causaloptim/man/interpret_bounds.html'>interpret_bounds</a></span><span class='op'>(</span><span class='va'>bounds</span><span class='op'>$</span><span class='va'>bounds</span>, <span class='va'>obj</span><span class='op'>$</span><span class='va'>parameters</span><span class='op'>)</span></span>
<span><span class='fu'><a href='https://rdrr.io/r/utils/str.html'>str</a></span><span class='op'>(</span><span class='va'>bounds_function</span><span class='op'>)</span></span></code></pre></div>
...
If one wants to bound a different effect using the same causal graph, the `update_effect` function can be used to save some computation time. It takes the object returned by `analyze_graph` and the new effect string and returns an object of class `linearcausalproblem` that can be optimized: `obj2 <- update_effect(obj, "p{Y(X = 1) = 1}")`. 
...
An overview of the main functions and their relations is depicted as a flow chart in Figure \@ref(fig:Overview). All functions may be called individually by the user at the `R`-console and all input, output and interaction available through the \CRANpkg{shiny} app has corresponding availability at the `R`-console as well. 
...
(ref:figcap) A function overview flow chart, listing user input, program output and overall structure. The function `specify_graph` launches the GUI. Undirected edges correspond to function calls and arrow heads to direct input. Objects are color coded according to their sources of information, with <span style='color: #CC0000;'>red</span> corresponding to the <span style='color: #CC0000;'>DAG</span>, <span style='color: #0000FF;'>blue</span> to the <span style='color: #0000FF;'>query</span> and <span style='color: #6600CC;'>purple</span> to a mixture of them, as in the optimization <span style='color: #6600CC;'>problem</span> of class `linearcausalproblem`. Finally, <span style='color: #009900;'>green</span> corresponds to the subsequent optimization process producing the <span style='color: #009900;'>bounds</span> of class `balkebound`.

<div class="layout-chunk" data-layout="l-body">
<div class="figure" style="text-align: center">
<img src="causaloptimDiagram.drawio.png" alt="(ref:figcap)" width="100%" />
<p class="caption">(\#fig:Overview)(ref:figcap)</p>
</div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="figure" style="text-align: center">
<img src="causaloptim-paper_files/figure-html5/mediation-fig-1.png" alt="A picture of the mediation DAG" width="100%" />
<p class="caption">(\#fig:mediation-fig)Causal DAG for mediation example</p>
</div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="sourceCode"><pre class="sourceCode r"><code class="sourceCode r"><span><span class='va'>b</span> <span class='op'>&lt;-</span> <span class='fu'>igraph</span><span class='fu'>::</span><span class='fu'><a href='https://r.igraph.org/reference/graph_from_literal.html'>graph_from_literal</a></span><span class='op'>(</span><span class='va'>X</span> <span class='op'>-</span><span class='op'>+</span> <span class='va'>Y</span>, <span class='va'>X</span> <span class='op'>-</span><span class='op'>+</span> <span class='va'>M</span>, <span class='va'>M</span> <span class='op'>-</span><span class='op'>+</span> <span class='va'>Y</span>, </span>
...
<span><span class='fu'><a href='https://r.igraph.org/reference/E.html'>E</a></span><span class='op'>(</span><span class='va'>b</span><span class='op'>)</span><span class='op'>$</span><span class='va'>edge.monotone</span> <span class='op'>&lt;-</span> <span class='fu'><a href='https://rdrr.io/r/base/c.html'>c</a></span><span class='op'>(</span><span class='fl'>0</span>, <span class='fl'>0</span>, <span class='fl'>0</span>, <span class='fl'>0</span>, <span class='fl'>0</span><span class='op'>)</span></span></code></pre></div>

</div>


Note that although the unmeasured confounder `Ur` appears to be designated as binary in this code, this is *not* assumed in the the computations, where it is treated as possibly continuous, e.g.

Using the data from Table IV of [@sjolandernaturaldirecteffects], we compute the observed conditional probabilities.

<div class="layout-chunk" data-layout="l-body">
<div class="sourceCode"><pre class="sourceCode r"><code class="sourceCode r"><span><span class='co'># parameters of the form pab_c, which represents </span></span>
...
<span><span class='va'>p11_1</span> <span class='op'>&lt;-</span> <span class='fl'>82</span><span class='op'>/</span><span class='fl'>1918</span> <span class='co'># P(Y=1,M=1|X=1)</span></span></code></pre></div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="sourceCode"><pre class="sourceCode r"><code class="sourceCode r"><span><span class='va'>CDE0_query</span> <span class='op'>&lt;-</span> <span class='st'>"p{Y(M = 0, X = 1) = 1} - p{Y(M = 0, X = 0) = 1}"</span></span>
...
<span>                                          p10_0 <span class='op'>=</span> <span class='va'>p10_0</span>, p10_1 <span class='op'>=</span> <span class='va'>p10_1</span>, </span>
<span>                                          p01_0 <span class='op'>=</span> <span class='va'>p01_0</span>, p01_1 <span class='op'>=</span> <span class='va'>p01_1</span>, </span>
<span>                                          p11_0 <span class='op'>=</span> <span class='va'>p11_0</span>, p11_1 <span class='op'>=</span> <span class='va'>p11_1</span><span class='op'>)</span></span></code></pre></div>
...
<div class="layout-chunk" data-layout="l-body">
<table>
<caption>(\#tab:mediation-bounds)Bounds on the controlled and natural direct effects.</caption>
 <thead>
  <tr>
...
<div class="layout-chunk" data-layout="l-body">
<div class="sourceCode"><pre class="sourceCode r"><code class="sourceCode r"><span><span class='va'>params</span> <span class='op'>&lt;-</span> <span class='fu'><a href='https://rdrr.io/r/base/list.html'>list</a></span><span class='op'>(</span>p00_0 <span class='op'>=</span> <span class='fl'>0.83</span>, p00_1 <span class='op'>=</span> <span class='fl'>0.88</span>, p00_2 <span class='op'>=</span> <span class='fl'>0.72</span>, </span>
...
<span>               p11_0 <span class='op'>=</span> <span class='fl'>0.01</span>, p11_1 <span class='op'>=</span> <span class='fl'>0.01</span>, p11_2 <span class='op'>=</span> <span class='fl'>0.03</span><span class='op'>)</span></span></code></pre></div>
...
<div class="layout-chunk" data-layout="l-body">
<div class="sourceCode"><pre class="sourceCode r"><code class="sourceCode r"><span><span class='co'># Input causal DAG</span></span>
...
<span><span class='fu'><a href='https://rdrr.io/r/base/Round.html'>round</a></span><span class='op'>(</span><span class='va'>numericbounds</span>, <span class='fl'>2</span><span class='op'>)</span></span></code></pre></div>
...
The methods and algorithms described in [@generalcausalbounds] to compute symbolic expressions for bounds on non-identifiable causal effects are implemented in the package \CRANpkg{causaloptim}. Our aim was to provide a user-friendly interface to these methods with a graphical interface to draw DAGs, specification of causal effects using standard notation for counterfactuals, and an efficient implementation of vertex enumeration to reduce computation times. These methods are applicable to a wide variety of causal inference problems which appear in biomedical research, economics, social sciences and more. Aside from the graphical interface, programming with the package is encouraged to promote reproducibility and advanced use. Our package includes automated unit tests and also tests for correctness by comparing the symbolic bounds derived using our program to independently derived bounds in particular settings. 
...
[causaloptim](https://cran.r-project.org/package=causaloptim), [shiny](https://cran.r-project.org/package=shiny), [bpbounds](https://cran.r-project.org/package=bpbounds), [igraph](https://cran.r-project.org/package=igraph), [rcdd](https://cran.r-project.org/package=rcdd)

## CRAN Task Views implied by cited packages {.appendix}

[CausalInference](https://cran.r-project.org/view=CausalInference), [GraphicalModels](https://cran.r-project.org/view=GraphicalModels), [Optimization](https://cran.r-project.org/view=Optimization), [WebTechnologies](https://cran.r-project.org/view=WebTechnologies)
