...
abstract: |
  ...
  that are typically smaller and often more accurate. The
  [**qCBA**](https://CRAN.R-project.org/package=qCBA) package supports
  models created with multiple packages for rule-based classification
  available in CRAN, including
  [**arc**](https://CRAN.R-project.org/package=arc),
  [**arulesCBA**](https://CRAN.R-project.org/package=arulesCBA),
  [**rCBA**](https://CRAN.R-project.org/package=rCBA) and
...
first, and with at least three packages (Hahsler et al. 2019), the most
popular such algorithm was Classification Based on Associations (CBA)
(Liu et al. 1998). There are also multiple newer approaches, such as
SBRL (Yang et al. 2017) or RCAR (Azmi and Berrado 2020), both available
in R (Yang et al. 2024; Michael Hahsler 2024).
...
MDLP (Fayyad and Irani 1993). Association rule learning then operates on
...
(Kliegr and Izquierdo 2023) is a collection of several algorithms that
...
experimentally shown in Kliegr and Izquierdo (2023), this makes the
models often more accurate and consistently smaller and thus more
interpretable.
...
[**rCBA**](https://CRAN.R-project.org/package=rCBA) (Kuchař and Kliegr
2019), [**arc**](https://CRAN.R-project.org/package=arc) (Kliegr 2016)
and [**arulesCBA**](https://CRAN.R-project.org/package=arulesCBA)
(Michael Hahsler 2024) introduced in Hahsler et al. (2019) rely on the
fast and proven [**arules**](https://CRAN.R-project.org/package=arules)
package (Hahsler et al. 2011) to mine association rules, which is also
the main dependency of the
[**qCBA**](https://CRAN.R-project.org/package=qCBA) R package.
...
the going example in the open-access publication (Kliegr and Izquierdo
2023), which contains graphical illustrations as well as formal
...
**Association rules** Algorithms such as Apriori (Agrawal and Srikant
1994) are used for association rule mining. An association rule has the
form $r: antecedent \rightarrow consequent$, where both $antecedent$ and
...
of CARs (Vanhoof and Depaire 2010). Some ARC algorithms, such as CBA,
use rule interest measures for this: rules are first ordered and then
some of them are removed using *data coverage pruning* and *default rule
pruning* (step 2 and step 3 of the CBA-CB algorithm as described in Liu
et al. (1998). Some ARC algorithms also include a rule with an empty
antecedent (called *default rule*) at the end of the rule list to ensure
that the classifier can always provide a prediction even if there is no
specific rule matching that particular transaction.
...
Other recent algorithms producing rule lists include SBRL (Yang et al.
2024). The other common ARC approach is based on *rule sets*, where
rules are not ordered, and multiple rules can be used for
classification, e.g., through voting. This second group includes
algorithms such as CMAR (Li et al. 2001) or CPAR (Yin and Han 2003).
...
combinatorial explosion (Kliegr and Kuchar 2019), the postprocessing by
QCBA is performed on a relatively small number of rules that passed the
...
Benchmarks in Kliegr and Izquierdo (2023) have shown that, on average,
the postprocessing with QCBA took a similar time as building the input
CBA model. The most expensive step can be extension, with its complexity
...
However, individual trees in algorithms such as C4.5 (Quinlan 1993) are
built in such a way that the resulting rules are non-overlapping, while
association rule learning outputs overlapping rules. Algorithms such as
Apriori will output all rules that are valid in the data, given the
user-specified thresholds. In contrast, decision tree induction
...
confidence threshold of 0.5 and support of 1% is recommended (Liu et al.
1998). However, since the `humtemp` dataset has fewer than 100 rows, the
support threshold would correspond to the support of just 1 transaction,
which would miss the purpose of this threshold to address overfitting by
eliminating rules that are backed by only a small number of instances.
We, therefore, set the minimum support threshold to 3 transactions
(expressed as a percentage in the code snippet). The `minlen` and
`maxlen` express that rules must contain at most two items in the
condition part (antecedent).
...
In this listing, coverage and lift, as defined in the Hahsler et al.
(2007) documentation, are additional rule quality measures not used by
the [**qCBA**](https://CRAN.R-project.org/package=qCBA) package. Count
...
2.  Rules are subject to *pruning*
    ...
    - *data coverage pruning*: the algorithm iterates through the rules
      in the order of precedence removing any rule which does not
      correctly classify at least one instance. If the rule correctly
      classifies at least one instance, it is retained and the instances
      removed (only for the purpose of subsequent steps of data coverage
      pruning).

    - *default rule pruning*: the algorithm iterates through the rules
      in the sort order, and cuts off the list once keeping the current
      rule would result in worse accuracy of the model than if a default
      rule was inserted at the place of the current rule and the rules
      below it were removed.
...
Figure [\[fig:post1\]](#fig:post1){reference-type="ref"
reference="fig:post1"}, where the green background corresponds to Class
2 predicted by the default rule. The CBA output contains several
additional statistics for each rule. The *ordered* versions of
confidence and support are computed only from those training instances
reaching the given rule. For the first rule, the ordered confidence is
identical to standard confidence. The ordered support is also
semantically the same but is expressed as an absolute count rather than
a proportion. To compute these values for the subsequent rules,
instances covered by rules higher in the list have been removed. The
cumulative confidence is an experimental measure described in
[**arc**](https://CRAN.R-project.org/package=arc) documentation.
...
correspond to the best-performing configuration #5 from (Kliegr and
Izquierdo 2023).
...
Figure [\[fig:post2\]](#fig:post2){reference-type="ref"
reference="fig:post2"} shows the QCBA model. Compared to the CBA model
in Figure [\[fig:post1\]](#fig:post1){reference-type="ref"
reference="fig:post1"}, QCBA removed two rules and refined the
boundaries of the remaining rules.
...
+-------------+------------+------------+------------+---------------+--------------+----------+
| dataset     | conf       | supp       | length     | $acc_{train}$ | $acc_{test}$ | rule     |
|             |            |            |            |               |              | count    |
+-------------+------------+------------+------------+---------------+--------------+----------+
| refit       | =          | =          | =          | =             | $na$         | =        |
+-------------+------------+------------+------------+---------------+--------------+----------+
| literal     | $\geq$     | $\geq (+)$ | $\leq (+)$ | $na$          | $na$         | $\leq$\* |
| pruning     |            |            |            |               |              | (+)      |
+-------------+------------+------------+------------+---------------+--------------+----------+
| trimming    | $\geq (+)$ | =          | =          | $\geq (+)$    | $na$         | =        |
+-------------+------------+------------+------------+---------------+--------------+----------+
...
unanimous effect of the preprocessing algorithm on the quality measure.
:::
::::
...
influence of these factors is presented in Kliegr and Izquierdo (2023).
This was performed on subsets of the KDD'99 Anomaly Detection dataset,
with the maximum dataset size being processed using all QCBA
optimizations, reaching about 40,000 rows and about the same number of
unique numerical values. The results show that the most computationally
intensive step is the extension step. For large datasets, the user may,
therefore, consider disabling it or tuning its `minCI` parameter, which
can influence the runtime. An important factor is also the number of
input rules for optimization. This is often related to the chosen base
...
result of pruning has been shown in benchmarks in Kliegr and Izquierdo
(2023).
...
(Kliegr and Izquierdo 2023). More details on the benchmarks are covered
in Section [Built-in benchmark support](#{sec:benchmark}).
...
- `rules`: list of rules in the model (instance of `rules` from `arules`
  package.
...
the heuristic algorithm from (Kliegr and Kuchar 2019). Therefore, no
support and confidence thresholds have to be passed. A more complex case
...
from (Kliegr and Kuchar 2019). The
...
[**arulesCBA**](https://CRAN.R-project.org/package=arulesCBA) (Hahsler
et al. 2019).
...
[**arulesCBA**](https://CRAN.R-project.org/package=arulesCBA)
implementations of CBA, CMAR, CPAR, PRM and FOIL2 (Quinlan and
Cameron-Jones 1993).
...
computational costs is presented in Kliegr and Izquierdo (2023), which
also includes the study of various data sizes and the effect of varying
the number of unique values in the dataset. A brief overview of the main
results was presented in Subsection [Computational costs for large
datasets](#{ss:costs}).
...
SBRL, Python packages producing IDS models (Lakkaraju et al. 2016) and
Weka libraries for RIPPER (Cohen 1995) and FURIA (Hühn and Hüllermeier
2009). Detailed benchmarking results are included in (Kliegr and
Izquierdo 2023).
...
The QCBA algorithm is described in detail in Kliegr and Izquierdo
(2023), the package documentation is available in Kliegr (2024), and
additional information is available at <https://github.com/kliegr/qcba>,
which also features an interactive RMarkdown tutorial supplementing this
paper.
...
International Conference on Very Large Data Bases* (San Francisco, CA,
USA), VLDB '94, 487--99.
:::

::: {#ref-azmi2020rcar .csl-entry}
Azmi, Mohamed, and Abdelaziz Berrado. 2020. "RCAR Framework: Building a
Regularized Class Association Rules Model in a Categorical Data Space."
*Proceedings of the 13th International Conference on Intelligent
Systems: Theories and Applications*, 1--6.
:::

::: {#ref-cohen1995fast .csl-entry}
Cohen, William W. 1995. "Fast Effective Rule Induction." *Twelfth
International Conference on Machine Learning*, 115--23.
:::

::: {#ref-FayyadI93 .csl-entry}
...
International Joint Conference on Uncertainly in Artificial Intelligence
(IJCAI93)*, 1022--29.
:::

::: {#ref-hahsler2011arules .csl-entry}
Hahsler, Michael, Sudheer Chelluboina, Kurt Hornik, and Christian
Buchta. 2011. "The Arules r-Package Ecosystem: Analyzing Interesting
Patterns from Large Transaction Data Sets." *Journal of Machine Learning
Research* 12 (Jun): 2021--25.
:::

::: {#ref-arules .csl-entry}
Hahsler, Michael, Bettina Grün, and Kurt Hornik. 2007. "Introduction to
Arules--Mining Association Rules and Frequent Item Sets." *SIGKDD
Explor* 2 (4): 1--28.
:::

::: {#ref-hahsler2019associative .csl-entry}
Hahsler, Michael, Ian Johnson, Tomáš Kliegr, and Jaroslav Kuchař. 2019.
"Associative Classification in R: Arc, arulesCBA, and rCBA." *R Journal*
9 (2).
:::

::: {#ref-huhn2009furia .csl-entry}
Hühn, Jens, and Eyke Hüllermeier. 2009. "FURIA: An Algorithm for
Unordered Fuzzy Rule Induction." *Data Mining and Knowledge Discovery*
19 (3): 293--319.
:::

::: {#ref-arcPackage .csl-entry}
Kliegr, Tomáš. 2016. *Association Rule Classification*.
<https://CRAN.R-project.org/package=arc>.
:::

::: {#ref-qcbaPackage .csl-entry}
Kliegr, Tomáš. 2024. *[qCBA]{.nocase}: Quantitative Classification by
Association Rules*. <https://CRAN.R-project.org/package=qCBA>.
:::

::: {#ref-kliegr2023qcba .csl-entry}
Kliegr, Tomáš, and Ebroul Izquierdo. 2023. "QCBA: Improving Rule
Classifiers Learned from Quantitative Data by Recovering Information
Lost by Discretisation." *Applied Intelligence* 53 (18): 20797--827.
:::

::: {#ref-kliegr2019tuning .csl-entry}
Kliegr, Tomás, and Jaroslav Kuchar. 2019. "Tuning Hyperparameters of
Classification Based on Associations (CBA)." *ITAT*, 9--16.
:::

::: {#ref-rcba .csl-entry}
Kuchař, Jaroslav, and Tomáš Kliegr. 2019. *rCBA: CBA Classifier for R*.
<https://CRAN.R-project.org/package=rCBA>.
:::

::: {#ref-lakkarajuinterpretable .csl-entry}
Lakkaraju, Himabindu, Stephen H. Bach, and Jure Leskovec. 2016.
"Interpretable Decision Sets: A Joint Framework for Description and
Prediction." *Proceedings of the 22Nd ACM SIGKDD International
...
::: {#ref-Liu98integratingclassification .csl-entry}
...
::: {#ref-arulesCBA .csl-entry}
Michael Hahsler, Tyler Giallanza, Ian Johnson. 2024. *arulesCBA:
Classification Based on Association Rules*.
<https://CRAN.R-project.org/package=arulesCBA>.
:::

::: {#ref-quinlan2014c4 .csl-entry}
Quinlan, J Ross. 1993. *C4.5: Programs for Machine Learning*. Morgan
Kaufmann.
:::

::: {#ref-quinlan1993foil .csl-entry}
Quinlan, J Ross, and R Mike Cameron-Jones. 1993. "FOIL: A Midterm
Report." *European Conference on Machine Learning*, 1--20.
:::

::: {#ref-arcReview .csl-entry}
Vanhoof, K., and B. Depaire. 2010. "Structure of Association Rule
Classifiers: A Review." *2010 International Conference on Intelligent
Systems and Knowledge Engineering (ISKE)*, 9--12.
:::

::: {#ref-yang2017scalable .csl-entry}
Yang, Hongyu, Cynthia Rudin, and Margo Seltzer. 2017. "Scalable Bayesian
Rule Lists." *International Conference on Machine Learning*, 3921--30.
:::

::: {#ref-sbrl .csl-entry}
Yang, Hongyu, Cynthia Rudin, and Margo Seltzer. 2024. *Sbrl: Scalable
Bayesian Rule Lists Model*. <https://CRAN.R-project.org/package=sbrl>.
:::

::: {#ref-yin2003cpar .csl-entry}
Yin, Xiaoxin, and Jiawei Han. 2003. "CPAR: Classification Based on
Predictive Association Rules." *Proceedings of the 2003 SIAM
International Conference on Data Mining*, 331--35.
:::
::::::::::::::::::::::::::
