...
Y=m(X)+\varepsilon,
\end{aligned}   (\#eq:grm)$$
where the error $\varepsilon$ is assumed to be centered with a finite
variance, i.e. $E(\varepsilon)=0$ and $var(\varepsilon)<\infty$. The
components of $X$ are independent with a known law
$P_X=\prod_{a=1}^dP_{X_a}$ on $\mathcal{X}$, a subset of $\mathbb{R}^d$.
The number $d$ of components of $X$ may be large. The model $m$ may
...
$m\in L^2(\mathcal{X},P_X)$. Based on the data points
$\{(X_i,Y_i)\}_{i=1}^n$, we estimate a meta-model that approximates the
Hoeffding decomposition of $m$. This meta-model belongs to a reproducing
...
and interactions between them [@DURRANDE201357]. The estimator of the
...
and the empirical norm [@huet:hal-01434895]. This procedure allows us to
select the subsets of variables $X_1,...,X_d$ that contribute to predict
$Y$. The estimated meta-model is used to perform sensitivity analysis,
and therefore, to determine the influence of each variable and groups of
them on the output variable $Y$.
...
associated with $X_v$ in Equation (\@ref(eq:sobol1)). Then Equation
(\@ref(eq:sobol1)) can be expressed as follows:
$$\begin{aligned}
\label{ch3sobol}
m(X)=m_0+\sum_{v\in\mathcal{P}}m_v(X_v).
\end{aligned}   (\#eq:ch3sobol)$$
...
in Equation (\@ref(eq:ch3sobol)) are square-integrable. As any two terms
of decomposition (\@ref(eq:ch3sobol)) are orthogonal, by squaring
...
(see Equation (\@ref(eq:chpacvardecop))) is numerically complicated or
...
of the components of $X$. In practice, expansion (\@ref(eq:ch3pc)) shall
...
(\@ref(eq:durandhoeff)) is its Hoeffding decomposition. As the function
...
(\@ref(eq:ch3sobol)). The number of the terms $f^*_v$ that should be
...
Equation (\@ref(eq:doubpen)) adapted to the multivariate setting,
$$\begin{aligned}
\label{ch3pen}
...
-   implement the RKHS ridge group sparse procedure and a special case
    ...
    penalty function (\@ref(eq:ch3pen))), in order to estimate the terms
    $f^*_v$ in the Hoeffding decomposition of the meta-model $f^*$
    leading to an estimation of the function $m$ (see Section
    [Optimization algorithms](#subsec:optim)),
-   choose the tuning parameters $\mu$ and $\gamma$ (see Equation
    (\@ref(eq:ch3pen))), using a procedure that leads to obtain the
...
small modification to one of its functions (see Remark [3](#ch3:remxun)
of Section [Calculation of the Gram matrices](#subsec:Gramm)).
...
*sensitivity* (see Section 4.5. of @JSSv051i01). In this case, the idea
...
extended-FAST method [@doi:10.1080/00401706.1999.10485594]. To reduce
...
Sobol indices](#subsec:SI)). Besides, the estimation of the Sobol
...
indices](#subsec:SI)). Moreover, the functions `sobolGP` and `fast99`
...
and [*DiceOptim*](https://CRAN.R-project.org/package=DiceOptim) (Deep
Inside Computer Experiments Kriging/Optim) [@JSSv051i01] implement the
...
The penalty function in the criterion (\@ref(eq:functional)) is the sum
...
the minimization problem (\@ref(eq:ch3prediction)) belonging to the RKHS
...
(\@ref(eq:ch3prediction)) satisfies,
$$\begin{aligned}
...
\end{aligned}   (\#eq:parametric)$$
If we consider only the second part of the penalty function in the
criterion (\@ref(eq:parametric)) ( i.e. set $\gamma=0$), we obtain the
...
criterion (\@ref(eq:Lasso)) insures the sparsity in the solution.
Therefore, for a given value of $\mu$, by implementing the RKHS group
lasso algorithm (see Section [RKHS group lasso](#subsec:optimGL)), a
...
prediction error are implemented as `RKHSMetMod`, `mu`$\_$`max`, and
...
materials), and illustrated in Example [1](#ex:1), Example [2](#ex:3),
and Examples [1](#ex:1), [2](#ex:3), [3](#ex:4), respectively.
...
`SI`$\_$`emp` function. This function is described in Section
\"Companion functions\" (supplementary materials) and illustrated in
Examples [1](#ex:1), [2](#ex:3), [3](#ex:4).

# Algorithms {#sec:Optalgo}

The *RKHSMetaMod* package implements two optimization algorithms: the
RKHS ridge group sparse (see Algorithm [2](#algo:RGS)) and the RKHS
group lasso (see Algorithm [1](#algo:GL)). These algorithms rely on the
...
-   Insure the positive definiteness of the matrices $K_v$:\
    The output of function `calc`$\_$`Kv` is one of the input arguments
    ...
    function `calc`$\_$`kv`. Let us briefly explain this part of the
    algorithm:\
    ...
    $\lambda_{v,i}+$`epsilon`, with `epsilon` being equal to
    $\lambda_{v,\text{max}}\times$`tol`. The value of `tol` is set as
...
The function `calc`$\_$`Kv` is described in Section \"Companion
...
(\@ref(eq:Lasso)) (if $\gamma=0$), or the minimization of the RKHS ridge
group sparse criterion presented in Equation (\@ref(eq:parametric)) (if
$\gamma\neq0$). In the following, the algorithms to solve these
...
criterion $C_g(f_0,\theta)$ (see Equation (\@ref(eq:Lasso))) is done
...
convergence. This procedure leads to Algorithm [1](#algo:GL) (see the
Appendix (supplementary materials) for more details on this procedure).

<figure id="algo:GL">
<img src="alg/algoGL.png" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Algorithm 1: RKHS group lasso algorithm:</figcaption>
</figure>

In the *RKHSMetaMod* package, the Algorithm [1](#algo:GL) is implemented
...
This procedure leads to Algorithm [2](#algo:RGS) (see the Appendix
(supplementary materials) for more details on this procedure).

<figure id="algo:RGS">
<img src="alg/algoRGS.png" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Algorithm 2: RKHS ridge group sparse algorithm:</figcaption>
</figure>

In the *RKHSMetaMod* package the Algorithm [2](#algo:RGS) is implemented
as `pen`$\_$`MetMod` function. This function is described in Section
\"Companion functions\" (supplementary materials) and illustrated in
Example [2](#ex:3).
...
-   First, the tuning parameter $\gamma$ is set as zero and a value of
    ...
    Algorithm [1](#algo:GL), contains exactly $qmax$ groups in its
    support is computed. This value is denoted by $\mu_{qmax}$.
-   Then, the RKHS ridge group sparse algorithm, Algorithm
    [2](#algo:RGS), is implemented by setting the tuning parameter $\mu$
    equal to $\mu_{qmax}$ and a grid of values of the tuning parameter
    $\gamma>0$.

This procedure leads to Algorithm [3](#algo:GLqmax).

<figure id="algo:GLqmax">
<img src="alg/algoGLqmax.png" style="width:100.0%"
alt="graphic without alt text" />
<figcaption>Algorithm 3: Algorithm to estimate RKHS meta-model with at
most <span
class="math inline"><em>q</em><em>m</em><em>a</em><em>x</em></span>
groups in its support:</figcaption>
</figure>
...
(\@ref(eq:parametric)) enforce sparsity to the solution, the estimator
...
::: {#tab:errorex1}
  ...
  : Table 2: Example [1](#ex:1): The columns of the table correspond to
  the different datasets with $n\in\{50,100,200\}$ and $d=5$. Each line
...
::: {#tab:bvarex1}
  ...
  : Table 3: Example [1](#ex:1): The columns of the table correspond to
  the different datasets with $n\in\{50,100,200\}$ and $d=5$. Each line
...
through all repetitions for $n=200$ and \"`matern`\" kernel are
displayed in Table [4](#trueSI).
...
::: {#trueSI}
  ...
  : Table 4: Example [1](#ex:1): The first line of the table gives the
  true values of the Sobol indices $\times100$. The second line gives
...
Taking into account the results obtained for this Example [1](#ex:1),
...
$d=10$ is generated. First, we use functions `calc`$\_$`Kv` and
`mu`$\_$`max` to compute the eigenvalues and eigenvectors of the
...
-   Set $\gamma=0$ and,
    ...
    Gather the obtained RKHS meta-models in a list, `res`$\_$`g` (while
    this job could be done using the function `RKHSMetMod` by setting
    $\gamma=0$, in this example, we use the function `RKHSgrplasso` in
    order to avoid the re-calculation of $K_v$'s at the next step).
    Thereafter, for each estimator in `res`$\_$`g`, the prediction error
    ...
    are displayed in Table [5](#tab:errorgex3).
    ...
    ::: {#tab:errorgex3}
      ...
      : Table 5: Example [2](#ex:3): Prediction errors associated with
...
-   Choose a smaller grid of values of $\mu$,
    ...
    `pen`$\_$`MetMod`. Calculate the prediction errors for the new
    ...
    `resgnew`. We set $\gamma_{(1:4)}=(0.2, 0.1, 0.01, 0.005)$, and we
    ...
    errors are displayed in Table [6](#tab:errorex3).
    ...
    ::: {#tab:errorex3}
      ...
      : Table 6: Example [2](#ex:3): Obtained prediction errors in step 2.
    ...
    displayed in Table [4](#trueSI) and $\widehat{S}_v$ be the estimated
    empirical Sobol indices. Then
    $$\label{relerr}
    RE=\sum_v{\vert\widehat{S}_v-S_v\vert/S_v}.   (\#eq:relerr)$$
    In Table [7](#tab:SIex3) the estimated empirical Sobol indices,
    ...
    ::: {#tab:SIex3}
      ...
      : Table 7: Example [2](#ex:3): The estimated empirical Sobol
      indices $\times100$ greater than $10^{-2}$. The last two columns
      show $\sum_v\widehat{S}_v$ and RE, respectively.
    :::
...
$n\times$vMax$=n\times 175$. Table [8](#timing) gives the execution time
for different functions used throughout the Examples
[1](#ex:1)-[3](#ex:4). In all examples we used a cluster of computers
...
::: {#timing}
  ...
  : Table 8: Example [3](#ex:4): The kernel used is \"`matern`\". The
  execution time for the functions `RKHSgrplasso` and `pen`$\_$`MetMod`
  is displayed in each row for two pairs of values of the tuning
  parameters $(\mu_1=\mu_{max}/(\sqrt{n}\times 2^7),\gamma=0.01)$ on up,
  and $(\mu_2=\mu_{max}/(\sqrt{n}\times 2^8),\gamma=0.01)$ on below. In
  the column $\vert S_{\widehat{f}}\vert$, the number of groups in the
  support of each estimated RKHS meta-model is displayed.
:::

As we can see, the execution time increases fast as $n$ increases. In
Figure [1](#timingplot) the plot of the logarithm of the time (in
seconds) versus the logarithm of $n$ is displayed for the functions
`calc`$\_$`Kv`, `mu`$\_$`max`, `RKHSgrplasso` and `pen`$\_$`MetMod`.

![Figure 1: Example [3](#ex:4): Timing plot for $d=10$,
$n\in\{100,300,500,1000,2000,5000\}$, and different functions of the
*RKHSMetaMod* package. The logarithm of the execution time (in seconds)
for the functions `RKHSgrplasso` and `pen`$\_$`MetMod` is displayed for
...
lines.](Rplot_log.png){#timingplot width="100%"
alt="graphic without alt text"}
...
`calc`$\_$`Kv` and `mu`$\_$`max`, and $\alpha\backsimeq2$ for the
functions `RKHSgrplasso` and `pen`$\_$`MetMod`.

Taking into account the results obtained for the prediction error and
the values of $(\widehat{\mu},\widehat{\gamma})$ in Example [2](#ex:3),
...
for the obtained meta-models using the functions `PredErr` and
`SI`$\_$`emp`:
...
Table [9](#tab:SIex4) gives the estimated empirical Sobol indices as
well as their sum, the values of RE (see Equation (\@ref(eq:relerr))),
...
::: {#tab:SIex4}
  ...
  : Table 9: Example [3](#ex:4): The estimated empirical Sobol indices
  $\times100$ greater than $10^{-2}$ associated with each estimated RKHS
  meta-model is printed. The last three columns show
  $\sum_v\widehat{S}_v$, RE, and the prediction error (Err),
  respectively. We have $\mu_1=\mu_{max}/(\sqrt{n}\times 2^{7})$,
...
In Figure [2](#SI) the result of the estimation quality and the Sobol
indices for the dataset with $n$ equal to $5000$, $d$ equal to $10$, and
$(\mu_2,\gamma)$ are displayed.

![Figure 2: Example [3](#ex:4): On the left, the RKHS meta-model versus
the g-function is plotted. On the right, the empirical Sobol indices in
the y-axis and vMax$=175$ groups in the x-axis are
displayed.](Rplot.png){#SI width="100%" alt="graphic without alt text"}
...
*DiceKriging* the range parameter $r$ (see Table [1](#kernels)) in the
...
$m(X)=3sin(5\pi X)+cos(7\pi X)$ over $[0,1]$. The same experimental
design as described in @Gu_2019 is considered: the design matrix $X$ is
a sequence of $12$ equally spaced points on $[0,1]$, and the response
variable $Y$ is calculated as $Y = m(X)$:
...
The prediction results are plotted in Figure [3](#Rplot01ex6). The black
...
![Figure 3: Example [4](#ex:comparisonPrediction): Prediction of the
modified sine wave function with $100$ test points, equally spaced in
$[0,1]$. The x-axis is the real output and the y-axis is the prediction.
...
are the predictive mean from *DiceKriging*.](Rplot01ex6.png){#Rplot01ex6
width="100%" alt="graphic without alt text"}

The meta-model results are plotted in Figure [4](#Rplotex6). The
...
noted by @Gu_2019, for that sine wave example, the meta-model from the
DiceKriging package \"degenerates to the fitted mean with spikes at the
design points\".

![Figure 4: Example [4](#ex:comparisonPrediction): The red curve is the
graph of the modified sine wave function with $100$ test points, equally
spaced in $[0,1]$. The black curve is the prediction produced by the
...
produced by the *RobustGaSP* package.](Rplotex6.png){#Rplotex6
width="100%" alt="graphic without alt text"}
...
(\@ref(eq:gfct)) with coefficients
$c_1=0,\:c_2=1,\:c_3=4.5,\:c_4=9,\:(c_a)_{a=5,6,7,8}=99$. With these
values of coefficients $c_a$, the variables $X_1$, $X_2$, $X_3$ and
$X_4$ explain $99.96\%$ of the variance of function $m(X)$ (see Table
[10](#trueSIcomparison)).
...
Kriging based meta-model is drawn in Figure [5](#comppred).

![Figure 5: Example [5](#ex:comparison): The x-axis is the real output
...
from *DiceKriging*.](comppred.png){#comppred width="100%"
alt="graphic without alt text"}
...
Kriging based meta-model are given in Table [10](#trueSIcomparison).
...
::: {#trueSIcomparison}
  ...
  : Table 10: Example [5](#ex:comparison): The true values of the Sobol
  indices $\times100$ greater than $10^{-2}$ are given in the first raw.
...
in Equation (\@ref(eq:relerr)), which is equal to $7.95$ for the Kriging
based meta-model, and $5.59$ for the RKHS meta-model. Comparing the
...
ridge group sparse criterion (\@ref(eq:parametric)) and the RKHS group
lasso criterion (\@ref(eq:Lasso)). Both of these algorithms rely on the
...
$k_{0a}$, $a=1,...,d$ in the function `calc`$\_$`Kv` of this package
(see Remark [3](#ch3:remxun)). The available kernels in the
...
(see Table [1](#kernels)). With regard to the problem being under study,
...
the `calc`$\_$`Kv` function. For the large values of $n$ and $d$ the
