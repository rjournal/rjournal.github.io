...
rmdcev are mdcev used to estimate all model specifications and mdcev.sim
used to simulate both demand and welfare implications. rmdcev is
...
advantages of apollo compared to rmdcev is that 1) it can estimate model
specifications without an outside good whereas rmdcev only estimates
models with an outside good, 2) users have more control over particular
parameter specifications such as which parameters are fixed at their
starting values and which are allowed to be random parameters, and 3) it
allows users to estimate the multiple discrete continuous nested extreme
value model and LC-random parameter MDCEV specifications.
...
maximization problem is $$\begin{aligned}
\begin{split}
 & \max_{x_{ik}, x_{i1}} U(x_{ik}, x_{i1}) \\
& s.t. \;\;\;y_i = \sum\limits^K_{k=2}p_{ik} x_{ik} + x_{i1}, \;\; x_{ik} \geq 0,\;\; k = 2,...,K,
  \end{split}
\end{aligned}$$ where $x_{ik}$ is the consumption level for alternative
$k$, $x_{i1}$ is consumption of the numeraire, $y_i$ is any arbitrary
budget amount (e.g. annual income), and $p_{ik}$ is the unit price of
alternative $k$.
...
\begin{split}
 \frac{U_{x_{ik}}}{U_{x_{i1}}} & \leq p_{ik},\; \; k = 1,....K , \\
  ...
  \end{split}
\end{aligned}$$ For alternatives with positive consumption levels, the
...
is defined explicitly using an expenditure function $$\label{eq:welfare}
CS^H = y - e(p^1, q^1, \bar{U}, \theta, \varepsilon),$$ where $\theta$
is the vector of structural parameters ($\psi_k, \alpha_k, \gamma_k$),
$\varepsilon$ is a vector or matrix of unobserved heterogeneity, and
...
baseline utility through a random error term as $$\label{eq:psi}
\psi_k=\psi(z_k,\varepsilon_k)= exp(\beta'z_k+\varepsilon_k),$$ where
$z_k$ is a set of variables that can include alternative-specific
...
4.  hybrid0-profile: set all $\alpha_k=\alpha_1=0$. $$\label{eq:hybrid0}
...
$g_k(.)$ is the right hand side of Equation
([\[eq:kt_g\]](#eq:kt_g){reference-type="ref" reference="eq:kt_g"}), and
...
The rmdcev package uses the compact structure of the determinant of the
Jacobian as derived by Bhat (2008) and defined as
...
where $P_{is}$ has the same form as Equations
([\[eq:ll_base\]](#eq:ll_base){reference-type="ref"
reference="eq:ll_base"}) and Equations
([\[eq:ll_kt_ee\]](#eq:ll_kt_ee){reference-type="ref"
reference="eq:ll_kt_ee"}) but is now class specific.
...
#\> alt quant price #\> 1 beach 6.5375 53.18359 #\> 2 birding 14.3835
44.01734 #\> 3 camping 2.5125 61.38326 #\> 4 cycling 9.4700 45.99470 #\>
5 fish 3.3435 86.22383 #\> 6 garden 21.5710 38.28073 #\> 7 golf 4.0260
134.10374 #\> 8 hiking 41.4150 37.53204 #\> 9 hunt_birds 0.4855
111.00176 #\> 10 hunt_large 0.9480 184.46812 #\> 11 hunt_trap 0.6290
95.33228 #\> 12 hunt_waterfowl 0.2085 159.66605 #\> 13 motor_land 3.7040
123.10169 #\> 14 motor_water 2.8390 139.63845 #\> 15 photo 8.6415
67.13733 #\> 16 ski_cross 2.6450 32.65243 #\> 17 ski_down 1.2065
151.01398
:::
...
::: example
data_mdcev \<- mdcev.data(data_rec, id.var = \"id\", alt.var = \"alt\",
choice = \"quant\")

#\> Sorting data by id.var then alt\... #\> Checking data\... #\> Data
is good
:::
...
The mdcev.data function also checks to ensure the data has the necessary
variables, and that all individuals spend positive amounts on the
numeraire good. If an individual does not have positive expenditures on
the numeraire good, an error message is given.
...
#\> \"gamma\", \"hybrid\", \"hybrid0\", \"kt_ee\"), n_classes = 1,
fixed_scale1 = 0, #\> single_scale = 0, trunc_data = 0, psi_ascs = NULL,
gamma_ascs = 1, #\> seed = \"123\", max_iterations = 2000,
jacobian_analytical_grad = 1, #\> initial.parameters = \"random\",
hessian = TRUE, algorithm = c(\"MLE\", #\> \"Bayes\"), flat_priors =
NULL, print_iterations = TRUE, #\> prior_psi_sd = 10, prior_gamma_sd =
10, prior_phi_sd = 10, #\> prior_alpha_shape = 1, prior_scale_sd = 1,
...
n_iterations = 200, n_chains = 4, #\> n_cores = 4, max_tree_depth = 10,
adapt_delta = 0.8, lkj_shape_prior = 4, #\> \...)
:::
...
-   psi_ascs Whether to include alternative-specific psi parameters. The
    first alternative is used as the reference category. Only specify to
    1 for MDCEV models.

-   fixed_scale1 Whether to fix the scale parameter at 1.

-   trunc_data Whether the estimation should be adjusted for truncation
    of non-numeraire alternatives. This option is useful if the data
    only includes individuals with positive non-numeraire consumption
...
-   print_iterations Whether to print intermediate iteration information
    or not.
...
-   initial.parameters The default for fixed and random parameter
    ...
    ::: example
    initial.parameters = list(psi = array(0, dim = c(K, num_psi)), gamma
    = array(1, dim = c(K, num_alt)), alpha = array(0.5, dim = c(K, 1)),
...
The formula is divided in three parts, separated by the symbol \| and is
...
variables in $\psi_k$ as in Equation
([\[eq:psi\]](#eq:psi){reference-type="ref" reference="eq:psi"}),
excluding ASCs. These can include alternative-specific and
individual-specific variables. Interaction terms between variables can
be included using the normal Formula syntax of z1:z2. This is
particularly useful for creating interaction terms to incorporate
observed preference heterogeneity for alternative-specific variables and
...
Alternative-specific variables such as z1 and z2 can be included in the
same way such as
...
but this can be omitted if -1 is used in the formula. For example, a LC
model with no alternative-specific variables in the $psi_k$ parameters
and university, ageindex and a constant determine the class membership
can be specified as
...
[\[eq:util_kt_ee\]](#eq:util_kt_ee){reference-type="ref"
reference="eq:util_kt_ee"})). For example, if there was an
...
We estimate a KT model by first calling mdcev.data on the **Recreation**
data. For these examples we are going to use a subset of 200 individuals
from the data.

::: example
data_model \<- mdcev.data(data_rec, subset = id \<= 200, id.var =
\"id\", alt.var = \"alt\", choice = \"quant\")
...
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 model = "gamma"}}%
{model = "gamma"}. We use maximum likelihood estimation by setting
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 algorithm = "MLE"}}%
{algorithm = "MLE"}.
...
Setting \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 print\_iterations = TRUE}}%
{print\_iterations = TRUE} will print out intermediate
...
utility function by changing the model argument to \texorpdfstring%
...
can be estimated by setting \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 model = "hybrid"}}%
{model = "hybrid"} as the next example
demonstrates.
...
call and the model call to \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "kt\_ee"}}%
{"kt\_ee"}.
...
the algorithm call to \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "Bayes"}}%
{"Bayes"}. Bayesian estimation is implemented
...
through these options in the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mdcev}}%
{mdcev} function:
...
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 prior\_psi\_sd}}%
{prior\_psi\_sd} standard deviation for normal prior with mean 0.
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 prior\_phi\_sd}}%
{prior\_phi\_sd} standard deviation for normal prior with mean 0.
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 prior\_gamma\_sd}}%
{prior\_gamma\_sd} standard deviation for half-normal prior with
  mean 1.
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 prior\_alpha\_shape}}%
{prior\_alpha\_shape} shape parameter for beta distribution.
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 prior\_scale\_sd}}%
{prior\_scale\_sd} standard deviation for half-normal prior with
  mean 0.
\end{itemize}
...
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 random\_parameters}}%
{random\_parameters} The form of the covariance matrix for the
...
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 n\_iterations}}%
{n\_iterations} The number of iterations to use in Bayesian
...
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 n\_chains}}%
{n\_chains} The number of independent Markov chains in Bayesian
  estimation.
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 n\_cores}}%
{n\_cores} The number of cores used to execute the Markov chains
  in parallel in Bayesian estimation. Can set using options(mc.cores =
  parallel::detectCores()).
\item
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 lkj\_shape\_prior}}%
{lkj\_shape\_prior} Prior for Cholesky matrix for correlated
  random parameters.
\end{itemize}
...
The output of the function can be accessed by calling \texorpdfstring%
...
posterior analysis in \href{https://CRAN.R-project.org/package=rstan}{\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rstan}}%
{rstan}},
\href{https://mc-stan.org/bayesplot/}{\href{https://CRAN.R-project.org/package=bayesplot}{\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont bayesplot}}%
{bayesplot}}}
\citep{gabrybayesplot2019}, and
\href{http://mc-stan.org/shinystan/}{\href{https://CRAN.R-project.org/package=shinystan}{\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont shinystan}}%
{shinystan}}}
...
individuals. We would like to include the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 university}}%
{university},
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 ageindex}}%
{ageindex}, and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 urban}}%
{urban} in the membership equation and we
include them in the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 formula}}%
{formula} interface. The constant for the
...
(\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 n\_classes}}%
{n\_classes}) is set greater than 1. The scale parameters are
fixed at 1 using \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 fixed\_scale1 = 1}}%
{fixed\_scale1 = 1}.
...
factors included in the membership equation with only \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 urban}}%
{urban}
...
If \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 initial.parameter}}%
{initial.parameter} are not provided, the default is to use
...
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mdcev\_lc[["mdcev\_fit"]]}}%
{mdcev\_lc[["mdcev\_fit"]]} object for comparison.
...
parameterized in many ways. The \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rmdcev}}%
{rmdcev} package defines the
...
parameters. We set the argument \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 random\_parameters = "uncorr"}}%
{random\_parameters = "uncorr"} to
...
the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 psi\_ascs = 0}}%
{psi\_ascs = 0} to omit the ASCs in the \(\psi_k\) parameters.
...
can be estimated by setting \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 random\_parameters = "corr"}}%
{random\_parameters = "corr"}.
...
\item
  ...
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 Error in chol.default(-H) : the leading minor of order 9 is not positive definite}}%
{Error in chol.default(-H) : the leading minor of order 9 is not positive definite}
  or \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 In sqrt(diag(cov\_mat)) : NaNs produced}}%
{In sqrt(diag(cov\_mat)) : NaNs produced}, this usually
...
\item
  ...
  estimation, users should consult the \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rstan}}%
{rstan} User Guide for
  ...
  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 help(rstan)}}%
{help(rstan)}.
\end{itemize}
...
The \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rmdcev}}%
{rmdcev} package includes simulation functions for calculating
...
scale parameter. \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rmdcev}}%
{rmdcev} allows errors to be drawn using uniform
...
zero consumption level. \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rmdcev}}%
{rmdcev} implements the
...
described by \citet{lloydsmithnew2018} and the \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rmdcev}}%
{rmdcev} extends
...
In \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rmdcev}}%
{rmdcev}, the functions for welfare and demand simulation have
been divided into 3 steps to allow users to parallelize operations as
necessary.

We first estimate the model using \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mdcev}}%
{mdcev} and we set
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 std\_errors = "mvn"}}%
{std\_errors = "mvn"} to generate multivariate normal draws as
...
scenario. For prices, \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rmdcev}}%
{rmdcev} is set up to accept additive changes
...
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 price\_change\_only = TRUE}}%
{price\_change\_only = TRUE} which ensures duplicate \(\psi\) and
...
\begin{example}
nalts <- mdcev_mle$stan_data\[\[\"J\"\]\] npols \<- 2

policies\<- CreateBlankPolicies(npols = npols, model = mdcev_mle,
...
policies object. For example, the following code will increase the value
of the first $\psi$ variable by 20% in policy scenario 2.

::: example
policies_2 \<- CreateBlankPolicies(npols = npols, model = mdcev_mle,
price_change_only = FALSE)

policies_2$dat_psi_p[[2]][, 1] <- policies_2$dat_psi_p\[\[2\]\]\[, 1\]
\* 1.2
:::
...
welfare \<- mdcev.sim(df_sim$df_indiv,
                     df_common = df_sim$df_common, sim_options =
df_sim$sim_options,
                     cond_err = 1,
                     nerrs = 25,
                     sim_type = "welfare")
...
The output of the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mdcev.sim}}%
{mdcev.sim} for welfare analysis is an object of
class \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mdcev.sim}}%
{mdcev.sim} which contains a list of matrices where each
...
to easily parallelize the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mdcev.sim}}%
{mdcev.sim} function using the
\href{https://CRAN.R-project.org/package=parallel}{\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont parallel}}%
{parallel}} package or similar packages.
...
\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont rmdcev}}%
{rmdcev}. Please refer to the previous section for an overview of
...
demand <- mdcev.sim(df_sim$df_indiv, df_common = df_sim$df_common,
                        sim_options = df_sim$sim_options, cond_err = 1,
nerrs = 25, sim_type = \"demand\")

#\> Using hybrid approach in simulation\... #\> 5.40e+06simulations
finished in0.07minutes.(1360202per second)

summary(demand)

#\> \# A tibble: 36 x 6 #\> \# Groups: policy \[2\] #\> policy alt mean
std.dev 'ci_lo2.5 #\> \<chr\> \<int\> \<dbl\> \<dbl\> \<dbl\> \<dbl\>
#\> 1 policy1 0 68946. 8.62 68933. 68962. #\> 2 policy1 1 6.24 0.02 6.2
6.28 #\> 3 policy1 2 11.2 0.05 11.1 11.2 #\> 4 policy1 3 2.34 0.02 2.31
2.37 #\> 5 policy1 4 7.24 0.04 7.18 7.3 #\> 6 policy1 5 3.76 0.02 3.72
3.78 #\> 7 policy1 6 20.7 0.06 20.7 20.8 #\> 8 policy1 7 5.29 0.01 5.28
5.3 #\> 9 policy1 8 36.1 0.15 35.8 36.4 #\> 10 policy1 9 0.55 0 0.53
0.55 #\> \# \... with 26 more rows
:::
...
The rmdcev package has the capability to simulate KT data. Simulated KT
data can be easily created for model assessment and Monte Carlo analysis
using the GenerateMDCEVData function. The following example will
generate a simulated data set with 1,000 individuals, 10 non-numeraire
alternatives, and particular parameter values.

::: example
model = \"gamma\" nobs = 1000 nalts = 10 sim.data \<-
GenerateMDCEVData(model = model, nobs = nobs, nalts = nalts, psi_j_parms
= c(-5, 0.5, 2), \# alt-specific variables psi_i_parms = c(-1.5, 3, -2,
1, 2), \# individual-specific variables gamma_parms =
stats::runif(nalts, 1, 10), alpha_parms = 0.5, scale_parms = 1)
...
hope that the publication of rmdcev will make KT modeling available to a
wider audience.
...
([\[eq:optimal_x\]](#eq:optimal_x){reference-type="ref"
reference="eq:optimal_x"}). Set the remaining alternative consumption
levels to zero and stop.
