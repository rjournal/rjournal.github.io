...
defined as
$$\label{eq:glm}  
g\left(E(y_i)\right) = \mathbf{w}_i^{\top}\boldsymbol{\beta} + \mathbf{X}_i \circ \mathbf{B},   (\#eq:glm)$$
...
$\mathbf X_i$ so that \@ref(eq:glm) can be expressed as a conventional
GLM with $d + PQ$ covariates. When $PQ$ is large, one can explore the
matrix structure of predictors and consider a low-rank tensor GLM so as
to reduce the number of parameters of interest while retaining the
variable-specific resolution. The main idea of tensor GLM is to model
$\mathbf{B}$ by a low-rank-constrained $\mathbf{B}$ so that $\mathbf{B}$
can be fully specified using fewer unconstrained parameters. See
...
$$\label{eq:lrtr}
g\left(E(y_i)\right) = \mathbf{w}_i^{\top}\boldsymbol{\beta} + \mathbf{X}_i \circ \left(\mathbf{B}_1\mathbf{B}_2^{\top}\right).    (\#eq:lrtr)$$
...
parameters in \@ref(eq:lrtr). According to the theory of GLM, the score
...
package
[*TensorTest2D*](https://CRAN.R-project.org/package=TensorTest2D). The
main function, `tensorReg2D`, in our
[*TensorTest2D*](https://CRAN.R-project.org/package=TensorTest2D)
package is used for following data analysis. The inputs are the response
vector, `y`, covariates matrix `X`, collecting tensor data, and vector
`W`, collecting adjustment information such as age and gender. The key
...
The package
[*TensorTest2D*](https://CRAN.R-project.org/package=TensorTest2D)
includes a data set, `omics`, which consists of a continuous response
...
$R\leq\min{\{P, Q\}}$ (see @ChangYang2021bioinformatics), there are
...
`summary(omicsMdl)` shows the model structure, summary statistics about
the residuals, and the table of significance tests for the coefficients.
On top of the result table, the model structure `y ~ I + X` of this case
is revealed where `I` is the intercept term and `X` is the matrix
covariate. The names of the coefficients appear in the first column of
the table. In addition to the `(Intercept)` and the terms of
$\mathbf{w}$, `Xi.j` is the coefficient of the $i$th row and $j$th
...
`ROWi:COLUMNj`. Those values in the summary table can also be returned
separately. Here, we print the attributes of the `tsglm` object
separately for the estimated coefficients and their standard deviations,
along with the $p$-values by the Wald test (see @Wald1943tams).
...
In our package
[*TensorTest2D*](https://CRAN.R-project.org/package=TensorTest2D), the
function `plot()` can be used to visualize the importance of the matrix
predictor. The output is an $P\times Q$ heat map that the plotted values
on it are controlled by the option `type`. If the unit of data varies
across the rows or columns in $X$, it is suggested to choose the
t-statistics of the coefficients (`type = "tval"`) instead of their
values `type = "coef"`. In addition, the function `plot()` also marks
...
`help("p.adjust")`) by the option `method` and specify the significance
level through the option `alpha`. We plot in Figure [1](#fig:omicsplot)
...
<figure id="fig:omicsplot">
<img
src="chen-chang-chen-tzeng-chang_files/figure-latex/omicsplot-1.png"
style="width:100.0%" alt="graphic without alt text" />
<figcaption>Figure 1: The image plot of the values for t-statistics of
matrix covariate in the omics data. The effective pixels identified by
the tensor regression model are marked out by the <span
class="math inline">⊠</span> symbol.</figcaption>
</figure>
...
[*dslabs*](https://CRAN.R-project.org/package=dslabs) package
[@rafael2019dslabs]. For the purpose of demonstration, we reduce the
...
[2](#fig:mnistpreproc). The original $28 \times 28$ images are thus
...
right of Figure [2](#fig:mnistpreproc). The mean image of the
pre-processed training set for each label in the MNIST database is shown
in Figure [3](#fig:mnistmean). These pre-processed images of
$10 \times 10$ pixels are included in the package
[*TensorTest2D*](https://CRAN.R-project.org/package=TensorTest2D) and
can be imported using the following commands:

<figure id="fig:mnistpreproc">
<img
src="chen-chang-chen-tzeng-chang_files/figure-latex/mnist_preproc.png"
style="width:95.0%" alt="graphic without alt text" />
<figcaption>Figure 2: Data pre-processing for the MNIST dataset. First,
the left subfigure shows the max-pooling step for reducing the image
size. Next, the center subfigure shows the edge-cutting step for
removing the noninformative pixels. Finally, the right subfigure shows
the data pre-process result.</figcaption>
</figure>

<figure id="fig:mnistmean">
<img
src="chen-chang-chen-tzeng-chang_files/figure-latex/mnist_preproc_out.png"
style="width:95.0%" alt="graphic without alt text" />
<figcaption>Figure 3: The mean plots of pre-processed images in the
training dataset. The value at each pixel of the mean plot is the
average grayscale value over the pre-processed images in the training
dataset.</figcaption>
</figure>
...
'5' randomly chosen from the MNIST training set `mnist_train`. In this
...
In the package
[*TensorTest2D*](https://CRAN.R-project.org/package=TensorTest2D), the
function `tensorReg2D()` is also used for fitting the logistic tensor
...
`train_y`, and the contaminated images, `train_Xn`, as inputs for model
training. There is no auxiliary information available to further
stratify the $y_i$'s, we specify a constant vector
`W = matrix(1, length(train_y), 1)` of length $n$, and if a rank $R = 4$
model is needed, we set `n_R = 4`. (In fact, the rank-4 model is the
...
ways. Similar to that Figure [1](#fig:omicsplot), we can make a plot
first for the values of t-statistics for the pixels by using the
`plot()` function as shown below this paragraph. Here, we adjust the
$p$-values according to the approach in [@benjamini1995controlling] by
setting the parameter `method = "fdr"`. The resulting plot is shown in
Figure [4](#fig:mnisttval), and most of the effective pixels can be
...
<figure id="fig:mnisttval">
<img
src="chen-chang-chen-tzeng-chang_files/figure-latex/mnisttval-1.png"
style="width:45.0%" alt="graphic without alt text" />
<figcaption>Figure 4: The image plot of the values for t-statistics of
matrix covariate in the handwritten label data. The effective pixels
identified by the logistic tensor regression model are marked out by the
<span class="math inline">⊠</span> marks.</figcaption>
</figure>
...
shown in Figure [5](#fig:mnistplot). On top of both images, there are
...
<figure id="fig:mnistplot">
<img
src="chen-chang-chen-tzeng-chang_files/figure-latex/mnistplot-1.png"
style="width:95.0%" alt="graphic without alt text" />
<figcaption>Figure 5: Effective pixels identified by the logistic tensor
rgression model. The important pixels to discriminate between labels 2
and 5 are marked by red and blue frames, which indicate positive and
negative coefficients, respectively.</figcaption>
</figure>
...
[*TensorTest2D*](https://CRAN.R-project.org/package=TensorTest2D)
provides the function `draw.coef()` to produce the marked image similar
to that in Figure [5](#fig:mnistplot). Different from the function
`plot.tsglm()`, users need to provide an input as the markers for
effective pixels. The markers in our example here are the LASSO
estimates, and by specifying `marks = l1B`, the pixels with non-zero
coefficients are then marked. In addition, by specifying
`markstyle = "bi-dir"`, as shown in Figure [6](#fig:mnistlasso), the
...
<figure id="fig:mnistlasso">
<img
src="chen-chang-chen-tzeng-chang_files/figure-latex/mnistlasso-1.png"
style="width:95.0%" alt="graphic without alt text" />
<figcaption>Figure 6: Effective pixels identified by the LASSO model.
...
coefficients, respectively.</figcaption>
</figure>
