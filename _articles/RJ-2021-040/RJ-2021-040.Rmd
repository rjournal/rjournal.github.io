...
- name: Valeria Policastro
  affiliation: Universitá degli Studi della Campania "Luigi Vanvitelli"
  address:
  - package author and creator
  - |-
  ...
  - |
    80131 Napoli, Italy
- name: Dario Righelli
  affiliation: Universitá di Padova
  address:
...
*robin* provides the best choice among the variety of the existing
methods for the network of interest. It is based on a procedure that
gives the opportunity to use the community detection techniques
implemented in the igraph package [@Csardi:2019] while providing the
user with the possibility to include other community detection
algorithms. *robin* initially detects if the community structure found
by some algorithms is statistically significant, then it compares the
different selected detection algorithms on the same network. *robin*
assumes undirected graphs without loops and multiple edges.

*robin* looks at the global stability of the detected partition and not
of single communities but accepts any detection algorithm and any random
...
algorithms in a theoretical way, such as [@Yang_et_al:2016], *robin*
aims to give a practical answer to such a comparison that can vary with
...
issue, we specify a perturbation strategy (see subsection
**[2.1](#sec:Perturbation_strategy)**) and a null model to build some
procedures based on a prefixed stability measure (see subsection
**[2.2](#sec:Stability_measure)**). Given:
...
$p$, as shown in Figure [1](#fig:PlotExample), and tests their
similarity by two types of functional statistical tests (see subsection
**[2.3](#sec:Statistical_Tests)**).

![Figure 1: []{#fig:PlotExample label="fig:PlotExample"} Example of
$Mc_{random}$ and $Mc$ curves generated by an M stability
measure](PlotExample.png){#fig:PlotExample width="100%"
...
degree preserving randomization by using the `rewire` function of the
...
In section **[4](#sec:example)**, we explore the $dk$ null random model
provided in [@Orsini_et_al:2015], whose code is available at
...
![Figure 2: Workflow to test the goodness of a community detection
algorithm.](Figure1.png){#fig:Flowchart1 width="100%"
alt="graphic without alt text"}
...
technique (see Figure [3](#fig:Flowchart2)):
...
![Figure 3: Workflow to compare two different community detection
algorithms.](Figure2.png){#fig:Flowchart2 width="100%"
alt="graphic without alt text"}

## Perturbation strategy {#sec:Perturbation_strategy}
...
graph. Therefore, in *robin*, the perturbation of a network preserves
...
## Stability measure {#sec:Stability_measure}
...
f\left(X\right)=1-X  
\end{aligned}$$
Only two of the four proposed stability measures, i.e., split-join and
VI, are distances. They differ in their dependency on the number of
clusters K: while the VI distance grows logarithmically with K, the
split-join metric grows with K toward the upper bound of 1. To make the
four different stability measures comparable, we normalized VI and
split-join between 0 and 1 (i.e., we divided the VI and the split-join
by their maximum, respectively $\log\left(n\right)$ and $2n$, where n
...
\end{aligned}   (\#eq:GPbayes-mean-cov)$$
...
Eq. (\@ref(eq:GPbayes-mean-cov)), the marginal likelihood becomes:
$$\begin{aligned}
...
*robin* uses the Bayes Factor (BF), which is approximated with a
log-ratio of marginal likelihoods of two GPs, each one representing the
hypothesis of differential (the profile has a significant underlying
signal) and non-differential expression (there is no underlying signal
in the profile, just random noise).
...
Once in the R environment, it is possible to install and load the
*robin* package with its dependencies, as follows:
...
[@Auguie2017], [*fdatest*](https://CRAN.R-project.org/package=fdatest)
[@PiniVantini:2015] ,
...
`plotComm`. The function `prepGraph`, required by the procedure, reads,
...
function `plotGraph`, with the aid of the *network3D* package, starting
from an igraph object loaded with `prepGraph`, shows an interactive 3D
...
*robin* embeds all the community detection algorithms present in
*igraph*. They can be classified as in [@Fortunato:2009]
...
`robinRobust` and `robinCompare`. These two functions create the
internal process for perturbation and measurement of communities
stability. In particular `robinRobust` tests the robustness of a chosen
detection algorithm and `robinCompare` compares two different detection
algorithms. The option `measure` in the `robinRobust` and `robinCompare`
functions provides the flexibility to choose between the four different
measures listed in the subsection **[2.2](#sec:Stability_measure)**.
...
original network. The function `rewire` randomly assigns a number of
...
[4](#fig:PlotComparison)-Left panel), and the second curve is obtained
in the same way but considering as the original graph the random graph
(red curve in Figure [4](#fig:PlotComparison)-Left panel). The
...
When `plotRobin` is used on the output of `robinCompare`, i.e. the
...
Figure [4](#fig:PlotComparison), each curve is obtained by computing the
...
<figure id="fig:PlotComparison">
<p><img src="PlotRobin.png" style="width:50.0%"
alt="graphic without alt text" /><img src="PlotCompare.png"
style="width:50.0%" alt="graphic without alt text" /></p>
<figcaption>Figure 4: Curves of the null model and the real data
generated by the VI stability measure and the Louvain detection
algorithm on the American College Football network (Left panel). Curves
of the Louvain and Fast greedy algorithm generated by the VI stability
measure on the American College Football network (Right panel) <span class="citation" data-cites="GirvanNewman:2002" aria-expanded="false">(<a href="#ref-GirvanNewman:2002" role="doc-biblioref">Girvan and Newman 2002</a>)</span>.</figcaption>
</figure>


## Testing

The GP test is implemented in `robinGPTest` and uses the R package
*gprege* [@KalaitzisLawrence2011]. The ITP test is implemented in
`robinFDATest` and uses the R package *fdatest* [@PiniVantini:2015]. The
area under the curves is calculated by the function `robinAUC` and
relies on the *DescTools* package. Figure [5](#fig:PlotComparisonFDA)
...
(left panel) [@GirvanNewman:2002] and corresponding adjusted
***p***-values (right panel).

![Figure 5: Curves of the Louvain and Fast greedy algorithm generated by
the VI stability measure using the Interval Testing Procedure on the
American College Football network (Left panel). Corresponding p-values
and adjusted p-values for all the intervals with the horizontal red line
on to the critical value 0.05 (Right
panel).](FDA.png){#fig:PlotComparisonFDA width="100%"
alt="graphic without alt text"}
...
::: {#tab-functions}
  ------------------------------------------------------------------------------------------------------------------------
  [Function]{.smallcaps}                                      [Description]{.smallcaps}
  ------------------------- ----------------------------------------------------------------------------------------------
  **Import/Manipulation**   
...
In Table [2](#tab-Time), we show the computational time evaluated on a
computer with an Intel 4 GHz i7-4790K processor and 24GB of memory. In
this example, we used $Louvain$ as a detection algorithm on the $LFR$
benchmark graphs [@Lancichinetti_2008]. The time complexity could be
mitigated using parallel computing, but this is not yet implemented.
...
# Example test: the American College football network {#sec:example}
...
different ones. We applied all the methods listed in subsection
**[3.3](#sec:procedures)** to this network, choosing as measure the VI
metric.
...
in *igraph* and can assume the following values: *fastGreedy*,
*infomap*, *walktrap*, *edgeBetweenness*, *spinglass*, *leadingEigen*,
*labelProp*, *louvain*. The function `compare` is contained in the
package *igraph* and permits the assessment of the distance between two
community structures according to the chosen method.

Table [3](#tab-VIresults) summarizes the VI results calculated between
...
Figure [6](#fig:PlotComparison_allInfomap) shows the results we
...
![Figure 6: Plot of the VI curves of Infomap against all other
...
*dk*-series with $d=2.1$ are shown in Figure [7](#fig:PlotGraph).

![Figure 7: Graph of the real data (Upper panel); graph of the CM null
model (Left - lower panel); graph of the $dk$-series null model with
$d=2.1$ (Right - lower panel).](network.png){#fig:PlotGraph width="100%"
alt="graphic without alt text"}
...
[8](#fig:PlotConfigurationModel)). In fact, when the modularity is low,
...
function `robinRobust` to assess the robustness of each detection
algorithm.
...
![Figure 8: Plot of the VI curves of the CM null model and all the
algorithms
...
Figure [9](#fig:PlotDkModel) shows the stability measure curves of each
...
the results in Table [3](#tab-VIresults).

![Figure 9: Plot of the VI curves of the *dk*-series null model with
$d=2.1$ and all the algorithms
implemented.](PlotDkModel.png){#fig:PlotDkModel width="100%"
alt="graphic without alt text"}
...
The results are shown in Table [4](#tab-BayesAIC) and agree with those
shown in Table [3](#tab-VIresults).
...
analysis, such as $GP$ regression and $ITP$. The core functions of the
package are `robinRobust` and `robinCompare`, which build the stability
...
for two detection algorithms, respectively. Moreover, `robinGPTest` and
`robinFDATest` implement the GP test and the ITP test. We illustrated
...
*igraph* version 1.2.4.2, *networkD3* version 0.4, *ggplot2* version
3.2.1, *gridExtra* version 2.3, *fdatest* version 2.1, *gprege* version
1.30.0, and *DescTools* version 0.99.31. R itself and all packages used
