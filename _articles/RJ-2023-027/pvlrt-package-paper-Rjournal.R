...
# Please edit pvlrt-package-paper-Rjournal.Rmd to modify this file
...
## ----table-summary-existing-packages, echo=FALSE, results=ifelse(knitr::is_latex_output(), 'asis','markup')----
if (knitr::is_html_output()) {
    ...
    mutate(
      Notes = case_when(
        note_tmp == "note1" ~ glue::glue(
          "These two packages are aimed towards drug safety."
        ),
        note_tmp == "note2" ~ glue::glue(
          "These packages are aimed towards vaccine-safety, \\
        particularly using sequential testing-based vaccine-safety \\
        assessment procedures as used by the FDA."
        ),
        note_tmp == "note3" ~ glue::glue(
          "This provides functions for handling \\
  ...
  out_tab <- pkg_summary_tab %>% 
    kableExtra::kbl(
      ...
      label = "pkg_compare",
      caption = glue::glue(
        "Existing CRAN packages with functionalities for pharmacovigilance."
      )
    ) %>%
    kableExtra::column_spec(column = 1:2, width = "18em")
  out_tab
...
## ----statin-46-heatmap-interactive-ploty, include=knitr::is_html_output()-----
...
## ----heatmap-zip-statin-html, fig.height=6, fig.width=12, out.width='100%', fig.align='center', include=knitr::is_html_output()----
pl_heat_statin_zip <- heatmap_pvlrt(
  test_statin_zip,
  AE = 15,
  show_n = FALSE,
