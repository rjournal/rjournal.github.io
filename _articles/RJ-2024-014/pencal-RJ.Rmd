...
Risk prediction models [@steyerberg2009] allow us to estimate the probability that an event of interest will occur in the future. Such models are commonly employed in the biomedical field to estimate the probability that an individual will experience an adverse event, and their output can be used to inform patients, monitor their disease progression, and guide treatment decisions.
...
where $\beta_s$ is a vector of fixed effect parameters, $u_{si} \sim N(0, D_s)$ is a vector of random effects, $\varepsilon_{si} \sim N(0, \sigma^2_s I_{m_i})$ is the error term vector, and $W_{si}$ and $Z_{si}$ are design matrices associated with $\beta_s$ and $u_{si}$. As an example, later in this article, we will consider an example where we let $y_{si}$ depend on the age $a_{ij}$ of subject $i$ at each visit and include a random intercept and random slope in the LMM:
...
The computation of the CBOCP is, by nature, repetitive, as it requires repeating steps 1, 2, and 3 over $B$ bootstrap samples, and computing predictions and performance measures both on the bootstrap sample and on the original dataset to estimate the optimism. Due to this repetitiveness, such computations can be easily parallelized to reduce computing time. Moreover, also the estimation of the $p$ LMMs / MLPMMs in step 1 can be trivially parallelized.
...
S3 classes and methods are implemented for the outputs of each modeling step:
...
Lastly, only repeated measurements taken up to the landmark time ($t_{ij} \leq t_L$) should be retained for modeling, whereas measurements taken after the landmark time ($t_{ij} > t_L$) should be discarded:
...
After choosing $t_L = 2$ as the landmark time, the number of subjects retained for model estimation is 278, of which 107 experience the event of interest, whereas the remaining 171 are right-censored:
...
We can observe that some longitudinal covariates exhibit strong skewness, as in the case of `serBilir`. Although, in principle, the LMM can be used to model variables with skewed distributions, this may sometimes lead to convergence problems or poor model fit. It can thus be advisable to transform such covariates to prevent these problems (but note that this is not a required modeling choice, and one may alternatively choose to avoid such transformation). For this reason, we log-transform those longitudinal covariates with skewed distribution before modeling them with LMMs:
...
The first step in the estimation of PRC involves modeling the evolution over time of the longitudinal predictors through mixed-effects models. Hereafter, we model each of the longitudinal predictors using the LMM given in equation \eqref{eq:lmmwithslope}, which comprises a random intercept and a random slope for age. Estimation of this model for each longitudinal predictor can be done using the function `fit_lmms`:
...
The `standardize` argument is used to determine whether the predicted random effects should be standardized prior to inclusion in the Cox model (default is `TRUE`). By default, `fit_prclmm` does not penalize baseline covariates, but this default behavior can be changed using the `pfac.base.covs` argument (not shown here).
...
The function `survplot_prc` allows visualizing predictions for a sample of individuals:
...
Note that the variables and their variable types in `new_ldata` and `new_sdata` should be the same as in the `ldata` and `sdata`; the only exception to this is that `new_sdata` does not need to contain information about survival, so unlike `sdata`, it does not comprise the `time` and `event` variables.
...
As explained in the Statistical Methods Section, estimation of the predictive performance in \CRANpkg{pencal} is done through a CBOCP that allows obtaining unbiased estimates of predictive performance as measured by the tdAUC, C index, and Brier score.
...
Computations were performed on an AMD EPYC 7662 processor with a 2 GHz CPU, using a single core for simulations 1, 2, and 3, and a number of cores ranging from 1 to 16 in simulation 4.
<!-- In simulations 1 and 2, computations were performed using a single core as adding cores does not provide a substantial improvement on the estimation of PRC. In simulation 3, where the number of cores can have a sizeable effect on computing time for the CBOCP, 8 and 16 cores were used. -->
...
To understand this initially decreasing, but later increasing pattern, it is useful to consider the computing time of each of the modeling steps separately. By looking at Figure \ref{fig:ctime4}, we notice that step 1, which involves the estimation of $p \cdot (B+1)$ LMMs, is the most time-consuming step; its computing time consistently decreases as the number of cores decreases (from 636.1 to 60.8 seconds).
...
As for step 3, we can see that it is the lightest step in terms of computing time. The pattern is consistently decreasing from 1 (35.4 seconds) to 8 cores (8.8 seconds), with a slight increase when using 16 cores (10.6 seconds).

In conclusion, the results of simulation 4 show how it may be advisable to parallelize computations to compute the CBOCP, but without using an excessive number of cores (especially for step 2). Our general advice is to use between 3 and 8 cores for optimal performance; nevertheless, we emphasize how the effect of the number of cores on computing time may differ from the patterns in Figure \ref{fig:ctime4} depending on a combination of factors such as $n$, $p$, the number of repeated measurements per subject, and $B$. Furthermore, we notice that within `pencal`, a different number of cores can be chosen for each modeling step; in the example of simulation 4, the optimal performance would be achieved using 16 cores for step 1, 4 cores for step 2, and 8 cores for step 3 (but using 8 cores for all 3 steps isn't much less efficient).
