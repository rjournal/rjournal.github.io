...
particular prediction [@szegedy2013intriguing]. In recent years, there
has been a growing research focus on enhancing trust in AI systems by
improving their interpretability. Broadly, interpretability methods can
be categorized into *post-hoc* and *ante-hoc* approaches
[@dovsilovic2018explainable]. *Post-hoc* methods aim to explain a
trained black-box model (like a typical Neural Network) using an
external, intrinsically interpretable ("white-box") model to approximate
or interpret the black-box decisions. In contrast, *ante-hoc* methods
aim to build and train inherently interpretable ("white-box") models
that balance high predictive accuracy with transparency.
...
@GNAM, used univariate multi-layer perceptrons without backpropagation
and required manual tuning. Subsequent approaches, like that of
@Bras-Geraldes2019, improved flexibility with parametric link functions
and bootstrap-based confidence intervals but remained constrained to
shallow architectures. More recent methods implemented in Python take
advantage of deep learning techniques: Neural Additive Models (NAMs)
[@NEURIPS2021_251bd044] train feature-specific subnetworks jointly with
backpropagation but can suffer from overfitting due to sharp activation
functions. GAMI-Net [@yang2021gami] extends this by allowing pairwise
interactions and incorporating sparsity and marginal clarity
constraints, but with an increased computational cost. Finally, IGANN
[@kraus2023interpretable] introduces a GANN based on gradient boosting
techniques with sparsified shallow networks, assuming initially linear
effects and avoiding deep architectures.

The development of the
[**neuralGAM**](https://CRAN.R-project.org/package=neuralGAM) package
has been motivated by recent contributions in the development of GANNs,
in particular, the method proposed by @neuralGAM to train a GANN using
independent neural networks using the local scoring (LS) and backfitting
(BF) algorithms.
...
package [@deepregression] implements distributional regression models
for mean, scale, and shape parameters using deep neural networks,
...
[**mgcv**](https://CRAN.R-project.org/package=mgcv) [@mgcv], which
...
package[@bamlss], in contrast, provides a more general Bayesian
framework for additive models that can incorporate neural network terms.
Estimation in this context is performed through Bayesian sampling or
variational inference, which naturally provides posterior uncertainty
quantification at the expense of higher computational costs. These
packages illustrate complementary methodological directions:
[**deepregression**](https://CRAN.R-project.org/package=deepregression)
...
package [@beck2018neuralnettools], for instance, offers visualizations
and sensitivity analyses tailored to neural networks. More general
frameworks such as [**iml**](https://CRAN.R-project.org/package=iml)
[@iml] and [**DALEX**](https://CRAN.R-project.org/package=DALEX)
[@dalex] implement techniques such as feature importance, partial
dependence, and Shapley values, supporting both local and global
...
Models [@nelder1972generalized] allow the use of other distribution
...
Additive Models [@hastie1990tibshirani] defined by
...
$f_1$ while subtracting the same constant value from $f_2$
[@hastie1990tibshirani].
...
::: {#dist-link}
  ...
  Gaussian, binomial and Poisson distributions [@hastie1990tibshirani].
:::
...
regression model in \@ref(eq:GAM), such as Bayesian approaches
[@lang2004bayesian], local polynomial kernel smoothers
[@wand1994kernel; @copeland1997local] or regression splines
[@de1978practical]. Unlike these quite common techniques, we propose to
use independent neural networks ---which are universal function
estimators [@hornik1989multilayer]--- to learn the contribution of each
covariate to the dependent variable, i.e., at each iteration of the
backfitting algorithm, we train a feed-forward Neural Network for each
covariate $X_j$ with the entire set of training data for one epoch. The
fits are improved at each epoch as the learned adjusted dependent
variable $Z_i$ approaches $Y_i$ at each iteration of the local scoring
algorithm. Algorithms [1](#local-scoring){reference-type="ref"
...
[@gal2016dropout], a widely used technique for estimating model
...
\begin{cases}
\widehat{\mathrm{Var}}_{\mathrm{np}}\!\bigl[f_j(x_j)\bigr], & \text{if} {f_j} \text{is a smooth term},\\[6pt]
\widehat{\mathrm{Var}}_{\mathrm{p}}\!\bigl[f_j(x_j)\bigr], & \text{if} {f_j} \text{is a linear term.}
\end{cases}$$
...
[@chollet2015keras] and
[**TensorFlow**](https://CRAN.R-project.org/package=TensorFlow)
[@tensorflow2015-whitepaper], high-level APIs for the implementation of
neural networks well-known for their simplicity, flexibility, and ease
of use. [**neuralGAM**](https://CRAN.R-project.org/package=neuralGAM)
also relies on other [R]{.sans-serif} packages for visualization
...
::: {#pkg:fun}
  ...
  `autoplot.neuralGAM`   Visualization of fitted components and prediction/confidence intervals using [**ggplot2**](https://CRAN.R-project.org/package=ggplot2) [@ggplot2], returning a `ggplot` object, with support for confidence and prediction intervals.
...
::: {#pkg:arg}
  ...
  `kernel_initializer`     Kernel initializer for the Dense layers. Defaults to Xavier initializer `"glorot_normal"` [@pmlr-v9-glorot10a].
  ...
  `…`                      Other arguments to pass on to the Adam optimizer [@kingma2014adam].
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
[@kingma2014adam] as an optimizer for stochastic gradient descent, whose
behaviour can be customized using the `loss`, `activation`,
...
and one based on `ggplot2` [@ggplot2]. With this latter plot method, the
`autoplot` function creates a `ggplot` object that can be modified later
by the user just using the functionality provided in the `ggplot2`
...
Figure [1](#fig:diagnose-example){reference-type="ref"
...
![Figure 1: Example output from the `diagnose` function for a Gaussian
model](figures/diagnose_neuralGAM.png){#fig:diagnose-example
...
$\epsilon$ are available at @neuralGAM.
...
summarizes the results, while Table [7](#tab:T7){reference-type="ref"
reference="tab:sim5"} shows the architecture and configuration of each
neural network.
...
Figures [2](#fig:compare_models){reference-type="ref"
reference="fig:compare_models"}a
and [2](#fig:compare_models){reference-type="ref"
reference="fig:compare_models"}b show the estimated partial effects for
each covariate for
[**neuralGAM**](https://CRAN.R-project.org/package=neuralGAM) and
[**deepregression**](https://CRAN.R-project.org/package=deepregression),
...
src="figures/neuralGAM_gaussian_CI_v2.png" style="width:100.0%"
alt="graphic without alt text" /></td>
</tr>
<tr>
<td style="text-align: center;"><span>a. <a
href="https://CRAN.R-project.org/package=neuralGAM"><strong>neuralGAM</strong></a></span></td>
</tr>
<tr>
<td style="text-align: center;"><img
src="figures/deepreg_gaussian_estimates.png" style="width:100.0%"
alt="graphic without alt text" /></td>
</tr>
<tr>
<td style="text-align: center;"><span>b. <a
href="https://CRAN.R-project.org/package=deepregression"><strong>deepregression</strong></a></span></td>
</tr>
</tbody>
</table>
<figcaption>Figure 2: Estimated functions from both approaches. Black
...
Figure[2](#fig:compare_models){reference-type="ref"
reference="fig:compare_models"} displays centered partial effects, which
isolates the contribution of a single smooth and makes the scale
directly comparable to the true generative functions. Consequently,
ensemble uncertainty bands are not on the same vertical scale as the
centered smooth effects, although the shapes of the functions remain
comparable.

![Figure 3: Estimated conditional mean functions with ensemble-based
...
Figure [3](#fig:ensemble-deepregression){reference-type="ref"
reference="fig:ensemble-deepregression"} highlights this difference in
the uncertainty estimation approach: while
[**neuralGAM**](https://CRAN.R-project.org/package=neuralGAM) produces
per-term confidence intervals via MC-Dropout
(Figure [2](#fig:compare_models){reference-type="ref"
reference="fig:compare_models"}),
[**deepregression**](https://CRAN.R-project.org/package=deepregression)
...
the `nycflights13` package [@nycflights13]. The data set includes
airline on-time data for flights departing from all the airports in New
York City during 2013. It also includes useful metadata on airlines,
...
::: {#tab:sim5}
  -------------------- ---------------------------------------------------------------- --------------------------------------------------------------------------------------------------------------------------------- -----------------------------------------------------------------
     ...
     **Model type**                  Additive neural GAM; binomial family                Distributional regression; additive predictor; spline bases from [**mgcv**](https://CRAN.R-project.org/package=mgcv); Bernoulli                           Feedforward MLP

   **Hidden layers**      Per-term subnetworks:*air_time*: 2 layers; others: 1 layer                                                   One spline-basis 'layer' per smooth                                                                      2 dense hidden layers

    **Units/layer**                  *air_time*: (256, 128); others: 128                                                            Spline basis size (TPRS, default $k=10$)                                                                     256, 128; output: 1

     **Activation**               swish (*air_time*, *temp*); tanh (*humid*)                                                              Linear (basis representation)                                                                   swish (hidden); sigmoid (output)

   **Regularization**                  Dropout $0.01$ per hidden layer                                                             Quadratic penalties on spline coefficients                                                         No explicit weight decay; early stopping

    **Optimization**    BF + LS; Adam (LR $0.001$); bf $10^{-2}$, ls $0.01$; loss: MSE                                    Penalized likelihood;TensorFlow initialized from spline bases                                       Adam (LR $0.001$); binary cross-entropy; batch size $128$

    **Uncertainty**         MC-Dropout CIs; 300 passes; rate $0.01$, $\alpha=0.05$                                                               Using ensembles                                                                                        None

      **Training**                   Iterative BF + LS until convergence                                                     Up to 100 epochs; early stopping $\text{patience} = 5$                                            Up to 100 epochs; early stopping $\text{patience} = 5$
  -------------------- ---------------------------------------------------------------- --------------------------------------------------------------------------------------------------------------------------------- -----------------------------------------------------------------

  : (#tab:T7) Model architecture and configuration for
  [**neuralGAM**](https://CRAN.R-project.org/package=neuralGAM) and
  [**deepregression**](https://CRAN.R-project.org/package=deepregression)
  in the simulated study. 
:::

All models use the same four covariates and are evaluated by AUC--ROC on
the test set. In addition, we also report the deviance explained on the
training set, and perform a comparison in terms of the interpretability
provided by analyzing the partial effect plots.
Table [7](#tab:T7){reference-type="ref" reference="tab:sim5"} summarizes
the architecture and hyperparameters of each method.

<figure id="fig:comparison_flights">
<table>
<caption> </caption>
<tbody>
<tr>
<td style="text-align: center;"><img
src="figures/neuralGAM-flights-swish.png" style="width:100.0%"
alt="graphic without alt text" /></td>
</tr>
<tr>
<td style="text-align: center;"><span>a. <a
...
<code>autoplot()</code>.</span></td>
</tr>
<tr>
<td style="text-align: center;"><span id="fig:neural_model_flights"
data-label="fig:neural_model_flights"></span> <img
src="figures/deepreg_airtime_binomial.png" style="width:100.0%"
alt="graphic without alt text" /></td>
</tr>
<tr>
<td style="text-align: center;"><span>b. <a
...
<code>plot()</code>.</span></td>
</tr>
<tr>
<td style="text-align: center;"><span id="fig:deepreg_airtime_binomial"
data-label="fig:deepreg_airtime_binomial"></span></td>
</tr>
</tbody>
</table>
<figcaption>Figure 4: Estimated partial effects for the four
...
showcase how to use *Swish* [@ramachandran2017searching], a
...
::: {#tab:comparison_realdata}
  ...
  : (#tab:T8) Predictive performance on the flight delay task.
:::

Table [8](#tab:T8){reference-type="ref"
...
methods, Figure [4](#fig:comparison_flights){reference-type="ref"
reference="fig:comparison_flights"} compares the estimated partial
effects of [**neuralGAM**](https://CRAN.R-project.org/package=neuralGAM)
and
[**deepregression**](https://CRAN.R-project.org/package=deepregression)
...
including conformal prediction [@karimi2023quantifying], bootstrap-based
