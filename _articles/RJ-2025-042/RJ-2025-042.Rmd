...
$b_{ij} = 1$ iff clone $i$ contains the mutation $j$ (Gusfield 1991).
The matrix $\boldsymbol{U}$ is an $s \times n$ matrix where $u_{ij}$ is
the fraction of clone $j$ in sample $i$.
...
2016; Husić et al. 2019; Fu et al. 2022; [Grigoriadis et al.]{.nocase}
...
manually crafted and limited in number, posing scalability issues.
...
position, can lead to more accurate VAF values (Petrackova et al. 2019).
...
- `k`: A numeric value that determines the linearity of the tree
  ...
  `k` is set to 1, all nodes have equal probabilities of being chosen as
  parents, resulting in a uniformly random topology.

- `selection`: A character string representing the evolutionary mode the
  tumor follows. This should be either \"positive\" or \"neutral\".

- `noisy`: A logical value (`TRUE` by default) indicating whether to add
  noise to the frequency matrix. If `TRUE`, noise is added to the
  frequency matrix. If `FALSE`, no noise is added.

- `depth`: A numeric value (30 by default) representing the mean depth
  of sequencing.

- `seed`: A numeric value (`Sys.time()` by default) used to set the seed
...
- `create_B`: This function generates a mutation matrix
  ...
  number of nodes and a value `k` determining the linearity of the tree
  topology.
...
- `create_U`: This function calculates the $\boldsymbol{U}$ matrix,
  ...
  a $\boldsymbol{B}$ matrix, the number of samples considered, the
  number of cells in each sample, and the evolutionary mode of the
  tumor.

- `create_F`: This function generates the $\boldsymbol{F}$ matrix, which
  ...
  on a pair of matrices, $\boldsymbol{U}$ and $\boldsymbol{B}$, and
  considers whether the mutations are heterozygous.

- `add_noise`: This function introduces sequencing noise into the
  noise-free $\boldsymbol{F}$ matrix generated by the `create_F` method.
...
- `parents`: A vector indicating the parent clones for each clone in the
  phylogenetic tree.
...
- `labels`: A vector containing the gene tags associated with the nodes
...
- `B`: A square $\boldsymbol{B}$ matrix that represents the phylogenetic
  tree.
...
- `B_mats`: A list of 10 trios of $\boldsymbol{B}$ matrices. Each trio
  ...
  relationships from a given $\boldsymbol{F}$ matrix. These matrices
  serve as illustrative examples for testing and exploring the package's
  functionalities.

- `palettes`: A data frame containing three predefined palettes for use
  with methods in **GeRnika** that require color palettes.
...
alt="graphic without alt text" />
<figcaption>Figure 3: Phylogenetic tree associated to the generated
<code>"Phylotree"</code> class object.</figcaption>
</figure>
...
alt="graphic without alt text" />
<figcaption>Figure 4: Phylogenetic trees associated to the generated
<code>"Phylotree"</code> class object, using the proportions associated
to the previously generated <span
class="math inline"><strong>U</strong></span> matrix.</figcaption>
</figure>
...
alt="graphic without alt text" />
<figcaption>Figure 5: <code>phylotree_real</code>,
<code>phylotree_alg2</code> and <code>phylotree_alg1</code>, from the
left to the right.</figcaption>
</figure>
...
alt="graphic without alt text" />
<figcaption>Figure 6: Maximal common subtrees between
...
Evolution." *Nature* 501 (September): 338--45.
<https://doi.org/10.1038/nature12625>.
:::

::: {#ref-clonevol .csl-entry}
Dang, HX, BS White, SM Foltz, et al. 2017. "ClonEvol: Clonal Ordering
and Visualization in Cancer Sequencing." *Annals of Oncology* 28 (12):
3076--82. <https://doi.org/10.1093/annonc/mdx517>.
:::

::: {#ref-davis2017tumor .csl-entry}
Davis, Alexander, Ruli Gao, and Nicholas Navin. 2017. "Tumor Evolution:
Linear, Branching, Neutral or Punctuated?" *Biochimica Et Biophysica
Acta (BBA)-Reviews on Cancer* 1867 (2): 151--61.
<https://doi.org/10.1016/j.bbcan.2017.01.003>.
:::

::: {#ref-phylowgs .csl-entry}
...
*Genome Biology* 16 (1): 1--20.
<https://doi.org/10.1186/s13059-015-0602-8>.
:::

::: {#ref-ancestree .csl-entry}
El-Kebir, Mohammed, Layla Oesper, Hannah Acheson-Field, and Benjamin J.
Raphael. 2015. "[Reconstruction of clonal trees and tumor composition
from multi-sample sequencing data]{.nocase}." *Bioinformatics* 31 (12):
62--70. <https://doi.org/10.1093/bioinformatics/btv261>.
:::

::: {#ref-spruce .csl-entry}
...
Perfect Phylogeny Mixtures." *Cell Systems* 3 (1): 43--53.
<https://doi.org/10.1016/j.cels.2016.07.004>.
:::

::: {#ref-machina .csl-entry}
...
*Nature Genetics* 50 (5): 718--26.
<https://doi.org/10.1038/s41588-018-0106-z>.
:::

::: {#ref-fu2022reconstructing .csl-entry}
...
Variations." *Bioinformatics* 38 (Supplement_1): i125--33.
<https://doi.org/10.1093/bioinformatics/btac253>.
:::

::: {#ref-contreedp .csl-entry}
...
(BIBM)*, 125--30. <https://doi.org/10.1101/2021.10.13.463978>.
:::

::: {#ref-grigoriadis2024conipher .csl-entry}
[Grigoriadis, Kristiana, Ariana Huebner, Abigail Bunkum, et
al.]{.nocase} 2024. "CONIPHER: A Computational Framework for Scalable
Phylogenetic Reconstruction with Error Correction." *Nature Protocols*
19 (1): 159--83. <https://doi.org/10.1038/s41596-023-00913-9>.
:::

::: {#ref-tuelip .csl-entry}
...
Trees." *Bioinformatics* 39 (Supplement_1): i204--12.
<https://doi.org/10.1093/bioinformatics/btad230>.
:::

::: {#ref-gusfield .csl-entry}
Gusfield, Dan. 1991. "Efficient Algorithms for Inferring Evolutionary
Trees." *Networks* 21 (1): 19--28.
<https://doi.org/10.1002/net.3230210104>.
:::
...
ILP." *Bioinformatics* 35 (5): 769--77.
<https://doi.org/10.1093/bioinformatics/bty683>.
:::

::: {#ref-canopy .csl-entry}
...
Evolutionary History by Next-Generation Sequencing." *Proceedings of the
National Academy of Sciences* 113 (37): E5528--37.
<https://doi.org/10.1073/pnas.1522203113>.
:::

::: {#ref-kimuraISA .csl-entry}
...
*Bioinformatics* 31 (9): 1349--56.
<https://doi.org/10.1093/bioinformatics/btv003>.
:::

::: {#ref-malikic2019phiscs .csl-entry}
...
Data." *Genome Research* 29 (11): 1860--77.
...
Deconvolution." *The Annals of Applied Statistics* 10: 2377--404.
<https://doi.org/10.1214/16-AOAS986>.
:::

::: {#ref-fishplot .csl-entry}
Miller, Christopher A, Joshua McMichael, Ha X Dang, et al. 2016.
"Visualizing Tumor Evolution with the Fishplot Package for R." *BMC
Genomics* 17: 1--3. <https://doi.org/10.1186/s12864-016-3195-z>.
:::

::: {#ref-calder .csl-entry}
Myers, Matthew A, Gryte Satas, and Benjamin J Raphael. 2019. "CALDER:
Inferring Phylogenetic Trees from Longitudinal Tumor Samples." *Cell
Systems* 8 (6): 514--22. <https://doi.org/10.1016/j.cels.2019.05.010>.
:::

::: {#ref-nowell1976clonal .csl-entry}
Nowell, Peter C. 1976. "The Clonal Evolution of Tumor Cell Populations."
*Science* 194 (4260): 23--28. <https://doi.org/10.1126/science.959840>.
:::

::: {#ref-depth_error .csl-entry}
...
::: {#ref-lichee .csl-entry}
Popic, Victoria, Raheleh Salari, Iman Hajirasouliha, Dorna
Kashef-Haghighi, Robert B West, and Serafim Batzoglou. 2015. "Fast and
Scalable Inference of Multi-Sample Cancer Lineages." *Genome Biology* 16
(1): 91. <https://doi.org/10.1186/s13059-015-0647-8>.
:::

::: {#ref-ross2016onconem .csl-entry}
Ross, Edith M, and Florian Markowetz. 2016. "OncoNEM: Inferring Tumor
Evolution from Single-Cell Sequencing Data." *Genome Biology* 17: 1--14.
<https://doi.org/10.1186/s13059-016-0929-9>.
:::

::: {#ref-clevrvis .csl-entry}
Sandmann, Sarah, Clara Inserte, and Julian Varghese. 2023. "ClevRvis:
Visualization Techniques for Clonal Evolution." *GigaScience* 12:
giad020. <https://doi.org/10.1093/gigascience/giad020>.
:::

::: {#ref-satas2020scarlet .csl-entry}
...
Constrained Mutation Losses." *Cell Systems* 10 (4): 323--32.
<https://doi.org/10.1016/j.cels.2020.04.001>.
:::

::: {#ref-bayclone .csl-entry}
...
*Pacific Symposium on Biocomputing Co-Chairs*, 467--78.
<https://doi.org/10.1142/9789814644730_0044>.
:::

::: {#ref-sollier2023compass .csl-entry}
...
Reconstruction from Amplicon Single-Cell Sequencing Data." *Nature
Communications* 14 (1): 4921.
...
Composition." *Nucleic Acids Research* 41: e165--65.
<https://doi.org/10.1093/nar/gkt641>.
:::

::: {#ref-tanner2019simulation .csl-entry}
...
in Silico Whole Exome Sequencing." *Bioinformatics* 35 (16): 2850--52.
<https://doi.org/10.1093/bioinformatics/bty1063>.
:::

::: {#ref-clonarch .csl-entry}
Wu, Jiaqi, and Mohammed El-Kebir. 2020. "ClonArch: Visualizing the
Spatial Clonal Architecture of Tumors." *Bioinformatics* 36
(Supplement_1): i161--68.
<https://doi.org/10.1093/bioinformatics/btaa471>.
:::

::: {#ref-bitphylogeny .csl-entry}
...
Intra-Tumor Phylogenies." *Genome Biology* 16 (1): 1--16.
<https://doi.org/10.1186/s13059-015-0592-6>.
:::
:::::::::::::::::::::::::::::::::::::
