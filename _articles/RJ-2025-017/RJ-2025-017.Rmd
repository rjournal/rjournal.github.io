...
date: '2025-09-09'
abstract: "Quantile regression directly models a conditional quantile of interest.
  The R package rqPen provides penalized quantile regression estimation using the
  lasso, elastic net, adaptive lasso, SCAD, and MCP penalties. It also provides extensions
  to group penalties, both for groups of predictors and grouping variable selection
  across quantiles. This paper presents the different approaches to penalized quantile
  regression and how to use the provided methods in rqPen. \n"
draft: no
author:
- name: Ben Sherwood
...
preamble: \usepackage{thumbpdf,lmodern,amsfonts,amsmath,mathtools,bm} \newcommand{\va}{{\bf
  a}} \newcommand{\vb}{{\bf b}} \newcommand{\vc}{{\bf c}} \newcommand{\vd}{{\bf d}}
  \newcommand{\ve}{{\bf e}} \newcommand{\vf}{{\bf f}} \newcommand{\vh}{{\bf h}} \newcommand{\vl}{{\bf
  l}} \newcommand{\vr}{{\bf r}} \newcommand{\vW}{{\bf W}} \newcommand{\vS}{{\bf S}}
  \newcommand{\vE}{{\bf E}} \newcommand{\vG}{{\bf G}} \newcommand{\vF}{{\bf F}} \newcommand{\vK}{{\bf
...
The ability to analyze large data sets is one of the major appeals of penalized regression methods. However, linear programming and second-order cone programming algorithms become computationally burdensome for large data sets. Further complicating matters is that the quantile loss function is non-differentiable, while popular algorithms for penalized objective functions rely on a differentiable loss function, for instance, \cite{friedman2009glmnet} (elastic net), \cite{scadAlg} (non-convex penalties), \cite{brehenyglasso} (group non-convex penalties), and \cite{Yang2015} (group lasso). \cite{huber_cd} proposed using a Huber-type approximation of the quantile loss and coordinate descent algorithm for solving elastic net penalized quantile regression which is implemented in the R package \CRANpkg{hqreg}. Assuming a good initial estimator is provided, \cite{qr_cd} proposed a coordinate descent algorithm (QICD) for non-convex penalties. The R package \CRANpkg{conquer} approximates the quantile loss with a convolution of the quantile loss and Kernel function \citep{lowdConv,highdConv}. 

<!-- \cite{quantGrowthVito} propose a method to estimate additive models that includes a penalty for the amount of wiggliness in the estimated curves. If a linear model is assumed this reduces to a lasso penalty with the quantile loss. The R package \CRANpkg{quantregGrowth} implements this approach providing the lasso and adaptive lasso penalty. In addition, it uses the algorithm of \cite{quantCurveNoCross} to avoid crossing quantiles.  -->

<!--- Using ADMM algorithms for penalized quantile regression has also been explored, albeit without active R packages. \citep{admm_nan_qr,lan_admm, admm_hui,WEN2023}. % and a coordinate descent algorithm (QICD) for nonconvex penalties \citep{qr_cd}. %IN ADDITION CITE OUR OWN WORK FOR GROUP LASSO AND R PACKAGE.--->    
...
(\#eq:qrModel)
\end{equation}
where \(P(\epsilon_i < 0 | {\bf x}_i)=\tau\). Define \(\rho_\tau(u) = u[\tau-I(u<0)]\), \citet{origQR} proposed estimating \@ref(eq:qrModel) by minimizing
\begin{equation}
\sum_{i=1}^n \rho_\tau(y_i-{\bf x}_i^\top\bm \beta),
(\#eq:qr)
\end{equation}
which is available in the package \CRANpkg{quantreg}. Let $m_i$ be the weight for observation $i$, $\bm w$ be a vector of weights for a predictor or group of predictors, and $\lambda$ and $a$ are penalty tuning parameters. The package \CRANpkg{rqPen} provides functions for estimating \(\bm \beta^\tau_0\) by minimizing
\begin{equation}
...
The following defines \(P_{\lambda,a}(\bm \beta)\) for the four different penalty functions, plus two important special cases.
...
The weights, \(w_j\), allow for different weights for predictors and must be non-negative. If \(w_j=0\) then that variable will be unpenalized. In \CRANpkg{rqPen} these weights are labeled as \code{penalty.factors} or \code{group.penalty.factors}. The lasso estimator provides the backbone for the algorithm of the three non-elastic net penalties. As \(\rho_\tau(x)+\rho_\tau(-x) = |x|\), the lasso estimator minimizes,
\begin{equation}
...
(\#eq:lassoAlt)
\end{equation}
The objective function \@ref(eq:lassoAlt) has the same form as \@ref(eq:qr) except for the scaling factor, \(n^{-1}\), and the weights. This approach of creating the augmented \(2p\) samples and then using standard quantile regression is implemented in \CRANpkg{rqPen} where the problem is solved using the \code{rq()} function from \CRANpkg{quantreg}. Note this approach is different from using \code{rq(method="lasso",...)} within \CRANpkg{quantreg}, which uses a linear programming approach but does not formulate the problem using augmented data.
...
(\#eq:enet)
\end{equation}
cannot be framed as a linear programming problem because of the ridge penalty. Thus for \(a \neq 1\), the \CRANpkg{rqPen} implementation of elastic net only uses the Huber approximation approach provided in \CRANpkg{hqreg}. While \CRANpkg{hqreg} provides a computational backbone, \CRANpkg{rqPen} provides SCAD, MCP, and adaptive lasso penalties that are not provided in \code{hqreg()}. In addition, \code{hqreg()} does not include any group penalties or allow for weights for observations, \(m_i\). If weights are specified then the group lasso with singleton groups, implemented in \CRANpkg{hrqglas}, is used instead of using \code{hqreg()} from \CRANpkg{hqreg}. 
...
The values of \(q\) in \code{rq.group.pen()} is limited to \(q \in \{1,2\}\) and can be changed with \code{norm=q}. If \(q=2\) then group variable selection will be all-or-nothing, that is all variables within a group will be selected or none of them will be. The choice of \(q=1\) allows for bi-level variable selection \citep{penBiLevel}. Users cannot specify \code{norm=1} and \code{penalty="gLASSO"}, default penalty of group lasso, because that is equivalent to the individual lasso penalty implemented in \code{rq.pen()}.
...
Cross-validation is the other approach implemented for choosing the optimal pair of \((\lambda,a)\). Consider the case of \(K\) folds, let \(n_k\) be the number of observations in the \(k\)th fold, \(y_i^k\), \({\bf x}_i^k\), and \(m_i^k\) be the \(i\)th  response, predictor vector, and weights for the \(k\)th fold and \(\hat{\bm \beta}^{\tau_b}_{-k,\lambda,a}\) be the fit for the \(b\)th quantile excluding the \(k\)th fold for given values of \(\lambda\) and \(a\). By default, even if Huber approximations are used, cross-validation is done using quantile loss. However, this can be changed by setting \code{cvFunc} in the later described functions \code{rq.group.pen.cv()}, \code{rq.gq.pen.cv()}, or \code{rq.pen.cv()}. For instance, \code{cvFunc=abs} will use absolute value loss regardless of the quantile being modeled. For simplicity of presentation, the following assumes quantile loss is used. The average quantile loss at a given fold is
\begin{equation}
C_k^\tau(\lambda,a) = \frac{1}{n_k} \sum_{i=1}^{n_k} m_i^k \rho_\tau(y_i^k-{{\bf x}_i^k}^\top\hat{\bm \beta}^{\tau}_{-k,\lambda,a}).
(\#eq:cv1)
\end{equation}
The cross-validation functions return two summaries of the cross-validation for selecting \((\lambda,a)\). The return value \code{btr} provides a table of the values of \(a\) and \(\lambda\) that minimize
\begin{equation}
...
\end{equation}
The average summary can be replaced by changing the parameter \code{cvSummary}. For instance, \code{cvSummary=median} would use the median values of \(C_k^\tau(\lambda,a)\). 

Users can also choose to optimize $\lambda$ and $a$, so they are the same values across all quantiles. When evaluating the cross-validation error they can choose to not provide equal weight to each quantile by entering quantile specific weights of $z_b$. The return value \code{gtr} provides results for the value of \(\lambda\) and \(a\) that minimize
\begin{equation}
...
By setting \code{groupError=FALSE} each observation is weighted equally and \@ref(eq:cv2) and \@ref(eq:cv3) are replaced by \(\sum_{k=1}^K n_k C_k^\tau(\lambda,a)\),
...
One reason the linear and second-order cone programming problems are computationally slow for larger values of \(p\) is they optimize across all potential predictors. \citet{tibshirani2012strong} proposed a strong rule for discarding predictors that can greatly decrease the computational complexity of penalized optimization. The strong rule assumes a differentiable loss function and thus is only implemented for the Huber approximations. In addition, the Huber approximation approaches use warm starts across a path of potential \(\lambda\) values. It starts with the largest potential value of \(\lambda\) then uses that solution as the initial value for the next largest potential value of \(\lambda\). This iterative process is continued until all solutions have been found. The linear programming approaches rely on \code{quantreg::rq()} which does not have an option for initial values. Thus warm starts are not implemented for those approaches. For an individual penalty, the calculations of the strong rule are done in the package \CRANpkg{hqreg}, and details can be found in \citet{huber_cd}. A similar approach has been used for group penalties \citep{SherwoodLi2022}.

# Description of functions
Below are the main functions of \CRANpkg{rqPen} and an S3 method that is unique to \CRANpkg{rqPen}, \code{bytau.plot()}.

<!-- \begin{itemize} -->
<!-- \item \code{rq.pen()} minimizes \@ref(eq:multtau) to provide estimates of a quantile regression model, for potentially multiple quantiles, using an individual penalty of either elastic-net, adaptive lasso, SCAD, or MCP. If not specified, a sequence of $\lambda$ values are automatically generated. Returns an \class{rq.pen.seq} S3 object that works with \code{bytau.plot()}, \code{coef()}, \code{plot()}, \code{predict()}, and \code{print()} methods. -->
<!-- \item \code{rq.gq.pen()} minimizes \@ref(eq:gql) to provide a quantile regression model for multiple quantiles using the group quantile lasso penalty. It also returns an \class{rq.pen.seq} S3 object and can be used with the same methods listed above. -->
<!-- \item \code{rq.group.pen()}  minimizes \@ref(eq:multtaug) and is a group penalty version of \code{rq.pen()}. Users have access to group versions of the lasso, adaptive lasso, SCAD, and MCP penalties. The lasso penalty is restricted to the L2-norm. For other penalties users can choose between the L1- or L2-norm. It also returns an \class{rq.pen.seq} S3 object  and can be used with the same methods listed above. -->
<!-- \item \code{rq.pen.cv()} automates K-folds cross-validation for selecting the tuning parameters $\lambda$ and $a$. If $\lambda$ is left undefined, a sequence of $\lambda$ values will be automatically generated. For $a$, the default values described earlier will be used, unless the user specifies a sequence. If multiple quantiles are modeled then it provides the optimal pair of $(\lambda,a)$ for each quantile and the optimal pair across all quantiles.  Returns an \class{rq.pen.seq.cv} S3 object that works with \code{bytau.plot()}, \code{coef()}, \code{plot()}, \code{predict()}, and \code{print()} methods. -->
<!-- \item \code{rq.gq.pen.cv()} is the group quantile lasso version of \code{rq.pen.cv()}. It does everything listed above, but for the group quantile penalty. It also returns an \class{rq.pen.seq.cv} object.  -->
<!-- \item \code{rq.group.pen.cv()} is the group version of \code{rq.pen.cv()}. It does everything listed above, but for group penalties. It also returns an \class{rq.pen.seq.cv} object. -->
<!-- \item \code{qic.select()} takes an \class{rq.pen.seq} or \class{rq.pen.seq.cv} object and provides the optimal values of $(\lambda,a)$ using an information criterion, as explained in the previous section. It returns an `\code{qic.select}' S3 object that works with \code{coef()}, \code{predict()}, and \code{print()} methods. -->
<!-- \item \code{bytau.plot()} plots coefficients estimates for each predictor as a function of $\tau$. -->
<!-- \end{itemize} -->

* \code{rq.pen()} minimizes \@ref(eq:multtau) to provide estimates of a quantile regression model, for potentially multiple quantiles, using an individual penalty of either elastic-net, adaptive lasso, SCAD, or MCP. If not specified, a sequence of $\lambda$ values is automatically generated. Returns an \class{rq.pen.seq} S3 object that works with \code{bytau.plot()}, \code{coef()}, \code{plot()}, \code{predict()}, and \code{print()} methods.
* \code{rq.gq.pen()} minimizes \@ref(eq:gql) to provide a quantile regression model for multiple quantiles using the group quantile lasso penalty. It also returns an \class{rq.pen.seq} S3 object and can be used with the same methods listed above.
* \code{rq.group.pen()}  minimizes \@ref(eq:multtaug) and is a group penalty version of \code{rq.pen()}. Users have access to group versions of the lasso, adaptive lasso, SCAD, and MCP penalties. The lasso penalty is restricted to the L2-norm. For other penalties, users can choose between the L1- or L2-norm. It also returns an \class{rq.pen.seq} S3 object  and can be used with the same methods listed above.
* \code{rq.pen.cv()} automates K-folds cross-validation for selecting the tuning parameters $\lambda$ and $a$. If $\lambda$ is left undefined, a sequence of $\lambda$ values will be automatically generated. For $a$, the default values described earlier will be used, unless the user specifies a sequence. If multiple quantiles are modeled then it provides the optimal pair of $(\lambda,a)$ for each quantile and the optimal pair across all quantiles.  Returns an \class{rq.pen.seq.cv} S3 object that works with \code{bytau.plot()}, \code{coef()}, \code{plot()}, \code{predict()}, and \code{print()} methods.
* \code{rq.gq.pen.cv()} is the group quantile lasso version of \code{rq.pen.cv()}. It does everything listed above, but for the group quantile penalty. It also returns an \class{rq.pen.seq.cv} object. 
* \code{rq.group.pen.cv()} is the group version of \code{rq.pen.cv()}. It does everything listed above, but for group penalties. It also returns an \class{rq.pen.seq.cv} object.
* \code{qic.select()} takes an \class{rq.pen.seq} or \class{rq.pen.seq.cv} object and provides the optimal values of $(\lambda,a)$ using an information criterion, as explained in the previous section. It returns a `\code{qic.select}' S3 object that works with \code{coef()}, \code{predict()}, and \code{print()} methods.
* \code{bytau.plot()} plots coefficient estimates for each predictor as a function of $\tau$.

The \code{rq.pen()} function provides estimates of conditional quantiles from minimizing a penalized objective function for a sequence of \(\lambda\) values.
...
The function \code{rq.pen()} requires a design matrix \code{x} and vector of response \code{y}. The predictors in the design matrix will be centered to have mean zero and standard deviation one when minimizing the penalized objective function unless \code{scalex} is set to FALSE. While the predictors are scaled, the coefficients are returned on the original scale of \code{x}. The quantiles modeled are set with \code{tau}. Users can choose the \code{penalty} function, with the default being lasso. A pre-specified lambda sequence can be set, \code{lambda}. If the user does not specify a sequence of \(\lambda\) values then \(\lambda_{min}=\epsilon\lambda_{max}\), where \(\epsilon\) can be set by \code{eps} and \code{nlambda} is the number of \(\lambda\) values considered. Though very small values of \(\lambda\) may be discarded if the coefficient estimates are not changing much with smaller values of \(\lambda\), unless \code{lambda.discard=FALSE}. For non-ridge or non-lasso penalties, a sequence of values can also be selected for \code{a}. If \code{a} is not set then default values depend on the penalty, elastic-net (\(a=0\)), adaptive lasso (\(a=1\)), SCAD (\(a=3.7\)) and MCP (\(a=3\)). Penalty factors can be set for predictors, \code{penalty.factor}, or quantiles, \code{tau.penalty.factor}. Observation weights are set using \code{weights}. The linear programming algorithms can provide very small, but non-zero estimates, and these coefficients are set to zero if they are below \code{coef.cutoff}. The choice of algorithm can be set using \code{alg}. Two linear programming algorithms are provided, the Barrodale and Roberts algorithm (br) \citep{br}, as described in \citet{crq1} and \citet{crq2}, and the Frisch-Newton (fn) approach, described in \citet{portnoy1997}. Both approaches rely on implementations in \CRANpkg{quantreg}. Setting \code{rq.pen(alg="huber")} uses the approach of \citet{huber_cd} implemented in \CRANpkg{hqreg}. In addition, they can set the maximum number of iterations, \code{max.iter}, or convergence criteria, \code{converge.eps}, though these only apply to the ``huber'' algorithms.
...
Using \code{coef()}, users can extract coefficients for specific quantiles, \code{tau}, and tuning parameters, \code{a} and \code{lambda}, from class \class{rq.pen.seq} object. Alternatively, they can directly specify the models and lambda values using \code{modelsIndex} and \code{lambdaIndex}, respectively. If none of these values are set then a matrix of coefficients for all quantiles and tuning parameters will be returned. \newline
...
With an \class{rq.pen.seq.cv} object, the \code{coef()} function returns coefficients based on the cross-validation results. If \code{cvmin=TRUE} then the tuning parameters associated with the minimum cross-validation error are returned, otherwise the one standard error rule is used. When \code{septau=TRUE} then the tuning parameters are optimized individually for each quantile. Users can identify the specific quantiles, \code{tau}, they wish to return. The default is to return results for all quantiles. Setting \code{useDefaults=FALSE} ignores the cross-validation results and users can use the arguments in \code{coef.rq.pen.seq()} to select coefficients. For instance, if \code{useDefaults=FALSE} and no other arguments are provided then all coefficients are returned.

Function \code{bytau.plot()} has a similar form for the \class{rq.pen.seq} and \class{rq.pen.seq.cv} objects. \newline
...
These functions produce a plot of how coefficients change with quantiles. The arguments in \code{bytau.plot.rq.pen.seq()} and \code{bytau.plot.rq.pen.seq.cv()} are the same as those covered for \code{coef()}, but with one major difference. Only one vector of coefficients can be provided for each quantile. When using an \class{rq.pen.seq} object, users must specify a single pair of \((\lambda,a)\). When using an \class{rq.pen.seq.cv} object, users can rely on the default tools for selecting coefficients or specify a single pair of \(\lambda\) and \(a\).

Both \class{rq.pen.seq} and \class{rq.pen.seq.cv} objects have \code{plot()} functions. Using \code{plot()} with an \class{rq.pen.seq.cv} object provides a plot of how the cross-validation error changes with \(\lambda\) for each quantile.
...
If \code{septau=TRUE} then a separate plot is created for each quantile, otherwise one plot is created that combines the cross-validation error across all quantiles. Users can limit the plots to a subset of quantiles, \code{tau}. If \code{logLambda=TRUE} then the x-axis will be \(\log(\lambda)\). The \code{main} text can be set otherwise the main title depends on the name of the variable and the value of \code{septau}.

While \code{plot()} for an \class{rq.pen.seq} object creates a plot of the coefficients as they change with \(\lambda\).
...
The \code{plot()} function will create a plot for each combination of \(\tau\) and \(a\). Users can specify specific values of \code{tau} and \code{a} if they only want to consider a subset. They can also limit the values of \code{lambda}. Choice of models and \(\lambda\) values can also be done using \code{modelsIndex} and \code{lambdaIndex}. The \code{main} text can be set. If \code{logLambda=TRUE} then the \(\lambda\) values are reported on the natural log scale.
...
Where "br" and "fn" are the Barrodale and Roberts \citep{crq1,crq2} and Frisch-Newton \citep{portnoy1997} interior point method for linear programming problems implemented in \fct{quantreg:::rq}. The following code takes the 25th value in the sequence of $\lambda$ values and compares the coefficients for the three different approaches creating three different \class{rq.pen.seq} objects.
...
The three different algorithms provide coefficient estimates that are different but similar. For larger data sets, with a large number of observations or variables, we recommend using the Huber approximation because this greatly reduces computational time while providing a reasonable solution \citep{huber_cd}. The following presents a plot of how the coefficient values from \code{qhuber} change with \(\lambda\).

```{r cache=TRUE, echo=TRUE, out.width="250px", out.height="250px", fig.align = 'center'}
plot(qhuber)
```

### Multiple quantiles

Next, we fit a model for multiple quantiles of $\tau \in \{.1,.5,.9\}$ and will also consider values of $a \in \{3,4\}$ for the additional tuning parameter for SCAD. A fit with multiple quantiles remains an \class{rq.pen.seq} object.
...
When jointly optimizing the tuning parameter selection the same values of \(\lambda\) and \(a\) will be selected for each quantile by minimizing \@ref(eq:jointQic). Otherwise, the optimal tuning parameters will be selected by minimizing \@ref(eq:qic) for each quantile separately. Users can specify \code{method="AIC"}, \code{method="BIC"} or \code{method="PBIC"} for a large \(p\) BIC proposed by \citet{qrbic}. When doing joint selection, users can also specify \code{weights}, \(z_b\) in \@ref(eq:jointQic). The default is to provide an equal weight, of one, for all quantiles.

The below code selects the tuning parameters of $(\lambda,a)$ for the \code{qmult} model using AIC. This code will create two \class{qic.select} objects, \code{qic\_sep} and \code{qic\_joint}.
...
Below provides the tuning parameters selected by the two different approaches. The \code{modelsInfo} attribute provides information on the model selected for each quantile.
...
\begin{itemize}
\item \code{tau}: The quantile being modeled.
\item \code{a}: The value of $a$ selected.
\item \code{minQIC}: The QIC value for the given quantile and tuning parameters.
\item \code{lambdaIndex}: The position in the sequence for the preceding $\lambda$ value.
\item \code{lambda}: The value of $\lambda$ at the minimum cross-validation error.
\end{itemize}

When defining \code{qic\_joint} we set \code{septau=FALSE}. Thus only a single pair of $(\lambda,a)$ is selected to the be the best for all nine quantiles. That is why the values of \code{lambda} and \code{a} are the same for all quantiles in \code{qic\_joint\$modelsInfo}, but can differ in \code{qic\_sep\$modelsInfo}. The coefficients can be extracted using \fct{coef}.
...
Users can make predictions from either the \class{qic.select} or \class{rq.pen.seq} objects. Prediction from a \class{qic.select} object will return predictions for all quantiles modeled.
...
The default for \fct{predict} for an \class{rq.pen.seq} object is to return predictions for all values of $\lambda$, $a$, and $\tau$. For \code{qmult} this results in 900 predictions for one row of predictors because there are 100 $\lambda$ values, 3 $a$ values, and 3 quantiles being modeled.
...
Following the \code{coef.rq.pen.seq} code presented in the previous section, users can specify predictions from specific values of $\lambda$, $\tau$, or $a$.
...
The function \fct{rq.pen.cv} creates an \class{rq.pen.seq.cv} object that can be used to select tuning parameters. Below we consider a similar model but use elastic net instead of SCAD. We set \code{tauWeights} to have $z_b=\tau_b(1-\tau_b)$, which provides more weights to errors at the median and less to the more extreme quantiles.
...
The \fct{plot} function with an \class{rq.pen.seq.cv} object can create a plot of cross-validation error for a single quantile, \@ref(eq:cv1), or across all quantiles, \@ref(eq:cv3). The following provides a plot for cross-validation error for $\tau=.5$ as a function of $\lambda$ and $a$.
...
For an \class{rq.pen.seq.cv} object the \fct{predict} function relies on the \fct{coef} function. Below are four predictions using the four combinations of \code{septau} and \code{cvmin}.
...
<!-- For \code{cvMin=TRUE}, the default setting, the model predictions are the same for \code{septau=TRUE} and \code{septau=FALSE}, because the optimal $(\lambda,a)$ pair is the same for each quantile and thus will be the optimal pair across all quantiles. However, there are some differences in the one standard error approach.  -->
The \fct{print} of an \class{rq.pen.seq.cv} object provides information about the cross-validation results.
...
\begin{itemize}
\item \code{tau}: The quantile being modeled.
\item \code{minCv}: The value of the minimum cross-validation error.
\item \code{lambda}: The value of $\lambda$ at the minimum cross-validation error.
\item \code{lambdaIndex}: The position in the sequence for the preceding $\lambda$ value.
\item \code{lambda1se}: The $\lambda$ value using the one standard error rule.
\item \code{a}: The optimal value of $a$ decided by the minimum cross-validation error, and the same value is used for the one standard error $\lambda$ value.
\item \code{cvse}: The standard error of the cross-validation error for the values of $(\lambda,a)$ that provide the smallest cross-validation error.
\item \code{modelsIndex}: The position of the corresponding \class{rq.pen.seq} object in the \code{\$fit\$models} list.
\item \code{nonzero}: The number of nonzero coefficients at the minimum value.
\item \code{nzse}: The number of nonzero coefficients at the one standard error rule.
\end{itemize}
...
Note the output for \code{coefficients(qic\_sep)} and \code{coefficients(qic\_joint)} both show the sparsity structure changes with the quantile being modeled. We find interpreting such results to be difficult. A penalty that enforces the same sparsity structure across quantiles would avoid this issue. The following code will minimize \@ref(eq:gql). Coefficients from the 13th entry in the \(\lambda\) sequence demonstrate that this approach provides consistency in variable selection across the quantiles. This property will hold for any value of \(\lambda\). The function \code{rq.gq.pen.cv()} can be used for cross-validation of this approach. The function \code{rq.gq.pen()} returns a \class{rq.pen.seq} object and thus functions such as \code{predict()}, \code{plot()}, and \code{qic.select()} will work as previously described.
...
Two important things to note from the above two plots. First, for both models, the unpenalized coefficients for \code{Lot\_Config} are never set to zero. Second, the group coefficients for \code{qAmes2} go to zero as a group. While the grouped coefficients for \code{qAmes1} tend to go to zero as a group but do not have to converge to zero at the same time. For instance, for \(\tau=.5\) and using the 40th value of \(\lambda\) the garage and overall condition coefficients from \code{qAmes1} provide a mix of zero and non-zero coefficients. This does not occur for the default setting of \(q=2\), \code{norm=2}.

```{r cache=TRUE, echo=TRUE}
coefficients(qAmes1,tau=.5, lambdaIndex=40)
```

The following plot presents how the coefficients for \code{Full\_Bath} change with \(\tau\) at the 40th \(\lambda\) value, where \code{vars=7} because \code{Full\_Bath} is the seventh coefficient.

```{r cache=TRUE, echo=TRUE, out.width="250px", out.height="250px", fig.align = 'center'}
bytau.plot(qAmes1,vars=7,lambdaIndex=40)
```

The \code{bytau.plot()} can also be used with an \class{rq.pen.seq.cv} object. The below code creates an \class{rq.pen.seq.cv} object using the group lasso penalty.

```{r cache=TRUE, echo=TRUE}
qgl <- rq.group.pen.cv(x_g,y_g,g,tau=tauVals)
```

The below code provides the plot for the coefficient of \code{GarageType\_BuiltIn} using the one standard error rule and the \(\lambda\) tuning parameter is selected to be optimal across all quantiles.

```{r cache=TRUE, echo=TRUE, out.width="250px", out.height="250px", fig.align = 'center'}
bytau.plot(qgl,vars=7,septau=FALSE,cvmin=FALSE)
```

Other functions such as \code{predict()}, \code{coef()} and \code{qic.select()} work the same for a group penalty method as they do for an individual penalty, presented in the Barro data set example. The \code{rq.pen()}, \code{rq.group.pen()}, and \code{rq.gq.pen()} functions return \class{rq.pen.seq} objects. While, the \code{rq.pen.cv()}, \code{rq.group.pen.cv()} and \code{rq.gq.pen.cv()} functions return \class{rq.pen.seq.cv} objects.
...
```{r lassoSpeedCompare, out.width="250px", out.height="250px", fig.cap = "Log time in seconds of lasso quantile regression using conquer and the Huber approximation in rqPen.", fig.align = 'center'}
...
```{r lassoObjCompare, out.width="250px", out.height="250px", fig.cap = "Ratio of quantile regression lasso objective function at the rqPen-huber and conquer solutions, where rqPen-br is the baseline.", fig.align = 'center'}
...
```{r lassoSpeedCompareGroup, out.width="250px", out.height="250px", fig.cap = "Log time in seconds of group lasso quantile regression using conquer and the Huber approximation in rqPen.", fig.align = 'center'}
...
```{r lassoObjCompareGroup, out.width="250px", out.height="250px", fig.cap = "Ratio of quantile regression group lasso objective function at the rqPen and conquer group lasso solutions.", fig.align = 'center'}
