...
ggplot(subset(times,method!="rqPen-br"), aes(x=as.factor(n),y=log(time+.01,10), fill=method)) + geom_boxplot()+facet_wrap(.~p)+ylab("Log Base 10 Time in seconds")+
  xlab("n")
dev.off()

pdf("lassoObjCompare2.pdf")
ggplot(subset(obj,method!="rq-conquer"), aes(x=as.factor(n),y=ratio, fill=method)) + geom_boxplot()+facet_wrap(.~p)+ylab("Objective Function Ratios")+
  xlab("n")
dev.off()
# 
# 
