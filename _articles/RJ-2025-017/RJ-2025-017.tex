...
By setting \code{groupError=FALSE} each observation is weighted equally and \eqref{eq:cv2} and \eqref{eq:cv3} are replaced by \(\sum_{k=1}^K n_k C_k^\tau(\lambda,a)\),
...
\tightlist
\item
  \code{rq.pen()} minimizes \eqref{eq:multtau} to provide estimates of a quantile regression model, for potentially multiple quantiles, using an individual penalty of either elastic-net, adaptive lasso, SCAD, or MCP. If not specified, a sequence of \(\lambda\) values is automatically generated. Returns an `\code{rq.pen.seq}' S3 object that works with \code{bytau.plot()}, \code{coef()}, \code{plot()}, \code{predict()}, and \code{print()} methods.
\item
  \code{rq.gq.pen()} minimizes \eqref{eq:gql} to provide a quantile regression model for multiple quantiles using the group quantile lasso penalty. It also returns an `\code{rq.pen.seq}' S3 object and can be used with the same methods listed above.
\item
  \code{rq.group.pen()} minimizes \eqref{eq:multtaug} and is a group penalty version of \code{rq.pen()}. Users have access to group versions of the lasso, adaptive lasso, SCAD, and MCP penalties. The lasso penalty is restricted to the L2-norm. For other penalties, users can choose between the L1- or L2-norm. It also returns an `\code{rq.pen.seq}' S3 object and can be used with the same methods listed above.
\item
  \code{rq.pen.cv()} automates K-folds cross-validation for selecting the tuning parameters \(\lambda\) and \(a\). If \(\lambda\) is left undefined, a sequence of \(\lambda\) values will be automatically generated. For \(a\), the default values described earlier will be used, unless the user specifies a sequence. If multiple quantiles are modeled then it provides the optimal pair of \((\lambda,a)\) for each quantile and the optimal pair across all quantiles. Returns an `\code{rq.pen.seq.cv}' S3 object that works with \code{bytau.plot()}, \code{coef()}, \code{plot()}, \code{predict()}, and \code{print()} methods.
\item
  \code{rq.gq.pen.cv()} is the group quantile lasso version of \code{rq.pen.cv()}. It does everything listed above, but for the group quantile penalty. It also returns an `\code{rq.pen.seq.cv}' object.
\item
  \code{rq.group.pen.cv()} is the group version of \code{rq.pen.cv()}. It does everything listed above, but for group penalties. It also returns an `\code{rq.pen.seq.cv}' object.
\item
  \code{qic.select()} takes an `\code{rq.pen.seq}' or `\code{rq.pen.seq.cv}' object and provides the optimal values of \((\lambda,a)\) using an information criterion, as explained in the previous section. It returns a `\code{qic.select}' S3 object that works with \code{coef()}, \code{predict()}, and \code{print()} methods.
\item
...
Using \code{coef()}, users can extract coefficients for specific quantiles, \code{tau}, and tuning parameters, \code{a} and \code{lambda}, from class `\code{rq.pen.seq}' object. Alternatively, they can directly specify the models and lambda values using \code{modelsIndex} and \code{lambdaIndex}, respectively. If none of these values are set then a matrix of coefficients for all quantiles and tuning parameters will be returned. \newline
...
These functions produce a plot of how coefficients change with quantiles. The arguments in \code{bytau.plot.rq.pen.seq()} and \code{bytau.plot.rq.pen.seq.cv()} are the same as those covered for \code{coef()}, but with one major difference. Only one vector of coefficients can be provided for each quantile. When using an `\code{rq.pen.seq}' object, users must specify a single pair of \((\lambda,a)\). When using an `\code{rq.pen.seq.cv}' object, users can rely on the default tools for selecting coefficients or specify a single pair of \(\lambda\) and \(a\).

Both `\code{rq.pen.seq}' and `\code{rq.pen.seq.cv}' objects have \code{plot()} functions. Using \code{plot()} with an `\code{rq.pen.seq.cv}' object provides a plot of how the cross-validation error changes with \(\lambda\) for each quantile.
...
While \code{plot()} for an `\code{rq.pen.seq}' object creates a plot of the coefficients as they change with \(\lambda\).
...
Where ``br'' and ``fn'' are the Barrodale and Roberts \citep{crq1,crq2} and Frisch-Newton \citep{portnoy1997} interior point method for linear programming problems implemented in \code{quantreg:::rq()}. The following code takes the 25th value in the sequence of \(\lambda\) values and compares the coefficients for the three different approaches creating three different `\code{rq.pen.seq}' objects.
...
\begin{center}\includegraphics[width=250px,height=250px]{RJ-2025-017_files/figure-latex/unnamed-chunk-10-1} \end{center}

\subsubsection{Multiple quantiles}\label{multiple-quantiles}

Next, we fit a model for multiple quantiles of \(\tau \in \{.1,.5,.9\}\) and will also consider values of \(a \in \{3,4\}\) for the additional tuning parameter for SCAD. A fit with multiple quantiles remains an `\code{rq.pen.seq}' object.
...
Users can make predictions from either the `\code{qic.select}' or `\code{rq.pen.seq}' objects. Prediction from a `\code{qic.select}' object will return predictions for all quantiles modeled.
...
The default for \code{predict()} for an `\code{rq.pen.seq}' object is to return predictions for all values of \(\lambda\), \(a\), and \(\tau\). For \code{qmult} this results in 900 predictions for one row of predictors because there are 100 \(\lambda\) values, 3 \(a\) values, and 3 quantiles being modeled.
...
The \code{plot()} function with an `\code{rq.pen.seq.cv}' object can create a plot of cross-validation error for a single quantile, \eqref{eq:cv1}, or across all quantiles, \eqref{eq:cv3}. The following provides a plot for cross-validation error for \(\tau=.5\) as a function of \(\lambda\) and \(a\).

\begin{verbatim}
plot(qcv,tau=.5)
\end{verbatim}

\begin{center}\includegraphics[width=250px,height=250px]{RJ-2025-017_files/figure-latex/unnamed-chunk-20-1} \end{center}
...
\begin{center}\includegraphics[width=250px,height=250px]{RJ-2025-017_files/figure-latex/unnamed-chunk-21-1} \end{center}

For an `\code{rq.pen.seq.cv}' object the \code{predict()} function relies on the \code{coef()} function. Below are four predictions using the four combinations of \code{septau} and \code{cvmin}.
...
The \code{print()} of an `\code{rq.pen.seq.cv}' object provides information about the cross-validation results.
...
\item \code{modelsIndex}: The position of the corresponding `\code{rq.pen.seq}' object in the \code{\$fit\$models} list.
...
\begin{center}\includegraphics[width=250px,height=250px]{RJ-2025-017_files/figure-latex/unnamed-chunk-29-1} \end{center}

\begin{verbatim}
plot(qAmes2,tau=.5)
\end{verbatim}

\begin{center}\includegraphics[width=250px,height=250px]{RJ-2025-017_files/figure-latex/unnamed-chunk-29-2} \end{center}
...
\begin{center}\includegraphics[width=250px,height=250px]{RJ-2025-017_files/figure-latex/unnamed-chunk-31-1} \end{center}

The \code{bytau.plot()} can also be used with an `\code{rq.pen.seq.cv}' object. The below code creates an `\code{rq.pen.seq.cv}' object using the group lasso penalty.
...
\begin{center}\includegraphics[width=250px,height=250px]{RJ-2025-017_files/figure-latex/unnamed-chunk-33-1} \end{center}

Other functions such as \code{predict()}, \code{coef()} and \code{qic.select()} work the same for a group penalty method as they do for an individual penalty, presented in the Barro data set example. The \code{rq.pen()}, \code{rq.group.pen()}, and \code{rq.gq.pen()} functions return `\code{rq.pen.seq}' objects. While, the \code{rq.pen.cv()}, \code{rq.group.pen.cv()} and \code{rq.gq.pen.cv()} functions return `\code{rq.pen.seq.cv}' objects.
