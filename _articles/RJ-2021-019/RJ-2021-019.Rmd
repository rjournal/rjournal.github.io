...
networks supported by bnlearn are restricted to CG models, which implies
that conditional distributions of discrete variables given continuous
ones are not permitted. Other packages like
...
Unlike MoTBFs and bnlearn, HydeNet does not provide functionality for
learning the network structure. HydeNet is especially appropriate for
modeling decision problems, since it implements influence diagrams,
...
natural way. The main difference with respect to package MoTBFs is that
MoTBF distributions do not belong to the exponential family, and in that
sense both packages are complementary.
...
-   $f$ can be written as
    ...
    $$\label{equ:motbfCore}
    	f(\mathbf{x}) = f(\mathbf{y},\mathbf{z}) =  \sum_{i=0}^k     \prod_{j=1}^c a_{i,\mathbf{y}}^{(j)}\, \psi_i\left( z_j \right),$$
...
-   There is a partition
    ...
    Equation [\[equ:motbfCore\]](#equ:motbfCore){reference-type="ref"
    reference="equ:motbfCore"}.
...
f^{(j)}_k(x | \mathbf{z}\in \Omega_{\boldsymbol{Z}}^j) =  \sum_{i=0}^k    \theta_{i}^{(j)}\, \psi_i^{(j)}(x).$$
...
solving the optimization problem $$\begin{aligned}
...
\end{aligned}$$ where the constraints ensure that the obtained parameter
estimates define a valid CDF, with $\alpha$ and $\beta$ being the
minimum and maximum values of the data sample, respectively. More
precisely, the first constraint guarantees that the corresponding
density is non-negative and the last two restrictions ensure that it
integrates to 1. The latter is equivalent to stating that the CDF should
be equal to 0 at the minimum and equal to 1 at the maximum. Note that
the estimated function is not actually a density, but a CDF instead. An
...
Bayesian information criterion (BIC) [@Sch78]: $$\label{eq:bic}
\text{BIC}(f,D) = \sum_{\ell=1}^N \log f(x_\ell) - \frac{k+2}{2} \log N.$$

This choice is motivated by [@Lan14], who showed that the estimators
based on Equation
[\[eq:program_MOP\]](#eq:program_MOP){reference-type="ref"
reference="eq:program_MOP"} are consistent in terms of the mean squared
error for all $x\in \Omega_X$.
...
program in Equation
[\[eq:program_MOP\]](#eq:program_MOP){reference-type="ref"
reference="eq:program_MOP"} to random vectors of arbitrary dimension
[@Per15b]. In the multivariate case, the sample is a set of
...
obtained by solving the optimization problem $$\begin{aligned}
...
Given a prior MoTBF density $f_{\emph{prior}}$ over a variable $X$,
[@Per15] obtain a posterior density $f_{\emph{post}}$ by making a linear
combination of the prior density and the density $f_{\emph{data}}$
...
($f_{\emph{prior}}$ and $f_{\emph{data}}$) and the expected
...
The MoTBFs package is designed using S3 objects. The functions provided
by the package implement the methods explained in the previous sections.
The package implements functions for learning univariate,
...
ecoli dataset [@Lichman:2013], which is provided along with the package.
The dataset contains information about *Escherichia coli* and consists
of $n = 336$ records, $8$ input variables, and $1$ output variable (the
class). It is a bacterium of the genus *Escherichia* that is commonly
found in the lower intestine of warm-blooded organisms. This dataset can
be downloaded from <http://archive.ics.uci.edu/ml/datasets/Ecoli>.

To begin the analysis, the package and the data are loaded by

::: example
\> install.packages(\"MoTBFs\") \> library(\"MoTBFs\") \>
data(\"ecoli\", package = \"MoTBFs\") \> str(ecoli)
...
$chg          : chr  "0.5" "0.5" "0.5" "0.5" ...$ aac : num 0.56 0.54
0.49 0.52 0.55 0.36 0.44 0.51 0.46 0.56 \...
...
The \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 ecoli}}%
{ecoli} dataset is a data frame with$`<!-- -->`{=html}336$rows corresponding to proteins and$`<!-- -->`{=html}9$columns corresponding to variables.
The dataset contains$`<!-- -->`{=html}4$discrete variables, stored as characters, and$`<!-- -->`{=html}5$continuous variables. The variables
provide measurements of the cells used for predicting the localization site of proteins. 
The first variable, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 Sequence.Name}}%
{Sequence.Name}, which
is the accession number for the SWISS-PROT database, and the output variable \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 class}}%
{class} will not be used
in this running example, and we will  therefore remove them from the data frame.
The discrete variables \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 lip}}%
{lip} and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 chg}}%
{chg} are binary attributes, where character numbers are used as states; \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "0.48"}}%
{"0.48"} and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "1"}}%
{"1"}, and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "0.5"}}%
{"0.5"} and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "1"}}%
{"1"}, respectively. For validation purposes, the dataset is split into 
...
>   trainingData <- dataTT$Training \> testData \<- dataTT$Test
\end{example}
...
network structure over the variables in the dataset. The structure can, for instance, be obtained, using the function \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 hc()}}%
{hc()}
from the 
\texorpdfstring%
{{\normalfont\fontseries{b}\selectfont bnlearn}}%
{bnlearn} package. This function returns a directed acyclic graph obtained from the dataset using 
a local search method. For the sake of simplicity, we have included the function \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 LearningHC()}}%
{LearningHC()} in our package,
which automatically converts into factors those columns that are non-numeric, before calling the function \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 hc()}}%
{hc()}
in \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont bnlearn}}%
{bnlearn}.  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 LearningHC()}}%
{LearningHC()} can also be used to discretize the dataset before calling \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 hc()}}%
{hc()}, but
...
\begin{figure}[htbp]
	...
	\caption{Directed acyclic graph learned from 80 percent of the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 ecoli}}%
{ecoli} dataset used as training data.}
	\label{fig:dag}
\end{figure}
...
We illustrate the learning of a univariate MoTBF density by considering the continuous variable \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mcg}}%
{mcg}.  
\begin{example}
...
The \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 univMoTBF()}}%
{univMoTBF()} function is used for learning univariate densities. The function is at the core of a collection of functions included in the package
to learn densities of class \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "motbf"}}%
{"motbf"} from data. Least squares optimization is used
to minimize the mean squared error between the empirical cumulative distribution and the estimated MoTBF.

The function takes two mandatory arguments, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 data}}%
{data} and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 POTENTIAL\_TYPE}}%
{POTENTIAL\_TYPE}, where the latter can
either be \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "MOP"}}%
{"MOP"} or \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "MTE"}}%
{"MTE"} if polynomial or exponential basis functions should be used, respectively.
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 univMoTBF()}}%
{univMoTBF()} also accepts optional arguments: it is possible to specify the domain over which the model will be fitted, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 evalRange}}%
{evalRange}, the exact number
of basis functions to be used, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 nparam}}%
{nparam}, and the maximum number of parameters in the function, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 maxParam}}%
{maxParam},
which selects the best fit using the log-likelihood score. 
If \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 nparam}}%
{nparam} or \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 maxParam}}%
{maxParam} are not given, then the Bayesian information criterion (BIC)
...
An overview of the obtained results is shown via \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 print()}}%
{print()} and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 summary()}}%
{summary()}.\footnote{In order to save space
...
The object returned by \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 univMoTBF()}}%
{univMoTBF()} is a list containing several elements, including its mathematical expression and other hidden elements related to the learning task.
The processing time is one of the values returned by this function and it can be extracted by \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 \$Time}}%
{\$Time}. 
...
\begin{figure}
	...
	\caption{Univariate learning with blue dashed line for MOPs and red solid line for MTEs overlaying the histogram of the training data of the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mcg}}%
{mcg} variable.}
	\label{fig:univariate}
\end{figure}

Figure \ref{fig:univariate} shows the model fits, provided by \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 univMoTBF()}}%
{univMoTBF()}, displayed using the generic
method \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 plot()}}%
{plot()}. \\

To evaluate the predictive ability of the models  we use the generic method \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 as.function()}}%
{as.function()} developed
for the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "motbf"}}%
{"motbf"} class to get the log-likelihood as well as \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 BICMoTBF()}}%
{BICMoTBF()} to obtain the BIC score.
...
This is done by function \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 rMoTBF()}}%
{rMoTBF()}.
For the sake of reproducibility, we fix the seed for the random numbers to be used by the
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 rMoTBF()}}%
{rMoTBF()} function, which is
set to$`<!-- -->`{=html}5$in this example. In the next code snippet, the previous function fitted with a polynomial basis, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 f2}}%
{f2}, will be used.
...
The$p$-value is notably above$`<!-- -->`{=html}0.05$, so there is no evidence to reject the null hypothesis
...
\begin{figure}[htb]     
	...
	\caption{Histogram of variable \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mcg}}%
{mcg} plotted over the histogram of the generated sample (a). 
		Illustration of the inverse transform method (b), red solid line is the CDF of the generated sample, 
		and black dashed line is the  empirical CDF of the training data for variable \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mcg}}%
{mcg}.}
	\label{fig:randomSample}
\end{figure}

Figure~\ref{fig:randomSample} shows two plots comparing the training data of variable \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mcg}}%
{mcg} 
and the sample simulated from the distribution learned using the same training data.

We can also manipulate the distributions with a collection of methods for class \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "motbf"}}%
{"motbf"}. Here is an example of the use of three of them,
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 coef()}}%
{coef()}, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 integralMoTBF()}}%
{integralMoTBF()}, and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 derivMoTBF()}}%
{derivMoTBF()}.
...
{{\normalfont\ttfamily\hyphenchar\font=-1 parametersJointMoTBF()}}%
{parametersJointMoTBF()} is used to solve the quadratic optimization problem. It returns
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 Parameters}}%
{Parameters}, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 Range}}%
{Range} and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 Time}}%
{Time}, among other values. The function \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 jointMoTBF()}}%
{jointMoTBF()} is used
for obtaining the analytical expression, where the returned object is of class \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "jointmotbf"}}%
{"jointmotbf"}. The expression 
is the only visible element, while the others can be retrieved using \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 attributes()}}%
{attributes()}.
In this example only two variables are used, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mcg}}%
{mcg} and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 alm1}}%
...
> attributes(P)$names \[1\] \"Function\" \"Domain\" \"Iterations\"
\"Time\" $class
[1] "jointmotbf"
\end{example}
...
\begin{figure}[htb]     
	...
	\caption{Filled contour (a), simple contour (b) and perspective (c) plots of the joint MoTBF $f(Y,X)$ of variables \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mcg}}%
{mcg} and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 alm1}}%
{alm1}.}
...
The plots in Figure~\ref{fig:joint} are generated using \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 plot()}}%
{plot()}, developed for objects of class 
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "jointmotbf"}}%
{"jointmotbf"}. This function accepts optional arguments such as \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 type}}%
{type}, where one can choose between 
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "perspective"}}%
{"perspective"} and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "contour"}}%
{"contour"}, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 ranges}}%
{ranges}, used to specify the plotting range, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 orientation}}%
{orientation}, which 
indicates the orientation of the perspective graph, and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 filled}}%
{filled} for getting a filled contour plot.

The function \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 print()}}%
{print()} can be used to obtain an expression of the learned joint density, while 
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 summary()}}%
{summary()} yields a more thorough excerpt of the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "jointmotbf"}}%
{"jointmotbf"} object.
...
As in the univariate case, the processing time, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 P\$Time}}%
{P\$Time}, can vary depending on the CPU, but the
learning outcome will always be  the same for a specific data sample.
 The \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 marginalJointMoTBF()}}%
{marginalJointMoTBF()} function computes the marginals of joint densities.
...
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 conditionalMethod()}}%
{conditionalMethod()}.
Five of its arguments are compulsory:  \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 data}}%
{data}, the dataset; \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 nameParents}}%
{nameParents}, a character
vector indicating the name of the parents; \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 nameChild}}%
{nameChild}, a character string containing the name of
the child; \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 numIntervals}}%
{numIntervals}, the maximum number of intervals for splitting the domain of the parent
variables; \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 POTENTIAL\_TYPE}}%
{POTENTIAL\_TYPE},  the type of basis function. Other arguments are optional,
like \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 maxParam}}%
{maxParam}, indicating the maximum number of parameters for each function, and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 s}}%
{s}, the
expert's relative confidence in any prior knowledge, and 
\texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 priorData}}%
{priorData} if prior knowledge is incorporated in the analysis. 

We will do the conditional analysis for only two variables in order to be able to make a 2-dimensional plot of
the obtained results using \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 plotConditional()}}%
{plotConditional()}. For example, taking into account the relationship found
by the dag, we consider the child variable \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 gvh}}%
{gvh} with parent variable \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mcg}}%
{mcg}.
...
\begin{figure}
	...
	\caption{Conditional density of \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 gvh}}%
{gvh} given \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 mcg}}%
{mcg}.}
...
even though we have set the argument \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 numIntervals}}%
{numIntervals} to five. This is because the BIC score is not
...
For doing this task, the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 MoTBFs\_Learning()}}%
{MoTBFs\_Learning()} function of the \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont MoTBFs}}%
{MoTBFs} package is used.
The graph is a mandatory argument, that can be of class \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "bn"}}%
{"bn"}, \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "graphNEL"}}%
{"graphNEL"} or \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "network"}}%
{"network"}.
Other mandatory arguments are the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 data}}%
{data}, the maximum number of intervals for splitting the domain of the parents, 
 and the type of basis function. The function also accepts additional arguments, but they are not listed here.

In the example, the DAG was obtained using the \texorpdfstring%
{{\normalfont\fontseries{b}\selectfont bnlearn}}%
{bnlearn} package and therefore it is
an object of class \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "bn"}}%
{"bn"}. As an example, 
we will use a maximum of$`<!-- -->`{=html}4$intervals and \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 "MTE"}}%
{"MTE"} potentials when learning the densities (i.e. exponential
basis functions).
...
The results are reported using the \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 printBN()}}%
{printBN()} function. Notice how nodes in the DAG with only
discrete parents contain as many functions as configurations of the parents, nodes that have continuous parents have 
at most$`<!-- -->`{=html}4$functions for each parent and nodes that have mixed parents contain as many functions as
...
The function \texorpdfstring%
{{\normalfont\ttfamily\hyphenchar\font=-1 BiC.MoTBFBN()}}%
{BiC.MoTBFBN()} can be used to compute the log-likelihood and the BIC score of a dataset
given the Bayesian network.

\begin{example}
> bic <- BiC.MoTBFBN(bn, data = testData)
> attributes(bic)$names \[1\] \"LogLikelihood\" \"BIC\"
...
::: example
\> set.seed(4) \> dataTT \<- TrainingandTestData(data, percentage_test =
0.99) \> trainingData \<- dataTT$Training
> testData <- dataTT$Test \> nrow(trainingData) \[1\] 13
:::
...
::: example
\> means \<- sapply(data, mean) \> set.seed(4) \> priorData \<-
generateNormalPriorData(dag, data = trainingData, size = 5000, + means =
means)
:::
...
priorData. Argument $s$ takes values on the interval $[0,N]$, where $N$
is the sample size, and is used to synchronize the support of the prior
knowledge and the sample. We refer the reader to [@Per15] for the
details. In this example we will use the aac variable from the data set,
have s = 5 as confidence level, and set \"MOP\" as potential type.

::: example
\> f \<- learnMoTBFpriorInformation(priorData$aac, trainingData$aac, + s
= 5, POTENTIAL_TYPE = \"MOP\") \> attributes(f)

$names
[1] "coeffs"            "posteriorFunction" "priorFunction"    
[4] "dataFunction"      "domain"  

> print(f)$coeffs \[1\] 0.5509206 0.4490794

$posteriorFunction
[1] 0.2911+2.7626*x+4.3721*x^2-30.4218*x^3-0.8389*x^4+965.8444*x^5-4183.3860*x^6
+7735.8433*x^7-7321.9450*x^8+3498.9449*x^9-671.5104*x^10$priorFunction
\[1\]
0.0673+1.7639\*x+11.8532\*x\^2-55.2199\*x\^3-1.5228\*x\^4+1753.1465\*x\^5-7593.4470\*x\^6
+14041.6676\*x\^7-13290.3826\*x\^8+6351.0879\*x\^9-1218.8879\*x\^10

$dataFunction
[1] 0.5656+3.9878*x-4.8055*x^2$domain \[1\] -0.1232 0.9531
:::

::: example
\> sum(log(as.function(f$posteriorFunction)(testData$aac))) \[1\]
134.566 \> sum(log(as.function(f$dataFunction)(testData$aac))) \[1\]
78.96405
:::
...
depicted in Figure [1](#fig:priorUniv){reference-type="ref"
reference="fig:priorUniv"}.
...
<figure id="fig:priorUniv">
<img src="FIGS/priorUnivariate.png" style="width:60.0%" />
<figcaption>Univariate density estimation using prior knowledge. Solid
black, dashed red and dotted blue lines represent the posterior, the
data, and the prior function, respectively.</figcaption>
</figure>
...
POTENTIAL_TYPE = \"MOP\", s = 5, priorData = priorData) \> BN \<-
MoTBFs_Learning(dag, trainingData, numIntervals = 2, POTENTIAL_TYPE =
\"MOP\")

\> logLikelihood.MoTBFBN(priorBN, data = testData) \[1\] 124.384 \>
logLikelihood.MoTBFBN(BN, data = testData) \[1\] 14.64589
:::
...
After a Bayesian network has been constructed, the MoTBFs package can be
used to obtain the conditional density of any variable in the network
...
::: example
\> data(\"ecoli\", package = \"MoTBFs\") \> data \<- ecoli\[,-c(1,9)\]
\> dag \<- LearningHC(data) \> bn \<- MoTBFs_Learning(dag, data = data,
numIntervals = 4, POTENTIAL_TYPE = \"MTE\")
:::
...
given that lip=\"0.48\", alm1 = 0.55 and gvh = 1. This is achieved by
using the function forward_sampling were we have chosen a sample size
equal to 10 specified by parameter size = 10.

::: example
\> obs \<- data.frame(lip = \"0.48\", alm1 = 0.55, gvh = 1,
stringsAsFactors=FALSE) \> node \<- \"alm2\" \> set.seed(5) \>
forward_sampling(bn, dag, target = node, evi = obs, size = 10, maxParam
= 15)

Processing Time: 0.209545850753784secs

$fx
[1] -4.3738+2.4552*exp(2*x)+1.5054*exp(-2*x)-0.2392*exp(4*x)+2.0045*exp(-4*x)$sample
mcg gvh lip chg aac alm1 alm2 1 0.7450156 1 0.48 0.5 0.3564026 0.55
0.53709571 2 0.2493266 1 0.48 0.5 0.4873396 0.55 0.55395502 3 0.4075408
1 0.48 0.5 0.5052861 0.55 0.74832025 4 0.3205169 1 0.48 0.5 0.4844040
0.55 0.82546475 5 0.4448844 1 0.48 0.5 0.4248584 0.55 0.07672959 6
0.5717975 1 0.48 0.5 0.4611889 0.55 0.35412707 7 0.7548104 1 0.48 0.5
0.5695499 0.55 0.68322911 8 0.5475842 1 0.48 0.5 0.7797320 0.55
0.45343460 9 0.3048408 1 0.48 0.5 0.5251502 0.55 0.73064241 10 0.7281756
1 0.48 0.5 0.3819332 0.55 0.70396470
:::
...
The MoTBFs package is designed to provide the required implementation to
tackle experimental data analysis with both discrete and continuous
...
Bayesian networks already provided by packages bnlearn and HydeNet, by
implementing MoTBF distributions, resulting in unrestricted network
...
MoTBFs is complementary to abn in the sense that the former is based on
MoTBF densities, which do not belong to the exponential family.
