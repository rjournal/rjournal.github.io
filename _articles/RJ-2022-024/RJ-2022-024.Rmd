...
[*gee*](https://CRAN.R-project.org/package=gee) [@carey19] and
[*geepack*](https://CRAN.R-project.org/package=geepack) [@halekoh06],
...
data related to dental health [@williamson03], pregnancy studies
[@chaurasia18], and longitudinal rehabilitation [@lorenz11], among
...
the reweighting approach developed by @williamson03 and describe how its
...
$\mathbf{V}_{i} = \{n_{i}, X_{i1}, \ldots, X_{in_{i}}\}$ and the set of
all observed data is
$\mathbf{V}=\{\mathbf{V}_{1}, \ldots, \mathbf{V}_{M} \}$. Informative
cluster size is defined as inequality between the marginal distribution
of the response $X$ and the distribution of $X$ conditional on cluster
size:
...
$\mathbf{V}_i=\{n_i^{(k)}, (X_{ij}, G_{ij})\}$, with observations
belonging to group $k$ denoted as the set
...
$\mathbf{X}^{*}=\{X_{1}^{*}, X_{2}^{*}, \ldots, X_{M}^{*}\}$, consists
of independent observations so an estimate of the parameter,
...
converges to
$\hat{\theta} = E\left[\hat{\theta}_{q}^{*} ~|~ \mathbf{V}\right]$ with
...
$$\begin{aligned}
	\hat{\theta} & = & E \left[ \hat{\theta}_{q}^* ~|~ \mathbf{V}\right] \nonumber \\
	& = & \frac{1}{M} \sum_{i=1}^{M} E \left[ X_{i}^{*} | \mathbf{V} \right] =  \frac{1}{M} \sum_{i=1}^{M} \frac{1}{n_{i}} \sum_{j=1}^{n_{i}} X_{ij}
\end{aligned}$$
...
on the observed data $\mathbf{V}$, the expectation of a resampled
...
resampled data
$\left(\mathbf{X}^{*}, \mathbf{G}^{*}\right) = \left \{ (X_{1}^{*}, G_{1}^{*}), \ldots, (X_{M}^{*}, G_{M}^{*}) \right\}$.
...
Let $\mathbf{V}=\left(V_{1},\ldots,V_{M}\right)$ be a collection of
...
3.  Calculate the test statistic from the collection of bootstrapped
    clusters, $T_{b}^{*}=T\left(\mathbf{V}_{b}^{*}\right)$,
    $\mathbf{V}_{b}^{*}=\left(V_{b1}^{*},\ldots,V_{bM}^{*}\right)$.
...
well-known analogue test for i.i.d.data (Table [1](#t:pckFun)). As such,
...
::: {#t:pckFun}
  ...
  **htestClust** function   Reweighted test(s)                          Classical analogue function
  ...
  : Table 1: Hypothesis testing functions available in the
  **htestClust** package. Each row gives the name of a **htestClust**
  function, the reweighted test the function performs, and the R
  function that executes the corresponding classical analogue test. All
  classical analogue functions are available in R through the *stats*
  package, except for `leveneTest()`, which is included in the
...
The *stats* library function `prop.test` does not operate on variables
in a data frame, but instead takes summary counts as its input. Argument
`x` can be a scalar representing the number of binomial successes,
whence `n` is required as the number of binomial trials. Alternatively,
`x` can be a one-dimensional table or matrix with two entries, whence
`n` is omitted. The remaining arguments customize the test in ways
familiar to most users.

The function `proptestClust` from *htestClust* operates on binary
variables in a data frame or on cluster-level summary counts. In this
function, `x` may be a binary variable measured over clusters, wherein
`id` is required as a vector of cluster identifiers. Alternatively, `x`
may instead be a two-dimensional table of within-cluster counts of
failures and successes, wherein `id` is omitted. As previously noted,
several options are available for variance estimation; these may be
selected by the user through the `variance` argument. Additional
...
functions produce `list` objects of class `htest` for which the `print`
...
`icstestClust`. Briefly, `icsPlot` plots a within-cluster summary
statistic of a variable, such as a mean, against the size of each
cluster. For quantitative variables, `icsPlot` produces a scatterplot of
...
cluster sizes. Table [2](#t:screen8) provides details on the variables
in the data set.
...
::: {#t:screen8}
  ...
  **Variable**   **Description**
  ...
  : Table 2: Variables in `screen8` data set. Each row gives the name of
  a variable included in the `screen8` data set and its associated
  description.
:::
...
using the `screen8` data set. Our illustration is not comprehensive, but
...
1.  Is the proportion of students having "proficient" standardized math
    test scores (65 or greater) more than 0.75?

2.  Are participation in extracurricular activity and gender
    independent?

3.  Are mean standardized math test scores different between male and
    female students?

4.  Are mean standardized reading test scores different among groups
    defined by extracurricular activities?

## Evaluating informative cluster size {#ss:icseval}
...
visualizing ICS through the `icsPlot` function. The arguments to
`icsPlot` specify the variable of interest, a cluster-identifying
variable, and a summary function to be applied to the variable within
each cluster. This summary can be any of `obs`, `mean`, `median`, `var`,
`IQR`, `range`, `prop`, producing plots of the observations themselves,
measure of location, or measures of variation against cluster size.
Option `prop` can only be used when the variable of interest is a
...
Figures [1](#f:ScorebyClSize) and [2](#f:GrpbyClSize) show potential
informativeness in cluster size for the `screen8` data. Cluster size
...
activities. []{#s:screen8 label="s:screen8"}

![Figure 1: Average scores in maths and reading by cluster size in
`screen8` data.](MathRead.png){#f:ScorebyClSize width="100%"
alt="graphic without alt text"}

![Figure 2: Plots of categorical variables by cluster size in `screen8`
data. Proportion of female students decreases with cluster size (left),
whereas student participation in sports-related extracurricular
activities increases with cluster size
(right).](GrpbyClSize.png){#f:GrpbyClSize width="100%"
alt="graphic without alt text"}

These empirical results can be verified using the test for ICS,
implemented through the function `icstestClust`, as illustrated below.
...
Within the `icstestClust` function, the type of test statistic, TF or
TCM as detailed earlier, is specified using the `test.method` argument,
and the number of bootstrap loops by argument `B`. Argument `print.it`
...
proportion via `proptestClust`. We specify a one-sided alternative and
use the default sandwich variance estimator evaluated at the null value
of the proportion (`variance = "sand.null"`), shown to perform best for
this test [@gregg20].
...
`htest`, producing familiar output through the `print` method for such
...
The second question suggests a test of independence of extracurricular
activity and gender. We start by producing cluster-weighted estimates of
the proportion of students participating in each activity within each
gender.
...
using `chisqtestClust`. Here, the default method of variance estimation
is method of moments (`variance = "MoM"`), demonstrated to be best for
the test of independence [@gregg20].
...
ICS in the `screen8` data can be demonstrated by implementing the
...
`ttestClust` function. We conclude that mean standardized test scores
...
named it as such to parallel the standard $t$-test means (`t.test` in
...
implementation used above, we note that `ttestClust` can also accept
...
marginal homogeneity, and variance listed in Table [1](#t:pckFun). Their
...
consider reweighted model-based methods such as those by @bible16,
@neuhaus11, and @wang11. Future research will also be devoted to
