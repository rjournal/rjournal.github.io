...
```{r raw, layout="l-body-outset", fig.cap=caption("scRNA-Seq cell scores visualized using langevitour."," The \"local\" guide is active. (A) Unstimulated cells. (B) All cells. (C) Colored by component 3. (D) Doublets hidden.")}
if (isHTML) {
    figureRaw
} else {
    plot_grid(
...
* To find a projection giving a good overview of the relationships between unstimulated PBMC cells, `r goInline('activate the "local" guide', "raw", "{guideType:'local'}")` from the drop-down list. The guided layout may not be able to perfectly separate all the clusters as UMAP can. However, small relative motions make clear where clusters are overlapping by accident rather than real proximity. For example, B cells and T cells are distinct.
...
```{r denoising, fig.height=4, fig.cap=caption("Noise reduction of scRNA-Seq cell scores. Original and denoised positions of 1,000 cells are shown, joined by lines.","")}
if (isHTML) {
    figureDenoising
} else {
...
```{r denoised, layout="l-body-outset", fig.height=4, fig.cap="Noise reduced scRNA-Seq cell scores."}
if (isHTML) {
    figureDenoised
} else {
...
* Doublets still lie between clusters in this denoised version. Compare this to the UMAP layout, where the doublets tend to be attached to one or other clusters. To make the clusters cleaner, `r goInline("hide the doublets", "denoised", "{labelInactive:['U Doublet','S Doublet']}")` by unticking the checkboxes on the doublet labels. `r go("Show all", "denoised", "{labelInactive:[]}")`

* To examine component C1, `r goInline("drag the C1 label on to the plot","denoised","{labelPos:{C1:[0,0.9]}}")`. This pulls out the monocyte cluster. It appears monocytes are associated with C1.
...
```{r gene, fig.height=3, fig.cap=caption("scRNA-Seq gene loadings visualized using langevitour. The points are genes and the variables are the loading for each component."," (A) Genes involved in component 3. (B) Genes involved in components 1 and 2.")}
if (isHTML) {
    figureGene
} else {
    plot_grid(
...
* Drag particular component labels onto the plot to examine them. Biological phenomena such as different cell types (`r goInline("C5","gene","{labelPos:{C5:[0,0.9]}}")`, `r goInline("C6","gene","{labelPos:{C6:[0,0.9]}}")`) or responses to the cytokine (`r goInline("C3","gene","{labelPos:{C3:[0,0.9]}}")`) 
