...
\colblue{The FoxP2 data set \citep{Chabout_etal:2016}, e.g., comprises the sequences of syllables making up the songs as well as the lengths of inter-syllable intervals (ISIs). 
...
\colblue{In the following section, we summarize the technical details of BMRMMs. 
...
\colblue{For example, the components $\blambda_{trans,x_1=1,x_2,\dots,x_p}(\cdot\mid y_{t-1})$ and $\blambda_{trans,x_1=2,x_2,\dots,x_p}(\cdot\mid y_{t-1})$ would be equal if levels 1 and 2 of covariate 1 have similar influences on transition dynamics for fixed levels for covariates $2, \dots, p$.} 
...
\colblue{Two levels of the covariate $j$, $\ell_1, \ell_2 \in \X_j = \{1,\dots,d_j\}$, are clustered together if and only if $z_{trans,j,\ell_1} = z_{trans,j,\ell_2}$.} 
...
{Inference is based on samples drawn from the posterior using a \colblue{Metropolis-Hastings-within-Gibbs MCMC algorithm. 
...
\colblue{Moreover,  the package includes a number of supplementary functions that use the results of the main function to produce numerical summaries, visualizations, and diagnostics.  
...
\colblue{The parameter \code{data} specifies the target data set and  needs to follow a certain structure. } 
...
The users can look at the included \colblue{simulated data set} \code{foxp2}  as an example.
...
\colblue{The number of covariates in the data set is specified by the argument \code{num.cov}.
...
{The \colblue{\code{random.effect}} parameter gives users the option to exclude the random individual effects. 
If \colblue{\code{random.effect}} is set to \code{FALSE}, the transition probabilities (and the mixture probabilities for {duration times}, if applicable) will only consider the influence of the covariate levels.} 
Similarly, the \colblue{\code{fixed.effect}} parameter allows users to exclude the fixed population effects. 
\colblue{The default values for \code{random.effect} and \code{fixed.effect} are both \code{TRUE}.}
The covariate indices for the two analyses can be specified by setting \code{trans.cov.index} and  \code{duration.cov.index}. 
\colblue{We note that indices specified by \code{trans.cov.index} and  \code{duration.cov.index} refer to the index of the covariate when the first covariate is given index 1,  thus different from its index in \code{data}.}

\colblue{Users can define \code{duration.distr} in the following three ways.} 

\begin{enumerate}
\item \colblue{If users set \code{duration.distr} to be \code{NULL},  which is the default setting,  then the duration times will be ignored and not modeled at all. } 
The BMMM described will be implemented to analyze the existing state transitions alone. 

\item \colblue{If \code{duration.distr} is set as \code{list(`mixDirichlet', unit)},  the duration times will be used to construct a new state \code{`dur.state'}, which will be analyzed along with the original set of states. }
...
\colblue{If there is a state transition from state \code{`a'} to \code{`b'} with a duration time of $15$ seconds and the \code{unit} is specified at $5$ seconds,  then the updated Markov sequence will contain three consecutive \code{`dur.state'} states, i.e.,  \code{(`a',  `dur.state', `dur.state', `dur.state', `b')}.  }
...
\item \colblue{If \code{duration.distr} is set to be \code{list(`mixgamma', shape, rate)},  the duration times are modeled as a continuous random variable using a flexible mixture of gamma kernels, as described for a BMRMM model.} 
...
By default, we consider the previous state $y_{s,t-1}$ as a covariate when we model the {duration times} as continuous variables, i.e.,  \colblue{\code{duration.incl.prev.state}} is set to \code{TRUE}. 
Users can set this parameter to \code{FALSE} if they wish to exclude the previous state when analyzing the {duration times}.
\colblue{The remaining parameters \code{simsize} and \code{burnin} denote the total number of MCMC iterations and the number of burn-ins, respectively. }
...
\resizebox{\columnwidth}{!}{\begin{tabular}{lll}
    ...
    \code{data} & the data set to be used following the required format & \\ 
         \code{num.cov} &  an integer giving the number of observed covariates in \code{data} & \\
                  \colblue{\code{cov.labels}} & a list of vectors giving names of all covariate levels &  \colblue{\code{NULL}} \\
                  \colblue{\code{state.labels}} &  a vector giving names of the states &  \colblue{\code{NULL}} \\
         \colblue{\code{random.effect}} &  \code{TRUE} if random individual effects are included &  \colblue{\code{TRUE}} \\
                  \colblue{\code{fixed.effect}} &  \code{TRUE} if fixed population effects are included &  \colblue{\code{TRUE}} \\
    ...
    \colblue{\code{duration.incl.prev.state}} & \code{TRUE} if $y_{t-1}$ acts as a covariate for the analysis of duration times  & \colblue{\code{TRUE}}\\
...
The \code{BMRMM} function returns \colblue{an object of class \code{BMRMM},  which either contains only  \code{results.trans} or both of  \code{results.trans} and \code{results.duration} if duration times follow a mixture gamma distribution.} 
...
\subsection{\colblue{Summarizing BMRMM results}}

\colblue{The \pkg{BMRMM} package provides an S3 method for summarizing results of a \code{BMRMM} object as follows. }

\begin{example}
summary.BMRMM(object, delta = 0.02, digits = 2, ...)
\end{example}

\colblue{The \code{object} must be of class \code{BMRMM}.  
...
\colblue{These two fields give the global test results from the inference of transition probabilities and duration times. 
...
\colblue{The \code{BMRMMsummary} object also contains local test results for transition probabilities. 
...
\colblue{Meanwhile, \code{trans.local.null.test} gives the probability of the null hypothesis that the difference between two covariate levels is not significant under each transition type. }
...
\subsection{\colblue{Visualizing results with BMRMM plotting functions}}

\colblue{The main plotting function of the package, \code{plot.BMRMMsummary}, is an S3 method  for class \code{BMRMMsummary}.  
...
\colblue{When duration times are analyzed as continuous variables using mixture gamma distributions, 
...
\colblue{Finally,  users can check the MCMC diagnostics with the traceplots and autocorrelation plots produced by the function \code{diag.BMRMM}.
...
\subsection{\colblue{Model selection scores for continuous duration times}}
...
\colblue{The function takes an \code{object} as its input, which must be an object of class \code{BMRMM}. 
...
\colblue{The real FoxP2 data set originates from the study by \cite{Chabout_etal:2016} and requires permission to use. 
...
\colblue{First, we obtain a \code{BMRMMsummary} object, \code{sm.fp2},  by calling the \code{summary.BMRMM} function on the returned results \code{res.fp2}.
...
\caption{Results for the \colblue{simulated \code{foxp2}} data set showing the global tests of significance of the covariates for the state transitions (left) and the ISIs (right).
...
\colblue{The plotting function can be called to visualize the posterior transition probabilities under different combinations of the covariate levels. }
...
\caption{Results for the  \colblue{simulated \code{foxp2}} data set showing the posterior mean and standard deviation for each transition type for selected covariate combinations, $(F,A)$ (top) and $(W,L)$ (bottom).
 }
\label{fig:post_mean}
\end{figure}
...
\colblue{The plotting function \code{plot.BMRMMsummary} gives the plots for all local test results if we set the \code{type} to be \code{`trans.local.mean.diff'} or \code{`trans.local.null.test'}. 
...
\caption{Results for the \colblue{simulated \code{foxp2}} data set showing local test results for genotypes fixing the social context, $U$ (top), $L$ (middle), and $A$ (bottom).
...
\caption{Results for the \colblue{simulated \code{foxp2}} data set showing the histograms of the ISIs with the estimated posterior gamma mixture density (left) and the histograms of the ISIs for each mixture component (right). 
 }
\label{fig:hist}
\end{figure}
...
\caption{Results for the \colblue{simulated \code{foxp2}} data set showing the MCMC diagnostic plots, including traceplots and autocorrelation plots. }
\label{fig:diag}
\end{figure}
...
\colblue{The \code{asthma} data set we use here is from the \pkg{SemiMarkov} package \citep{listwon2015semimarkov}.
...
\colblue{Future improvements to the package may include more options for the distribution types of transition probabilities and duration times beyond the currently available mixture Dirichlet and mixture gamma distributions, respectively.}  



\colblue{\section{Acknowledgements} \label{sec:ack}
