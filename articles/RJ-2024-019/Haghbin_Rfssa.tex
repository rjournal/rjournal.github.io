...
\end{figure}
As a motivating example, consider the \code{Callcenter} dataset in Figure 
...
objects of these functions. The \code{funts($\cdot$)}, \code{fssa($\cdot$)} and 
...
the following arguments:
 \begin{itemize}
 	...
 	\item[-] \code{start} and \code{end}: Specify the time of the first and last observations. They can be a 
...
\vspace{.2in}	
	...
	\end{subfigure}
	\caption{The plot types of the generated plots by \code{plotly\_funts($\cdot$)}}
...
After obtaining an object of class \code{fssa}, the user may then choose to perform reconstruction using the \code{freconstruct($\cdot$)} function or perform forecasting using \code{fforecast($\cdot$)}. The reconstruction and forecasting functions both return a list of \code{funts} objects with length $m$ (number of groups). We note that even though it is common to perform forecasting using a combination of groups that best reconstruct the original signal, the user may try forecasting using several different combinations of groups.
...
\end{itemize}
To streamline user interactions further, we have developed a \code{print($\cdot$)} method for the \code{fforecast} class, making it more convenient to view and assess forecasted FTS data. To illustrate these enhancements, consider the continuation of the Callcenter data example in the following:
\begin{example}
...
\begin{figure}[h!]
	...
	\label{fig:forecast}
\end{figure}
The next example will forecast the \code{Callcenter} FTS one week into the future, based on the first $358$ days of the year, by leveraging the FSSA R-forecasting and V-forecasting methods as the results that had been given in Figure \ref{fig:for_call}.
\begin{example}
...
\begin{itemize}
\item \textsf{`Input Data'}: In this sub-panel, the user can either (a) use the functional datasets available in the \CRANpkg{Rfssa} package (e.g., \code{Callcenter} data or remote sensing datasets); (b) simulate MFTS \citep[see][for details on simulation 
