...
Section \hyperref[sec:applications]{6} present applications of the \CRANpkg{iAR} package to astronomical data.
...
respectively. If \(y_{t_j}|y_{t_{j-1}} \sim\) Gamma\((\alpha_{t_j},\beta_{t_j})\), where \(\alpha_{t_j}\) and \(\beta_{t_j}\) are the shape and scale parameters respectively, then \(y_{t_j}\) will be a iAR-Gamma process if \(\alpha_{t_j}\) and \(\beta_{t_j}\) are such that the following two equations are satisfied,
...
The state-space representations of the models described in Section \hyperref[models]{2} allow to obtain the maximum likelihood estimators of the parameters of the models using the Kalman recursions. Given the transition matrix \(F_{t_j}\), the observation matrix \(G_{t_j}\), the transition error variance \(Q_{t_j}\) and the observation error variance \(R_{t_j}\) the Kalman recursions can be obtained as,
...
\begin{figure}
\includegraphics[alt={Simulated Irregularly Observed Autoregressive process.}]{iARPackage_files/figure-latex/iAR-1} \caption{Simulated Irregularly Observed Autoregressive process.}\label{fig:iAR}
\end{figure}
...
\begin{figure}
\includegraphics[alt={Simulated Complex Irregularly Observed Autoregressive process. The red line corresponds to the fitted values estimated with the Kalman recursions.}]{iARPackage_files/figure-latex/CIARfit-1} \caption{Simulated Complex Irregularly Observed Autoregressive process. The blue line corresponds to the fitted values estimated with the Kalman recursions.}\label{fig:CIARfit}
\end{figure}
...
The sim method applied on a CiAR object also allows to generate an irregularly observed autoregressive process with negative autocorrelation as follows:
...
\begin{figure}
\includegraphics[alt={Simulated Bivariate Irregularly Observed Autoregressive process.}]{iARPackage_files/figure-latex/BIAR-1} \caption{Simulated Bivariate Irregularly Observed Autoregressive process.}\label{fig:BIAR}
\end{figure}
...
#> Coefficients:
#>      Estimate St. Error t value Pr(>|t|)
#> phiR     0.89      0.01   104.67     0.00
#> phiI     0.29      0.01    20.61     0.00
#> 
#> Information criteria:
#> AIC: -4065.88 
#> BIC: -4060.67
...
In addition, the method fit allows estimating the contemporary correlation parameter rho. The usage of this method is the following:
...
\begin{figure}
\includegraphics[width=1\linewidth]{iARPackage_files/figure-latex/Benchmark-1} \caption{Distribution of the computation times (in microseconds) of the functions carma (growth package), drw (Rdrw package) and the loglik method (iAR package).}\label{fig:Benchmark}
\end{figure}
...
\begin{figure}
\includegraphics[alt={Mean computation time (in seconds) of the estimation methods in the iAR  package.}]{iARPackage_files/figure-latex/Computation-1} \caption{Mean computation time (in seconds) of the estimation methods in the iAR package.}\label{fig:Computation}
\end{figure}

In addition, Table \ref{tab:timestable} reports the mean CPU times (in seconds). For example, the iAR model increases from \ensuremath{5\times 10^{-4}} seconds (n = 50) to 0.0037 seconds (n = 1000), while the CiAR and BiAR models increase from 0.0069 to 0.1392 and from 0.0126 to 0.2392 seconds, respectively. These results are consistent with the approximately linear growth observed in the previous figure. In particular, when n increases by a factor of 20, the computation time increases by a factor of about 20 for CiAR and BiAR, and by a factor of about 5 for iAR.
...
n & iAR & CiAR & BiAR\\
\hline
50 & 0.0005 & 0.0069 & 0.0126\\
\hline
100 & 0.0006 & 0.0130 & 0.0266\\
\hline
250 & 0.0011 & 0.0376 & 0.0620\\
\hline
500 & 0.0018 & 0.0762 & 0.1305\\
\hline
1000 & 0.0037 & 0.1392 & 0.2392\\
\hline
\end{tabular}
\end{table}
...
\begin{figure}
\includegraphics[alt={Figure a) shows the raw DMCEP time series and Figure b) shows this time series folded by the first period of the variable star. Figure c) shows the residuals of the harmonic fit and Figure d) shows this residuals folded by the second period of the variable star.}]{iARPackage_files/figure-latex/dmcep-1} \caption{Figure a) shows the raw DMCEP time series and Figure b) shows this time series folded by the first period of the variable star. Figure c) shows the residuals of the harmonic fit and Figure d) shows this residuals folded by the second period of the variable star.}\label{fig:dmcep}
\end{figure}

\subsection{Pairing two astronomical time series}\label{pairing-two-astronomical-time-series}

Among the models available in the \CRANpkg{iAR} package, the BiAR model allows two time series to be modeled jointly. However, this model assumes that two time series are measured at the same times. This assumption is not necessarily satisfied. For example, in astronomy an object can be observed with multiple filters, producing distinct time series of brightness that are not perfectly synchronized. To address this problem, a two-step procedure is proposed. The first step consists of pairing two time series by their observational times, in order to pair the measurements taken at the same time. In the case that for a given time there is a measurement in only one time series, a missing value is assigned in the remaining one. In the second step these missing values are imputed with a interpolation procedure.
