## This file generates Figure 6d of our manuscript. ##
...
g <- function(theta, d) {
    mvdc(claytonCopula(theta, dim = d), rep("norm", d),
         rep(list(list(mean = 0, sd = 1)), d))
}
...
pplt <- ggplot(data = df, aes(x = x)) +
    ...
    scale_size_manual(
        name = "legend",
        values = c("Fasano-Franceschini" = 1.5, "Cramer" = 1.5,
                   "DiProPerm" = 2.0)
    ) +
...
saveRDS(pplt, file = "data/fig6d.rds")
