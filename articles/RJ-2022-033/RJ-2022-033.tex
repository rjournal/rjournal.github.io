...
The generalized (complete exponential) Bell polynomials arise from the multivariate Faà di Bruno's formula, whose computation has been included in the latest version of the \CRANpkg{kStatistics} package. In enumerative combinatorics, Faà di Bruno's formula is employed in dealing with formal power series. In particular the multivariate Faà di Bruno's formula gives the \(\boldsymbol{i}\)-th coefficient of the composition (E. Di Nardo, Guarino, and Senato 2011)
\begin{equation}
...
Most routines of the \CRANpkg{kStatistics} package use the partitions of a multi-index \(\boldsymbol{i}.\) Therefore, before describing any of these routines, we recall the notion of multi-index partition and describe the algorithm for its construction as implemented in the \texttt{mkmSet} function of the package.
...
the number of set partitions corresponding to the same multi-index partition, that is \(\boldsymbol{i}!/\Lambda! \mathfrak{m}(\Lambda)!.\) In the latest version of the \CRANpkg{kStatistics} package, among the input parameters of the \texttt{mkmSet} function, an input flag parameter has been inserted aiming to print the multi-index partitions in a more compact form. See the following example.

\hskip-0.5cm\textbf{\emph{Example 2:}} To get all the partitions of \((2,1)\) run
...
The \texttt{parts} function of the \CRANpkg{partitions} package (Hankin 2006) lists all the partitions of a given integer, but in decreasing order. Instead the \texttt{get.partitions} function of the \CRANpkg{nilde} package (Arnqvist et al. 2021) finds all the partitions of a given integer with a fixed length \(l(\lambda)\) (Voinov and Pya Arnqvist 2017). If \(l(\lambda)\) is equal to the given integer, the \texttt{get.partitions} function lists all the partitions in increasing order.
...
The \texttt{nKS} function generates the numerical value of the \(i\)-th \(k\)-statistic starting from a data sample. The computation relies on the following polynomials
\begin{equation}
...
\hskip-0.5cm The main steps of the \texttt{nKM} function are summarized in the following.
...
To simplify the notation, in the following we show how to compute the \(\boldsymbol{i}\)-th polykay of \(N\) numerical data \(\{a_1, \ldots, a_N\}\) using the \texttt{nPS} function for \(\boldsymbol{i} = (i_1, i_2)\). Set \(i=i_1+i_2\) and suppose \(i \leq N.\) The computation relies on the so-called logarithmic polynomials
...
\hskip-0.5cm The main steps of the \texttt{nPS} function are summarized in the following.
...
\hskip-0.5cm\textbf{\emph{Example 9:}} To get \(h_{(1,1)}(y_1, y_2)\) run
...
When the multi-index \(\boldsymbol{i}\) reduces to an integer \(i,\) formulae \(\eqref{cummom}\) are the classical expressions of univariate moments in terms of univariate cumulants and vice versa. The \texttt{mom2cum} and \texttt{cum2mom} functions do the same when the input is an integer.
...
\hskip-0.5cm\textbf{\emph{Example 15:}} To get \(G_4( a_1, a_2, a_3, a_4; y_1, y_2, y_3, y_4)\) run
...
The \texttt{MFB} function generates all the summands of \(\eqref{multfaa2}\). Its first step is to find the set \(\tilde{p}(n,\boldsymbol{i})\) of all the compositions of \(\boldsymbol{i}\) in \(n\) parts, that is all the \(n\)-tuples
...
Consider the partitions of \((2,1)\) as given in Example 2. Note that \texttt{{[}(\ 2\ 1\ )(\ 0\ 0\ ){]}} and \texttt{{[}(\ 0\ 0\ )(\ 2\ 1\ ){]}} are obtained adding a zero column to the partition \texttt{{[}(\ 2\ 1\ ),\ \ 1\ {]}}, and then permuting the two columns. No zero columns are added to \texttt{{[}(\ 2\ 0\ )(\ 0\ 1\ ){]}} as the length of the partition is \(2.\) The same is true for \texttt{{[}(\ 0\ 1\ )(\ 2\ 0\ ){]}} or \texttt{{[}(\ 1\ 1\ )(\ 1\ 0\ ){]}} which are only permuted.
...
The polynomial families discussed in the previous sections are generated using the \texttt{MFB} function. Indeed, the generalized (complete exponential) Bell polynomials in \(\eqref{(sol22ter)}\) are coefficients of the following formal power series
\begin{equation}
...
\subsubsection{\texorpdfstring{Examples of how to generate polynomials not included in the \CRANpkg{kStatistics} package}{Examples of how to generate polynomials not included in the  package}}\label{examples-of-how-to-generate-polynomials-not-included-in-the-package}}

In the following we give some suggestions on how to use the \texttt{R} codes of the \CRANpkg{kStatistics} package to generate additional polynomial families.
...
\hskip-0.5cm\textbf{\emph{Example 24:}} The following code shows how to evaluate \(F_{11}(y)\) in \(y=7.\)
...
Future works consist in expanding the \CRANpkg{kStatistics} package by including extensions of the multivariate Faà di Bruno's formula, as addressed in Bernardini, Natalini, and Ricci (2005) and references therein, aiming to manage nested compositions, as the \texttt{BellY} function in the Wolfram Language and System does. Moreover, further procedures can be inserted relied on symbolic strategies not apparently related to the multivariate Faà di Bruno's formula but referable to this formula, as for example the central Bell polynomials (Kim, Kim, and Jang 2019).

The results in this paper were obtained using the \CRANpkg{kStatistics} \(2.1.1\) package. The package is currently available with a general public license (GPL) from the Comprehensive \texttt{R} Archive Network.
...
\leavevmode\vadjust pre{\hypertarget{ref-nilde}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2186441}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2427666}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR538226}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR3375628}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR1937238}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-DERIV}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR1325915}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-Leeuw}{}}%
De Leeuw, J. 2012. {``Multivariate {C}umulates in {R}.''} \url{https://escholarship.org/uc/item/1fw1h53c}.

\leavevmode\vadjust pre{\hypertarget{ref-MR3529335}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR3412229}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-packkS}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-packkS1}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2773373}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR3437172}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR4129101}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-AIP}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2576676}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR3809541}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-ferreira1997cumulants}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-6100593}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-1458151}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2005276}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2303087}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-sym11020288}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2326318}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2515761}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR907286}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR1781515}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-PrivaultArxiv}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-systems9020046}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-Peccati}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-PRIVAULT2021109161}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-rao1999some}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR505584}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-robson1957applications}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR741185}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR1890491}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2301629}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR3707509}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR1915279}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-smith2020tutorial}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-9754714}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2721349}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR1280717}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-Voinov1439641}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-MR2725499}{}}%
...
Elvira Di Nardo\\
University of Turin\\%
Department of Mathematics ``G.Peano''\\ Via Carlo Alberto 10, 10123 Turin (Italy)\\
%
