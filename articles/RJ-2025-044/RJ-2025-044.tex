...
\subsection{ATSMs with unspanned economic risks: theoretical background}\label{S:ATSMTheory}
...
\subsubsection{Model cross-sectional dimension (Q-dynamics)}\label{S:Qdyn}

\paragraph{Single-country specifications (individual Q-dynamics model classes)}\label{single-country-specifications-individual-q-dynamics-model-classes}
...
\paragraph{Multicountry specifications (joint Q-dynamics model classes)}\label{multicountry-specifications-joint-q-dynamics-model-classes}
...
\subsubsection{Model time series dimension (P-dynamics)}\label{S:Pdyn}
...
\paragraph{JPS-based models}\label{jps-based-models}
...
\paragraph{GVAR-based models}\label{S:GVARtheory}
...
\paragraph{JLL-based models}\label{S:JLL}
...
\subsubsection{Estimation procedures}\label{S:ATSMestimation}
...
\subsection{The ATSMs available at the MultiATSM package}\label{S:ATSMoptions}
...
\subsection{Package dataset}\label{S:SectionData}
...
\subsection{Required user inputs}\label{S:SectionInputs}

\subsubsection{Fundamental inputs}\label{fundamental-inputs}
...
\subsubsection{Model-specific inputs}\label{model-specific-inputs}

\paragraph{GVARlist and JLLlist}\label{S:SectionGVARJLLinputs}
...
\paragraph{BRWlist}\label{S:SectionBRWinputs}
...
\subsubsection{Additional inputs for numerical and graphical outputs}\label{S:SectionNumOut}
...
\paragraph{Bootstrap settings}\label{S:SectionBootstrap}
...
\paragraph{Out-of-sample forecast settings}\label{S:SectionForecast}
...
\subsection{Model estimation}\label{S:SectionEstimation}
...
\subsubsection{Spanned factors}\label{S:SpaFac}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{RJ-2025-044_files/figure-latex/pca-L-1.pdf}}
...
\subsubsection{The P-dynamics estimation}\label{S:PdynEst}
...
\paragraph{VAR}\label{S:SectionVAR}
...
\paragraph{GVAR}\label{S:SectionGVAR}
...
\paragraph{JLL}\label{S:SectionJLL}
...
\subsubsection{ATSM estimation}\label{atsm-estimation}
...
\subsection{Full implementation of ATSMs}\label{S:SectionImplementation}

\subsubsection{Package workflow}\label{package-workflow}
...
\subsubsection{Complete implementation}\label{complete-implementation}
...
\begin{figure}
\includegraphics[width=1\linewidth]{RJ-2025-044_files/figure-latex/FitYields-1} \caption{Chinese bond yield maturities with model fit comparisons. \emph{Model-fit} reflects estimation using only risk-neutral ($\mathbb{Q}$) dynamics parameters, while \emph{Model-implied} incorporates both physical ($\mathbb{P}$) and risk-neutral ($\mathbb{Q}$) dynamics. The $x$-axes represent time in months and the $y$-axis is in natural units.}\label{fig:FitYields}
\end{figure}

\begin{figure}
\includegraphics[width=1\linewidth]{RJ-2025-044_files/figure-latex/IRF-1} \caption{IRFs from the Brazilian bond yields to global economic activity. Size of the shock is one-standard deviation. The black lines are the point estimates. Gray dashed lines are the bounds of the 95\% confidence intervals and the green lines correspond to the median of these intervals. The $x$-axes are expressed in months and the $y$-axis is in natural units.}\label{fig:IRF}
\end{figure}

\begin{figure}
\includegraphics[width=1\linewidth]{RJ-2025-044_files/figure-latex/FEVD-1} \caption{FEVD from the Brazilian bond yield with maturity 60 months. The $x$-axis represents the forecast horizon in months and the $y$-axis is in natural units.}\label{fig:FEVD}
\end{figure}

\begin{figure}
\includegraphics[width=1\linewidth]{RJ-2025-044_files/figure-latex/TermPremia-1} \caption{Chinese sovereign yield curve decomposition showing (i) expected future short rates and (ii) term premia components. The $x$-axis represents time in months and the $y$-axis is expressed in percentage points.}\label{fig:TermPremia}
\end{figure}
...
\subsubsection*{A: Supplementary functions}\label{a-supplementary-functions}
\addcontentsline{toc}{subsubsection}{A: Supplementary functions}

\paragraph{Importing data from Excel files}\label{importing-data-from-excel-files}
...
\paragraph{Transition matrix and star factors}\label{transition-matrix-and-star-factors}
...
\subsubsection*{B: Additional theoretical considerations}\label{b-additional-theoretical-considerations}
\addcontentsline{toc}{subsubsection}{B: Additional theoretical considerations}

\paragraph{Bond yield decomposition}\label{bond-yield-decomposition}
...
\subsubsection*{C: Replication of existing research}\label{c-replication-of-existing-research}
\addcontentsline{toc}{subsubsection}{C: Replication of existing research}

\paragraph{Joslin, Priebisch and Singleton (2014)}\label{joslin-priebisch-and-singleton-2014}
...
\paragraph{Candelon and Moura (2024)}\label{candelon-and-moura-2024}
...
\paragraph{Candelon and Moura (2023)}\label{candelon-and-moura-2023}
