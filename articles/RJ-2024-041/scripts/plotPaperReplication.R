...
#Figure 1:
plotNMFgpumatrix <- plotTimeComparison_SingleMatrix(f=NMFgpumatrix, fgpu = NMFgpumatrix, namePlot="Non negative factorization")

ggsave(plotNMFgpumatrix,
       filename = "./plotNMFgpumatrix.pdf",
       device = "pdf",
       height = 8.2, width = 8.2, units = "in")
#Figure 2 & Figure 3
plotLRGResultSparse <- plotLRG(f=match.fun("LR_GradientConjugate_gpumatrix"),
                   fgpu=match.fun("LR_GradientConjugate_gpumatrix"),
                   sparsity = 0.9,
                   typeMatrixPlotX = c("MKL-R matrix",
                                      "GPUm f64 cuda",
                                      "GPUm f64 cuda sparse",
                   ...
                   typeMatrixPloty = c("MKL-R matrix",
                                       "GPUm f64 cuda",
                                       "GPUm f64 cuda",
                                       "GPUm f64 cpu",
                                       "GPUm f64 cpu",
...
plotLRGResult <- plotLRG(f=match.fun("LR_GradientConjugate_gpumatrix"),
                   fgpu=match.fun("LR_GradientConjugate_gpumatrix"))

ggsave(plotLRGResult,
       filename = "./LRGC_glm.pdf",
       device = "pdf",
       height = 8.2, width = 8.2, units = "in")

ggsave(plotLRGResultSparse,
       filename = "./LRGC.pdf",
       device = "pdf",
       height = 8.2, width = 9.2, units = "in")

#Figure 4
library(boot)
plotGLMRes <- plotGLM()
ggsave(plotGLMRes,
       filename = "./plotGLMRes.pdf",
       device = "pdf",
       height = 8.2, width = 8.2, units = "in")

#Figure 5
plotRowMeans <- plotTimeComparison_SingleMatrix(f=rowMeans, fgpu=GPUmatrix::rowMeans, namePlot="Means of the rows")
plotSolve <- plotTimeComparison_SingleMatrix(f=solve,fgpu=GPUmatrix::solve, namePlot="Inverse of a Matrix")
plotSvd <- plotTimeComparison_SingleMatrix(f=match.fun("svd"),fgpu=GPUmatrix::svd, namePlot="Singular Value Decomposition")
plotExp <- plotTimeComparison_SingleMatrix(f=match.fun("exp"),fgpu=match.fun("exp"), namePlot="Exponential of each element")
plotProduct <- plotTimeComparison_TwoMatrix(f=match.fun("*"),fgpu=match.fun("*"), namePlot="Element-wise product")
plotMatProduct <- plotTimeComparison_TwoMatrix(f=match.fun("%*%"),fgpu=match.fun("%*%"), namePlot="Matrix product")
...
plotSvd <- plotSvd+ theme(legend.position = "none")+scale_y_log10()+labs(y = NULL,x="Size matrix n×n", title="Singular Value Decomposition")
...
ggsave(ComputationTimeGraph,
       filename = "./performanceComparison.pdf",
       device = "pdf",
       height = 10.2, width = 7.2, units = "in")
#Figure 6
plotProduct <- plotTimeComparisonSparse_TwoMatrix(f=match.fun("*"),fgpu=match.fun("*"), namePlot="Element-wise product",typeMatrixPlot = c("Matrix lib", "GPUm f32 cpu sparse", "GPUm f64 cpu sparse", "GPUm f32 cuda sparse", "GPUm f64 cuda sparse"),sparsity_x = T, sparsity_y=T)

plotMatProduct <- plotTimeComparisonSparse_TwoMatrix(f=match.fun("%*%"),fgpu=match.fun("%*%"), namePlot="Matrix #product",
                                              typeMatrixPlot = c("Matrix lib", "GPUm f32 cpu sparse", "GPUm f64 #cpu sparse", "GPUm f32 cuda sparse", "GPUm f64 cuda sparse"),
                                              typeMatrixPlotSecond = c("MKL-R matrix", "GPUm f32 cpu", "GPUm f64 #cpu", "GPUm f32 cuda", "GPUm f64 cuda"),sparsity_x = T)

legendPlot <- get_legend(plotProduct)
plotProduct <- plotProduct+ theme(legend.position = "none")+scale_y_log10()+labs(y = "Time in seconds (log10#-scale)",x="Size and percentaje of not null elements", title="Element-wise product")
plotMatProduct <- plotMatProduct+ theme(legend.position = "none")+scale_y_log10()+labs(y = NULL,x="Size and #percentaje of not null elements", title="Matrix product")
SparsePerformanceComparison <- grid.arrange(plotProduct,
                                    plotMatProduct,
                                    ncol = 2, nrow=1,
                                    bottom=legendPlot)
ggsave(SparsePerformanceComparison,
      filename = "./SparsePerformanceComparison.pdf",
      device = "pdf",
      height = 5.2, width = 9.2, units = "in")
