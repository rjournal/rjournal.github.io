...
\title{\pkg{fairmodels}: a Flexible Tool for Bias Detection,
...
also equips the user with a so-called \code{fairness\_object}, an
object aggregating possibly many models, information about data, and
fairness metrics. \code{fairness\_object} can later be transformed
...
\code{fairness\_check}. It returns \code{fairness\_object}, which
can be visualized or processed by other functions. This will be further
explained in the ``Structure'' section. When calling
\code{fairness\_check} for the first time, the following three
arguments are mandatory:

\begin{itemize}
\tightlist
\item
  \code{explainer} - an object that combines model and data that gives
  a unified interface for predictions. It is a wrapper over a model
  created with the \CRANpkg{DALEX} \citep{JMLRv19} package.
\item
  \code{protected} - a factor, vector containing sensitive attributes
...
\item
  \code{privileged} - a character/factor denoting a level in the
...
created with \code{fairness\_check()} might be summarized with the
\code{print()} function. Total loss is the sum of all fairness
...
the example below. Now \code{fairness\_object} (in code:
\code{fobject}) wraps three models together with different labels and
cutoffs for subgroups. The \code{fairness\_object} can be later used
as a basis for another \code{fairness\_object}. In detail, while
running \code{fairness\_check()} for the first time,
...
providing explainers with a \code{fairness\_object}, those arguments
are not necessary as they are already a part of the previously created
\code{fairness\_object}.
...
columns and has access to the \code{Sex} feature. The second is random
...
objects. The \code{label} argument specifies how these models are
named on plots.
...
numerical way with the \code{print()} function.
...
\code{"fairness\_object"}, a unified structure to wrap model explainer
or multiple model explainers and other \code{fairness\_objects} in a
...
wrapped by \code{fairness\_objects}) must have different labels.
...
models \pkg{fairmodels} allows creating \code{fairness\_objects} in
other ways:

\begin{itemize}
\tightlist
\item
  \code{explainers\ \%\textgreater{}\%\ fairness\_check()} - possibly
  many explainers can be passed to \code{fairness\_check()},
\item
  \code{fairness\_objects\ \%\textgreater{}\%\ fairness\_check()} -
  explainers stored in \code{fairness\_objects} passed to
  \code{fairness\_check()} will be aggregated into one
  \code{fairness\_object},
\item
  \code{explainer} \&
  \code{fairness\_objects\ \%\textgreater{}\%\ fairness\_check()} -
  explainers passed directly and explainers from
  \code{fairness\_objects} will be aggregated into one
  \code{fairness\_object}.
\end{itemize}
...
\code{fairness\_check()} is unnecessary.

To create a \code{fairness\_object}, at least one explainer needs to
be passed to \code{fairness\_check()} function, which returns the said
object. \code{fairness\_object} metrics for each subgroup are
calculated from the separate confusion matrices.

The \code{fairness\_object} has numerous fields. Some of them are:

\begin{itemize}
\tightlist
\item
  \code{parity\_loss\_metric\_data} - data.frame containing parity
  loss for each metric and classifier,
\item
  \code{groups\_data} - list of metric scores for each metric and
  model,
\item
  \code{group\_confusion\_matrices} - list of values in confusion
  matrices for each model and metric,
\item
  \code{explainers} - list of DALEX explainers. When explainers and/or
  \code{fairness\_object} are added, then explainers and/or explainers
  extracted from \code{fairness\_object} are added to that list,
\item
  \code{label} - character vector of labels for each explainer.
\item
  \code{...} - other fields.
\end{itemize}

The \code{fairness\_object} methods are used to create numerous
...
call \code{parity\_loss}, has an attractive property. Due to the usage
...
The \code{parity\_loss} relates strictly to ratios. The classifier is
more fair if \code{parity\_loss} is low. This property is helpful in
visualizations.

There are several modifying functions that operate on
\code{fairness\_object}. Their usage will return other objects. The
...
\code{plot()} function. Additionally, a special plotting function
works immediately on \code{fairness\_object}, which is
\code{plot\_density}. The user can directly specify which metrics
...
Plots visualizing different aspects of \code{parity\_loss} can be
created with one of the following pipelines:

\begin{itemize}
\tightlist
\item
  \code{fairness\_object\ \%\textgreater{}\%\ modifying\_function(...)\ \%\textgreater{}\%\ plot()}~\\
  ...
  next pipeline.
\item
  \code{fairness\_object\ \%\textgreater{}\%\ plot\_fairmodels(type\ =\ modifying\_function,\ ...)}~\\
...
\code{modifying\_function} with function names. Four examples of
additional graphical functions available in the \code{fairmodels} can
...
see how cutoff manipulation might change the \code{parity\_loss}. Find
...
\code{fairness\_check()}. An alternative is to use bias mitigation
...
\item
  ...
  The \code{roc\_pivot} method is implemented based on
...
\item
  ...
  subgroup. Users may pick \code{parity\_loss} metrics of their choice
  and find the minimal \code{parity\_loss}. It is part of
  \code{ceteris\_paribus\_cutoff()} function. Based on picked metrics,
...
\item
  ...
  \tightlist
  \item
    \code{data/explainer\ \%\textgreater{}\%\ method}~\\
    Returns either weights, indexes, or changed data depending on the
    method used.
  \item
    \code{data/explainer\ \%\textgreater{}\%\ pre\_process\_data(data,\ protected,\ y,\ type\ =\ ...)}~\\
    Always returns data.frame. In case of weights data has additional
    column called \code{\_weights\_}.
  \end{itemize}
\item
  ...
  \tightlist
  \item
    \code{fairness\_object\ \%\textgreater{}\%\ ceteris\_paribus\_cutoff(subgroup,\ ...)\ \textbackslash{}\%\textgreater{}\textbackslash{}\%\ print/plot}~\\
    This is the pipeline for creating ceteris paribus cutoff print and
    plot.
  \item
    \code{explainer\ \%\textgreater{}\%\ roc\_pivot(protected,\ privileged,\ ...)}~\\
    The pipeline will return explainer with \code{y\_hat} field
    changed.
  \end{itemize}
\end{itemize}
...
be used along with the previously created \code{lm\_model}. So
firstly, we create a new dataset using \code{pre\_process\_data} and
...
\code{explainer\_lm} with the post-processing function
\code{roc\_pivot}. We set parameter \code{theta\ =\ 0.05} for a
...
In the end, we create \code{fairness\_object} with explainers obtained
...
\code{base}. This is not always the case - sometimes, eliminating bias
