...
abstract: The 'CHANGES IN R 3.6.2' article from the 2019-2 issue.
author: R Core Team
date: '2019-12-01'
date_received: ~
journal:
...
-   print()ing and auto-printing no longer differs for `function`s with
...
-   Improvements in `smoothEnds(x, *)` working with `NA`s (towards
...
-   `table()` no longer drops an `"NaN"` factor level, and better obeys
    ...
    the case of `exclude = NULL` and `NA`s, these are tabulated
    correctly (again).
...
-   `nchar(x, *)` and `nzchar(x)` gain a new argument `keepNA` which
    governs how the result for `NA`s in `x` is determined. For
...
-   `R CMD javareconf` now supports the use of symbolic links for
    ...
    possible to use
    `R``\ `{=latex}`CMD``\ `{=latex}`javareconf``\ `{=latex}`JAVA_HOME=/usr/lib/jvm/java-1.7.0`
