...
\textbf{PLEASE NOTE CAREFULLY:} 

\begin{quote}

The journal's production software is rather unforgiving.  Failure to
adhere to the instructions here may result in a delay to the reviewing,
and possible publication, of your article.
...
\item Download \href{https://journal.r-project.org/share/RJtemplate.zip}{\file{RJtemplate.zip}} which contains the latest versions of \file{RJwrapper.tex}, \file{RJtemplate.tex}, \file{RJournal.sty} and other useful files.
...
\item Then submit the following files to the Editor-in-Chief:
  \begin{itemize}
  \item If the submission is of the ``add-on packages'' type, a motivating letter 
  (PDF or plain text, please) explaining how you meet our requirements described 
  above (\ref{add-on-req}).
  ...
  \item You should have only one \file{.bib} file, and only two
  \file{.tex} files, one of which is \file{RJwrapper.tex}.  
  \end{itemize}
\end{itemize}

\textit{Article length:} The \emph{R Journal} has no page limit.
However, there is a tendency for longer papers to be subject to longer
reviewing time, and excessive length may reduce your article's
readership, should it be published.  If possible, try to limit your
article's length to, say, 8-10 pages, excluding figures and bibliography.
...
Note that reproducibility requires that the author provide any datasets
used in the examples.  If this would be infeasible, due to data privacy
or data volume considerations, please do not use such datasets as
examples. 
...
\item v0.16 2019/08/26 --- Noted importance of strictly following the
above rules, and stated there must be only one \file{.bib}, and only one
\file{.tex} file other than \file{RJwrapper.tex}.

\item v0.17 2019/09/07 --- Added a comment about article length.

\item v0.17 2019/09/11 --- Added a comment about need to provide
datasets.
